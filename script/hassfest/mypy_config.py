...
from menuai.const import REQUIRED_PYTHON_VER
...
NO_IMPLICIT_REEXPORT_MODULES: set[str] = {
    "menuai.components",
    "menuai.components.application_credentials.*",
    "menuai.components.diagnostics.*",
    "menuai.components.spotify.*",
    "menuai.components.stream.*",
    "menuai.components.update.*",
}

HEADER: Final = """
# Automatically generated by menuaifest.
#
# To update, run python3 -m script.menuaifest -p mypy_config
...
def _generate_and_validate_mypy_config(config: Config) -> str:
    ...
    for module in parsed_modules:
        if module.startswith("menuai.components"):
    ...
    # By default enable no_implicit_reexport only for menuai.*
    # Disable it afterwards for all components
    components_section = "mypy-menuai.*"
    ...
    components_section = "mypy-menuai.components.*"
    mypy_config.add_section(components_section)
    for key in STRICT_SETTINGS:
...
def validate(integrations: dict[str, Integration], config: Config) -> None:
    ...
    if _get_strict_typing_path(config).read_text() != strict_typing_content:
        config.add_error(
            "mypy_config",
            "File .strict_typing is not up to date. Run python3 -m script.menuaifest",
            fixable=True,
        )
    ...
    if _get_mypy_ini_path(config).read_text() != mypy_content:
        config.add_error(
            "mypy_config",
            "File mypy.ini is not up to date. Run python3 -m script.menuaifest",
            fixable=True,
        )
