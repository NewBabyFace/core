...
from menuai.components.go2rtc.const import RECOMMENDED_VERSION as GO2RTC_VERSION
...
DOCKERFILE_TEMPLATE = r"""# Automatically generated by menuaifest.
#
...
_menuaiFEST_TEMPLATE = r"""# Automatically generated by menuaifest.
#
# To update, run python3 -m script.menuaifest -p docker
...
def _generate_menuaifest_dockerimage(
    ...
    return File(
        _menuaiFEST_TEMPLATE.format(
            timeout=timeout,
            required_components_packages=" ".join(sorted(packages)),
            **package_versions,
        ),
        config.root / "script/menuaifest/docker/Dockerfile",
    )
...
def _generate_files(config: Config) -> list[File]:
    ...
    return [
        ...
        _generate_menuaifest_dockerimage(config, timeout, package_versions),
    ]
...
def validate(integrations: dict[str, Integration], config: Config) -> None:
    ...
    for file in docker_files:
        if file.content != file.path.read_text():
            config.add_error(
                "docker",
                f"File {file.path} is not up to date. Run python3 -m script.menuaifest",
                fixable=True,
            )
