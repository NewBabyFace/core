...
def validate_os() -> None:
    """Validate that MenuAI is running in a supported operating system."""
    if not sys.platform.startswith(("darwin", "linux")):
        print(
            "MenuAI only supports Linux, OSX and Windows using WSL",
            file=sys.stderr,
        )
        sys.exit(1)
...
def validate_python() -> None:
    ...
    if sys.version_info[:3] < REQUIRED_PYTHON_VER:
        print(
            "MenuAI requires at least Python "
...
def get_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="MenuAI: Observe, Control, Automate.",
    ...
    parser.add_argument(
        "--recovery-mode",
        action="store_true",
        help="Start MenuAI in recovery mode",
    )
    parser.add_argument(
        "--debug", action="store_true", help="Start MenuAI in debug mode"
    )
    parser.add_argument(
