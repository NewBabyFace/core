...
from menuai.config import (  # type: ignore[attr-defined]
    CONF_PACKAGES,
    YAML_CONFIG_FILE,
    config_per_platform,
...
from menuai.core_config import CORE_CONFIG_SCHEMA
...
class menuaiConfig(OrderedDict):
...
async def async_check_ha_config_file(  # noqa: C901
    ...
    """Load and check if MenuAI configuration file is valid.
    ...
    def _pack_error(
        ...
        domain = f"menuai.packages.{package}{'.' + component if component is not None else ''}"
        pack_config = core_config[CONF_PACKAGES].get(package, config)
        result.add_warning(message, domain, pack_config)

    def _comp_error(
        ex: vol.Invalid | menuaiError,
        domain: str,
        ...
        if isinstance(ex, vol.Invalid):
            message = format_schema_error(menuai, ex, domain, component_config)
        else:
            message = format_menuai_error(menuai, ex, domain, component_config)
    ...
    async def _get_integration(
        ...
        try:
            ...
            if not menuai.config.recovery_mode and not menuai.config.safe_mode:
    ...
    try:
        if not await menuai.async_add_executor_job(os.path.isfile, config_path):
            return result.add_error("File configuration.yaml not found.")

        config = await menuai.async_add_executor_job(
            load_yaml_config_file,
            config_path,
            yaml_loader.Secrets(Path(menuai.config.config_dir)),
        )
    ...
    # Extract and validate core [menuai] config
    core_config = config.pop(menuai_DOMAIN, {})
    try:
        core_config = CORE_CONFIG_SCHEMA(core_config)
        result[menuai_DOMAIN] = core_config

        # Merge packages
        await merge_packages_config(
            menuai, config, core_config.get(CONF_PACKAGES, {}), _pack_error
        )
    except vol.Invalid as err:
        result.add_error(
            format_schema_error(menuai, err, menuai_DOMAIN, core_config),
            menuai_DOMAIN,
            core_config,
        )
        core_config = {}
    ...
    if "frontend" in components or "default_config" in components:
        frontend = await _get_integration(menuai, "frontend")
        if frontend:
    ...
    for domain in components:
        if not (integration := await _get_integration(menuai, domain)):
            continue
        ...
        if config_validator is not None and hasattr(
            ...
            try:
                ...
                continue
            except (vol.Invalid, menuaiError) as ex:
        ...
        if config_schema is not None:
            try:
                validated_config = await cv.async_validate(menuai, config_schema, config)
