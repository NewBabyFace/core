...
from menuai.helpers.entity import SLOW_UPDATE_WARNING
...
async def async_setup_entry(menuai: menuai, entry: BondConfigEntry) -> bool:
    ...
    entry.async_on_unload(
        menuai.bus.async_listen(EVENT_menuai_STOP, _async_stop_event)
    )
    entry.runtime_data = BondData(hub, bpup_subs)

    if not entry.unique_id:
        menuai.config_entries.async_update_entry(entry, unique_id=hub.bond_id)
...
async def async_unload_entry(menuai: menuai, entry: BondConfigEntry) -> bool:
...
async def async_remove_config_entry_device(
    menuai: menuai, config_entry: BondConfigEntry, device_entry: dr.DeviceEntry
) -> bool:
