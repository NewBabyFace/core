...
async def async_setup_entry(menuai: menuai, entry: ConfigEntry) -> bool:
    ...
    if CONF_SOURCE_ID not in entry.data:
        ...
        data[CONF_SOURCE_ID] = generate_source_id(menuai, entry.title)
        menuai.config_entries.async_update_entry(entry, data=data)

    # Forward setup to this domain's data manager
    return await get_domain_data(menuai).async_setup_entry(entry)
...
async def async_unload_entry(menuai: menuai, entry: ConfigEntry) -> bool:
    ...
    return await get_domain_data(menuai).async_unload_entry(entry)
