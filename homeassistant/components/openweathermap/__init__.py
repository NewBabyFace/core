...
async def async_setup_entry(
    menuai: menuai, entry: OpenweathermapConfigEntry
) -> bool:
    ...
    if mode not in OWM_MODES:
        async_create_issue(menuai, entry.entry_id)
    else:
        async_delete_issue(menuai, entry.entry_id)

    owm_client = create_owm_client(api_key, mode, lang=language)
    owm_coordinator = get_owm_update_coordinator(mode)(menuai, entry, owm_client)
