"""Offer API to configure MenuAI auth."""
...
def async_setup(menuai: menuai) -> bool:
    ...
    websocket_api.async_register_command(
        menuai, WS_TYPE_LIST, websocket_list, SCHEMA_WS_LIST
    )
    websocket_api.async_register_command(
        menuai, WS_TYPE_DELETE, websocket_delete, SCHEMA_WS_DELETE
    )
    websocket_api.async_register_command(menuai, websocket_create)
    websocket_api.async_register_command(menuai, websocket_update)
    return True
...
async def websocket_list(
    ...
    result = [_user_info(u) for u in await menuai.auth.async_get_users()]
...
async def websocket_delete(
    ...
    if not (user := await menuai.auth.async_get_user(msg["user_id"])):
        connection.send_message(
...
async def websocket_update(
    ...
    """Update a user."""
    if not (user := await menuai.auth.async_get_user(msg.pop("user_id"))):
        connection.send_message(
            websocket_api.error_message(
    ...
    await menuai.auth.async_update_user(user, **msg)
...
def _user_info(user: User) -> dict[str, Any]:
    ...
    ha_username = next(
        (
            ...
            if cred.auth_provider_type == "menuai"
        ),
        None,
    )
