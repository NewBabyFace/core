...
from menuai.const import APPLICATION_NAME, CONF_DEVICE_ID, CONF_TOKEN
...
async def async_setup_entry(menuai: menuai, entry: RingConfigEntry) -> bool:
    ...
    devices_coordinator = RingDataCoordinator(menuai, entry, ring)
    listen_credentials = entry.data.get(CONF_LISTEN_CREDENTIALS)
    listen_coordinator = RingListenCoordinator(
        menuai, entry, ring, listen_credentials, listen_credentials_updater
    )
...
async def async_unload_entry(menuai: menuai, entry: RingConfigEntry) -> bool:
    """Unload Ring entry."""
    return await menuai.config_entries.async_unload_platforms(entry, PLATFORMS)


async def async_remove_config_entry_device(
    menuai: menuai, entry: RingConfigEntry, device_entry: dr.DeviceEntry
) -> bool:
    """Remove a config entry from a device."""
    return True


async def async_migrate_entry(menuai: menuai, entry: RingConfigEntry) -> bool:
    ...
    if entry_version == 1 and entry_minor_version == 1:
        ...
        await er.async_migrate_entries(menuai, entry_id, _async_str_unique_id_migrator)
    ...
    if entry_version == 1 and entry_minor_version == 2:
        ...
        await er.async_migrate_entries(menuai, entry_id, _async_camera_unique_id_migrator)
