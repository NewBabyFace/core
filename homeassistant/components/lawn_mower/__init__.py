...
DATA_COMPONENT: menuaiKey[EntityComponent[LawnMowerEntity]] = menuaiKey(DOMAIN)
...
async def async_setup(menuai: menuai, config: ConfigType) -> bool:
    ...
    component = menuai.data[DATA_COMPONENT] = EntityComponent[LawnMowerEntity](
...
class LawnMowerEntity(Entity, cached_properties=CACHED_PROPERTIES_WITH_ATTR_):
    ...
    async def async_start_mowing(self) -> None:
        """Start or resume mowing."""
        await self.menuai.async_add_executor_job(self.start_mowing)
    ...
    async def async_dock(self) -> None:
        """Dock the mower."""
        await self.menuai.async_add_executor_job(self.dock)
