...
from menuai.const import CONF_URL, EVENT_menuai_STOP, Platform
...
async def async_setup_entry(
    menuai: menuai, entry: MusicAssistantConfigEntry
) -> bool:
    """Set up Music Assistant from a config entry."""
    http_session = async_get_clientsession(menuai, verify_ssl=False)
    ...
    listen_task = asyncio.create_task(_client_listen(menuai, entry, mass, init_ready))
    ...
    async def handle_player_removed(event: MassEvent) -> None:
        ...
        if menuai_device := dev_reg.async_get_device({(DOMAIN, event.object_id)}):
            dev_reg.async_update_device(
                menuai_device.id, remove_config_entry_id=entry.entry_id
            )
...
async def async_remove_config_entry_device(
    ...
    if player_id is None:
        ...
        return False
    mass = get_music_assistant_client(menuai, config_entry.entry_id)
