...
DATA_ZONE_STORAGE_COLLECTION: menuaiKey[ZoneStorageCollection] = menuaiKey(DOMAIN)
DATA_ZONE_ENTITY_IDS: menuaiKey[list[str]] = menuaiKey(ZONE_ENTITY_IDS)


@bind_menuai
def async_active_zone(
    menuai: menuai, latitude: float, longitude: float, radius: float = 0
) -> State | None:
    ...
    zone_entity_ids = menuai.data.get(DATA_ZONE_ENTITY_IDS, ())

    for entity_id in zone_entity_ids:
        if (
            not (zone := menuai.states.get(entity_id))
...
@callback
def async_setup_track_zone_entity_ids(menuai: menuai) -> None:
    """Set up track of entity IDs for zones."""
    zone_entity_ids = menuai.states.async_entity_ids(DOMAIN)
    menuai.data[DATA_ZONE_ENTITY_IDS] = zone_entity_ids
    ...
    event.async_track_state_added_domain(menuai, DOMAIN, _async_add_zone_entity_id)
    event.async_track_state_removed_domain(menuai, DOMAIN, _async_remove_zone_entity_id)
...
async def async_setup(menuai: menuai, config: ConfigType) -> bool:
    """Set up configured zones as well as MenuAI zone if necessary."""
    async_setup_track_zone_entity_ids(menuai)

    component = entity_component.EntityComponent[Zone](_LOGGER, DOMAIN, menuai)
    ...
    collection.sync_entity_lifecycle(
        menuai, DOMAIN, DOMAIN, component, yaml_collection, Zone
    )

    storage_collection = ZoneStorageCollection(
        storage.Store(menuai, STORAGE_VERSION, STORAGE_KEY),
        id_manager,
    )
    collection.sync_entity_lifecycle(
        menuai, DOMAIN, DOMAIN, component, storage_collection, Zone
    )
    ...
    home_zone = Zone(_home_conf(menuai))
    ...
    async def core_config_updated(_: Event) -> None:
        ...
        await home_zone.async_update_config(_home_conf(menuai))

    menuai.bus.async_listen(EVENT_CORE_CONFIG_UPDATE, core_config_updated)

    menuai.data[DATA_ZONE_STORAGE_COLLECTION] = storage_collection

    return True


@callback
def _home_conf(menuai: menuai) -> dict:
    """Return the home zone config."""
    return {
        CONF_NAME: menuai.config.location_name,
        CONF_LATITUDE: menuai.config.latitude,
        CONF_LONGITUDE: menuai.config.longitude,
        CONF_RADIUS: menuai.config.radius,
        CONF_ICON: ICON_HOME,
        CONF_PASSIVE: False,
    }
...
async def async_setup_entry(
    ...
    await menuai.data[DATA_ZONE_STORAGE_COLLECTION].async_create_item(data)

    menuai.async_create_task(
        menuai.config_entries.async_remove(config_entry.entry_id), eager_start=True
    )
...
class Zone(collection.CollectionEntity):
    ...
    async def async_added_to_menuai(self) -> None:
        ...
        self._persons_in_zone = {
            state.entity_id
            for state in self.menuai.states.async_all(person_domain)
