...
async def async_get_trace(
    menuai: menuai, key: str, run_id: str
) -> dict[str, BaseTrace]:
    """Return the requested trace."""
    # Restore saved traces if not done
    await async_restore_traces(menuai)

    return menuai.data[DATA_TRACE][key][run_id].as_extended_dict()


async def async_list_contexts(
    menuai: menuai, key: str | None
    ...
    if key is not None:
        values = {key: menuai.data[DATA_TRACE].get(key)}
    else:
        values = menuai.data[DATA_TRACE]
...
def _get_debug_traces(menuai: menuai, key: str) -> list[dict[str, Any]]:
    """Return a serializable list of debug traces for a script or automation."""
    if traces_for_key := menuai.data[DATA_TRACE].get(key):
        return [trace.as_short_dict() for trace in traces_for_key.values()]
    return []


async def async_list_traces(
    menuai: menuai, wanted_domain: str, wanted_key: str | None
) -> list[dict[str, Any]]:
    ...
    if not wanted_key:
        ...
        for key in menuai.data[DATA_TRACE]:
            domain = key.split(".", 1)[0]
            if domain == wanted_domain:
                traces.extend(_get_debug_traces(menuai, key))
    else:
        traces = _get_debug_traces(menuai, wanted_key)

    return traces


def async_store_trace(
    menuai: menuai, trace: ActionTrace, stored_traces: int
) -> None:
    """Store a trace if its key is valid."""
    if key := trace.key:
        traces = menuai.data[DATA_TRACE]
...
def _async_store_restored_trace(menuai: menuai, trace: RestoredTrace) -> None:
...
async def async_restore_traces(menuai: menuai) -> None:
    """Restore saved traces."""
    if DATA_TRACES_RESTORED in menuai.data:
        return

    menuai.data[DATA_TRACES_RESTORED] = True

    store = menuai.data[DATA_TRACE_STORE]
    try:
    ...
    for key, traces in restored_traces.items():
        ...
        for json_trace in reversed(traces):
            if (
                (stored_traces := menuai.data[DATA_TRACE].get(key))
            ...
            # Catch any exception to not blow up if the stored trace is invalid
                ...
                continue
            _async_store_restored_trace(menuai, trace)
