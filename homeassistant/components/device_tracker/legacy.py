...
from menuai.helpers.typing import ConfigType, GPSType, StateType
...
from menuai.util.yaml import dump
...
def see(
    ...
    menuai.services.call(DOMAIN, SERVICE_SEE, data)


@callback
def async_setup_integration(menuai: menuai, config: ConfigType) -> None:
    ...
    tracker_future: asyncio.Future[DeviceTracker] = menuai.loop.create_future()

    async def async_platform_discovered(
        p_type: str, info: dict[str, Any] | None
    ) -> None:
        """Load a platform."""
        platform = await async_create_platform_type(menuai, config, p_type, {})

        if platform is None or platform.type != PLATFORM_TYPE_LEGACY:
            return

        tracker = await tracker_future
        await platform.async_setup_legacy(menuai, tracker, info)
    ...
    menuai.async_create_task(
        _async_setup_integration(menuai, config, tracker_future), eager_start=True
    )
...
async def _async_setup_integration(
    ...
    """Set up the legacy integration."""
    tracker = await get_tracker(menuai, config)
    ...
    legacy_platforms = await async_extract_config(menuai, config)

    setup_tasks = [
        create_eager_task(legacy_platform.async_setup_legacy(menuai, tracker))
    ...
    cancel_update_stale = async_track_utc_time_change(
        menuai, tracker.async_update_stale, second=range(0, 60, 5)
    )
            ...
            try:
                ...
                if hasattr(self.platform, "async_get_scanner"):
                    scanner = await self.platform.async_get_scanner(
                        menuai, {DOMAIN: self.config}
                    )
                elif hasattr(self.platform, "get_scanner"):
                    scanner = await menuai.async_add_executor_job(
                    ...
                    setup = await self.platform.async_setup_scanner(
                        menuai, self.config, tracker.async_see, discovery_info
                    )
                elif hasattr(self.platform, "setup_scanner"):
                    setup = await menuai.async_add_executor_job(
                    ...
                    raise menuaiError("Invalid legacy device_tracker platform.")  # noqa: TRY301

                if scanner is not None:
                    async_setup_scanner_platform(
                        menuai, self.config, scanner, tracker.async_see, self.type
                    )
...
async def async_extract_config(
    ...
    for platform in await asyncio.gather(
        *(
            async_create_platform_type(menuai, config, p_type, p_config)
...
async def async_create_platform_type(
    menuai: menuai, config: ConfigType, p_type: str, p_config: dict
...
def async_setup_scanner_platform(
    ...
    async def async_device_tracker_scan(now: datetime | None) -> None:
        ...
        for mac in found_devices:
            ...
            zone_home = menuai.states.get(ENTITY_ID_HOME)
            ...
            menuai.async_create_task(async_see_device(**kwargs), eager_start=True)
    ...
    menuai.async_create_task(async_device_tracker_scan(None), eager_start=True)
...
async def get_tracker(menuai: menuai, config: ConfigType) -> DeviceTracker:
    """Create a tracker."""
    ...
    devices = await async_load_config(yaml_path, menuai, consider_home)
    return DeviceTracker(menuai, consider_home, track_new, defaults, devices)
...
class DeviceTracker:
    ...
    async def async_see(
        ...
        if mac is None and dev_id is None:
            raise menuaiError("Neither mac or device id passed in")
        if mac is not None:
            mac = str(mac).upper()
        ...
        self.menuai.async_create_task(
            self.async_update_config(
                self.menuai.config.path(YAML_DEVICES), dev_id, device
            ),
            eager_start=True,
        )
    ...
    async def async_update_config(self, path: str, dev_id: str, device: Device) -> None:
        ...
        async with self._is_updating:
            await self.menuai.async_add_executor_job(
                update_config, self.menuai.config.path(YAML_DEVICES), dev_id, device
            )
    ...
    async def async_setup_tracked_device(self) -> None:
        ...
        for device in self.devices.values():
            if device.track and not device.last_seen:
                # async_added_to_menuai is unlikely to suspend so
    ...
    async def async_update(self) -> None:
        ...
        if self.location_name:
            ...
            zone_state = zone.async_active_zone(
                self.menuai, self.gps[0], self.gps[1], self.gps_accuracy
            )
...
class DeviceScanner:
    ...
    async def async_scan_devices(self) -> list[str]:
        ...
        assert self.menuai is not None, (
            "menuai should be set by async_setup_scanner_platform"
        )
        return await self.menuai.async_add_executor_job(self.scan_devices)
    ...
    async def async_get_device_name(self, device: str) -> str | None:
        ...
        return await self.menuai.async_add_executor_job(self.get_device_name, device)
    ...
    async def async_get_extra_attributes(self, device: str) -> dict:
        ...
        return await self.menuai.async_add_executor_job(self.get_extra_attributes, device)


async def async_load_config(
    path: str, menuai: menuai, consider_home: timedelta
) -> list[Device]:
    ...
    result: list[Device] = []
    try:
        devices = await menuai.async_add_executor_job(load_yaml_config_file, path)
    ...
    for dev_id, device in devices.items():
        ...
        try:
            ...
            async_log_schema_error(exp, dev_id, devices, menuai)
            async_notify_setup_error(menuai, DOMAIN)
        else:
            result.append(Device(menuai, **device))
    return result
...
def remove_device_from_config(menuai: menuai, device_id: str) -> None:
