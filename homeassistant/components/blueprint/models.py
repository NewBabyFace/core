...
class Blueprint:
    ...
    def validate(self) -> list[str] | None:
        """Test if the MenuAI installation supports this blueprint.

        Return list of errors if not valid.
        """
        errors = []
        metadata = self.metadata
        min_version = metadata.get(CONF_menuai, {}).get(CONF_MIN_VERSION)

        if min_version is not None and AwesomeVersion(__version__) < AwesomeVersion(
            min_version
        ):
            errors.append(f"Requires at least MenuAI {min_version}")
...
class DomainBlueprints:
    ...
    def __init__(
        ...
        blueprint_in_use: Callable[[menuai, str], bool],
        reload_blueprint_consumers: Callable[[menuai, str], Awaitable[None]],
        ...
        menuai.data.setdefault(DOMAIN, {})[domain] = self

    @property
    def blueprint_folder(self) -> pathlib.Path:
        """Return the blueprint folder."""
        return pathlib.Path(self.menuai.config.path(BLUEPRINT_FOLDER, self.domain))
    ...
    def _load_blueprints(self) -> dict[str, Blueprint | BlueprintException | None]:
        ...
        blueprint_folder = pathlib.Path(
            self.menuai.config.path(BLUEPRINT_FOLDER, self.domain)
        )
    ...
    async def async_get_blueprints(
        ...
        async with self._load_lock:
            return await self.menuai.async_add_executor_job(self._load_blueprints)
    ...
    async def async_get_blueprint(self, blueprint_path: str) -> Blueprint:
        ...
        async with self._load_lock:
            ...
            try:
                blueprint = await self.menuai.async_add_executor_job(
    ...
    async def async_remove_blueprint(self, blueprint_path: str) -> None:
        ...
        if self._blueprint_in_use(self.menuai, blueprint_path):
            raise BlueprintInUse(self.domain, blueprint_path)
        path = self.blueprint_folder / blueprint_path
        await self.menuai.async_add_executor_job(path.unlink)
        self._blueprints[blueprint_path] = None
    ...
    def _create_file(
        ...
        path = pathlib.Path(
            self.menuai.config.path(BLUEPRINT_FOLDER, self.domain, blueprint_path)
        )
        exists = path.exists()
    ...
    async def async_add_blueprint(
        ...
        """Add a blueprint."""
        overrides_existing = await self.menuai.async_add_executor_job(
        ...
        if overrides_existing:
            await self._reload_blueprint_consumers(self.menuai, blueprint_path)
    ...
    async def async_populate(self) -> None:
        ...
        integration = await loader.async_get_integration(self.menuai, self.domain)
        ...
        def populate() -> None:
            ...
            shutil.copytree(
                ...
                self.blueprint_folder / menuai_DOMAIN,
            )

        await self.menuai.async_add_executor_job(populate)
