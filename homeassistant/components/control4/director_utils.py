...
async def _update_variables_for_config_entry(
    menuai: menuai, entry: Control4ConfigEntry, variable_names: set[str]
...
async def update_variables_for_config_entry(
    ...
    try:
        return await _update_variables_for_config_entry(menuai, entry, variable_names)
    except BadToken:
        _LOGGER.debug("Updating Control4 director token")
        await refresh_tokens(menuai, entry)
        return await _update_variables_for_config_entry(menuai, entry, variable_names)


async def refresh_tokens(menuai: menuai, entry: Control4ConfigEntry):
    ...
    _LOGGER.debug("Saving new tokens in menuai data")
