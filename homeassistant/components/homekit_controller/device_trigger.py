    ...
    def async_attach_trigger(
        ...
        def event_handler(ev: dict[str, Any]) -> None:
            if sub_type != HK_TO_HA_INPUT_EVENT_VALUES[ev["value"]]:
                return
            menuai.async_run_menuai_job(job, {"trigger": {**trigger_data, **config}})
...
async def async_setup_triggers_for_entry(
    ...
    def async_add_characteristic(service: Service) -> bool:
        ...
        if TRIGGERS in menuai.data and device_id in menuai.data[TRIGGERS]:
            return False
        ...
        trigger = async_get_or_create_trigger_source(conn.menuai, device_id)
...
def async_get_or_create_trigger_source(
    ...
    trigger_sources: dict[str, TriggerSource] = menuai.data.setdefault(TRIGGERS, {})
    if not (source := trigger_sources.get(device_id)):
        source = TriggerSource(menuai)
...
def async_fire_triggers(
    ...
    trigger_sources: dict[str, TriggerSource] = conn.menuai.data.get(TRIGGERS, {})
...
async def async_get_triggers(
    ...
    if device_id not in menuai.data.get(TRIGGERS, {}):
        return []

    device: TriggerSource = menuai.data[TRIGGERS][device_id]
...
async def async_attach_trigger(
    ...
    return async_get_or_create_trigger_source(menuai, device_id).async_attach_trigger(
