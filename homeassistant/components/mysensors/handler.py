...
HANDLERS: decorator.Registry[
    str, Callable[[menuai, GatewayId, Message], None]
] = decorator.Registry()


@HANDLERS.register("set")
@callback
def handle_set(menuai: menuai, gateway_id: GatewayId, msg: Message) -> None:
    """Handle a mysensors set message."""
    validated = validate_set_msg(gateway_id, msg)
    _handle_child_update(menuai, gateway_id, validated)


@HANDLERS.register("internal")
@callback
def handle_internal(menuai: menuai, gateway_id: GatewayId, msg: Message) -> None:
    """Handle a mysensors internal message."""
    internal = msg.gateway.const.Internal(msg.sub_type)
    if (handler := HANDLERS.get(internal.name)) is None:
        return
    handler(menuai, gateway_id, msg)


@HANDLERS.register("I_BATTERY_LEVEL")
@callback
def handle_battery_level(
    menuai: menuai, gateway_id: GatewayId, msg: Message
) -> None:
    """Handle an internal battery level message."""
    _handle_node_update(menuai, gateway_id, msg)


@HANDLERS.register("I_HEARTBEAT_RESPONSE")
@callback
def handle_heartbeat(menuai: menuai, gateway_id: GatewayId, msg: Message) -> None:
    """Handle an heartbeat."""
    _handle_node_update(menuai, gateway_id, msg)
...
def handle_presentation(
    ...
    if msg.child_id == SYSTEM_CHILD_ID:
        discover_mysensors_node(menuai, gateway_id, msg.node_id)


@callback
def _handle_child_update(
    menuai: menuai, gateway_id: GatewayId, validated: dict[Platform, list[DevId]]
) -> None:
    ...
    for platform, dev_ids in validated.items():
        devices = get_mysensors_devices(menuai, platform)
        ...
        if new_dev_ids:
            discover_mysensors_platform(menuai, gateway_id, platform, new_dev_ids)
