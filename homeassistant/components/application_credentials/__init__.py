...
STORAGE_KEY = DOMAIN
STORAGE_VERSION = 1
DATA_COMPONENT: menuaiKey[ApplicationCredentialsStorageCollection] = menuaiKey(DOMAIN)
...
class ApplicationCredentialsStorageCollection(collection.DictStorageCollection):
    ...
    async def _process_create_data(self, data: dict[str, str]) -> dict[str, str]:
        ...
        if not await _get_platform(self.menuai, domain):
    ...
    async def async_delete_item(self, item_id: str) -> None:
        ...
        entries = self.menuai.config_entries.async_entries(current[CONF_DOMAIN])
...
async def async_setup(menuai: menuai, config: ConfigType) -> bool:
    ...
    menuai.data[DATA_COMPONENT] = storage_collection

    collection.DictStorageCollectionWebsocket(
        storage_collection, DOMAIN, DOMAIN, CREATE_FIELDS, UPDATE_FIELDS
    ).async_setup(menuai)

    websocket_api.async_register_command(menuai, handle_integration_list)
    websocket_api.async_register_command(menuai, handle_config_entry)

    config_entry_oauth2_flow.async_add_implementation_provider(
        menuai, DOMAIN, _async_provide_implementation
    )
...
async def async_import_client_credential(
    ...
    await menuai.data[DATA_COMPONENT].async_import_item(item)
...
async def _async_provide_implementation(
    ...
    platform = await _get_platform(menuai, domain)
    if not platform:
        return []

    credentials = menuai.data[DATA_COMPONENT].async_client_credentials(domain)
    if hasattr(platform, "async_get_auth_implementation"):
        return [
            await platform.async_get_auth_implementation(menuai, auth_domain, credential)
            for auth_domain, credential in credentials.items()
        ]
    authorization_server = await platform.async_get_authorization_server(menuai)
    return [
        AuthImplementation(menuai, auth_domain, credential, authorization_server)
        for auth_domain, credential in credentials.items()
    ]
...
async def _async_config_entry_app_credentials(
    ...
    if not await _get_platform(menuai, config_entry.domain) or not (
        auth_domain := config_entry.data.get("auth_implementation")
    ):
        return None

    for item in menuai.data[DATA_COMPONENT].async_items():
...
async def _get_platform(
    menuai: menuai, integration_domain: str
) -> ApplicationCredentialsProtocol | None:
    """Register an application_credentials platform."""
    try:
        integration = await async_get_integration(menuai, integration_domain)
...
async def _async_integration_config(menuai: menuai, domain: str) -> dict[str, Any]:
    platform = await _get_platform(menuai, domain)
    if platform and hasattr(platform, "async_get_description_placeholders"):
        placeholders = await platform.async_get_description_placeholders(menuai)
...
async def handle_integration_list(
    ...
    domains = await async_get_application_credentials(menuai)
    result = {
        "domains": domains,
        "integrations": {
            domain: await _async_integration_config(menuai, domain) for domain in domains
        },
    }
