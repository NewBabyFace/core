...
async def async_remove_automations(menuai: menuai, device_id: str) -> None:
    """Remove automations for a Tasmota device."""
    await device_trigger.async_remove_triggers(menuai, device_id)
...
async def async_setup_entry(menuai: menuai, config_entry: ConfigEntry) -> None:
    ...
    async def async_device_removed(
        ...
        await async_remove_automations(menuai, event.data["device_id"])
    ...
    async def async_discover(
        ...
        if tasmota_automation.automation_type == AUTOMATION_TYPE_TRIGGER:
            await device_trigger.async_setup_trigger(
                menuai, tasmota_automation, config_entry, discovery_hash
            )

    menuai.data[DATA_REMOVE_DISCOVER_COMPONENT.format("device_automation")] = (
