...
from menuai.const import CONF_ADDRESS, CONF_PORT, Platform
...
async def async_setup_entry(menuai: menuai, entry: AuroraAbbConfigEntry) -> bool:
    """Set up Aurora ABB PowerOne from a config entry."""

    comport = entry.data[CONF_PORT]
    address = entry.data[CONF_ADDRESS]
    coordinator = AuroraAbbDataUpdateCoordinator(menuai, entry, comport, address)
...
async def async_unload_entry(menuai: menuai, entry: AuroraAbbConfigEntry) -> bool:
