...
async def async_setup_entry(menuai: menuai, entry: OverseerrConfigEntry) -> bool:
    """Set up Overseerr from a config entry."""

    coordinator = OverseerrCoordinator(menuai, entry)

    await coordinator.async_config_entry_first_refresh()

    entry.runtime_data = coordinator

    webhook_manager = OverseerrWebhookManager(menuai, entry)
...
async def async_unload_entry(menuai: menuai, entry: OverseerrConfigEntry) -> bool:
    """Unload a config entry."""
    return await menuai.config_entries.async_unload_platforms(entry, PLATFORMS)


async def async_remove_entry(menuai: menuai, entry: OverseerrConfigEntry) -> None:
...
class OverseerrWebhookManager:
    ...
    def webhook_urls(self) -> list[str]:
        ...
        urls = [
            async_generate_url(
                self.menuai, self.entry.data[CONF_WEBHOOK_ID], prefer_external=external
            )
    ...
    async def register_webhook(self) -> None:
        ...
        if cloud.async_active_subscription(self.menuai):
            ...
            url = await _async_cloudhook_generate_url(self.menuai, self.entry)
            if await self.test_and_set_webhook(url):
                return
    ...
    async def handle_webhook(
        ...
        async_dispatcher_send(menuai, EVENT_KEY, data)
        return menuaiView.json({"message": "ok"})

    async def unregister_webhook(self) -> None:
        """Unregister webhook."""
        async_unregister(self.menuai, self.entry.data[CONF_WEBHOOK_ID])
