...
from menuai.const import ATTR_ENTITY_ID, ATTR_STATE
...
async def async_handle_state_update(menuai, context, msg):
    ...
    async_dispatcher_send(menuai, f"konnected.{entity_id}.update", state)


@HANDLERS.register("temp")
async def async_handle_temp_update(menuai, context, msg):
    ...
    if entity_id:
        async_dispatcher_send(menuai, f"konnected.{entity_id}.update", temp)


@HANDLERS.register("humi")
async def async_handle_humi_update(menuai, context, msg):
    ...
    if entity_id:
        async_dispatcher_send(menuai, f"konnected.{entity_id}.update", humi)


@HANDLERS.register("addr")
async def async_handle_addr_update(menuai, context, msg):
    ...
    if entity_id := context.get(addr):
        ...
        msg["addr"] = addr
        async_dispatcher_send(menuai, SIGNAL_DS18B20_NEW, msg)
