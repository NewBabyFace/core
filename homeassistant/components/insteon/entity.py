...
class InsteonEntity(Entity):
    ...
    def device_info(self) -> DeviceInfo:
        ...
        return DeviceInfo(
            ...
            configuration_url=f"menuai://insteon/device/config/{self._insteon_device.id}",
        )
    ...
    async def async_added_to_menuai(self) -> None:
        ...
        self.async_on_remove(
            async_dispatcher_connect(self.menuai, load_signal, self._async_read_aldb)
        )
        print_signal = f"{self.entity_id}_{SIGNAL_PRINT_ALDB}"
        async_dispatcher_connect(self.menuai, print_signal, self._print_aldb)
        default_links_signal = f"{self.entity_id}_{SIGNAL_ADD_DEFAULT_LINKS}"
        async_dispatcher_connect(
            self.menuai, default_links_signal, self._async_add_default_links
        )
    ...
    async def _async_read_aldb(self, reload):
        ...
        self._print_aldb()
        async_dispatcher_send(self.menuai, SIGNAL_SAVE_DEVICES)
