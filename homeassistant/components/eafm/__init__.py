"""UK Environment Agency Flood Monitoring Integration."""

from menuai.const import Platform
from menuai.core import menuai

from .coordinator import EafmConfigEntry, EafmCoordinator

PLATFORMS = [Platform.SENSOR]


async def async_setup_entry(menuai: menuai, entry: EafmConfigEntry) -> bool:
    """Set up flood monitoring sensors for this config entry."""
    coordinator = EafmCoordinator(menuai, entry=entry)
    await coordinator.async_config_entry_first_refresh()
    entry.runtime_data = coordinator
    await menuai.config_entries.async_forward_entry_setups(entry, PLATFORMS)
    return True


async def async_unload_entry(menuai: menuai, entry: EafmConfigEntry) -> bool:
    """Unload flood monitoring sensors."""
    return await menuai.config_entries.async_unload_platforms(entry, PLATFORMS)
