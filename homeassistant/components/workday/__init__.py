...
async def _async_validate_country_and_province(
    menuai: menuai, entry: ConfigEntry, country: str | None, province: str | None
) -> None:
    ...
    try:
        with async_pause_setup(menuai, SetupPhases.WAIT_IMPORT_PACKAGES):
            ...
            await menuai.async_add_import_executor_job(country_holidays, country)
    ...
    try:
        with async_pause_setup(menuai, SetupPhases.WAIT_IMPORT_PACKAGES):
            ...
            # the holidays library and it is not thread safe to import it in parallel
            # https://github.com/python/cpython/issues/83065
            await menuai.async_add_import_executor_job(
...
async def async_setup_entry(menuai: menuai, entry: ConfigEntry) -> bool:
    ...
    await _async_validate_country_and_province(menuai, entry, country, province)
    ...
    if country and CONF_LANGUAGE not in entry.options:
        with async_pause_setup(menuai, SetupPhases.WAIT_IMPORT_PACKAGES):
            ...
            # the holidays library and it is not thread safe to import it in parallel
            # https://github.com/python/cpython/issues/83065
            cls: HolidayBase = await menuai.async_add_import_executor_job(
                partial(country_holidays, country, subdiv=province)
            )
