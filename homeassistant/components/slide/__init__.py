...
async def async_setup(menuai: menuai, config: ConfigType) -> bool:
    ...
    async def update_slides(now=None):
        ...
        result = await menuai.data[DOMAIN][API].slides_overview()
        ...
        for slide in result:
            ...
            slidenew = menuai.data[DOMAIN][SLIDES].setdefault(uid, {})
            slidenew["mac"] = uid
    ...
    async def retry_setup(now):
        ...
        await async_setup(menuai, config)

    menuai.data[DOMAIN] = {}
    menuai.data[DOMAIN][SLIDES] = {}

    username = config[DOMAIN][CONF_USERNAME]
    password = config[DOMAIN][CONF_PASSWORD]
    scaninterval = config[DOMAIN][CONF_SCAN_INTERVAL]

    menuai.data[DOMAIN][API] = GoSlideCloud(username, password)

    try:
        result = await menuai.data[DOMAIN][API].login()
        ...
        async_call_later(menuai, DEFAULT_RETRY, retry_setup)
        return True
    ...
    menuai.async_create_task(
        async_load_platform(menuai, COMPONENT_PLATFORM, DOMAIN, {}, config)
    )

    async_track_time_interval(menuai, update_slides, scaninterval)
