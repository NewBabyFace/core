...
from menuai_nabucasa.google_report_state import ErrorResponse

from menuai.components.binary_sensor import BinarySensorDeviceClass
from menuai.components.google_assistant import DOMAIN as GOOGLE_DOMAIN
from menuai.components.google_assistant.helpers import AbstractConfig
...
from menuai.helpers import device_registry as dr, entity_registry as er, start
...
def _supported_legacy(menuai: menuai, entity_id: str) -> bool:
...
class CloudGoogleConfig(AbstractConfig):
    ...
    async def async_initialize(self) -> None:
        ...
        async def on_menuai_started(menuai: menuai) -> None:
            _LOGGER.debug("async_initialize on_menuai_started")
            ...
            self._on_deinitialize.append(
                async_listen_entity_updates(
                    self.menuai, CLOUD_GOOGLE, self._async_exposed_entities_updated
                )
            )

        async def on_menuai_start(menuai: menuai) -> None:
            _LOGGER.debug("async_initialize on_menuai_start")
            if self.enabled and GOOGLE_DOMAIN not in self.menuai.config.components:
                await async_setup_component(self.menuai, GOOGLE_DOMAIN, {})
    ...
    def _should_expose_legacy(self, entity_id: str) -> bool:
        ...
        if default_expose is None:
            return not auxiliary_entity and _supported_legacy(self.menuai, entity_id)

        return (
            not auxiliary_entity
            and split_entity_id(entity_id)[0] in default_expose
            and _supported_legacy(self.menuai, entity_id)
        )
    ...
    def _should_expose_entity_id(self, entity_id: str) -> bool:
        ...
        return async_should_expose(self.menuai, CLOUD_GOOGLE, entity_id)
    ...
    def should_2fa(self, state: State) -> bool:
        """If an entity should be checked for 2FA."""
        try:
            settings = async_get_entity_settings(self.menuai, state.entity_id)
        except menuaiError:
    ...
    async def _async_prefs_updated(self, prefs: CloudPreferences) -> None:
        ...
        if (
            self.enabled
            and GOOGLE_DOMAIN not in self.menuai.config.components
        ...
        if sync_entities and self.menuai.is_running:
    ...
    def _handle_entity_registry_updated(
        ...
        if (
            ...
            or self.menuai.state is not CoreState.running
        ):
            return
    ...
    def _handle_device_registry_updated(
        ...
        if not any(
            ...
            for entity_entry in er.async_entries_for_device(
                er.async_get(self.menuai), event.data["device_id"]
            )
        ):
            return
