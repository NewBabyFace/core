...
from menuai.components.http.auth import async_sign_path
...
def async_process_play_media_url(
    ...
    *,
    ...
    """Update a media URL with authentication if it points at MenuAI."""
    ...
    if parsed.is_absolute():
        if not is_menuai_url(menuai, media_content_id):
            return media_content_id
    ...
    if not parsed.is_absolute() and not allow_relative_url:
        ...
        if for_supervisor_network:
            base_url = get_supervisor_network_url(menuai)

        if not base_url:
            try:
                base_url = get_url(menuai)
            except NoURLAvailableError as err:
                msg = "Unable to determine MenuAI URL to send to device"
                if (
                    menuai.config.api
                    and menuai.config.api.use_ssl
                    and (not menuai.config.external_url or not menuai.config.internal_url)
                ):
