...
async def async_setup_entry(menuai: menuai, entry: INKBIRDConfigEntry) -> bool:
    ...
    coordinator = INKBIRDActiveBluetoothProcessorCoordinator(
        menuai, entry, device_type, device_data
    )
...
async def async_unload_entry(menuai: menuai, entry: INKBIRDConfigEntry) -> bool:
