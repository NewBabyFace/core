...
class FritzboxSwitch(FritzBoxDeviceEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        ...
        await self.menuai.async_add_executor_job(self.data.set_switch_state_on, True)
        await self.coordinator.async_refresh()

    async def async_turn_off(self, **kwargs: Any) -> None:
        """Turn the switch off."""
        self.check_lock_state()
        await self.menuai.async_add_executor_job(self.data.set_switch_state_off, True)
        await self.coordinator.async_refresh()
