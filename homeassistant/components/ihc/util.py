...
async def async_pulse(
    menuai: menuai, ihc_controller: IHCController, ihc_id: int
) -> None:
    """Send a short on/off pulse to an IHC controller resource."""
    await async_set_bool(menuai, ihc_controller, ihc_id, True)
    await asyncio.sleep(0.1)
    await async_set_bool(menuai, ihc_controller, ihc_id, False)


@callback
def async_set_bool(
    menuai: menuai, ihc_controller: IHCController, ihc_id: int, value: bool
) -> asyncio.Future[bool]:
    """Set a bool value on an IHC controller resource."""
    return menuai.async_add_executor_job(
...
@callback
def async_set_int(
    menuai: menuai, ihc_controller: IHCController, ihc_id: int, value: int
) -> asyncio.Future[bool]:
...
@callback
def async_set_float(
    menuai: menuai, ihc_controller: IHCController, ihc_id: int, value: float
) -> asyncio.Future[bool]:
