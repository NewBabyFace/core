...
from menuai.const import CONF_CODE, CONF_NAME
...
async def async_setup_entry(menuai: menuai, entry: YaleConfigEntry) -> bool:
    ...
    coordinator = YaleDataUpdateCoordinator(menuai, entry)
...
async def update_listener(menuai: menuai, entry: YaleConfigEntry) -> None:
...
async def async_migrate_entry(menuai: menuai, entry: YaleConfigEntry) -> bool:
    """Migrate old entry."""
    ...
    if entry.version == 1:
        ...
        menuai.config_entries.async_update_entry(entry, options=new_options, version=2)
