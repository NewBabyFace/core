...
from menuai.auth.permissions.const import POLICY_CONTROL
from menuai.const import ATTR_ENTITY_ID, ENTITY_MATCH_ALL, ENTITY_MATCH_NONE
from menuai.core import menuai, ServiceCall
from menuai.exceptions import Unauthorized, UnknownUser
from menuai.helpers.dispatcher import async_dispatcher_send
from menuai.helpers.service import async_extract_entity_ids
...
def async_setup_services(menuai: menuai) -> None:
    ...
    async def async_extract_from_service(call: ServiceCall) -> list[str]:
        ...
        if call.data.get(ATTR_ENTITY_ID) == ENTITY_MATCH_ALL:
            ...
            return [
                entity_id
                for entity_id in menuai.data[DATA_AMCREST][CAMERAS]
        ...
        call_ids = await async_extract_entity_ids(menuai, call)
        entity_ids = []
        for entity_id in menuai.data[DATA_AMCREST][CAMERAS]:
    ...
    async def async_service_handler(call: ServiceCall) -> None:
        ...
        for entity_id in await async_extract_from_service(call):
            async_dispatcher_send(menuai, service_signal(call.service, entity_id), *args)

    for service, params in CAMERA_SERVICES.items():
        menuai.services.async_register(DOMAIN, service, async_service_handler, params[0])
