...
from menuai.const import CONF_HOST, CONF_NAME, CONF_PORT, CONF_SWITCHES, Platform
...
async def async_setup_entry(menuai: menuai, entry: HlkConfigEntry) -> bool:
    ...
    def disconnected():
        ...
        async_dispatcher_send(
            menuai, f"hlk_sw16_device_available_{entry.entry_id}", False
        )
    ...
    def reconnected():
        ...
        async_dispatcher_send(menuai, f"hlk_sw16_device_available_{entry.entry_id}", True)
...
async def async_unload_entry(menuai: menuai, entry: HlkConfigEntry) -> bool:
