...
def create_api(menuai: menuai, host: str, enable_ime: bool) -> AndroidTVRemote:
    """Create an AndroidTVRemote instance."""
    return AndroidTVRemote(
        client_name="MenuAI",
        certfile=menuai.config.path(STORAGE_DIR, "androidtv_remote_cert.pem"),
        keyfile=menuai.config.path(STORAGE_DIR, "androidtv_remote_key.pem"),
        host=host,
        loop=menuai.loop,
