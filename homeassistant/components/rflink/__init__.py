...
async def async_setup(menuai: menuai, config: ConfigType) -> bool:
    ...
    menuai.data[DATA_ENTITY_LOOKUP] = {
        EVENT_KEY_COMMAND: defaultdict(list),
        EVENT_KEY_SENSOR: defaultdict(list),
    }
    menuai.data[DATA_ENTITY_GROUP_LOOKUP] = {EVENT_KEY_COMMAND: defaultdict(list)}

    # Allow platform to specify function to register new unknown devices
    menuai.data[DATA_DEVICE_REGISTER] = {}
    ...
    def event_callback(event):
        ...
        if event_type not in menuai.data[DATA_ENTITY_LOOKUP]:
        ...
        if is_group_event:
            entity_ids = menuai.data[DATA_ENTITY_GROUP_LOOKUP][event_type].get(
                event_id, []
            )
        else:
            entity_ids = menuai.data[DATA_ENTITY_LOOKUP][event_type][event_id]
        ...
        if entity_ids:
            ...
            for entity in entity_ids:
                ...
                async_dispatcher_send(menuai, SIGNAL_HANDLE_EVENT.format(entity), event)
        elif not is_group_event:
            # If device is not yet known, register with platform (if loaded)
            if event_type in menuai.data[DATA_DEVICE_REGISTER]:
                ...
                menuai.data[DATA_ENTITY_LOOKUP][event_type][event_id].append(
                    TMP_ENTITY.format(event_id)
                )
                menuai.async_create_task(
                    menuai.data[DATA_DEVICE_REGISTER][event_type](event),
                    eager_start=False,
                )
            else:
    ...
    def reconnect(_: Exception | None = None) -> None:
        ...
        async_dispatcher_send(menuai, SIGNAL_AVAILABILITY, False)

        # If HA is not stopping, initiate new connection
        if menuai.state is not CoreState.stopping:
            _LOGGER.warning("Disconnected from Rflink, reconnecting")
            menuai.async_create_task(connect(), eager_start=False)

    _reconnect_job = menuaiJob(reconnect, "Rflink reconnect", cancel_on_shutdown=True)
    ...
    async def connect():
        ...
        try:
            ...
            async_call_later(menuai, reconnect_interval, _reconnect_job)
            return

        # There is a valid connection to a Rflink device now so
        # mark entities as available
        async_dispatcher_send(menuai, SIGNAL_AVAILABILITY, True)
        ...
        menuai.bus.async_listen_once(
            EVENT_menuai_STOP, lambda x: transport.close()
        )

        _LOGGER.debug("Connected to Rflink")

    menuai.async_create_task(connect(), eager_start=False)
    async_dispatcher_connect(menuai, SIGNAL_EVENT, event_callback)
    ...
    menuai.bus.async_listen(EVENT_LOGGING_CHANGED, handle_logging_changed)
