...
async def async_setup_entry(menuai: menuai, entry: AugustConfigEntry) -> bool:
    """Set up August from a config entry."""
    session = async_create_august_clientsession(menuai)
    august_gateway = AugustGateway(Path(menuai.config.config_dir), session)
    try:
        await async_setup_august(menuai, entry, august_gateway)
...
async def async_remove_entry(menuai: menuai, entry: AugustConfigEntry) -> None:
    """Remove an August config entry."""
    ir.async_delete_issue(menuai, DOMAIN, "yale_brand_migration")


async def async_unload_entry(menuai: menuai, entry: AugustConfigEntry) -> bool:
    """Unload a config entry."""
    return await menuai.config_entries.async_unload_platforms(entry, PLATFORMS)


async def async_setup_august(
    menuai: menuai, entry: AugustConfigEntry, august_gateway: AugustGateway
) -> None:
    ...
    if august_gateway.api.brand == Brand.YALE_HOME:
        _async_create_yale_brand_migration_issue(menuai, entry)
    await august_gateway.async_authenticate()
    await august_gateway.async_refresh_access_token_if_needed()
    data = entry.runtime_data = AugustData(menuai, august_gateway)
...
async def async_remove_config_entry_device(
    menuai: menuai, config_entry: AugustConfigEntry, device_entry: dr.DeviceEntry
) -> bool:
