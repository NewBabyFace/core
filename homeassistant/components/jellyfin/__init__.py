...
async def async_setup_entry(menuai: menuai, entry: JellyfinConfigEntry) -> bool:
    ...
    device_name = ascii(menuai.config.location_name)

    client = create_client(device_id=device_id, device_name=device_name)

    try:
        user_id, connect_result = await validate_input(menuai, dict(entry.data), client)
    ...
    coordinator = JellyfinDataUpdateCoordinator(
        menuai, entry, client, server_info, user_id
    )
...
async def async_unload_entry(menuai: menuai, entry: JellyfinConfigEntry) -> bool:
    """Unload a config entry."""
    return await menuai.config_entries.async_unload_platforms(entry, PLATFORMS)


async def async_remove_config_entry_device(
    menuai: menuai, config_entry: JellyfinConfigEntry, device_entry: dr.DeviceEntry
) -> bool:
