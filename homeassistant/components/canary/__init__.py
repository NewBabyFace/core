...
from menuai.const import CONF_PASSWORD, CONF_TIMEOUT, CONF_USERNAME, Platform
...
async def async_setup_entry(menuai: menuai, entry: CanaryConfigEntry) -> bool:
    ...
    try:
        canary_api = await menuai.async_add_executor_job(_get_canary_api_instance, entry)
    except (ConnectTimeout, HTTPError) as error:
        _LOGGER.error("Unable to connect to Canary service: %s", str(error))
        raise ConfigEntryNotReady from error

    coordinator = CanaryDataUpdateCoordinator(menuai, entry, api=canary_api)
...
async def async_unload_entry(menuai: menuai, entry: CanaryConfigEntry) -> bool:
    """Unload a config entry."""
    return await menuai.config_entries.async_unload_platforms(entry, PLATFORMS)


async def _async_update_listener(menuai: menuai, entry: CanaryConfigEntry) -> None:
