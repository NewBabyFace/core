...
def _get_device_trigger_data(menuai: menuai, device_id: str) -> tuple[str, dict]:
    ...
    return get_zha_data(menuai).device_trigger_cache[device_id]
...
async def async_validate_trigger_config(
    ...
    _, triggers = _get_device_trigger_data(menuai, config[CONF_DEVICE_ID])
...
async def async_attach_trigger(
    ...
    try:
        ieee, triggers = _get_device_trigger_data(menuai, config[CONF_DEVICE_ID])
    ...
    if trigger_key not in triggers:
        raise menuaiError(f"Unable to find trigger {trigger_key}")
...
async def async_get_triggers(
    ...
    """
    try:
        _, triggers = _get_device_trigger_data(menuai, device_id)
    except KeyError as err:
