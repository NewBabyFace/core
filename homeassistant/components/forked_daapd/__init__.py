...
async def async_setup_entry(menuai: menuai, entry: ForkedDaapdConfigEntry) -> bool:
    """Set up forked-daapd from a config entry by forwarding to platform."""
    host: str = entry.data[CONF_HOST]
    port: int = entry.data[CONF_PORT]
    password: str = entry.data[CONF_PASSWORD]
    forked_daapd_api = ForkedDaapdAPI(
        async_get_clientsession(menuai), host, port, password
    )
    forked_daapd_updater = ForkedDaapdUpdater(menuai, forked_daapd_api, entry.entry_id)
...
async def async_unload_entry(
    menuai: menuai, entry: ForkedDaapdConfigEntry
) -> bool:
    """Remove forked-daapd component."""
    status = await menuai.config_entries.async_unload_platforms(entry, PLATFORMS)
    if status:
