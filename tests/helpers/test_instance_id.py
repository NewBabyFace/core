...
async def test_get_id_empty(menuai: menuai, menuai_storage: dict[str, Any]) -> None:
    """Get unique ID."""
    uuid = await instance_id.async_get(menuai)
    assert uuid is not None
    # Assert it's stored
    assert menuai_storage["core.uuid"]["data"]["uuid"] == uuid
...
async def test_get_id_load_fail(
    ...
    menuai_storage["core.uuid"] = None  # Invalid, will make store.async_load raise
    ...
    assert (
        "Could not read menuai instance ID from 'core.uuid' or '.uuid', a "
...
async def test_get_id_migrate(
    ...
    with (
        patch("menuai.util.json.load_json", return_value={"uuid": "1234"}),
