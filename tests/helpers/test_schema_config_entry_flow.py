...
async def test_name(menuai: menuai, entity_registry: er.EntityRegistry) -> None:
    ...
    assert wrapped_entity_config_entry_title(menuai, entity_id) == "ceiling"

    # State set, use name from state
    menuai.states.async_set(entity_id, "on", {"friendly_name": "State Name"})
    assert wrapped_entity_config_entry_title(menuai, entity_id) == "State Name"
    ...
    assert wrapped_entity_config_entry_title(menuai, entity_id) == "Original Name"
    assert wrapped_entity_config_entry_title(menuai, entry.id) == "Original Name"

    # Entity has customized name
    entity_registry.async_update_entity("switch.ceiling", name="Custom Name")
    assert wrapped_entity_config_entry_title(menuai, entity_id) == "Custom Name"
    assert wrapped_entity_config_entry_title(menuai, entry.id) == "Custom Name"


@pytest.mark.parametrize("marker", [vol.Required, vol.Optional])
async def test_config_flow_advanced_option(
    menuai: menuai, manager: data_entry_flow.FlowManager, marker
) -> None:
...
async def test_menu_step(menuai: menuai) -> None:
    """Test menu step."""
...
async def test_schema_none(menuai: menuai) -> None:
...
async def test_last_step(menuai: menuai) -> None:
    """Test SchemaFlowFormStep with schema set to None."""
...
async def test_next_step_function(menuai: menuai) -> None:
...
async def test_suggested_values(
    menuai: menuai, manager: data_entry_flow.FlowManager
) -> None:
...
async def test_options_flow_state(menuai: menuai) -> None:
    ...
    options_handler = menuai.config_entries.options._progress[result["flow_id"]]
