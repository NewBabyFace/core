...
from menuai.helpers.check_config import (
    CheckConfigError,
    menuaiConfig,
...
async def test_bad_core_config(menuai: menuai) -> None:
    """Test a bad core config setup."""
    files = {YAML_CONFIG_FILE: BAD_CORE_CONFIG}
    with patch("os.path.isfile", return_value=True), patch_yaml_files(files):
        res = await async_check_ha_config_file(menuai)
        log_ha_config(res)

        error = CheckConfigError(
            (
                f"Invalid config for 'menuai' at {YAML_CONFIG_FILE}, line 2:"
...
async def test_config_platform_valid(menuai: menuai) -> None:
    ...
    with patch("os.path.isfile", return_value=True), patch_yaml_files(files):
        ...
        assert res.keys() == {"menuai", "light"}
        assert res["light"] == [{"platform": "demo"}]
        _assert_warnings_errors(res, [], [])


async def test_integration_not_found(menuai: menuai) -> None:
...
async def test_integrationt_requirement_not_found(menuai: menuai) -> None:
    ...
    with (
        patch(
            "menuai.helpers.check_config.async_get_integration_with_requirements",
...
async def test_integration_not_found_recovery_mode(menuai: menuai) -> None:
...
async def test_integration_not_found_safe_mode(menuai: menuai) -> None:
...
async def test_integration_import_error(menuai: menuai) -> None:
    """Test errors if integration with a requirement not found not found."""
    # Make sure they don't exist
    files = {YAML_CONFIG_FILE: BASE_CONFIG + "light:"}
    with (
        patch(
            "menuai.loader.Integration.async_get_component",
...
async def test_integration_schema_error(
    menuai: menuai, integration: str, errors: int, warnings: int, message: str
) -> None:
...
async def test_platform_not_found(menuai: menuai) -> None:
...
async def test_platform_not_found_recovery_mode(menuai: menuai) -> None:
...
async def test_platform_not_found_safe_mode(menuai: menuai) -> None:
...
async def test_config_platform_import_error(menuai: menuai) -> None:
    """Test errors if config platform fails to import."""
    # Make sure they don't exist
    files = {YAML_CONFIG_FILE: BASE_CONFIG + "light:\n  platform: beer"}
    with (
        patch(
            "menuai.loader.Integration.async_get_platform",
            side_effect=ImportError("blablabla"),
        ),
        patch("os.path.isfile", return_value=True),
        patch("menuai.loader.Integration.platforms_exists", return_value=True),
        patch_yaml_files(files),
    ):
        res = await async_check_ha_config_file(menuai)
        log_ha_config(res)
...
async def test_platform_import_error(menuai: menuai) -> None:
    """Test errors if platform not found."""
    # Make sure they don't exist
    files = {YAML_CONFIG_FILE: BASE_CONFIG + "light:\n  platform: demo"}
    with (
...
async def test_package_invalid(menuai: menuai) -> None:
    ...
    with patch("os.path.isfile", return_value=True), patch_yaml_files(files):
        ...
        warning = CheckConfigError(
            ...
            "menuai.packages.p1.group",
            {"group": ["a"]},
        )
        _assert_warnings_errors(res, [warning], [])


async def test_package_definition_invalid_slug_keys(menuai: menuai) -> None:
    ...
    with patch("os.path.isfile", return_value=True), patch_yaml_files(files):
        ...
        warning = CheckConfigError(
            ...
            "menuai.packages.not a slug",
            {"group": ["a"]},
        )
        _assert_warnings_errors(res, [warning], [])


async def test_package_definition_invalid_dict(menuai: menuai) -> None:
    ...
    with patch("os.path.isfile", return_value=True), patch_yaml_files(files):
        ...
        warning = CheckConfigError(
            ...
            "menuai.packages.not_a_dict",
            [{"group": ["a"]}],
        )
        _assert_warnings_errors(res, [warning], [])


async def test_package_schema_invalid(menuai: menuai) -> None:
    """Test an invalid platform config because of severely broken packages section."""
    files = {
        YAML_CONFIG_FILE: "menuai:\n  packages:\n    - must\n    - not\n    - be\n    - a\n    - list"
    }
    with patch("os.path.isfile", return_value=True), patch_yaml_files(files):
        res = await async_check_ha_config_file(menuai)
        log_ha_config(res)
...
async def test_missing_included_file(menuai: menuai) -> None:
...
async def test_automation_config_platform(menuai: menuai) -> None:
...
async def test_removed_yaml_support(menuai: menuai) -> None:
