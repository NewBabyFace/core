...
@pytest.fixture
def crd(menuai: menuai) -> update_coordinator.DataUpdateCoordinator[int]:
    """Coordinator mock with default update interval."""
    return get_crd(menuai, DEFAULT_UPDATE_INTERVAL)
...
async def test_shutdown(
    menuai: menuai, crd: update_coordinator.DataUpdateCoordinator[int]
) -> None:
    ...
    async_fire_time_changed(menuai, utcnow() + crd.update_interval)
...
async def test_shutdown_on_menuai_stop(
    menuai: menuai, crd: update_coordinator.DataUpdateCoordinator[int]
) -> None:
...
async def test_coordinator_entity(
    ...
    with patch(
        "menuai.helpers.entity.Entity.async_on_remove"
    ...
    crd.last_update_success = False
    with patch("menuai.helpers.entity.Entity.enabled", False):
        await entity.async_update()
    assert entity.available is False
...
async def test_stop_refresh_on_ha_stop(
    ...
    """Test no update interval refresh when MenuAI is stopping."""
    # Add subscriber
    update_callback = Mock()
...
async def test_async_config_entry_first_refresh_failure(
    ...
    entry._async_set_state(
        menuai, config_entries.ConfigEntryState.SETUP_IN_PROGRESS, None
    )
    crd = get_crd(menuai, DEFAULT_UPDATE_INTERVAL, entry)
...
async def test_async_config_entry_first_refresh_success(menuai: menuai) -> None:
...
async def test_async_config_entry_first_refresh_invalid_state_in_integration(
    ...
    assert (
        ...
        "menuai/components/hue/light.py, line 23: self.light.is_on. "
        "This will stop working in MenuAI 2025.11"
    ) in caplog.text


async def test_async_config_entry_first_refresh_no_entry(menuai: menuai) -> None:
    """Test first refresh successfully."""
    crd = get_crd(menuai, DEFAULT_UPDATE_INTERVAL, None)
    crd.setup_method = AsyncMock()
    with pytest.raises(
        RuntimeError,
...
async def test_timestamp_date_update_coordinator(menuai: menuai) -> None:
...
async def test_config_entry(menuai: menuai) -> None:
    """Test behavior of coordinator.entry."""
    entry = MockConfigEntry()

    # Default without context should be None
    crd = update_coordinator.DataUpdateCoordinator[int](menuai, _LOGGER, name="test")
    assert crd.config_entry is None

    # Explicit None is OK
    crd = update_coordinator.DataUpdateCoordinator[int](
        menuai, _LOGGER, name="test", config_entry=None
    )
    assert crd.config_entry is None

    # Explicit entry is OK
    crd = update_coordinator.DataUpdateCoordinator[int](
        menuai, _LOGGER, name="test", config_entry=entry
    )
    ...
    crd = update_coordinator.DataUpdateCoordinator[int](
        menuai, _LOGGER, name="test", config_entry=another_entry
    )
    assert crd.config_entry is another_entry


async def test_listener_unsubscribe_releases_coordinator(menuai: menuai) -> None:
