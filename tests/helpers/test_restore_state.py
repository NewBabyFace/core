...
from menuai.helpers.reload import async_get_platform_without_config_entry
from menuai.helpers.restore_state import (
    DATA_RESTORE_STATE,
    STORAGE_KEY,
    RestoreEntity,
    RestoreStateData,
...
async def test_caching_data(menuai: menuai) -> None:
    ...
    menuai.data.pop(DATA_RESTORE_STATE)

    with (
        patch(
            "menuai.helpers.restore_state.Store.async_load",
            side_effect=menuaiError,
        ),
        patch("menuai.helpers.restore_state.Store.async_save"),
    ):
        # Failure to load should not be treated as fatal
        await async_load(menuai)
    ...
    with patch(
        "menuai.helpers.restore_state.Store.async_save"
    ) as mock_write_data:
        await async_load(menuai)
        await menuai.async_block_till_done()
...
async def test_periodic_write(menuai: menuai) -> None:
    ...
    with patch(
        ...
        async_fire_time_changed(menuai, dt_util.utcnow() + timedelta(minutes=30))
        await menuai.async_block_till_done()
...
async def test_save_persistent_states(menuai: menuai) -> None:
    ...
    with patch(
        ...
        await RestoreStateData.async_save_persistent_states(menuai)
        await menuai.async_block_till_done()
...
async def test_menuai_starting(menuai: menuai) -> None:
    """Test that we cache data."""
    ...
    menuai.states.async_set("input_boolean.b1", "on")
    ...
    assert state.entity_id == "input_boolean.b1"
    assert state.state == "on"
    menuai.states.async_remove("input_boolean.b1")

    # Assert that no data was written yet, since menuai is still starting.
    assert not mock_write_data.called

    # Finish menuai startup
    with patch(
        "menuai.helpers.restore_state.Store.async_save"
    ) as mock_write_data:
...
async def test_dump_data(menuai: menuai) -> None:
    """Test that we cache data."""
    states = [
    ...
    platform = MockEntityPlatform(menuai, domain="input_boolean")
    entity = Entity()
    entity.menuai = menuai
    ...
    for state in states:
        menuai.states.async_remove(state.entity_id)
...
async def test_dump_error(menuai: menuai) -> None:
    """Test that we cache data."""
    states = [
        State("input_boolean.b0", "on"),
        State("input_boolean.b1", "on"),
...
async def test_load_error(menuai: menuai) -> None:
    """Test that we cache data."""
    entity = RestoreEntity()
    entity.menuai = menuai
    entity.entity_id = "input_boolean.b1"
    ...
    with patch(
        "menuai.helpers.storage.Store.async_load",
        side_effect=menuaiError,
    ):
        state = await entity.async_get_last_state()
...
async def test_state_saved_on_remove(menuai: menuai) -> None:
...
async def test_restore_entity_end_to_end(
    ...
    class MockRestoreEntity(RestoreEntity):
        ...
        async def async_added_to_menuai(self) -> Coroutine[Any, Any, None]:
    ...
    assert menuai.states.get(entity_id).state == "stored"

    await data.async_dump_states()
    await menuai.async_block_till_done()

    storage_data = menuai_storage[STORAGE_KEY]["data"]
