...
from menuai.helpers import importlib

from tests.common import MockModule


async def test_async_import_module(menuai: menuai) -> None:
    """Test importing a module."""
    mock_module = MockModule()
    with patch(
        "menuai.helpers.importlib.importlib.import_module",
        return_value=mock_module,
    ):
        module = await importlib.async_import_module(menuai, "test.module")

    assert module is mock_module


async def test_async_import_module_on_helper(menuai: menuai) -> None:
    """Test importing the importlib helper."""
    module = await importlib.async_import_module(
        menuai, "menuai.helpers.importlib"
    )
...
async def test_async_import_module_failures(menuai: menuai) -> None:
    ...
    with (
        ...
        await importlib.async_import_module(menuai, "test.module")
    ...
    with (
        ...
        assert await importlib.async_import_module(menuai, "test.module") is mock_module
...
async def test_async_import_module_concurrency(
    menuai: menuai, eager_start: bool
) -> None:
    ...
    with patch(
        ...
        task1 = menuai.async_create_task(
            importlib.async_import_module(menuai, "test.module"), eager_start=eager_start
        )
