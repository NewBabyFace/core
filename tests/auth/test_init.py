...
from menuai.auth.const import GROUP_ID_ADMIN, MFA_SESSION_EXPIRATION
...
@pytest.fixture
def mock_menuai(menuai: menuai) -> menuai:
    """MenuAI mock with minimum amount of data set to make it work with auth."""
    return menuai


async def test_auth_manager_from_config_validates_config(mock_menuai) -> None:
...
async def test_auth_manager_from_config_auth_modules(mock_menuai) -> None:
...
async def test_create_new_user(menuai: menuai) -> None:
    ...
    menuai.bus.async_listen("user_added", user_added)
...
async def test_login_as_existing_user(mock_menuai) -> None:
...
async def test_saving_loading(
    ...
    store2 = auth_store.AuthStore(menuai)
...
async def test_cannot_retrieve_expired_access_token(menuai: menuai) -> None:
    """Test that we cannot retrieve expired access tokens."""
    manager = await auth.auth_manager_from_config(menuai, [], [])
...
async def test_generating_system_user(menuai: menuai) -> None:
    ...
    manager = await auth.auth_manager_from_config(menuai, [], [])
    user = await manager.async_create_system_user("menuai.io")
    ...
    user = await manager.async_create_system_user(
        "menuai.io", group_ids=[GROUP_ID_ADMIN], local_only=True
    )
    token = await manager.async_create_refresh_token(user)
    assert user.system_generated
...
async def test_refresh_token_requires_client_for_user(menuai: menuai) -> None:
...
async def test_refresh_token_type(menuai: menuai) -> None:
...
async def test_refresh_token_type_long_lived_access_token(menuai: menuai) -> None:
...
async def test_refresh_token_provider_validation(mock_menuai) -> None:
...
async def test_cannot_deactive_owner(mock_menuai) -> None:
    """Test that we cannot deactivate the owner."""
    manager = await auth.auth_manager_from_config(mock_menuai, [], [])
...
async def test_remove_refresh_token(menuai: menuai) -> None:
...
async def test_remove_expired_refresh_token(menuai: menuai) -> None:
    ...
    with freeze_time(now + timedelta(days=30)):
        async_fire_time_changed(menuai, now + timedelta(days=30))
    ...
    with freeze_time(now + timedelta(days=89, hours=23)):
        async_fire_time_changed(menuai, now + timedelta(days=89, hours=23))
        await menuai.async_block_till_done()
    ...
    with freeze_time(now + timedelta(days=90, seconds=5)):
        async_fire_time_changed(menuai, now + timedelta(days=90, seconds=5))
        await menuai.async_block_till_done()
        assert manager.async_get_refresh_token(refresh_token1.id) is None
        assert manager.async_get_refresh_token(refresh_token2.id)

    with freeze_time(now + timedelta(days=120, seconds=5)):
        async_fire_time_changed(menuai, now + timedelta(days=120, seconds=5))
        await menuai.async_block_till_done()
        assert manager.async_get_refresh_token(refresh_token1.id) is None
        assert manager.async_get_refresh_token(refresh_token2.id) is None


async def test_update_expire_at_refresh_token(menuai: menuai) -> None:
...
async def test_register_revoke_token_callback(mock_menuai) -> None:
...
async def test_unregister_revoke_token_callback(mock_menuai) -> None:
...
async def test_create_access_token(mock_menuai) -> None:
...
async def test_create_long_lived_access_token(mock_menuai) -> None:
...
async def test_one_long_lived_access_token_per_refresh_token(mock_menuai) -> None:
...
async def test_login_with_auth_module(mock_menuai) -> None:
...
async def test_login_with_multi_auth_module(mock_menuai) -> None:
...
async def test_auth_module_expired_session(mock_menuai) -> None:
    """Test login as existing user."""
    manager = await auth.auth_manager_from_config(
        mock_menuai,
        [
...
async def test_async_remove_user(menuai: menuai) -> None:
    """Test removing a user."""
    events = async_capture_events(menuai, "user_removed")
    manager = await auth.auth_manager_from_config(
        menuai,
        [
            {
...
async def test_async_remove_user_fail_if_remove_credential_fails(
    ...
    with (
        patch.object(menuai.auth, "async_remove_credentials", side_effect=ValueError),
        pytest.raises(ValueError),
    ):
        await menuai.auth.async_remove_user(menuai_admin_user)


async def test_new_users(mock_menuai) -> None:
...
async def test_rename_does_not_change_refresh_token(mock_menuai) -> None:
...
async def test_event_user_updated_fires(menuai: menuai) -> None:
    ...
    events = async_capture_events(menuai, EVENT_USER_UPDATED)
...
async def test_access_token_with_invalid_signature(mock_menuai) -> None:
...
async def test_access_token_with_null_signature(mock_menuai) -> None:
...
async def test_access_token_with_empty_signature(mock_menuai) -> None:
...
async def test_access_token_with_empty_key(mock_menuai) -> None:
...
async def test_reject_access_token_with_impossible_large_size(mock_menuai) -> None:
    """Test rejecting access tokens with impossible sizes."""
    manager = await auth.auth_manager_from_config(mock_menuai, [], [])
    assert manager.async_validate_access_token("a" * 10000) is None


async def test_reject_token_with_invalid_json_payload(mock_menuai) -> None:
...
async def test_reject_token_with_not_dict_json_payload(mock_menuai) -> None:
...
async def test_access_token_that_expires_soon(mock_menuai) -> None:
...
async def test_access_token_from_the_future(mock_menuai) -> None:
