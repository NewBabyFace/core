...
from menuai.components.mopeka.const import DOMAIN
...
async def test_sensors_unusable_signal(menuai: menuai) -> None:
    ...
    inject_bluetooth_service_info(menuai, PRO_UNUSABLE_SIGNAL_SERVICE_INFO)
    await menuai.async_block_till_done()
    assert len(menuai.states.async_all("sensor")) == 4

    temp_sensor = menuai.states.get("sensor.pro_plus_eeff_temperature")
    ...
    tank_sensor = menuai.states.get("sensor.pro_plus_eeff_tank_level")
...
async def test_sensors_poor_signal(menuai: menuai) -> None:
    ...
    assert len(menuai.states.async_all("sensor")) == 0
    inject_bluetooth_service_info(menuai, PRO_SERVICE_INFO)
    await menuai.async_block_till_done()
    assert len(menuai.states.async_all("sensor")) == 4
...
async def test_sensors_good_signal(menuai: menuai) -> None:
    ...
    assert len(menuai.states.async_all("sensor")) == 0
    inject_bluetooth_service_info(menuai, PRO_GOOD_SIGNAL_SERVICE_INFO)
    await menuai.async_block_till_done()
    assert len(menuai.states.async_all("sensor")) == 4
