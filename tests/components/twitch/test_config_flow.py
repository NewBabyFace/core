...
from menuai.components.twitch.const import (
    CONF_CHANNELS,
    DOMAIN,
    OAUTH2_AUTHORIZE,
)
...
async def test_full_flow(
    ...
    await _do_get_token(menuai, result, menuai_client_no_auth, scopes)
    ...
    assert result["options"] == {CONF_CHANNELS: ["internetofthings", "menuai"]}
...
async def test_reauth_from_import(
    ...
    assert entry.options == {CONF_CHANNELS: ["internetofthings", "menuai"]}
...
async def test_reauth_wrong_account(
    ...
    twitch_mock.return_value.get_users = lambda *args, **kwargs: get_generator(
        menuai, "get_users_2.json", TwitchUser
    )
    result = await config_entry.start_reauth_flow(menuai)
    assert result["type"] is FlowResultType.FORM
    assert result["step_id"] == "reauth_confirm"
