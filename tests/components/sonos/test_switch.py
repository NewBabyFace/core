...
from menuai.components.sonos.const import DATA_SONOS_DISCOVERY_MANAGER
from menuai.components.sonos.switch import (
    ATTR_DURATION,
    ATTR_ID,
...
from menuai.const import ATTR_TIME, STATE_OFF, STATE_ON
...
async def test_entity_registry(
    menuai: menuai, async_autosetup_sonos, entity_registry: er.EntityRegistry
) -> None:
...
async def test_switch_attributes(
    ...
    alarm_state = menuai.states.get(alarm.entity_id)
    ...
    surround_music_full_volume_state = menuai.states.get(
    ...
    with patch.object(menuai.data[DATA_SONOS_DISCOVERY_MANAGER], "async_shutdown") as m:
    ...
    status_light_state = menuai.states.get(status_light.entity_id)
    assert status_light_state.state == STATE_ON

    touch_controls = entity_registry.entities["switch.zone_a_touch_controls"]
    touch_controls_state = menuai.states.get(touch_controls.entity_id)
