...
async def test_diagnostics_v1(
    menuai: menuai, menuai_client: ClientSessionGenerator, mock_bridge_v1: Mock
) -> None:
    """Test diagnostics v1."""
    await setup_platform(menuai, mock_bridge_v1, [])
    config_entry = menuai.config_entries.async_entries("hue")[0]
...
async def test_diagnostics_v2(
    ...
    await setup_platform(menuai, mock_bridge_v2, [])
    config_entry = menuai.config_entries.async_entries("hue")[0]
    result = await get_diagnostics_for_config_entry(menuai, menuai_client, config_entry)
