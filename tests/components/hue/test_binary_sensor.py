...
async def test_binary_sensors(
    ...
    await setup_platform(menuai, mock_bridge_v2, Platform.BINARY_SENSOR)
    ...
    # test motion sensor
    sensor = menuai.states.get("binary_sensor.hue_motion_sensor_motion")
    ...
    sensor = menuai.states.get("binary_sensor.entertainmentroom_1")
    assert sensor is not None
    assert sensor.state == "off"
    ...
    # test contact sensor
    sensor = menuai.states.get("binary_sensor.test_contact_sensor_opening")
    assert sensor is not None
    assert sensor.state == "off"
    ...
    # test tamper sensor
    sensor = menuai.states.get("binary_sensor.test_contact_sensor_tamper")
    assert sensor is not None
    assert sensor.state == "off"
    ...
    sensor = menuai.states.get("binary_sensor.test_camera_motion")
    assert sensor is not None
...
async def test_binary_sensor_add_update(
    menuai: menuai, mock_bridge_v2: Mock
) -> None:
