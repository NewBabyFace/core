...
async def test_switch(
    ...
    await setup_platform(menuai, mock_bridge_v2, Platform.SWITCH)
    ...
    test_entity = menuai.states.get("switch.hue_motion_sensor_motion_sensor_enabled")
    ...
    test_entity = menuai.states.get("switch.automation_timer_test")
    assert test_entity is not None
...
async def test_switch_turn_off_service(
    ...
    assert menuai.states.get(test_entity_id).state == "on"
...
async def test_switch_added(menuai: menuai, mock_bridge_v2: Mock) -> None:
