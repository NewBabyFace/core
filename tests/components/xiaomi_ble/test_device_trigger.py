...
from menuai.components.xiaomi_ble.const import CONF_SUBTYPE, DOMAIN
...
async def _async_setup_xiaomi_device(
    menuai: menuai, mac: str, data: Any | None = None
):
...
async def test_event_button_press(menuai: menuai) -> None:
    """Make sure that a button press event is fired."""
    mac = "54:EF:44:E3:9C:BC"
    data = {"bindkey": "5b51a7c91cde6707c9ef18dfda143a58"}
    entry = await _async_setup_xiaomi_device(menuai, mac, data)
    events = async_capture_events(menuai, "xiaomi_ble_event")
...
async def test_event_unlock_outside_the_door(menuai: menuai) -> None:
    """Make sure that a unlock outside the door event is fired."""
    mac = "D7:1F:44:EB:8A:91"
    entry = await _async_setup_xiaomi_device(menuai, mac)
    events = async_capture_events(menuai, "xiaomi_ble_event")
...
async def test_event_successful_fingerprint_match_the_door(menuai: menuai) -> None:
...
async def test_event_motion_detected(menuai: menuai) -> None:
...
async def test_event_dimmer_rotate(menuai: menuai) -> None:
...
async def test_triggers_for_invalid__model(
    ...
    triggers = await async_get_device_automations(
        menuai, DeviceAutomationType.TRIGGER, invalid_model_id
    )
    assert triggers == []
