...
async def test_switch_state(
    ...
    assert menuai.states.get("switch.test_switch_switch_1").state is not STATE_ON
    ...
    assert menuai.states.get("switch.test_switch_switch_1").state is STATE_ON
...
async def test_switch_turn_off(
    ...
    assert menuai.states.get("switch.test_switch_watchdog").state is STATE_ON
    await menuai.services.async_call(
        SWITCH_DOMAIN,
        SERVICE_TURN_OFF,
...
async def test_switch_device_class(
    ...
    assert (
        menuai.states.get("switch.test_switch_switch_1").attributes["device_class"]
        == SwitchDeviceClass.OUTLET
    )
    assert (
        menuai.states.get("switch.test_switch_watchdog").attributes["device_class"]
...
async def test_switch_no_name(
    ...
    assert (
        menuai.states.get("switch.test_switch_single").attributes["friendly_name"]
        == "Test Switch Single"
    )
...
async def test_switch_snapshot(
    ...
    mock_homee.nodes = [build_mock_node("switches.json")]
    mock_homee.get_node_by_id.return_value = mock_homee.nodes[0]
    with patch("menuai.components.homee.PLATFORMS", [Platform.SWITCH]):
        await setup_integration(menuai, mock_config_entry)
