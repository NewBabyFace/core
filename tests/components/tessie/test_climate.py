...
from menuai.components.tessie.const import TessieClimateKeeper
...
async def test_climate(
    ...
    with patch(
        "menuai.components.tessie.climate.start_climate_preconditioning",
    ...
    with (
        patch(
            "menuai.components.tessie.climate.set_temperature",
            return_value=TEST_RESPONSE,
        ) as mock_set,
        patch(
            "menuai.components.tessie.climate.start_climate_preconditioning",
    ...
    with patch(
        "menuai.components.tessie.climate.set_climate_keeper_mode",
        return_value=TEST_RESPONSE,
    ) as mock_set:
        await menuai.services.async_call(
            CLIMATE_DOMAIN,
    ...
    with patch(
        "menuai.components.tessie.climate.stop_climate",
        return_value=TEST_RESPONSE,
    ) as mock_set:
        await menuai.services.async_call(
            CLIMATE_DOMAIN,
