...
from menuai.components.tessie.coordinator import (
...
async def test_coordinator_online(
    menuai: menuai, mock_get_state, mock_get_status, freezer: FrozenDateTimeFactory
) -> None:
    """Tests that the coordinator handles online vehicles."""

    await setup_platform(menuai, PLATFORMS)
    ...
    assert menuai.states.get("binary_sensor.test_status").state == STATE_ON


async def test_coordinator_asleep(
    menuai: menuai, mock_get_status, freezer: FrozenDateTimeFactory
) -> None:
    ...
    await menuai.async_block_till_done()
    mock_get_status.assert_called_once()
    assert menuai.states.get("binary_sensor.test_status").state == STATE_OFF
...
async def test_coordinator_clienterror(
    ...
    await menuai.async_block_till_done()
    mock_get_status.assert_called_once()
    assert menuai.states.get("binary_sensor.test_status").state == STATE_UNAVAILABLE
...
async def test_coordinator_live_error(
    menuai: menuai, mock_live_status, freezer: FrozenDateTimeFactory
) -> None:
    ...
    assert menuai.states.get("sensor.energy_site_solar_power").state == STATE_UNAVAILABLE


async def test_coordinator_info_error(
    menuai: menuai, mock_site_info, freezer: FrozenDateTimeFactory
) -> None:
    ...
    assert (
        menuai.states.get("sensor.energy_site_vpp_backup_reserve").state
        == STATE_UNAVAILABLE
    )


async def test_coordinator_live_reauth(menuai: menuai, mock_live_status) -> None:
...
async def test_coordinator_info_reauth(menuai: menuai, mock_site_info) -> None:
