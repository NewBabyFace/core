...
from menuai.components.stream.core import StreamOutput
from menuai.components.stream.worker import StreamState

from .common import generate_h264_video, stream_teardown

_LOGGER = logging.getLogger(__name__)

TEST_TIMEOUT = 7.0  # Lower than 9s MenuAI timeout
...
def stream_worker_sync() -> Generator[WorkerSync]:
    ...
    sync = WorkerSync()
    with patch(
        "menuai.components.stream.worker.StreamState.discontinuity",
...
def hls_sync():
    ...
    with (
        patch(
            "menuai.components.stream.core.StreamOutput.recv",
            side_effect=sync.recv,
            autospec=True,
        ),
        patch(
            "menuai.components.stream.core.StreamOutput.part_recv",
            side_effect=sync.part_recv,
            autospec=True,
        ),
        patch(
            "menuai.components.stream.hls.web.HTTPBadRequest",
            side_effect=sync.bad_request,
        ),
        patch(
            "menuai.components.stream.hls.web.HTTPNotFound",
            side_effect=sync.not_found,
        ),
        patch(
            "menuai.components.stream.hls.web.Response",
            new=sync.response,
        ),
    ):
        yield sync
...
def should_retry() -> Generator[Mock]:
    ...
    with patch(
        "menuai.components.stream._should_retry", return_value=False
