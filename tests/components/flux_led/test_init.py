...
async def test_configuring_flux_led_causes_discovery(menuai: menuai) -> None:
    """Test that specifying empty config does discovery."""
    with (
        patch(
            "menuai.components.flux_led.discovery.AIOBulbScanner.async_scan"
        ) as scan,
        patch(
            "menuai.components.flux_led.discovery.AIOBulbScanner.getBulbInfo"
        ) as discover,
    ):
        ...
        async_fire_time_changed(menuai, utcnow() + flux_led.DISCOVERY_INTERVAL)
        await menuai.async_block_till_done()
...
async def test_config_entry_retry_right_away_on_discovery(menuai: menuai) -> None:
...
async def test_coordinator_retry_right_away_on_discovery_already_setup(
    ...
    async_fire_time_changed(menuai, now + timedelta(seconds=50))
    await menuai.async_block_till_done()
    state = menuai.states.get(entity_id)
...
async def test_config_entry_fills_unique_id_with_directed_discovery(
    menuai: menuai, discovery: dict[str, str], title: str
) -> None:
...
async def test_time_sync_startup_and_next_day(menuai: menuai) -> None:
...
async def test_name_removed_when_it_matches_entry_title(menuai: menuai) -> None:
...
async def test_entry_is_reloaded_when_title_changes(menuai: menuai) -> None:
    ...
    with _patch_discovery(), _patch_wifibulb():
        ...
        menuai.config_entries.async_update_entry(config_entry, title="Shop Light")
        assert config_entry.title == "Shop Light"
        await menuai.async_block_till_done()

    assert (
        menuai.states.get("light.bulb_rgbcw_ddeeff").attributes[ATTR_FRIENDLY_NAME]
        == "Shop Light"
    )
