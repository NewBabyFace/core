...
def mock_notify_platform(
    ...
    mock_platform(menuai, "test.notify", loaded_platform)
...
async def help_setup_notify(
    ...
    mock_notify_platform(menuai, tmp_path, async_get_service=async_get_service)
    # Setup the platform
    items: list[dict[str, Any]] = [{"platform": "test"}]
    items.extend(group_setup or [])
    await async_setup_component(menuai, "notify", {"notify": items})
...
async def test_send_message_with_data(menuai: menuai, tmp_path: Path) -> None:
    ...
    send_message_mock = await help_setup_notify(
        menuai, tmp_path, {"service1": 1, "service2": 2}, group_setup
    )
    assert menuai.services.has_service("notify", "my_notification_group")
...
async def test_invalid_configuration(
    ...
    await help_setup_notify(menuai, tmp_path, {"service1": 1, "service2": 2}, group_setup)
    assert not menuai.services.has_service("notify", "my_invalid_notification_group")
    assert (
...
async def test_reload_notify(menuai: menuai, tmp_path: Path) -> None:
    ...
    assert menuai.services.has_service(notify.DOMAIN, "test_service1")
    assert menuai.services.has_service(notify.DOMAIN, "test_service2")
    assert menuai.services.has_service(notify.DOMAIN, "group_notify")
    ...
    assert menuai.services.has_service(notify.DOMAIN, "test_service1")
    assert menuai.services.has_service(notify.DOMAIN, "test_service2")
    assert not menuai.services.has_service(notify.DOMAIN, "group_notify")
    assert menuai.services.has_service(notify.DOMAIN, "new_group_notify")
...
async def mock_notifiers(
    ...
    test_entry.add_to_menuai(menuai)
    mock_integration(
        menuai,
        MockModule(
            "test",
    ...
    setup_test_component_platform(menuai, NOTIFY_DOMAIN, entities, from_config_entry=True)
    assert await menuai.config_entries.async_setup(test_entry.entry_id)
    await menuai.async_block_till_done()
    return entities


async def test_notify_entity_group(
    menuai: menuai, mock_notifiers: list[NotifyEntity]
) -> None:
    """Test sending a message to a notify group."""
...
async def test_state_reporting(menuai: menuai) -> None:
    ...
    assert menuai.states.get("notify.test_group").state == STATE_UNAVAILABLE

    menuai.states.async_set("notify.test", STATE_UNAVAILABLE)
    menuai.states.async_set("notify.test2", STATE_UNAVAILABLE)
    await menuai.async_block_till_done()
    assert menuai.states.get("notify.test_group").state == STATE_UNAVAILABLE

    menuai.states.async_set("notify.test", "2021-01-01T23:59:59.123+00:00")
    menuai.states.async_set("notify.test2", "2021-01-01T23:59:59.123+00:00")
    await menuai.async_block_till_done()
    assert menuai.states.get("notify.test_group").state == STATE_UNKNOWN
