...
from menuai.components.daikin.const import KEY_MAC
from menuai.config_entries import SOURCE_USER, SOURCE_ZEROCONF
from menuai.const import CONF_API_KEY, CONF_HOST, CONF_PASSWORD
...
def mock_daikin():
    ...
    with patch(
        "menuai.components.daikin.config_flow.DaikinFactory"
    ) as Appliance:
...
def mock_daikin_discovery():
    ...
    with patch("menuai.components.daikin.config_flow.Discovery") as Discovery:
...
async def test_user(menuai: menuai, mock_daikin) -> None:
    """Test user config."""
...
async def test_abort_if_already_setup(menuai: menuai, mock_daikin) -> None:
    """Test we abort if Daikin is already setup."""
    MockConfigEntry(domain="daikin", unique_id=MAC).add_to_menuai(menuai)
    result = await menuai.config_entries.flow.async_init(
        "daikin",
        context={"source": SOURCE_USER},
...
async def test_device_abort(menuai: menuai, mock_daikin, s_effect, reason) -> None:
    """Test device abort."""
...
async def test_api_password_abort(menuai: menuai) -> None:
    """Test device abort."""
    result = await menuai.config_entries.flow.async_init(
        "daikin",
        context={"source": SOURCE_USER},
...
async def test_discovery_zeroconf(
    menuai: menuai, mock_daikin, mock_daikin_discovery, source, data, unique_id
) -> None:
    ...
    MockConfigEntry(domain="daikin", unique_id=unique_id).add_to_menuai(menuai)
    result = await menuai.config_entries.flow.async_init(
        "daikin",
