...
async def test_one_cover(menuai: menuai, rfxtrx) -> None:
    """Test with 1 cover."""
    ...
    state = menuai.states.get("cover.lightwaverf_siemens_0213c7_242")
    assert state
...
async def test_several_covers(menuai: menuai, rfxtrx) -> None:
    """Test with 3 covers."""
    ...
    state = menuai.states.get("cover.lightwaverf_siemens_f394ab_1")
    assert state
    assert state.state == "closed"
    assert state.attributes.get("friendly_name") == "LightwaveRF, Siemens f394ab:1"

    state = menuai.states.get("cover.rollertrol_009ba8_1")
    assert state
    assert state.state == "closed"
    assert state.attributes.get("friendly_name") == "RollerTrol 009ba8:1"


async def test_discover_covers(menuai: menuai, rfxtrx_automatic) -> None:
    """Test with discovery of covers."""
    rfxtrx = rfxtrx_automatic

    await rfxtrx.signal("0a140002f38cae010f0070")
    state = menuai.states.get("cover.lightwaverf_siemens_f38cae_1")
    assert state
    assert state.state == "open"

    await rfxtrx.signal("0a1400adf394ab020e0060")
    state = menuai.states.get("cover.lightwaverf_siemens_f394ab_2")
    assert state
    assert state.state == "open"


async def test_duplicate_cover(menuai: menuai, rfxtrx) -> None:
...
async def test_rfy_cover(menuai: menuai, rfxtrx) -> None:
    ...
    state = menuai.states.get("cover.rfy_010203_1")
    assert state
    ...
    state = menuai.states.get("cover.rfy_010203_2")
    assert state
    ...
    state = menuai.states.get("cover.rfy_010203_3")
    assert state
    rfxtrx.transport.send.mock_calls = []
