...
async def test_one_light(menuai: menuai, rfxtrx) -> None:
    """Test with 1 light."""
    ...
    state = menuai.states.get("light.ac_213c7f2_16")
    assert state
...
async def test_state_restore(menuai: menuai, rfxtrx, state, brightness) -> None:
    """State restoration."""

    entity_id = "light.ac_213c7f2_16"

    mock_restore_cache(
        menuai, [State(entity_id, state, attributes={ATTR_BRIGHTNESS: brightness})]
    )
    ...
    assert menuai.states.get(entity_id).attributes.get(ATTR_BRIGHTNESS) == brightness


async def test_several_lights(menuai: menuai, rfxtrx) -> None:
    """Test with 3 lights."""
    ...
    state = menuai.states.get("light.ac_213c7f2_48")
    assert state
    assert state.state == STATE_UNKNOWN
    assert state.attributes.get("friendly_name") == "AC 213c7f2:48"

    state = menuai.states.get("light.ac_118cdea_2")
    assert state
    assert state.state == STATE_UNKNOWN
    assert state.attributes.get("friendly_name") == "AC 118cdea:2"

    state = menuai.states.get("light.ac_1118cdea_2")
    assert state
    assert state.state == STATE_UNKNOWN
...
async def test_discover_light(menuai: menuai, rfxtrx_automatic) -> None:
    """Test with discovery of lights."""
    rfxtrx = rfxtrx_automatic

    await rfxtrx.signal("0b11009e00e6116202020070")
    state = menuai.states.get("light.ac_0e61162_2")
    assert state
    assert state.state == "on"
