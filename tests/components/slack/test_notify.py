...
from menuai.components.slack import DOMAIN
from menuai.components.slack.notify import (
    ATTR_THREAD_TS,
    CONF_DEFAULT_CHANNEL,
    SlackNotificationService,
)
from menuai.const import ATTR_ICON, CONF_API_KEY, CONF_NAME, CONF_PLATFORM
...
async def test_message_includes_default_icon_url() -> None:
    ...
    mock_client = Mock()
    mock_client.chat_postMessage = AsyncMock()
    expected_icon = "https://example.com/menuai.png"
    service = SlackNotificationService(
        None, mock_client, CONF_DATA | {ATTR_ICON: expected_icon}
    )
