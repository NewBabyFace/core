...
def mock_get_mac_address() -> Generator[Mock]:
    ...
    with patch(
        "menuai.components.dlna_dmr.config_flow.get_mac_address", autospec=True
    ) as gma_mock:
...
def mock_setup_entry() -> Generator[Mock]:
    ...
    with patch(
        "menuai.components.dlna_dmr.async_setup_entry", return_value=True
    ) as setup_entry_mock:
        yield setup_entry_mock


async def test_user_flow_undiscovered_manual(menuai: menuai) -> None:
...
async def test_user_flow_selected(menuai: menuai, ssdp_scanner_mock: Mock) -> None:
...
async def test_user_flow_uncontactable(
    menuai: menuai, domain_data_mock: Mock
) -> None:
...
async def test_user_flow_wrong_st(menuai: menuai, domain_data_mock: Mock) -> None:
...
async def test_ssdp_flow_success(menuai: menuai) -> None:
    ...
    logging.getLogger("menuai.components.dlna_dmr.config_flow").setLevel(
        logging.DEBUG
    )
    result = await menuai.config_entries.flow.async_init(
        DOMAIN,
...
async def test_ssdp_flow_duplicate_location(
    menuai: menuai, config_entry_mock: MockConfigEntry, mock_get_mac_address: Mock
) -> None:
...
async def test_ssdp_add_mac(
    menuai: menuai, config_entry_mock_no_mac: MockConfigEntry
) -> None:
...
async def test_ssdp_ignore_device(menuai: menuai) -> None:
    """Test SSDP discovery ignores certain devices."""
    discovery = dataclasses.replace(MOCK_DISCOVERY)
    discovery.x_menuai_matching_domains = {DOMAIN, "other_domain"}
    assert discovery.x_menuai_matching_domains
    result = await menuai.config_entries.flow.async_init(
        DOMAIN,
        context={"source": config_entries.SOURCE_SSDP},
        data=discovery,
...
async def test_ignore_flow(menuai: menuai, ssdp_scanner_mock: Mock) -> None:
...
async def test_get_mac_address_ipv4(
    menuai: menuai, mock_get_mac_address: Mock
) -> None:
...
async def test_options_flow(
    ...
    config_entry_mock.add_to_menuai(menuai)
    result = await menuai.config_entries.options.async_init(config_entry_mock.entry_id)
