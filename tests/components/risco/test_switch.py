...
async def _check_cloud_state(
    ...
    with patch.object(
        ...
        assert menuai.states.get(entity_id).state == expected_bypassed
...
async def test_cloud_bypass(
    ...
    with patch("menuai.components.risco.RiscoCloud.bypass_zone") as mock:
...
async def test_local_states(
    ...
    await _check_local_state(menuai, two_zone_local, True, FIRST_ENTITY_ID, 0, callback)
    await _check_local_state(menuai, two_zone_local, False, FIRST_ENTITY_ID, 0, callback)
    await _check_local_state(menuai, two_zone_local, True, SECOND_ENTITY_ID, 1, callback)
    await _check_local_state(menuai, two_zone_local, False, SECOND_ENTITY_ID, 1, callback)


async def test_local_bypass(
    menuai: menuai, two_zone_local, setup_risco_local
) -> None:
    """Test bypassing a zone."""
