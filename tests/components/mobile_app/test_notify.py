...
async def setup_push_receiver(
    menuai: menuai, aioclient_mock: AiohttpClientMocker, menuai_admin_user: MockUser
) -> None:
    ...
    loaded_late_entry.add_to_menuai(menuai)
    assert await menuai.config_entries.async_setup(loaded_late_entry.entry_id)
    await menuai.async_block_till_done()

    assert menuai.services.has_service("notify", "mobile_app_loaded_late")

    assert await menuai.config_entries.async_remove(loaded_late_entry.entry_id)
    await menuai.async_block_till_done()

    assert menuai.services.has_service("notify", "mobile_app_test")
    assert not menuai.services.has_service("notify", "mobile_app_loaded_late")
...
async def setup_websocket_channel_only_push(
    ...
    assert menuai.services.has_service("notify", "mobile_app_websocket_push_name")


async def test_notify_works(
    menuai: menuai, aioclient_mock: AiohttpClientMocker, setup_push_receiver
) -> None:
    """Test notify works."""
    assert menuai.services.has_service("notify", "mobile_app_test") is True
    ...
    assert call_json["registration_info"]["app_id"] == "io.menuai.mobile_app"
...
async def test_notify_ws_confirming_works(
    ...
    msg_result = await client.receive_json()
    confirm_id = msg_result["event"].pop("menuai_confirm_id")
...
async def test_notify_ws_not_confirming(
    ...
    with patch(
        "menuai.components.mobile_app.push_notification.PUSH_CONFIRM_TIMEOUT", 0
    ):
        await menuai.services.async_call(
            "notify", "mobile_app_test", {"message": "Hello world 2"}, blocking=True
        )
