...
from menuai.components.aws_s3.backup import (
...
from menuai.components.aws_s3.const import (
    CONF_ENDPOINT_URL,
    ...
    assert response["result"]["backups"] == [
        {
            ...
            "menuai_included": test_backup.menuai_included,
            "menuai_version": test_backup.menuai_version,
...
async def test_agents_list_backups_with_corrupted_metadata(
    ...
    # Create agent
    agent = S3BackupAgent(menuai, mock_config_entry)
...
async def test_cache_expiration(
    ...
    # Create agent
    agent = S3BackupAgent(menuai, mock_entry)
...
async def test_listeners_get_cleaned_up(menuai: menuai) -> None:
    ...
    assert DATA_BACKUP_AGENT_LISTENERS not in menuai.data
