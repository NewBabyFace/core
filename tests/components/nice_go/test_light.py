...
async def test_update_light_state(
    ...
    assert menuai.states.get("light.test_garage_3_light") is None
...
async def test_unsupported_device_type(
    ...
    assert menuai.states.get("light.test_garage_4_light") is None
    assert (
...
async def test_auth_failed_error(
    ...
    with pytest.raises(menuaiError, match="Error while turning on the light"):
        await menuai.services.async_call(
            LIGHT_DOMAIN,
            SERVICE_TURN_ON,
            {ATTR_ENTITY_ID: "light.test_garage_1_light"},
    ...
    with pytest.raises(menuaiError, match="Error while turning off the light"):
        await menuai.services.async_call(
            LIGHT_DOMAIN,
            SERVICE_TURN_OFF,
            {ATTR_ENTITY_ID: "light.test_garage_2_light"},
