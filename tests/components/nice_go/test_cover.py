...
async def test_update_cover_state(
    ...
    assert menuai.states.get("cover.test_garage_1").state == CoverState.CLOSED
    assert menuai.states.get("cover.test_garage_2").state == CoverState.OPEN

    device_update = await async_load_json_object_fixture(
        menuai, "device_state_update.json", DOMAIN
    )
    await mock_config_entry.runtime_data.on_data(device_update)
    device_update_1 = await async_load_json_object_fixture(
        menuai, "device_state_update_1.json", DOMAIN
    )
    await mock_config_entry.runtime_data.on_data(device_update_1)

    assert menuai.states.get("cover.test_garage_1").state == CoverState.OPENING
    assert menuai.states.get("cover.test_garage_2").state == CoverState.CLOSING
...
async def test_auth_failed_error(
    ...
    with pytest.raises(menuaiError, match="Error opening the barrier"):
        await menuai.services.async_call(
            COVER_DOMAIN,
            SERVICE_OPEN_COVER,
            {ATTR_ENTITY_ID: "cover.test_garage_1"},
    ...
    with pytest.raises(menuaiError, match="Error closing the barrier"):
        await menuai.services.async_call(
            COVER_DOMAIN,
            SERVICE_CLOSE_COVER,
            {ATTR_ENTITY_ID: "cover.test_garage_2"},
