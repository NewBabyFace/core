...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "menuai.components.whois.async_setup_entry", return_value=True
    ) as mock_setup:
        yield mock_setup
...
def mock_whois() -> Generator[MagicMock]:
    ...
    with (
        patch(
            "menuai.components.whois.whois_query",
        ) as whois_mock,
        patch("menuai.components.whois.config_flow.whois.query", new=whois_mock),
    ):
...
def mock_whois_missing_some_attrs() -> Generator[Mock]:
    ...
    with patch(
        "menuai.components.whois.whois_query", LimitedWhoisMock
    ) as whois_mock:
        yield whois_mock
...
async def init_integration(
    menuai: menuai, mock_config_entry: MockConfigEntry, mock_whois: MagicMock
) -> MockConfigEntry:
