...
async def test_async_get_node_status_sensor_entity_id(
    ...
    assert async_get_node_status_sensor_entity_id(menuai, device.id) is None
...
async def test_async_get_nodes_from_area_id(
    ...
    assert not async_get_nodes_from_area_id(menuai, area.id)


async def test_get_value_state_schema_boolean_config_value(
    menuai: menuai, client, aeon_smart_switch_6
) -> None:
...
async def test_async_get_provisioning_entry_from_device_id(
    menuai: menuai, client, device_registry: dr.DeviceRegistry, integration
) -> None:
    ...
    with pytest.raises(
        ...
        await async_get_provisioning_entry_from_device_id(menuai, non_zwave_device.id)
    ...
    not_loaded_config_entry.add_to_menuai(menuai)
    ...
    with pytest.raises(
        ...
        await async_get_provisioning_entry_from_device_id(menuai, not_loaded_device.id)
