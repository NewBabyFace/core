...
from menuai.components.light import ATTR_SUPPORTED_COLOR_MODES, ColorMode
...
from menuai.components.zwave_js.discovery import (
    FirmwareVersionRange,
    ZWaveDiscoverySchema,
    ZWaveValueDiscoverySchema,
)
from menuai.components.zwave_js.discovery_data_template import (
    DynamicCurrentTempClimateDataTemplate,
)
from menuai.components.zwave_js.helpers import get_device_id
from menuai.const import ATTR_ENTITY_ID, STATE_OFF, STATE_UNKNOWN, EntityCategory
...
async def test_aeon_smart_switch_6_state(
    ...
    state = menuai.states.get("button.smart_switch_6_reset_accumulated_values")
    assert state


async def test_iblinds_v2(menuai: menuai, client, iblinds_v2, integration) -> None:
    """Test that an iBlinds v2.0 multilevel switch value is discovered as a cover."""
    node = iblinds_v2
    assert node.device_class.specific.label == "Unused"

    state = menuai.states.get("light.window_blind_controller")
    assert not state

    state = menuai.states.get("cover.window_blind_controller")
    assert state


async def test_zvidar_state(menuai: menuai, client, zvidar, integration) -> None:
...
async def test_ge_12730(menuai: menuai, client, ge_12730, integration) -> None:
    ...
    state = menuai.states.get("light.in_wall_smart_fan_control")
    assert not state

    state = menuai.states.get("fan.in_wall_smart_fan_control")
    assert state
...
async def test_inovelli_lzw36(
    ...
    state = menuai.states.get("light.family_room_combo")
    assert state.state == "off"

    state = menuai.states.get("fan.family_room_combo_2")
    assert state


async def test_vision_security_zl7432(
    menuai: menuai, client, vision_security_zl7432, integration
) -> None:
...
async def test_lock_popp_electric_strike_lock_control(
    menuai: menuai, client, lock_popp_electric_strike_lock_control, integration
) -> None:
    """Test that the Popp Electric Strike Lock Control gets discovered correctly."""
    assert menuai.states.get("lock.node_62") is not None
    assert (
        menuai.states.get("binary_sensor.node_62_the_current_status_of_the_door")
        is not None
    )
    assert menuai.states.get("select.node_62_current_lock_mode") is not None


async def test_fortrez_ssa3_siren(
    menuai: menuai, client, fortrezz_ssa3_siren, integration
) -> None:
    """Test Fortrezz SSA3 siren gets discovered correctly."""
    assert menuai.states.get("select.siren_and_strobe_alarm") is not None


async def test_firmware_version_range_exception(menuai: menuai) -> None:
...
async def test_merten_507801(
    menuai: menuai, client, merten_507801, integration
) -> None:
    """Test that Merten 507801 multilevel switch value is discovered as a cover."""
    node = merten_507801
    assert node.device_class.specific.label == "Unused"

    state = menuai.states.get("light.connect_roller_shutter")
    assert not state

    state = menuai.states.get("cover.connect_roller_shutter")
    assert state
...
async def test_shelly_001p10_disabled_entities(
    ...
    state = menuai.states.get("cover.wave_shutter")
    assert state
...
async def test_zooz_zen72(
    ...
    assert len(menuai.states.async_entity_ids(NUMBER_DOMAIN)) == 1
    assert len(menuai.states.async_entity_ids(BUTTON_DOMAIN)) == 2  # includes ping
...
async def test_light_device_class_is_null(
    menuai: menuai, client, light_device_class_is_null, integration
) -> None:
    ...
    assert menuai.states.get("light.bar_display_cases")
...
async def test_aeotec_smart_switch_7(
    ...
    state = menuai.states.get("light.smart_switch_7")
    assert state
    assert state.attributes[ATTR_SUPPORTED_COLOR_MODES] == [
        ColorMode.HS,
    ]

    state = menuai.states.get("switch.smart_switch_7")
    assert state

    state = menuai.states.get("button.smart_switch_7_reset_accumulated_values")
    assert state
