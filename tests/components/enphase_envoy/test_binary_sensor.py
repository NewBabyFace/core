...
async def test_binary_sensor(
    ...
    with patch(
        "menuai.components.enphase_envoy.PLATFORMS", [Platform.BINARY_SENSOR]
    ):
...
async def test_binary_sensor_data(
    ...
    assert (entity_state := menuai.states.get(f"{entity_base}_{sn}_communicating"))
    assert entity_state.state == STATE_ON
    assert (entity_state := menuai.states.get(f"{entity_base}_{sn}_grid_status"))
    assert entity_state.state == STATE_ON
    ...
    for sn in mock_envoy.data.encharge_inventory:
        assert (entity_state := menuai.states.get(f"{entity_base}_{sn}_communicating"))
        assert entity_state.state == STATE_ON
        assert (entity_state := menuai.states.get(f"{entity_base}_{sn}_dc_switch"))
        assert entity_state.state == STATE_ON
