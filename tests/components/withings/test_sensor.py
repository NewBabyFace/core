...
from menuai.components.withings import DOMAIN
...
async def test_all_entities(
    ...
    """Test all entities."""
    with patch("menuai.components.withings.PLATFORMS", [Platform.SENSOR]):
...
async def test_update_failed(
    ...
    state = menuai.states.get("sensor.henk_weight")
...
async def test_update_new_measurement_creates_new_sensor(
    ...
    assert menuai.states.get("sensor.henk_fat_mass") is None
    ...
    assert menuai.states.get("sensor.henk_fat_mass")
...
async def test_update_new_goals_creates_new_sensor(
    ...
    assert menuai.states.get("sensor.henk_step_goal") is None
    assert menuai.states.get("sensor.henk_weight_goal")
    ...
    assert menuai.states.get("sensor.henk_step_goal")
...
async def test_activity_sensors_unknown_next_day(
    ...
    assert menuai.states.get("sensor.henk_steps_today")
    ...
    assert menuai.states.get("sensor.henk_steps_today").state == STATE_UNKNOWN
...
async def test_activity_sensors_same_result_same_day(
    ...
    assert menuai.states.get("sensor.henk_steps_today").state == "1155"
...
async def test_activity_sensors_created_when_existed(
    ...
    await menuai.config_entries.async_reload(polling_config_entry.entry_id)
    await menuai.async_block_till_done()
...
async def test_activity_sensors_created_when_receive_activity_data(
    ...
    assert menuai.states.get("sensor.henk_steps_today") is None
...
async def test_sleep_sensors_created_when_existed(
    ...
    assert menuai.states.get("sensor.henk_deep_sleep")
    assert menuai.states.get("sensor.henk_deep_sleep").state != STATE_UNKNOWN
...
async def test_sleep_sensors_created_when_receive_sleep_data(
    ...
    assert menuai.states.get("sensor.henk_deep_sleep") is None
...
async def test_workout_sensors_created_when_existed(
    ...
    assert menuai.states.get("sensor.henk_last_workout_type")
    assert menuai.states.get("sensor.henk_last_workout_type").state != STATE_UNKNOWN
...
async def test_workout_sensors_created_when_receive_workout_data(
    ...
    assert menuai.states.get("sensor.henk_last_workout_type") is None
...
async def test_device_sensors_created_when_device_data_received(
    ...
    assert menuai.states.get("sensor.body_battery") is None
    ...
    assert menuai.states.get("sensor.body_battery")
...
async def test_device_two_config_entries(
    ...
    assert menuai.states.get("sensor.body_battery") is not None

    second_polling_config_entry.add_to_menuai(menuai)
    await menuai.config_entries.async_setup(second_polling_config_entry.entry_id)

    assert menuai.states.get("sensor.not_henk_temperature") is not None

    assert "Platform withings does not generate unique IDs" not in caplog.text

    await menuai.config_entries.async_unload(polling_config_entry.entry_id)
    await menuai.async_block_till_done()

    assert menuai.states.get("sensor.body_battery").state == STATE_UNAVAILABLE
    ...
    await menuai.config_entries.async_setup(polling_config_entry.entry_id)
    await menuai.async_block_till_done()
