...
async def mock_update_data(self: Alpha2Base, menuai: menuai) -> None:
    """Mock Alpha2Base.update_data."""
    data = xmltodict.parse(await async_load_fixture(menuai, "static2.xml", DOMAIN))
...
async def init_integration(menuai: menuai) -> MockConfigEntry:
    ...
    with patch(
        "menuai.components.moehlenhoff_alpha2.coordinator.Alpha2Base.update_data",
