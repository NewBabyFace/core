...
from menuai.components.speedtestdotnet.coordinator import (
...
async def test_setup_failed(menuai: menuai, mock_api: MagicMock) -> None:
...
async def test_entry_lifecycle(menuai: menuai, mock_api: MagicMock) -> None:
...
async def test_server_not_found(menuai: menuai, mock_api: MagicMock) -> None:
    ...
    state = menuai.states.get("sensor.speedtest_ping")
    assert state is not None
    assert state.state == STATE_UNAVAILABLE


async def test_get_best_server_error(menuai: menuai, mock_api: MagicMock) -> None:
    """Test configured server id is not found."""
