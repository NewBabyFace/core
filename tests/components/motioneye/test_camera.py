...
from menuai.components.camera import async_get_image, async_get_mjpeg_stream
...
from menuai.const import ATTR_DEVICE_ID, ATTR_ENTITY_ID, CONF_URL
...
async def test_setup_camera(menuai: menuai) -> None:
    """Test a basic camera."""
...
async def test_setup_camera_auth_fail(menuai: menuai) -> None:
    """Test a successful camera."""
    client = create_mock_motioneye_client()
    client.async_client_login = AsyncMock(side_effect=MotionEyeClientInvalidAuthError)
    await setup_mock_motioneye_config_entry(menuai, client=client)
    assert not menuai.states.get(TEST_CAMERA_ENTITY_ID)


async def test_setup_camera_client_error(menuai: menuai) -> None:
    """Test a successful camera."""
    client = create_mock_motioneye_client()
...
async def test_setup_camera_empty_data(menuai: menuai) -> None:
    """Test a successful camera."""
    client = create_mock_motioneye_client()
...
async def test_setup_camera_bad_data(menuai: menuai) -> None:
...
async def test_setup_camera_without_streaming(menuai: menuai) -> None:
...
async def test_setup_camera_new_data_error(menuai: menuai) -> None:
...
async def test_setup_camera_new_data_without_streaming(menuai: menuai) -> None:
...
async def test_unload_camera(menuai: menuai) -> None:
    """Test unloading camera."""
    client = create_mock_motioneye_client()
    entry = await setup_mock_motioneye_config_entry(menuai, client=client)
    assert menuai.states.get(TEST_CAMERA_ENTITY_ID)
...
async def test_get_still_image_from_camera(
    aiohttp_server: Callable[[], TestServer], menuai: menuai
) -> None:
    ...
    await setup_mock_motioneye_config_entry(
        menuai, config_entry=config_entry, client=client
    )
    await menuai.async_block_till_done()
    ...
    with pytest.raises(menuaiError):
        await async_get_image(menuai, TEST_CAMERA_ENTITY_ID, timeout=1)
...
async def test_get_stream_from_camera(
    ...
    await async_get_mjpeg_stream(menuai, MockRequest(b"", "test"), TEST_CAMERA_ENTITY_ID)
...
async def test_device_info(
    ...
    entry = await setup_mock_motioneye_config_entry(menuai)
...
async def test_get_stream_from_camera_with_broken_host(
    ...
    client = create_mock_motioneye_client()
    config_entry = create_mock_motioneye_config_entry(menuai, data={CONF_URL: "http://"})
    ...
    with pytest.raises(HTTPBadGateway):
        await async_get_mjpeg_stream(menuai, Mock(), TEST_CAMERA_ENTITY_ID)


async def test_set_text_overlay_bad_extra_key(menuai: menuai) -> None:
    ...
    with pytest.raises(vol.error.MultipleInvalid):
        await menuai.services.async_call(DOMAIN, SERVICE_SET_TEXT_OVERLAY, data)


async def test_set_text_overlay_bad_entity_identifier(menuai: menuai) -> None:
...
async def test_set_text_overlay_bad_empty(menuai: menuai) -> None:
    """Test text overlay with incorrect input data."""
    client = create_mock_motioneye_client()
    await setup_mock_motioneye_config_entry(menuai, client=client)
    with pytest.raises(vol.error.MultipleInvalid):
        await menuai.services.async_call(DOMAIN, SERVICE_SET_TEXT_OVERLAY, {})


async def test_set_text_overlay_bad_no_left_or_right(menuai: menuai) -> None:
    """Test text overlay with incorrect input data."""
    client = create_mock_motioneye_client()
    await setup_mock_motioneye_config_entry(menuai, client=client)
...
async def test_set_text_overlay_good(menuai: menuai) -> None:
...
async def test_set_text_overlay_good_entity_id(menuai: menuai) -> None:
...
async def test_set_text_overlay_bad_device(menuai: menuai) -> None:
    """Test a working text overlay."""
    client = create_mock_motioneye_client()
    await setup_mock_motioneye_config_entry(menuai, client=client)
...
async def test_set_text_overlay_no_such_camera(menuai: menuai) -> None:
    """Test a working text overlay."""
    client = create_mock_motioneye_client()
    await setup_mock_motioneye_config_entry(menuai, client=client)
...
async def test_request_action(menuai: menuai) -> None:
    ...
    await menuai.services.async_call(DOMAIN, SERVICE_ACTION, data)
    await menuai.async_block_till_done()
    assert client.async_action.call_args == call(TEST_CAMERA_ID, data[CONF_ACTION])


async def test_request_snapshot(menuai: menuai) -> None:
    ...
    await menuai.services.async_call(DOMAIN, SERVICE_SNAPSHOT, data)
    await menuai.async_block_till_done()
