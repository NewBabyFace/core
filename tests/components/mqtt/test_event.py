...
from menuai.components import event, mqtt
from menuai.components.mqtt.event import MQTT_EVENT_ATTRIBUTES_BLOCKED
...
async def test_setting_event_value_via_mqtt_message(
    ...
    async_fire_mqtt_message(
        menuai, "test-topic", '{"event_type": "press", "duration": "short" }'
    )
    state = menuai.states.get("event.test")
...
async def test_availability_when_connection_lost(
    ...
    await help_test_availability_when_connection_lost(
        menuai, mqtt_mock_entry, event.DOMAIN
    )
...
async def test_availability_without_topic(
    ...
    await help_test_availability_without_topic(
        menuai, mqtt_mock_entry, event.DOMAIN, DEFAULT_CONFIG
    )
...
async def test_default_availability_list_single(
    ...
    await help_test_default_availability_list_single(
        menuai, mqtt_mock_entry, caplog, event.DOMAIN, DEFAULT_CONFIG
    )
...
async def test_unique_id(
    ...
    await help_test_unique_id(menuai, mqtt_mock_entry, event.DOMAIN)
...
async def test_discovery_removal_event(
    ...
    await help_test_discovery_removal(menuai, mqtt_mock_entry, event.DOMAIN, data)
...
async def test_discovery_update_event_template(
    ...
    async_fire_mqtt_message(menuai, "menuai/event/bla/config", json.dumps(config1))
    await menuai.async_block_till_done()
    async_fire_mqtt_message(menuai, "event/state1", '{"val":100}')
    await menuai.async_block_till_done()
    state = menuai.states.get("event.beer")
    assert state is not None
    assert state.attributes.get("val") == "100"

    async_fire_mqtt_message(menuai, "menuai/event/bla/config", json.dumps(config2))
    await menuai.async_block_till_done()
    async_fire_mqtt_message(menuai, "event/state1", '{"val":100}')
    await menuai.async_block_till_done()
    state = menuai.states.get("event.beer")
...
async def test_discovery_broken(
    ...
    await help_test_discovery_broken(menuai, mqtt_mock_entry, event.DOMAIN, data1, data2)
...
async def test_entity_device_info_with_hub(
    ...
    async_fire_mqtt_message(menuai, "menuai/event/bla/config", data)
    await menuai.async_block_till_done()
...
async def test_entity_debug_info_message(
    ...
    await help_test_entity_debug_info_message(
        menuai, mqtt_mock_entry, event.DOMAIN, DEFAULT_CONFIG, None
    )
...
async def test_entity_category(
    ...
    await help_test_entity_category(menuai, mqtt_mock_entry, event.DOMAIN, DEFAULT_CONFIG)
