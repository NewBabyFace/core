...
from menuai.components import mqtt, vacuum
from menuai.components.mqtt import vacuum as mqttvacuum
from menuai.components.mqtt.const import CONF_COMMAND_TOPIC, CONF_STATE_TOPIC
from menuai.components.mqtt.vacuum import (
    ALL_SERVICES,
...
from menuai.const import CONF_NAME, ENTITY_MATCH_ALL, STATE_UNKNOWN
...
async def test_default_supported_features(
    ...
    entity = menuai.states.get("vacuum.mqtttest")
...
@pytest.mark.parametrize("menuai_config", [CONFIG_ALL_SERVICES])
async def test_all_commands(
    ...
    await common.async_set_fan_speed(menuai, "medium", "vacuum.mqtttest")
    mqtt_mock.async_publish.assert_called_once_with(
        "vacuum/set_fan_speed", "medium", 0, False
    )
    mqtt_mock.async_publish.reset_mock()

    await common.async_send_command(menuai, "44 FE 93", entity_id="vacuum.mqtttest")
    mqtt_mock.async_publish.assert_called_once_with(
    ...
    await common.async_send_command(
        menuai, "44 FE 93", {"key": "value"}, entity_id="vacuum.mqtttest"
    )
...
async def test_command_without_command_topic(
    ...
    await common.async_start(menuai, "vacuum.test")
    mqtt_mock.async_publish.assert_not_called()
    mqtt_mock.async_publish.reset_mock()

    await common.async_set_fan_speed(menuai, "low", "vacuum.test")
    mqtt_mock.async_publish.assert_not_called()
    mqtt_mock.async_publish.reset_mock()

    await common.async_send_command(menuai, "some command", entity_id="vacuum.test")
    mqtt_mock.async_publish.assert_not_called()
    mqtt_mock.async_publish.reset_mock()
...
async def test_status(
    ...
    await mqtt_mock_entry()
    state = menuai.states.get("vacuum.mqtttest")
    ...
    async_fire_mqtt_message(menuai, "vacuum/state", message)
    state = menuai.states.get("vacuum.mqtttest")
...
async def test_status_invalid_json(
    ...
    async_fire_mqtt_message(menuai, "vacuum/state", '{"asdfasas false}')
    state = menuai.states.get("vacuum.mqtttest")
    assert state.state == STATE_UNKNOWN


@pytest.mark.parametrize("menuai_config", [DEFAULT_CONFIG_2])
async def test_availability_when_connection_lost(
    menuai: menuai, mqtt_mock_entry: MqttMockHAClientGenerator
) -> None:
    """Test availability after MQTT disconnection."""
    await help_test_availability_when_connection_lost(
        menuai, mqtt_mock_entry, vacuum.DOMAIN
    )
...
async def test_availability_without_topic(
    ...
    await help_test_availability_without_topic(
        menuai, mqtt_mock_entry, vacuum.DOMAIN, DEFAULT_CONFIG_2
    )
...
async def test_update_with_json_attrs_not_dict(
    ...
    await help_test_update_with_json_attrs_not_dict(
        menuai, mqtt_mock_entry, caplog, vacuum.DOMAIN, DEFAULT_CONFIG_2
    )
...
async def test_unique_id(
    ...
    await help_test_unique_id(menuai, mqtt_mock_entry, vacuum.DOMAIN)
...
async def test_discovery_removal_vacuum(
    ...
    await help_test_discovery_removal(menuai, mqtt_mock_entry, vacuum.DOMAIN, data)
...
async def test_discovery_update_vacuum(
    ...
    await help_test_discovery_update(
        menuai, mqtt_mock_entry, vacuum.DOMAIN, config1, config2
    )
...
async def test_discovery_update_unchanged_vacuum(
    ...
    with patch(
        "menuai.components.mqtt.vacuum.MqttStateVacuum.discovery_update"
    ) as discovery_update:
        await help_test_discovery_update_unchanged(
            menuai, mqtt_mock_entry, vacuum.DOMAIN, data1, discovery_update
        )
...
async def test_discovery_broken(
    ...
    await help_test_discovery_broken(menuai, mqtt_mock_entry, vacuum.DOMAIN, data1, data2)
...
async def test_setup_manual_entity_from_yaml(
    ...
    assert menuai.states.get(f"{platform}.mqtttest")
