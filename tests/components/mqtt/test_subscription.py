...
from menuai.components.mqtt.subscription import (
...
async def test_subscribe_topics(
    ...
    async_fire_mqtt_message(menuai, "test-topic1", "test-payload1")
    assert len(calls1) == 1
    assert calls1[0][0].topic == "test-topic1"
    assert calls1[0][0].payload == "test-payload1"
    assert len(calls2) == 0

    async_fire_mqtt_message(menuai, "test-topic2", "test-payload2")
    assert len(calls1) == 1
    assert len(calls2) == 1
    ...
    async_fire_mqtt_message(menuai, "test-topic1", "test-payload")
    async_fire_mqtt_message(menuai, "test-topic2", "test-payload")
...
async def test_modify_topics(
    ...
    async_fire_mqtt_message(menuai, "test-topic1_1", "test-payload")
    assert len(calls1) == 2
