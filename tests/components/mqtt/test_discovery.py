...
from menuai.components.mqtt.abbreviations import (
    ABBREVIATIONS,
    DEVICE_ABBREVIATIONS,
)
from menuai.components.mqtt.const import SUPPORTED_COMPONENTS
from menuai.components.mqtt.discovery import (
    MQTT_DISCOVERY_DONE,
    MQTT_DISCOVERY_NEW,
...
from menuai.components.mqtt.models import ReceiveMessage
...
from menuai.data_entry_flow import AbortFlow, FlowResult
...
TEST_SINGLE_CONFIGS = [
    (
        "menuai/device_automation/0AFFD2/bla1/config",
        {
    ...
    (
        "menuai/sensor/0AFFD2/bla2/config",
        {
            "device": {"identifiers": ["0AFFD2"], "name": "test_device"},
            "o": {"name": "Foo2Mqtt", "sw": "1.40.2", "url": "https://www.foo2mqtt.io"},
...
TEST_DEVICE_DISCOVERY_TOPIC = "menuai/device/0AFFD2/config"
...
async def help_check_discovered_items(
    ...
    state = menuai.states.get("sensor.test_device_mqtt_sensor")
    assert state is not None

    # Check the tag works
    async_fire_mqtt_message(menuai, "foobar/tags/bla3/see", DEFAULT_TAG_SCAN)
...
async def mock_mqtt_flow(
    menuai: menuai, mqtt_data_flow_calls: list[MqttServiceInfo]
...
async def test_subscribing_config_topic(
    ...
    discovery_topic = "menuai"
    await async_start(menuai, discovery_topic, entry)
...
@pytest.mark.parametrize(
    ("topic", "log"),
    [
        ("menuai/binary_sensor/bla/not_config", False),
        ("menuai/binary_sensor/rörkrökare/config", True),
        ("menuai/device/bla/not_config", False),
        ("menuai/device/rörkrökare/config", True),
    ],
)
async def test_invalid_topic(
    ...
    with patch(
        "menuai.components.mqtt.discovery.async_dispatcher_send"
...
@pytest.mark.parametrize(
    "discovery_topic",
    ["menuai/binary_sensor/bla/config", "menuai/device/bla/config"],
)
async def test_invalid_json(
    ...
    with patch(
        ...
        async_fire_mqtt_message(menuai, discovery_topic, "not json")
        await menuai.async_block_till_done()
...
async def test_discovery_schema_error(
    ...
    with patch(
        f"menuai.components.mqtt.{domain}.DISCOVERY_SCHEMA",
...
async def test_invalid_config(
    ...
    async_fire_mqtt_message(
        menuai,
        "menuai/alarm_control_panel/bla/config",
...
async def test_only_valid_components(
    ...
    with patch(
        ...
        async_fire_mqtt_message(
            menuai, f"menuai/{invalid_component}/bla/config", "{}"
        )
...
@pytest.mark.parametrize(
    ...
    [
        ("menuai/binary_sensor/bla/config", ("binary_sensor", "bla")),
        ("menuai/binary_sensor/node/bla/config", ("binary_sensor", "node bla")),
    ],
...
async def test_correct_config_discovery_component(
    ...
    state = menuai.states.get("binary_sensor.test_device1_beer")

    assert state is not None
    assert state.name == "test_device1 Beer"
    assert discovery_hash in menuai.data["mqtt"].discovery_already_discovered
...
@pytest.mark.parametrize(
    ("discovery_topic", "discovery_hash"),
    [
        ("menuai/device/some_id/config", ("binary_sensor", "some_id bla")),
        (
            "menuai/device/node_id/some_id/config",
...
@pytest.mark.parametrize(
    ...
    [
        (
            "menuai/binary_sensor/bla/config",
            (
...
async def test_discovery_integration_info(
    ...
    state = menuai.states.get("binary_sensor.bla_beer")
...
async def test_discovery_migration_to_device_base(
    ...
    await help_check_discovered_items(menuai, device_registry, tag_mock)
    ...
    assert (
        ...
        "'0AFFD2 bla1' which was previously discovered on topic menuai/"
        ...
        "menuai/device/0AFFD2/config from external application Foo2Mqtt, "
    ...
    assert (
        ...
        "menuai/sensor/0AFFD2/bla2/config from external application Foo2Mqtt, "
        "version: 1.40.2; the conflicting discovery message was received on topic "
        "menuai/device/0AFFD2/config from external application Foo2Mqtt, "
        "version: 1.50.0; for support visit https://www.foo2mqtt.io" in caplog.text
    )
    assert (
        ...
        "was previously discovered on topic menuai/tag/0AFFD2/bla3/config "
        "from external application Foo2Mqtt, version: 1.40.2; the conflicting "
        "discovery message was received on topic menuai/device/0AFFD2/config "
    ...
    assert (
        ...
        "menuai/device_automation/0AFFD2/bla1/config. To complete migration, "
        ...
        "menuai/device_automation/0AFFD2/bla1/config" in caplog.text
    )
    assert (
        ...
        "on topic menuai/sensor/0AFFD2/bla2/config. To complete migration, "
        ...
        "menuai/sensor/0AFFD2/bla2/config" in caplog.text
    )
    ...
    assert (
        "Received a conflicting MQTT discovery message for device_automation "
        "'0AFFD2 bla1' which was previously discovered on topic menuai/device"
        "/0AFFD2/config from external application Foo2Mqtt, version: 1.50.0; the "
        "conflicting discovery message was received on topic menuai/"
        "device_automation/0AFFD2/bla1/config from external application Foo2Mqtt, "
    ...
    assert (
        ...
        "previously discovered on topic menuai/device/0AFFD2/config from "
        "external application Foo2Mqtt, version: 1.50.0; the conflicting discovery "
        "message was received on topic menuai/tag/0AFFD2/bla3/config from "
...
async def test_discovery_migration_unique_id(
    ...
    discovery_topic = "menuai/sensor/0AFFD2/bla2/config"
...
async def test_discovery_rollback_to_single_base(
    ...
    assert (
        ...
        "menuai/device/0AFFD2/config. To complete rollback, publish a platform "
...
async def test_discovery_availability(
    ...
    await menuai.async_block_till_done()
    state = menuai.states.get("binary_sensor.beer_bla")
    assert state is not None
...
async def test_discovery_component_availability_overridden(
    ...
    await menuai.async_block_till_done()
    state = menuai.states.get("binary_sensor.none_beer")
    assert state is not None
...
async def test_discovery_with_invalid_integration_info(
    ...
    """Test sending in correct JSON."""
    await mqtt_mock_entry()
    async_fire_mqtt_message(menuai, discovery_topic, config_message)
    await menuai.async_block_till_done()
...
async def test_discover_fan(
    ...
    async_fire_mqtt_message(
        menuai,
        "menuai/fan/bla/config",
        '{ "name": "Beer", "command_topic": "test_topic" }',
    )
    await menuai.async_block_till_done()

    state = menuai.states.get("fan.beer")

    assert state is not None
    assert state.name == "Beer"
    assert ("fan", "bla") in menuai.data["mqtt"].discovery_already_discovered
...
async def test_discover_climate(
    ...
    async_fire_mqtt_message(menuai, "menuai/climate/bla/config", data)
    await menuai.async_block_till_done()

    state = menuai.states.get("climate.ClimateTest")

    assert state is not None
    assert state.name == "ClimateTest"
    assert ("climate", "bla") in menuai.data["mqtt"].discovery_already_discovered
...
async def test_discover_alarm_control_panel(
    ...
    async_fire_mqtt_message(menuai, "menuai/alarm_control_panel/bla/config", data)
    await menuai.async_block_till_done()

    state = menuai.states.get("alarm_control_panel.AlarmControlPanelTest")

    assert state is not None
    assert state.name == "AlarmControlPanelTest"
    assert ("alarm_control_panel", "bla") in menuai.data[
        "mqtt"
...
@pytest.mark.parametrize(
    ...
    [
        (
            "menuai/alarm_control_panel/object/bla/config",
        ...
        (
            "menuai/binary_sensor/object/bla/config",
        ...
        (
            "menuai/button/object/bla/config",
        ...
        (
            "menuai/camera/object/bla/config",
        ...
        (
            "menuai/climate/object/bla/config",
        ...
        (
            "menuai/cover/object/bla/config",
        ...
        (
            "menuai/humidifier/object/bla/config",
        ...
        (
            "menuai/number/object/bla/config",
        ...
        (
            "menuai/scene/object/bla/config",
        ...
        (
            "menuai/select/object/bla/config",
        ...
        (
            "menuai/sensor/object/bla/config",
        ...
        (
            "menuai/switch/object/bla/config",
        ...
        (
            "menuai/light/object/bla/config",
        ...
        (
            "menuai/vacuum/object/bla/config",
        ...
        (
            "menuai/valve/object/bla/config",
        ...
        (
            "menuai/device_tracker/object/bla/config",
...
async def test_discovery_with_object_id(
    ...
    await mqtt_mock_entry()
    async_fire_mqtt_message(menuai, topic, config)
    await menuai.async_block_till_done()
    ...
    assert (domain, "object bla") in menuai.data["mqtt"].discovery_already_discovered
...
async def test_discovery_incl_nodeid(
    ...
    async_fire_mqtt_message(
        menuai,
        "menuai/binary_sensor/my_node_id/bla/config",
        '{ "name": "Beer", "state_topic": "test-topic" }',
    )
    await menuai.async_block_till_done()

    state = menuai.states.get("binary_sensor.beer")

    assert state is not None
    assert state.name == "Beer"
    assert ("binary_sensor", "my_node_id bla") in menuai.data[
        "mqtt"
    ].discovery_already_discovered
...
async def test_removal(
    ...
    async_fire_mqtt_message(menuai, "menuai/binary_sensor/bla/config", "")
    await menuai.async_block_till_done()
    state = menuai.states.get("binary_sensor.beer")
    assert state is None
...
async def test_rapid_rediscover(
    ...
    state = menuai.states.get("binary_sensor.milk")
    assert state is not None
...
async def test_rapid_rediscover_unique(
    ...
    async_fire_mqtt_message(
        menuai,
        "menuai/binary_sensor/bla2/config",
        '{ "name": "Ale", "state_topic": "test-topic", "unique_id": "very_unique" }',
    )
    await menuai.async_block_till_done()
    state = menuai.states.get("binary_sensor.ale")
    assert state is not None
    assert len(events) == 1
    ...
    assert len(menuai.states.async_entity_ids("binary_sensor")) == 2
    state = menuai.states.get("binary_sensor.ale")
    assert state is not None
    state = menuai.states.get("binary_sensor.milk")
    assert state is not None
...
@pytest.mark.parametrize(
    ...
    [
        (
            {
                "menuai/sensor/sens1/config": "{"
                '"device":{"identifiers":["0AFFD2"]},'
                '"state_topic": "foobar/sensor1",'
                '"unique_id": "unique1",'
                '"name": "sensor1"'
                "}",
                "menuai/sensor/sens2/config": "{"
                '"device":{"identifiers":["0AFFD2"]},'
...
async def test_cleanup_device_manual(
    ...
    for discovery_topic, discovery_payload in discovery_payloads.items():
        async_fire_mqtt_message(menuai, discovery_topic, discovery_payload)
...
async def test_cleanup_device_mqtt(
    ...
    base_discovery_topic = "menuai/sensor/bla_base/config"
    base_entity_id = "sensor.none_sensor_base"
    async_fire_mqtt_message(menuai, base_discovery_topic, data)
    await menuai.async_block_till_done()
    ...
    assert entity_entry is not None
    state = menuai.states.get(base_entity_id)
    assert state is not None
    ...
    async_fire_mqtt_message(menuai, discovery_topic, "")
    await menuai.async_block_till_done()
    await menuai.async_block_till_done()
...
async def test_cleanup_device_mqtt_device_discovery(
    ...
    discovery_topic = "menuai/device/bla/config"
    discovery_payload = (
        '{ "device":{"identifiers":["0AFFD2"]},'
        '  "o": {"name": "foobar"},'
        '  "cmps": {"sens1": {'
    ...
    async_fire_mqtt_message(menuai, discovery_topic, discovery_payload_update1)
    await menuai.async_block_till_done()
    ...
    async_fire_mqtt_message(menuai, discovery_topic, discovery_payload_update2)
    await menuai.async_block_till_done()
    device_entry = device_registry.async_get_device(identifiers={("mqtt", "0AFFD2")})
...
async def test_cleanup_device_multiple_config_entries(
    ...
    async_fire_mqtt_message(menuai, "menuai/sensor/bla/config", sensor_data)
    async_fire_mqtt_message(menuai, "menuai/tag/bla/config", tag_data)
    async_fire_mqtt_message(
        menuai, "menuai/device_automation/bla/config", trigger_data
    )
    await menuai.async_block_till_done()
    ...
    state = menuai.states.get("sensor.none_mqtt_sensor")
    assert state is not None
    ...
    mqtt_mock.async_publish.assert_has_calls(
        [
            call("menuai/sensor/bla/config", None, 0, True),
            call("menuai/tag/bla/config", None, 0, True),
            call("menuai/device_automation/bla/config", None, 0, True),
        ],
        any_order=True,
    )
...
async def test_cleanup_device_multiple_config_entries_mqtt(
    ...
    async_fire_mqtt_message(menuai, "menuai/sensor/bla/config", "")
    async_fire_mqtt_message(menuai, "menuai/tag/bla/config", "")
    async_fire_mqtt_message(menuai, "menuai/device_automation/bla/config", "")
...
async def test_discovery_expansion(
    ...
    async_fire_mqtt_message(menuai, "menuai/switch/bla/config", data)
    await menuai.async_block_till_done()

    state = menuai.states.get("switch.DiscoveryExpansionTest1")
    assert state and state.state == STATE_UNAVAILABLE

    async_fire_mqtt_message(menuai, "avail_item2/some/base/topic", "available")
    await menuai.async_block_till_done()

    state = menuai.states.get("switch.DiscoveryExpansionTest1")
    assert state is not None
    assert state.name == "DiscoveryExpansionTest1"
    assert ("switch", "bla") in menuai.data["mqtt"].discovery_already_discovered
    assert state.state == STATE_UNKNOWN

    async_fire_mqtt_message(menuai, "test_topic/some/base/topic", "ON")

    state = menuai.states.get("switch.DiscoveryExpansionTest1")
    assert state and state.state == STATE_ON

    async_fire_mqtt_message(menuai, "some/base/topic/avail_item1", "not_available")
    await menuai.async_block_till_done()
...
async def test_discovery_expansion_2(
    ...
    async_fire_mqtt_message(menuai, "some/base/topic/avail_item1", "available")
    await menuai.async_block_till_done()
...
async def test_discovery_expansion_3(
    ...
    async_fire_mqtt_message(menuai, "menuai/switch/bla/config", data)
    await menuai.async_block_till_done()
    assert menuai.states.get("switch.DiscoveryExpansionTest1") is None
...
async def test_discovery_expansion_without_encoding_and_value_template_1(
    ...
    async_fire_mqtt_message(menuai, "some/base/topic/avail_item1", b"\x01")
    await menuai.async_block_till_done()
    ...
    async_fire_mqtt_message(menuai, "some/base/topic/avail_item1", b"\x00")
...
async def test_missing_discover_abbreviations(
    ...
    await menuai.async_add_executor_job(_add_missing)
...
async def test_no_implicit_state_topic_switch(
    ...
    state = menuai.states.get("switch.Test1")
    assert state is not None
    ...
    async_fire_mqtt_message(menuai, "menuai/switch/bla/state", "ON")
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            {
                mqtt.CONF_DISCOVERY_PREFIX: "my_home/menuai/register",
            },
        )
    ],
)
async def test_complex_discovery_topic_prefix(
    ...
    async_fire_mqtt_message(
        menuai,
        ("my_home/menuai/register/binary_sensor/node1/object1/config"),
        '{ "name": "Beer", "state_topic": "test-topic" }',
    )
    await menuai.async_block_till_done()
    ...
    assert state is not None
    assert state.name == "Beer"
    assert ("binary_sensor", "node1 object1") in menuai.data[
        "mqtt"
    ].discovery_already_discovered
...
@patch("menuai.components.mqtt.client.UNSUBSCRIBE_COOLDOWN", 0.0)
@pytest.mark.parametrize(
...
async def test_mqtt_integration_discovery_flow_fitering_on_redundant_payload(
    menuai: menuai, mqtt_client_mock: MqttMockPahoClient, reason: str
) -> None:
    ...
    with (
        patch(
            "menuai.components.mqtt.discovery.async_get_mqtt",
        ...
        await menuai.async_block_till_done(wait_background_tasks=True)
        async_fire_mqtt_message(menuai, "comp/discovery/bla/config", "initial message")
        await menuai.async_block_till_done(wait_background_tasks=True)
        ...
        await menuai.async_block_till_done(wait_background_tasks=True)
        async_fire_mqtt_message(menuai, "comp/discovery/bla/config", "update message")
        await menuai.async_block_till_done(wait_background_tasks=True)
...
async def test_mqtt_discovery_flow_starts_once(
    ...
    """
    mock_integration(
        menuai, MockModule(domain="comp", async_setup_entry=AsyncMock(return_value=True))
    )
    mock_platform(menuai, "comp.config_flow", None)
    ...
    with (
        ...
        async_fire_mqtt_message(menuai, "comp/discovery/bla/config1", "initial message")
        await menuai.async_block_till_done(wait_background_tasks=True)
        ...
        with caplog.at_level(logging.DEBUG):
            async_fire_mqtt_message(
                menuai, "comp/discovery/bla/config1", "initial message"
            )
            await menuai.async_block_till_done(wait_background_tasks=True)
        ...
        async_fire_mqtt_message(menuai, "comp/discovery/bla/config1", "update message")
        await menuai.async_block_till_done(wait_background_tasks=True)
        ...
        async_fire_mqtt_message(menuai, "comp/discovery/bla/config2", "initial message")
        await menuai.async_block_till_done(wait_background_tasks=True)
        ...
        async_fire_mqtt_message(menuai, "comp/discovery/bla/config2", "update message")
        await menuai.async_block_till_done(wait_background_tasks=True)
        ...
        async_fire_mqtt_message(menuai, "comp/discovery/bla/config2", "")
        await menuai.async_block_till_done(wait_background_tasks=True)
        ...
        assert len(menuai.config_entries.async_entries(domain="comp")) == 1
        ...
        await menuai.config_entries.async_remove(entry.entry_id)
        assert len(menuai.config_entries.async_entries(domain="comp")) == 0
...
async def test_clear_config_topic_disabled_entity(
    ...
    config = {
        "state_topic": "menuai_test/sensor/sbfspot_0/sbfspot_12345/",
    ...
    async_fire_mqtt_message(
        menuai,
        "menuai/sensor/sbfspot_0/sbfspot_12345/config",
        json.dumps(config),
    )
    await menuai.async_block_till_done()
    ...
    async_fire_mqtt_message(
        menuai,
        "menuai/sensor/sbfspot_0/sbfspot_12345_1/config",
    ...
    async_fire_mqtt_message(
        menuai,
        "menuai/sensor/sbfspot_0/sbfspot_12345_2/config",
        json.dumps(config_not_unique2),
    )
    await menuai.async_block_till_done()
    assert "Platform mqtt does not generate unique IDs" in caplog.text

    assert menuai.states.get("sensor.abc123_sbfspot_12345") is None  # disabled
    assert menuai.states.get("sensor.abc123_sbfspot_12345_1") is not None  # enabled
    assert menuai.states.get("sensor.abc123_sbfspot_12345_2") is None  # not unique
    ...
    assert (
        call("menuai/sensor/sbfspot_0/sbfspot_12345/config", None, 0, True)
        in mqtt_mock.async_publish.mock_calls
    )
    assert (
        call("menuai/sensor/sbfspot_0/sbfspot_12345_1/config", None, 0, True)
        in mqtt_mock.async_publish.mock_calls
    )
...
async def test_clean_up_registry_monitoring(
    ...
    await mqtt_mock_entry()
    hooks: dict = menuai.data["mqtt"].discovery_registry_hooks
    # discover an entity that is not enabled by default
    config1 = {
        "name": "sbfspot_12345",
        "state_topic": "menuai_test/sensor/sbfspot_0/sbfspot_12345/",
...
async def test_unique_id_collission_has_priority(
    ...
    assert menuai.states.get("sensor.abc123_sbfspot_12345_1") is None  # not enabled
    assert menuai.states.get("sensor.abc123_sbfspot_12345_2") is None  # not unique
...
async def test_update_with_bad_config_not_breaks_discovery(
    ...
    config1 = {
        "name": "sbfspot_12345",
        "state_topic": "menuai_test/sensor/sbfspot_0/state",
    }
    async_fire_mqtt_message(
        menuai,
        "menuai/sensor/sbfspot_0/config",
        json.dumps(config1),
    )
    await menuai.async_block_till_done()
    assert menuai.states.get("sensor.sbfspot_12345") is not None
    ...
    config3 = {
        "name": "sbfspot_12345",
        "state_topic": "menuai_test/sensor/sbfspot_0/new_state_topic",
    }
    async_fire_mqtt_message(
        menuai,
        "menuai/sensor/sbfspot_0/config",
    ...
    async_fire_mqtt_message(
        menuai,
        "menuai_test/sensor/sbfspot_0/new_state_topic",
        "new_value",
    )
    await menuai.async_block_till_done()

    state = menuai.states.get("sensor.sbfspot_12345")
...
async def test_discovery_dispatcher_signal_type_messages(
    menuai: menuai, signal_message: SignalTypeFormat[MQTTDiscoveryPayload]
) -> None:
    ...
    unsub = async_dispatcher_connect(
        menuai, signal_message.format(*domain_id_tuple), _callback
    )
    async_dispatcher_send(menuai, signal_message.format(*domain_id_tuple), test_data)
    await menuai.async_block_till_done()
    assert len(calls) == 1
...
async def test_shared_state_topic(
    ...
    async_fire_mqtt_message(menuai, "foobar/sensor-shared", "New state")
    ...
    async_fire_mqtt_message(menuai, "foobar/sensor3", "New state3")
    entity_id = entity_ids[2]
    state = menuai.states.get(entity_id)
    assert state is not None
...
async def test_discovery_with_late_via_device_discovery(
    ...
    via_device_discovery_topic = "menuai/switch/very_unique/config"
    async_fire_mqtt_message(
        menuai,
