...
from menuai.components.mqtt.models import (
    MessageCallbackType,
    MqttCommandTemplateException,
    MqttValueTemplateException,
    ReceiveMessage,
)
from menuai.components.mqtt.schemas import MQTT_ENTITY_DEVICE_INFO_SCHEMA
...
from menuai.helpers import device_registry as dr, entity_registry as er, template
...
async def test_command_template_variables(
    menuai: menuai, mqtt_mock_entry: MqttMockHAClientGenerator, config: ConfigType
) -> None:
    ...
    async_fire_mqtt_message(menuai, "menuai/select/bla/config", json.dumps(config))
    await menuai.async_block_till_done()
    ...
    with patch(
        "menuai.helpers.template.TemplateStateFromEntityId", MagicMock()
...
async def test_command_template_fails(menuai: menuai) -> None:
    """Test the exception handling of an MQTT command template."""
    tpl = template.Template("{{ value * 2 }}", menuai=menuai)
    cmd_tpl = mqtt.MqttCommandTemplate(tpl)
...
async def test_value_template_value(menuai: menuai) -> None:
    """Test the rendering of MQTT value template."""

    variables = {"id": 1234, "some_var": "beer"}

    # test rendering value
    tpl = template.Template("{{ value_json.id }}", menuai=menuai)
    val_tpl = mqtt.MqttValueTemplate(tpl)
    assert val_tpl.async_render_with_possible_json_value('{"id": 4321}') == "4321"

    # test variables at rendering
    tpl = template.Template("{{ value_json.id }} {{ some_var }} {{ code }}", menuai=menuai)
    ...
    tpl = template.Template("{{ value_json.id | as_datetime }}", menuai=menuai)
    val_tpl = mqtt.MqttValueTemplate(tpl)
    assert (
    ...
    tpl2 = template.Template("{{ this.entity_id }}", menuai=menuai)
    ...
    with patch(
        ...
        tpl3 = template.Template("{{ this.entity_id }}", menuai=menuai)
...
async def test_value_template_fails(menuai: menuai) -> None:
    """Test the rendering of MQTT value template fails."""
    entity = MockEntity(entity_id="sensor.test")
    entity.menuai = menuai
    entity.platform = MockEntityPlatform(menuai)
    tpl = template.Template("{{ value_json.some_var * 2 }}", menuai=menuai)
    val_tpl = mqtt.MqttValueTemplate(tpl, entity=entity)
...
async def test_service_call_mqtt_entry_does_not_publish(
    ...
    """Test the service call if topic is missing."""
    assert await async_setup_component(menuai, mqtt.DOMAIN, {})
    with pytest.raises(
...
async def test_mqtt_publish_action_call_with_raw_data(
    ...
    with patch(
        "menuai.components.mqtt.models.literal_eval"
...
async def test_publish_function_with_bad_encoding_conditions(
    ...
    await mqtt.async_publish(
        menuai, "some-topic", "test-payload", qos=0, retain=False, encoding=None
    )
    assert (
...
async def test_handle_logging_on_writing_the_entity_state(
    ...
    await mqtt_mock_entry()
    await menuai.async_block_till_done()
    async_fire_mqtt_message(menuai, "test/state", b"initial_state")
    await menuai.async_block_till_done()
    ...
    with patch(
        "menuai.helpers.entity.Entity.async_write_ha_state",
        side_effect=ValueError("Invalid value for sensor"),
    ):
        async_fire_mqtt_message(menuai, "test/state", b"payload causing errors")
        await menuai.async_block_till_done()
        state = menuai.states.get("sensor.test_sensor")
        assert state is not None
        assert state.state == "initial_state"
...
async def test_receiving_non_utf8_message_gets_logged(
    ...
    async_fire_mqtt_message(menuai, "test-topic", b"\x9a")
...
async def test_receiving_message_with_non_utf8_topic_gets_logged(
    ...
    # pylint: disable-next=import-outside-toplevel
    from menuai.components.mqtt.models import MqttData
...
async def test_reload_entry_with_restored_subscriptions(
    ...
    with patch("menuai.config.load_yaml_config_file", return_value={}):
        await menuai.config_entries.async_setup(entry.entry_id)

    mock_debouncer.clear()
    await mqtt.async_subscribe(menuai, "test-topic", record_calls)
    await mqtt.async_subscribe(menuai, "wild/+/card", record_calls)
    # cooldown
    await mock_debouncer.wait()

    async_fire_mqtt_message(menuai, "test-topic", "test-payload")
    async_fire_mqtt_message(menuai, "wild/any/card", "wild-card-payload")
    ...
    async_fire_mqtt_message(menuai, "test-topic", "test-payload2")
    async_fire_mqtt_message(menuai, "wild/any/card", "wild-card-payload2")
    ...
    async_fire_mqtt_message(menuai, "test-topic", "test-payload3")
    async_fire_mqtt_message(menuai, "wild/any/card", "wild-card-payload3")
...
@pytest.mark.parametrize("menuai_config", [{mqtt.DOMAIN: {"light": {"name": "test"}}}])
...
async def test_default_entry_setting_are_applied(
    ...
    async_fire_mqtt_message(menuai, "menuai/sensor/bla/config", data)
    await menuai.async_block_till_done()
...
async def test_message_callback_exception_gets_logged(
    ...
    await mqtt.async_subscribe(menuai, "test-topic", bad_handler)
    async_fire_mqtt_message(menuai, "test-topic", "test")
    await menuai.async_block_till_done()
...
async def test_message_partial_callback_exception_gets_logged(
    ...
    await mqtt.async_subscribe(
        menuai, "test-topic", partial(parial_handler, bad_handler, {"some_attr"})
    )
    await mock_debouncer.wait()
    async_fire_mqtt_message(menuai, "test-topic", "test")
    await menuai.async_block_till_done()
...
async def test_mqtt_ws_subscription(
    ...
    async_fire_mqtt_message(menuai, "test-topic", "test1")
    async_fire_mqtt_message(menuai, "test-topic", "test2")
    async_fire_mqtt_message(menuai, "test-topic", b"\xde\xad\xbe\xef")
    ...
    async_fire_mqtt_message(menuai, "test-topic", "test1", 2)
    async_fire_mqtt_message(menuai, "test-topic", "test2", 2)
    async_fire_mqtt_message(menuai, "test-topic", b"\xde\xad\xbe\xef", 2)
...
async def test_mqtt_ws_subscription_not_admin(
    ...
    await mqtt_mock_entry()
    client = await menuai_ws_client(menuai, access_token=menuai_read_only_access_token)
    await client.send_json({"id": 5, "type": "mqtt/subscribe", "topic": "test-topic"})
    response = await client.receive_json()
...
async def test_dump_service(
    ...
    async_fire_mqtt_message(menuai, "bla/1", "test1")
    async_fire_mqtt_message(menuai, "bla/2", "test2")

    with patch("menuai.components.mqtt.open", mopen):
        async_fire_time_changed(menuai, utcnow() + timedelta(seconds=3))
        await menuai.async_block_till_done()
...
async def test_mqtt_ws_get_device_debug_info_binary(
    ...
    async_fire_mqtt_message(menuai, "menuai/camera/bla/config", data)
    await menuai.async_block_till_done()
    ...
    async_fire_mqtt_message(menuai, "foobar/image", small_png)
    await menuai.async_block_till_done()
        ...
        "entities": [
            {
                ...
                "discovery_data": {
                    "payload": config,
                    "topic": "menuai/camera/bla/config",
                },
                "transmitted": [],
            }
        ],
...
async def test_debug_info_multiple_devices(
    ...
    for dev in devices:
        ...
        async_fire_mqtt_message(
            menuai, f"menuai/{domain}/{device_id}/config", data
        )
        await menuai.async_block_till_done()
    ...
    for dev in devices:
        ...
        assert discovery_data["topic"] == f"menuai/{domain}/{device_id}/config"
...
async def test_debug_info_multiple_entities_triggers(
    ...
    for c in config:
        ...
        async_fire_mqtt_message(
            menuai, f"menuai/{domain}/{discovery_id}/config", data
        )
        await menuai.async_block_till_done()
    ...
    for c in config:
        ...
        assert {
            "topic": f"menuai/{domain}/{discovery_id}/config",
            "payload": c["config"],
        } in discovery_data
...
async def test_debug_info_non_mqtt(
    ...
    assert await async_setup_component(
        menuai, mqtt.DOMAIN, {mqtt.DOMAIN: {domain: {"platform": "test"}}}
    )

    debug_info_data = debug_info.info_for_device(menuai, device_entry.id)
    assert len(debug_info_data["entities"]) == 0
    assert len(debug_info_data["triggers"]) == 0
...
async def test_debug_info_wildcard(
    ...
    async_fire_mqtt_message(menuai, "sensor/abc", "123")
...
async def test_debug_info_same_topic(
    ...
    start_dt = datetime(2019, 1, 1, 0, 0, 0, tzinfo=dt_util.UTC)
    freezer.move_to(start_dt)
    async_fire_mqtt_message(menuai, "sensor/status", "123", qos=0, retain=False)
...
async def test_debug_info_qos_retain(
    ...
    async_fire_mqtt_message(menuai, "sensor/abc", "123", qos=0, retain=True)
    # simulate an update message
    async_fire_mqtt_message(menuai, "sensor/abc", "123", qos=0, retain=False)
    # simpulate someone else subscribed and retained messages were replayed
    async_fire_mqtt_message(menuai, "sensor/abc", "123", qos=1, retain=True)
    # simulate an update message
    async_fire_mqtt_message(menuai, "sensor/abc", "123", qos=1, retain=False)
    # simulate an update message
    async_fire_mqtt_message(menuai, "sensor/abc", "123", qos=2, retain=False)
...
async def test_unload_config_entry(
    ...
    assert await async_setup_component(menuai, mqtt.DOMAIN, {})
    assert menuai.services.has_service(mqtt.DOMAIN, "dump")
    assert menuai.services.has_service(mqtt.DOMAIN, "publish")
    ...
    mqtt.publish(menuai, "just_in_time", "published", qos=0, retain=False)
    await menuai.async_block_till_done()
...
async def test_publish_or_subscribe_without_valid_config_entry(
    ...
    with pytest.raises(menuaiError):
        await mqtt.async_subscribe(menuai, "some-topic", record_calls, qos=0)
...
async def test_disabling_and_enabling_entry(
    ...
    with patch(
        "menuai.components.mqtt.entity.mqtt_config_entry_enabled",
        return_value=False,
    ):
        # Discovery of mqtt tag
        async_fire_mqtt_message(menuai, "menuai/tag/abc/config", config_tag)

        # Late discovery of mqtt entities
        async_fire_mqtt_message(
            menuai,
            "menuai/alarm_control_panel/abc/config",
            config_alarm_control_panel,
        )
        async_fire_mqtt_message(menuai, "menuai/light/abc/config", config_light)
    ...
    await menuai.config_entries.async_set_disabled_by(entry.entry_id, None)
    await menuai.async_block_till_done()
    await menuai.async_block_till_done()
    new_mqtt_config_entry = entry
    assert new_mqtt_config_entry.state is ConfigEntryState.LOADED

    assert menuai.states.get("light.test") is not None
    assert menuai.states.get("alarm_control_panel.test") is not None


@pytest.mark.parametrize(
    ("menuai_config", "unique"),
    [
        (
            {
                mqtt.DOMAIN: {
...
async def test_setup_manual_items_with_unique_ids(
    ...
    assert menuai.states.get("light.test1") is not None
    assert (menuai.states.get("light.test2") is not None) == unique
...
async def test_link_config_entry(
    ...
    async_fire_mqtt_message(
        menuai, "menuai/sensor/bla/config", json.dumps(config_discovery)
    )
    await menuai.async_block_till_done()
    await menuai.async_block_till_done()

    assert menuai.states.get("sensor.test_manual") is not None
    assert menuai.states.get("sensor.test_discovery") is not None
    entity_names = ["test_manual", "test_discovery"]

    # Check if both entities were linked to the MQTT config entry
    mqtt_config_entry = menuai.config_entries.async_entries(mqtt.DOMAIN)[0]
    mqtt_platforms = async_get_platforms(menuai, mqtt.DOMAIN)
    ...
    with patch("menuai.components.mqtt.async_client.AsyncMQTTClient"):
        await menuai.config_entries.async_reload(mqtt_config_entry.entry_id)
        await menuai.async_block_till_done()
...
async def test_reload_config_entry(
    ...
    async_fire_mqtt_message(menuai, "test-topic_manual1", "manual1_intial")
    async_fire_mqtt_message(menuai, "test-topic_manual3", "manual3_intial")

    assert (state := menuai.states.get("sensor.test_manual1")) is not None
    assert state.attributes["friendly_name"] == "test_manual1"
    assert state.state == "manual1_intial"
    assert (state := menuai.states.get("sensor.test_manual3")) is not None
    ...
    with patch(
        "menuai.config.load_yaml_config_file", return_value=menuai_config_new
    ):
        assert await menuai.config_entries.async_reload(entry.entry_id)
        assert entry.state is ConfigEntryState.LOADED
        await menuai.async_block_till_done()
    ...
    assert (state := menuai.states.get("sensor.test_manual2_new")) is not None
    ...
    assert (state := menuai.states.get("sensor.test_discovery")) is not None
    assert state.state is STATE_UNAVAILABLE
    # The entity is not loaded anymore
    assert _check_entities() == 2

    async_fire_mqtt_message(menuai, "test-topic_manual1_updated", "manual1_update")
    async_fire_mqtt_message(menuai, "test-topic_manual2", "manual2_update")
    async_fire_mqtt_message(menuai, "test-topic_manual3", "manual3_update")
    ...
    async_fire_mqtt_message(
        menuai, "test-topic_manual1_updated", "manual1_update_after_reload"
    )
    async_fire_mqtt_message(menuai, "test-topic_manual2", "manual2_update_after_reload")
    async_fire_mqtt_message(menuai, "test-topic_manual3", "manual3_update_after_reload")
...
async def test_reload_with_invalid_config(
    ...
    await mqtt_mock_entry()
    assert menuai.states.get("sensor.test") is not None

    # Reload with an invalid config and assert again
    invalid_config = {"mqtt": "some_invalid_config"}
    with patch(
        "menuai.config.load_yaml_config_file", return_value=invalid_config
    ):
        with pytest.raises(menuaiError):
            await menuai.services.async_call(
                "mqtt",
...
async def test_reload_with_new_platform_config(
    ...
    assert menuai.states.get("sensor.test") is not None
    assert menuai.states.get("binary_sensor.test") is not None
...
async def test_multi_platform_discovery(
    ...
    for platform, config in entity_configs.items():
        for set_number in range(2):
            ...
            topic = f"menuai/{platform}/bla_{set_number}/config"
            async_fire_mqtt_message(menuai, topic, json.dumps(set_config))
    for platform, config in non_entity_configs.items():
        topic = f"menuai/{platform}/bla/config"
        async_fire_mqtt_message(menuai, topic, json.dumps(config))
    await menuai.async_block_till_done()
    for set_number in range(2):
