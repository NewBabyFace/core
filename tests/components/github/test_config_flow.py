...
from menuai.components.github.config_flow import get_repositories
from menuai.components.github.const import (
    CONF_REPOSITORIES,
...
async def test_flow_with_remove_while_activating(
    ...
    assert menuai.config_entries.flow.async_get(result["flow_id"])
...
async def test_starred_pagination_with_paginated_result(menuai: menuai) -> None:
    """Test pagination of starred repositories with paginated result."""
    with patch(
        "menuai.components.github.config_flow.GitHubAPI",
        ...
        repos = await get_repositories(menuai, MOCK_ACCESS_TOKEN)

    assert len(repos) == 2
    assert repos[-1] == DEFAULT_REPOSITORIES[0]


async def test_starred_pagination_with_no_starred(menuai: menuai) -> None:
...
async def test_starred_pagination_with_exception(menuai: menuai) -> None:
...
async def test_options_flow(
    ...
    menuai.config_entries.async_update_entry(
        mock_config_entry,
        options={
            CONF_REPOSITORIES: ["menuai/core", "menuai/architecture"]
        },
    )
    ...
    result = await menuai.config_entries.options.async_configure(
        result["flow_id"],
        user_input={CONF_REPOSITORIES: ["menuai/core"]},
    )

    assert "menuai/architecture" not in result["data"][CONF_REPOSITORIES]
