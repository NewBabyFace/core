...
from menuai.components.ecobee.switch import DATE_FORMAT
...
async def test_ventilator_20min_attributes(menuai: menuai) -> None:
    """Test the ventilator switch on home attributes are correct."""
    await setup_platform(menuai, SWITCH_DOMAIN)

    state = menuai.states.get(VENTILATOR_20MIN_ID)
    assert state.state == "off"


async def test_ventilator_20min_when_on(menuai: menuai, data) -> None:
...
async def test_ventilator_20min_when_off(menuai: menuai, data) -> None:
    """Test the ventilator switch goes on."""
...
async def test_ventilator_20min_when_empty(menuai: menuai, data) -> None:
    """Test the ventilator switch goes on."""
...
async def test_turn_on_20min_ventilator(menuai: menuai) -> None:
    """Test the switch 20 min timer (On)."""

    with patch(
        "menuai.components.ecobee.Ecobee.set_ventilator_timer"
...
async def test_turn_off_20min_ventilator(menuai: menuai) -> None:
...
async def test_aux_heat_only_turn_on(menuai: menuai) -> None:
...
async def test_aux_heat_only_turn_off(menuai: menuai) -> None:
