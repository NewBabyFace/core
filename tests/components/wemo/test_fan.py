...
from menuai.components.wemo import fan
...
async def test_available_after_update(
    ...
    await entity_test_helpers.test_avaliable_after_update(
        menuai, pywemo_registry, pywemo_device, wemo_entity, FAN_DOMAIN
    )


async def test_turn_off_state(menuai: menuai, wemo_entity) -> None:
    """Test that the device state is updated after turning off."""
    await entity_test_helpers.test_turn_off_state(menuai, wemo_entity, FAN_DOMAIN)
...
async def test_fan_set_humidity_service(
    menuai: menuai, pywemo_device, wemo_entity, test_input, expected
) -> None:
...
async def test_fan_set_percentage(
    menuai: menuai, pywemo_device, wemo_entity, percentage, expected_fan_mode
) -> None:
...
async def test_fan_mode_high_initially(menuai: menuai, pywemo_device) -> None:
    """Verify the FanMode is set to High when turned on."""
    pywemo_device.fan_mode = FanMode.Off
    wemo_entity = await async_create_wemo_entity(menuai, pywemo_device, "")
    await menuai.services.async_call(
        FAN_DOMAIN,
        SERVICE_TURN_ON,
        {ATTR_ENTITY_ID: [wemo_entity.entity_id]},
