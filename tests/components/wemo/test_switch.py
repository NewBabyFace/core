...
from menuai.components.wemo.switch import (
...
async def test_available_after_update(
    ...
    await entity_test_helpers.test_avaliable_after_update(
        menuai, pywemo_registry, pywemo_device, wemo_entity, SWITCH_DOMAIN
    )


async def test_turn_off_state(menuai: menuai, wemo_entity) -> None:
    """Test that the device state is updated after turning off."""
    await entity_test_helpers.test_turn_off_state(menuai, wemo_entity, SWITCH_DOMAIN)


async def test_insight_state_attributes(menuai: menuai, pywemo_registry) -> None:
    """Verify the switch attributes are set for the Insight device."""
    await async_setup_component(menuai, HA_DOMAIN, {})
    with create_pywemo_device(pywemo_registry, "Insight") as insight:
        wemo_entity = await async_create_wemo_entity(menuai, insight, "")
        attributes = menuai.states.get(wemo_entity.entity_id).attributes
...
async def test_maker_state_attributes(menuai: menuai, pywemo_registry) -> None:
    """Verify the switch attributes are set for the Insight device."""
    await async_setup_component(menuai, HA_DOMAIN, {})
    with create_pywemo_device(pywemo_registry, "Maker") as maker:
        wemo_entity = await async_create_wemo_entity(menuai, maker, "")
        attributes = menuai.states.get(wemo_entity.entity_id).attributes
