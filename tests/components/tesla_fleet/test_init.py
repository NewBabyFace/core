...
from menuai.components.tesla_fleet.const import AUTHORIZE_URL
from menuai.components.tesla_fleet.coordinator import (
...
from menuai.components.tesla_fleet.models import TeslaFleetData
...
async def test_load_unload(
    ...
    assert await menuai.config_entries.async_unload(normal_config_entry.entry_id)
...
async def test_oauth_refresh_expired(
    ...
    with patch(
        "menuai.components.tesla_fleet.OAuth2Session.async_ensure_token_valid",
...
async def test_vehicle_refresh_error(
    ...
    assert (state := menuai.states.get("sensor.test_battery_level"))
...
async def test_vehicle_sleep(
    ...
    with patch(
        "menuai.components.tesla_fleet.coordinator.VEHICLE_INTERVAL",
        TEST_INTERVAL,
    ):
        await setup_platform(menuai, normal_config_entry)
...
async def test_signing(
    ...
    with patch(
        "menuai.components.tesla_fleet.TeslaFleetApi.get_private_key"
...
async def test_bad_implementation(
    ...
    await setup_platform(menuai, bad_config_entry)
    assert bad_config_entry.state is ConfigEntryState.SETUP_ERROR

    # Ensure reauth flow starts
    assert any(bad_config_entry.async_get_active_flows(menuai, {"reauth"}))
    result = await bad_config_entry.start_reauth_flow(menuai)
