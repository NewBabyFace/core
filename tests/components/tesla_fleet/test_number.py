...
async def test_number(
    ...
    await setup_platform(menuai, normal_config_entry, [Platform.NUMBER])
...
async def test_number_offline(
    ...
    await setup_platform(menuai, normal_config_entry, [Platform.NUMBER])
    state = menuai.states.get("number.test_charge_current")
    assert state.state == STATE_UNKNOWN


@pytest.mark.usefixtures("entity_registry_enabled_by_default")
async def test_number_services(
    menuai: menuai, mock_vehicle_data, normal_config_entry: MockConfigEntry
) -> None:
