"""Test the MenuAI hardware firmware config flow."""
...
from menuai.components.menuaiio import AddonInfo, AddonState
...
async def mock_test_firmware_platform(
    ...
    mock_integration(menuai, mock_module)
...
def mock_addon_info(
    ...
    *,
    ...
    mock_flasher_manager = Mock(spec_set=get_zigbee_flasher_addon_manager(menuai))
    ...
    mock_otbr_manager = Mock(spec_set=get_otbr_addon_manager(menuai))
    ...
    with (
        patch(
            "menuai.components.menuai_hardware.firmware_config_flow.get_otbr_addon_manager",
            return_value=mock_otbr_manager,
        ),
        patch(
            "menuai.components.menuai_hardware.util.get_otbr_addon_manager",
            return_value=mock_otbr_manager,
        ),
        patch(
            "menuai.components.menuai_hardware.firmware_config_flow.get_zigbee_flasher_addon_manager",
            return_value=mock_flasher_manager,
        ),
        patch(
            "menuai.components.menuai_hardware.firmware_config_flow.is_menuaiio",
            return_value=is_menuaiio,
        ),
        patch(
            "menuai.components.menuai_hardware.util.is_menuaiio",
            return_value=is_menuaiio,
        ),
        patch(
...
async def test_config_flow_zigbee(menuai: menuai) -> None:
...
async def test_config_flow_zigbee_skip_step_if_installed(menuai: menuai) -> None:
...
async def test_config_flow_auto_confirm_if_running(menuai: menuai) -> None:
    """Test the config flow skips the confirmation step the hardware is already used."""
    with patch(
        "menuai.components.menuai_hardware.firmware_config_flow.guess_firmware_info",
...
async def test_config_flow_thread(menuai: menuai) -> None:
    """Test the config flow."""
    result = await menuai.config_entries.flow.async_init(
        TEST_DOMAIN, context={"source": "hardware"}
    )
...
async def test_config_flow_thread_addon_already_installed(menuai: menuai) -> None:
...
async def test_config_flow_zigbee_not_menuaiio(menuai: menuai) -> None:
    """Test when the stick is used with a non-menuaiio setup."""
    result = await menuai.config_entries.flow.async_init(
        TEST_DOMAIN, context={"source": "hardware"}
    )
...
async def test_options_flow_zigbee_to_thread(menuai: menuai) -> None:
...
@pytest.mark.usefixtures("addon_store_info")
async def test_options_flow_thread_to_zigbee(menuai: menuai) -> None:
