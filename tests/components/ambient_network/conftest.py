...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "menuai.components.ambient_network.async_setup_entry", return_value=True
    ) as mock_setup_entry:
...
async def mock_aioambient(open_api: OpenAPI):
    ...
    with (
        patch(
            "menuai.components.ambient_network.config_flow.OpenAPI",
            return_value=open_api,
        ),
        patch(
            "menuai.components.ambient_network.OpenAPI",
            return_value=open_api,
        ),
    ):
        yield
...
async def setup_platform(
    ...
    assert (
        await menuai.config_entries.async_setup(config_entry.entry_id) == expected_outcome
    )
