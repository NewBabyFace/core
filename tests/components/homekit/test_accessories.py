...
from menuai.components.homekit.accessories import (
    HomeAccessory,
    HomeBridge,
    HomeDriver,
)
...
from menuai.components.homekit.util import format_version
...
async def test_accessory_cancels_track_state_change_on_stop(
    ...
    acc = HomeAccessory(
        menuai, hk_driver, "Home Accessory", entity_id, 2, {"platform": "isy994"}
    )
    with patch(
        "menuai.components.homekit.accessories.HomeAccessory.async_update_state"
    ):
        acc.run()
    await acc.stop()


async def test_home_accessory(menuai: menuai, hk_driver) -> None:
    """Test HomeAccessory class."""
    entity_id = "sensor.accessory"
    entity_id2 = "light.accessory_that_exceeds_the_maximum_maximum_maximum_maximum_maximum_maximum_maximum_allowed_length"

    menuai.states.async_set(entity_id, None)
    menuai.states.async_set(entity_id2, STATE_UNAVAILABLE)
    ...
    acc2 = HomeAccessory(menuai, hk_driver, "Home Accessory", entity_id2, 3, {})
    ...
    assert format_version(menuai_version).startswith(
    ...
    menuai.states.async_set(entity_id, None)
    await menuai.async_block_till_done()
    acc = HomeAccessory(menuai, hk_driver, "test_name", entity_id, 6, None)
    serv = acc.services[0]  # SERV_ACCESSORY_INFO
...
async def test_accessory_with_missing_basic_service_info(
    ...
    assert serv.get_characteristic(CHAR_NAME).value == "Home Accessory"
    assert serv.get_characteristic(CHAR_MANUFACTURER).value == "MenuAI Sensor"
    assert serv.get_characteristic(CHAR_MODEL).value == "Sensor"
...
async def test_accessory_with_hardware_revision(menuai: menuai, hk_driver) -> None:
...
async def test_battery_service(
    menuai: menuai, hk_driver, caplog: pytest.LogCaptureFixture
) -> None:
    """Test battery service."""
    entity_id = "homekit.accessory"
    menuai.states.async_set(entity_id, None, {ATTR_BATTERY_LEVEL: 50})
    await menuai.async_block_till_done()

    acc = HomeAccessory(menuai, hk_driver, "Battery Service", entity_id, 2, None)
    ...
    with patch(
        ...
        menuai.states.async_set(entity_id, None, {ATTR_BATTERY_LEVEL: 15})
        await menuai.async_block_till_done()
        state = menuai.states.get(entity_id)
        mock_async_update_state.assert_called_with(state)
    ...
    with patch(
        ...
        menuai.states.async_set(entity_id, None, {ATTR_BATTERY_LEVEL: "error"})
        await menuai.async_block_till_done()
        state = menuai.states.get(entity_id)
        mock_async_update_state.assert_called_with(state)
    ...
    with patch(
        ...
        acc = HomeAccessory(menuai, hk_driver, "Battery Service", entity_id, 3, None)
        assert acc._char_battery.value == 0
        assert acc._char_low_battery.value == 0
        assert acc._char_charging.value == 2
...
async def test_linked_battery_sensor(
    ...
    menuai.states.async_set(entity_id, "open", {ATTR_BATTERY_LEVEL: 100})
    menuai.states.async_set(linked_battery, 50, None)
    await menuai.async_block_till_done()
    ...
    menuai.states.async_set(linked_battery, 10, None)
    await menuai.async_block_till_done()
    assert acc._char_battery.value == 10
    assert acc._char_low_battery.value == 1
    ...
    with patch(
        ...
        menuai.states.async_set(entity_id, "open", {ATTR_BATTERY_LEVEL: 90})
        await menuai.async_block_till_done()
    assert acc._char_battery.value == 10
    ...
    with patch(
        ...
        menuai.states.async_set(linked_battery, "error", None)
        await menuai.async_block_till_done()
    assert acc._char_battery.value == 10
    assert "ERROR" not in caplog.text

    # Test charging & low battery threshold
    menuai.states.async_set(linked_battery, 20, {ATTR_BATTERY_CHARGING: True})
    await menuai.async_block_till_done()
    ...
    menuai.states.async_set(linked_battery, 100, {ATTR_BATTERY_CHARGING: False})
    await menuai.async_block_till_done()
    assert acc._char_battery.value == 100
    assert acc._char_low_battery.value == 0
    assert acc._char_charging.value == 0

    menuai.states.async_remove(linked_battery)
    await menuai.async_block_till_done()
    assert acc._char_battery.value == 100
    assert acc._char_low_battery.value == 0
    assert acc._char_charging.value == 0
...
async def test_linked_battery_charging_sensor(
    ...
    menuai.states.async_set(entity_id, "open", {ATTR_BATTERY_LEVEL: 100})
    menuai.states.async_set(linked_battery_charging_sensor, STATE_ON, None)
    await menuai.async_block_till_done()
    ...
    with patch(
        ...
        menuai.states.async_set(linked_battery_charging_sensor, STATE_OFF, None)
        acc.run()
        await menuai.async_block_till_done()
        state = menuai.states.get(entity_id)
        mock_async_update_state.assert_called_with(state)
    ...
    with patch(
        ...
        menuai.states.async_remove(linked_battery_charging_sensor)
        acc.run()
        await menuai.async_block_till_done()
    assert acc._char_charging.value == 1
...
async def test_battery_appears_after_startup(
    ...
    acc = HomeAccessory(menuai, hk_driver, "Accessory without battery", entity_id, 2, {})
    assert acc._char_battery is None
...
async def test_call_service(
    ...
    acc = HomeAccessory(menuai, hk_driver, "Home Accessory", entity_id, 2, {})
    call_service = async_mock_service(menuai, "cover", "open_cover")
...
def test_home_bridge(hk_driver) -> None:
    ...
    bridge = HomeBridge("menuai", hk_driver, BRIDGE_NAME)
    assert bridge.menuai == "menuai"
...
def test_home_bridge_setup_message(hk_driver) -> None:
    ...
    bridge = HomeBridge("menuai", hk_driver, "test_name")
