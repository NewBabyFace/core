...
from menuai.components.homekit.type_media_players import (
    MediaPlayer,
    ReceiverMediaPlayer,
...
async def test_media_player_set_state(
    ...
    acc = MediaPlayer(menuai, hk_driver, "MediaPlayer", entity_id, 2, config)
    acc.run()
    await menuai.async_block_till_done()
    ...
    menuai.states.async_set(entity_id, STATE_PLAYING, base_attrs)
    await menuai.async_block_till_done()
    assert acc.chars[FEATURE_PLAY_PAUSE].value is True
    assert acc.chars[FEATURE_PLAY_STOP].value is True

    menuai.states.async_set(entity_id, STATE_PAUSED, base_attrs)
    await menuai.async_block_till_done()
    assert acc.chars[FEATURE_PLAY_PAUSE].value is False

    menuai.states.async_set(entity_id, STATE_IDLE, base_attrs)
    await menuai.async_block_till_done()
    assert acc.chars[FEATURE_PLAY_STOP].value is False

    # Set from HomeKit
    call_turn_on = async_mock_service(menuai, MEDIA_PLAYER_DOMAIN, "turn_on")
    call_turn_off = async_mock_service(menuai, MEDIA_PLAYER_DOMAIN, "turn_off")
    call_media_play = async_mock_service(menuai, MEDIA_PLAYER_DOMAIN, "media_play")
    call_media_pause = async_mock_service(menuai, MEDIA_PLAYER_DOMAIN, "media_pause")
    call_media_stop = async_mock_service(menuai, MEDIA_PLAYER_DOMAIN, "media_stop")
    call_toggle_mute = async_mock_service(menuai, MEDIA_PLAYER_DOMAIN, "volume_mute")
...
async def test_media_player_television(
    ...
    await menuai.async_block_till_done()
    acc = TelevisionMediaPlayer(menuai, hk_driver, "MediaPlayer", entity_id, 2, None)
    acc.run()
    await menuai.async_block_till_done()
    ...
    call_media_play_pause = async_mock_service(
        menuai, MEDIA_PLAYER_DOMAIN, "media_play_pause"
    )
    call_toggle_mute = async_mock_service(menuai, MEDIA_PLAYER_DOMAIN, "volume_mute")
    call_select_source = async_mock_service(menuai, MEDIA_PLAYER_DOMAIN, "select_source")
    call_volume_up = async_mock_service(menuai, MEDIA_PLAYER_DOMAIN, "volume_up")
    call_volume_down = async_mock_service(menuai, MEDIA_PLAYER_DOMAIN, "volume_down")
    call_volume_set = async_mock_service(menuai, MEDIA_PLAYER_DOMAIN, "volume_set")
    ...
    menuai.states.async_set(entity_id, STATE_PLAYING)
    await menuai.async_block_till_done()
    acc.char_remote_key.client_update_value(11)
    await menuai.async_block_till_done()
    assert call_media_pause
...
async def test_tv_restore(
    ...
    acc = TelevisionMediaPlayer(
        menuai, hk_driver, "MediaPlayer", "media_player.simple", 2, None
    )
    ...
    acc = TelevisionMediaPlayer(
        menuai, hk_driver, "MediaPlayer", "media_player.all_info_set", 3, None
    )
    assert acc.category == 31
    assert acc.chars_tv == [CHAR_REMOTE_KEY]
    assert acc.chars_speaker == [
...
async def test_media_player_receiver(
    menuai: menuai, hk_driver: HomeDriver, caplog: pytest.LogCaptureFixture
) -> None:
    """Test if television accessory with unsafe characters."""
    ...
    await menuai.async_block_till_done()
    acc = ReceiverMediaPlayer(menuai, hk_driver, "MediaPlayer", entity_id, 2, None)
    acc.run()
    await menuai.async_block_till_done()
