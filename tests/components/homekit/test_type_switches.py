...
from menuai.components.homekit.type_switches import (
    LawnMower,
    Outlet,
    SelectSwitch,
    Switch,
...
from menuai.core import Event, menuai, split_entity_id
...
async def test_outlet_set_state(
    ...
    acc = Outlet(menuai, hk_driver, "Outlet", entity_id, 2, None)
    acc.run()
    await menuai.async_block_till_done()
    ...
    # Set from HomeKit
    call_turn_on = async_mock_service(menuai, "switch", "turn_on")
    call_turn_off = async_mock_service(menuai, "switch", "turn_off")
...
async def test_switch_set_state(
    menuai: menuai, hk_driver, entity_id, attrs, events: list[Event]
) -> None:
    """Test if accessory and HA are updated accordingly."""
    domain = split_entity_id(entity_id)[0]

    menuai.states.async_set(entity_id, None, attrs)
    await menuai.async_block_till_done()
    acc = Switch(menuai, hk_driver, "Switch", entity_id, 2, None)
    acc.run()
    await menuai.async_block_till_done()
    ...
    menuai.states.async_set(entity_id, STATE_ON, attrs)
    await menuai.async_block_till_done()
    assert acc.char_on.value is True

    menuai.states.async_set(entity_id, STATE_OFF, attrs)
    await menuai.async_block_till_done()
    assert acc.char_on.value is False

    # Set from HomeKit
    call_turn_on = async_mock_service(menuai, domain, "turn_on")
    call_turn_off = async_mock_service(menuai, domain, "turn_off")
...
async def test_valve_switch_set_state(
    ...
    acc = ValveSwitch(menuai, hk_driver, "Valve", entity_id, 2, {CONF_TYPE: TYPE_FAUCET})
    acc.run()
    await menuai.async_block_till_done()
    assert acc.category == 29  # Faucet
    assert acc.char_valve_type.value == 3  # Water faucet

    acc = ValveSwitch(menuai, hk_driver, "Valve", entity_id, 3, {CONF_TYPE: TYPE_SHOWER})
    acc.run()
    await menuai.async_block_till_done()
    ...
    acc = ValveSwitch(
        menuai, hk_driver, "Valve", entity_id, 4, {CONF_TYPE: TYPE_SPRINKLER}
    )
    acc.run()
    await menuai.async_block_till_done()
    ...
    acc = ValveSwitch(menuai, hk_driver, "Valve", entity_id, 5, {CONF_TYPE: TYPE_VALVE})
    acc.run()
    await menuai.async_block_till_done()
    ...
    # Set from HomeKit
    call_turn_on = async_mock_service(menuai, "switch", SERVICE_TURN_ON)
    call_turn_off = async_mock_service(menuai, "switch", SERVICE_TURN_OFF)
...
async def test_valve_set_state(
    ...
    acc = Valve(menuai, hk_driver, "Valve", entity_id, 5, {CONF_TYPE: TYPE_VALVE})
    acc.run()
    await menuai.async_block_till_done()
    ...
    menuai.states.async_set(entity_id, STATE_OPEN)
    await menuai.async_block_till_done()
    assert acc.char_active.value == 1
    assert acc.char_in_use.value == 1
    ...
    # Set from HomeKit
    call_turn_on = async_mock_service(menuai, "valve", SERVICE_OPEN_VALVE)
    call_turn_off = async_mock_service(menuai, "valve", SERVICE_CLOSE_VALVE)
...
async def test_vacuum_set_state_with_returnhome_and_start_support(
    ...
    acc = Vacuum(menuai, hk_driver, "Vacuum", entity_id, 2, None)
    acc.run()
    await menuai.async_block_till_done()
    assert acc.aid == 2
    assert acc.category == 8  # Switch
    ...
    # Set from HomeKit
    call_start = async_mock_service(menuai, VACUUM_DOMAIN, SERVICE_START)
    call_return_to_base = async_mock_service(
        menuai, VACUUM_DOMAIN, SERVICE_RETURN_TO_BASE
    )
...
async def test_vacuum_set_state_without_returnhome_and_start_support(
    ...
    # Set from HomeKit
    call_turn_on = async_mock_service(menuai, VACUUM_DOMAIN, SERVICE_TURN_ON)
    call_turn_off = async_mock_service(menuai, VACUUM_DOMAIN, SERVICE_TURN_OFF)
...
async def test_lawn_mower_set_state(
    ...
    acc = LawnMower(menuai, hk_driver, "LawnMower", entity_id, 2, None)
    acc.run()
    await menuai.async_block_till_done()
    assert acc.aid == 2
    assert acc.category == 8  # Switch
    ...
    # Set from HomeKit
    call_turn_on = async_mock_service(menuai, LAWN_MOWER_DOMAIN, SERVICE_START_MOWING)
    call_turn_off = async_mock_service(menuai, LAWN_MOWER_DOMAIN, SERVICE_DOCK)
...
async def test_script_switch(
    ...
    call_turn_on = async_mock_service(menuai, domain, "test")
    call_turn_off = async_mock_service(menuai, domain, "turn_off")
...
async def test_input_select_switch(
    menuai: menuai, hk_driver, events: list[Event], domain
) -> None:
    ...
    await menuai.async_block_till_done()
    acc = SelectSwitch(menuai, hk_driver, "SelectSwitch", entity_id, 2, None)
    acc.run()
    await menuai.async_block_till_done()
    ...
    call_select_option = async_mock_service(menuai, domain, SERVICE_SELECT_OPTION)
...
async def test_button_switch(
    ...
    call_press = async_mock_service(menuai, domain, "press")
