...
def setup_entry() -> None:
    ...
    with patch(
        "menuai.components.skybell.async_setup_entry",
        return_value=True,
    ):
        yield
...
async def test_flow_user_cannot_connect(menuai: menuai, skybell_mock) -> None:
    """Test user initialized flow with unreachable server."""
    skybell_mock.async_initialize.side_effect = exceptions.SkybellException(menuai)
    result = await menuai.config_entries.flow.async_init(
        DOMAIN, context={"source": SOURCE_USER}, data=CONF_DATA
    )
    assert result["type"] is FlowResultType.FORM
...
async def test_invalid_credentials(menuai: menuai, skybell_mock) -> None:
    """Test that invalid credentials throws an error."""
    skybell_mock.async_initialize.side_effect = (
        exceptions.SkybellAuthenticationException(menuai)
    )
    result = await menuai.config_entries.flow.async_init(
        DOMAIN, context={"source": SOURCE_USER}, data=CONF_DATA
    )
...
async def test_flow_user_unknown_error(menuai: menuai, skybell_mock) -> None:
    """Test user initialized flow with unreachable server."""
...
async def test_step_reauth_failed(menuai: menuai, skybell_mock) -> None:
