...
async def test_sensors(menuai: menuai, device_registry: dr.DeviceRegistry) -> None:
    ...
    state = menuai.states.get("sensor.mysite_load_power")
    ...
    state = menuai.states.get("sensor.mysite_load_frequency")
    ...
    state = menuai.states.get("sensor.mysite_load_voltage")
    ...
    state = menuai.states.get("sensor.mysite_load_current")
    ...
    assert float(menuai.states.get("sensor.mysite_load_export").state) == 1056.8
    assert float(menuai.states.get("sensor.mysite_load_import").state) == 4693.0

    state = menuai.states.get("sensor.mysite_battery_power")
    assert state.state == "-8.55"

    assert float(menuai.states.get("sensor.mysite_battery_export").state) == 3620.0
    assert float(menuai.states.get("sensor.mysite_battery_import").state) == 4216.2

    state = menuai.states.get("sensor.mysite_solar_power")
    assert state.state == "10.49"

    assert float(menuai.states.get("sensor.mysite_solar_export").state) == 9864.2
    assert float(menuai.states.get("sensor.mysite_solar_import").state) == 28.2

    state = menuai.states.get("sensor.mysite_charge")
    ...
    state = menuai.states.get("sensor.mysite_backup_reserve")
    ...
    assert menuai.states.get("sensor.mysite_load_power").state == STATE_UNKNOWN
    assert menuai.states.get("sensor.mysite_load_frequency").state == STATE_UNKNOWN
    assert menuai.states.get("sensor.mysite_backup_reserve").state == STATE_UNKNOWN

    assert (
        float(menuai.states.get("sensor.mysite_tg0123456789ab_battery_capacity").state)
        == 14.715
    )
    assert (
        float(menuai.states.get("sensor.mysite_tg0123456789ab_battery_voltage").state)
        == 245.7
    )
    assert (
        float(menuai.states.get("sensor.mysite_tg0123456789ab_frequency").state) == 50.0
    )
    assert float(menuai.states.get("sensor.mysite_tg0123456789ab_current").state) == 0.3
    assert int(menuai.states.get("sensor.mysite_tg0123456789ab_power").state) == -100
    assert (
        float(menuai.states.get("sensor.mysite_tg0123456789ab_battery_export").state)
        == 2358.235
    )
    assert (
        float(menuai.states.get("sensor.mysite_tg0123456789ab_battery_import").state)
        == 2693.355
    )
    assert (
        float(menuai.states.get("sensor.mysite_tg0123456789ab_battery_remaining").state)
        == 14.715
    )
    assert (
        str(menuai.states.get("sensor.mysite_tg0123456789ab_grid_state").state)
        == "grid_compliant"
    )
    assert (
        float(menuai.states.get("sensor.mysite_tg9876543210ba_battery_capacity").state)
        == 15.137
    )
    assert (
        float(menuai.states.get("sensor.mysite_tg9876543210ba_battery_voltage").state)
        == 245.6
    )
    assert (
        float(menuai.states.get("sensor.mysite_tg9876543210ba_frequency").state) == 50.0
    )
    assert float(menuai.states.get("sensor.mysite_tg9876543210ba_current").state) == 0.1
    assert int(menuai.states.get("sensor.mysite_tg9876543210ba_power").state) == -100
    assert (
        float(menuai.states.get("sensor.mysite_tg9876543210ba_battery_export").state)
        == 509.907
    )
    assert (
        float(menuai.states.get("sensor.mysite_tg9876543210ba_battery_import").state)
        == 610.483
    )
    assert (
        float(menuai.states.get("sensor.mysite_tg9876543210ba_battery_remaining").state)
        == 15.137
    )
    assert (
        str(menuai.states.get("sensor.mysite_tg9876543210ba_grid_state").state)
        == "grid_compliant"
    )


async def test_sensor_backup_reserve_unavailable(menuai: menuai) -> None:
    ...
    state = menuai.states.get("sensor.powerwall_backup_reserve")
...
async def test_sensors_with_empty_meters(menuai: menuai) -> None:
    ...
    assert menuai.states.get("sensor.mysite_solar_power") is None
