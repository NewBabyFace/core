...
from menuai.components.powerwall.const import (
    AUTH_COOKIE_KEY,
    CONFIG_ENTRY_COOKIE,
    DOMAIN,
)
...
async def test_update_data_reauthenticate_on_access_denied(menuai: menuai) -> None:
    ...
    with (
        ...
        async_fire_time_changed(menuai, utcnow() + datetime.timedelta(minutes=1))
        await menuai.async_block_till_done()
...
async def test_init_uses_cookie_if_present(menuai: menuai) -> None:
...
async def test_init_uses_password_if_no_cookies(menuai: menuai) -> None:
...
async def test_init_saves_the_cookie(menuai: menuai) -> None:
    ...
    with (
        ...
        patch("menuai.components.powerwall.CookieJar", return_value=mock_jar),
    ):
        auth_cookie = Morsel()
...
async def test_retry_ignores_cookie(menuai: menuai) -> None:
...
async def test_reauth_ignores_and_clears_cookie(menuai: menuai) -> None:
...
async def test_init_retries_with_password(menuai: menuai) -> None:
