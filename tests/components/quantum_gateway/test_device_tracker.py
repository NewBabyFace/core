"""Tests for the quantum_gateway device tracker."""

from unittest.mock import AsyncMock

import pytest
from requests import RequestException

from menuai.const import STATE_HOME
from menuai.core import menuai

from . import setup_platform

from tests.components.device_tracker.test_init import mock_yaml_devices  # noqa: F401


@pytest.mark.usefixtures("yaml_devices")
async def test_get_scanner(menuai: menuai, mock_scanner: AsyncMock) -> None:
    """Test creating a quantum gateway scanner."""
    await setup_platform(menuai)

    device_1 = menuai.states.get("device_tracker.desktop")
    assert device_1 is not None
    assert device_1.state == STATE_HOME

    device_2 = menuai.states.get("device_tracker.ff_ff_ff_ff_ff_ff")
    assert device_2 is not None
    assert device_2.state == STATE_HOME


@pytest.mark.usefixtures("yaml_devices")
async def test_get_scanner_error(menuai: menuai, mock_scanner: AsyncMock) -> None:
    """Test failure when creating a quantum gateway scanner."""
    mock_scanner.side_effect = RequestException("Error")
    await setup_platform(menuai)

    assert "quantum_gateway.device_tracker" not in menuai.config.components


@pytest.mark.usefixtures("yaml_devices")
async def test_scan_devices_error(menuai: menuai, mock_scanner: AsyncMock) -> None:
    """Test failure when scanning devices."""
    mock_scanner.return_value.scan_devices.side_effect = RequestException("Error")
    await setup_platform(menuai)

    assert "quantum_gateway.device_tracker" in menuai.config.components

    device_1 = menuai.states.get("device_tracker.desktop")
    assert device_1 is None

    device_2 = menuai.states.get("device_tracker.ff_ff_ff_ff_ff_ff")
    assert device_2 is None
