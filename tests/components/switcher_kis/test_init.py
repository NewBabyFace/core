...
from menuai.components.switcher_kis.const import DOMAIN, MAX_UPDATE_INTERVAL_SEC
...
async def test_update_fail(
    ...
    async_fire_time_changed(
        menuai, dt_util.utcnow() + timedelta(seconds=MAX_UPDATE_INTERVAL_SEC - 2)
    )
    await menuai.async_block_till_done()
...
async def test_entry_unload(menuai: menuai, mock_bridge) -> None:
    """Test entry unload."""
    entry = await init_integration(menuai)
    assert mock_bridge
