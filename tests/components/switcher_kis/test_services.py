...
from menuai.components.switcher_kis.const import (
    CONF_AUTO_OFF,
    CONF_TIMER_MINUTES,
    DOMAIN,
...
@pytest.mark.parametrize("mock_bridge", [[DUMMY_WATER_HEATER_DEVICE]], indirect=True)
async def test_set_auto_off_service(menuai: menuai, mock_bridge, mock_api) -> None:
    ...
    with patch(
        "menuai.components.switcher_kis.entity.SwitcherApi.set_auto_shutdown"
...
async def test_plug_unsupported_services(
    menuai: menuai, mock_bridge, mock_api, caplog: pytest.LogCaptureFixture
) -> None:
