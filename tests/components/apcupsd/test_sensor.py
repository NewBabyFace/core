...
from menuai.components.apcupsd.coordinator import REQUEST_REFRESH_COOLDOWN
...
async def test_sensor(
    ...
    with patch("menuai.components.apcupsd.PLATFORMS", [Platform.SENSOR]):
        config_entry = await async_init_integration(menuai, status=MOCK_STATUS)
    await snapshot_platform(menuai, entity_registry, snapshot, config_entry.entry_id)


async def test_state_update(menuai: menuai) -> None:
...
async def test_manual_update_entity(menuai: menuai) -> None:
    ...
    # Setup menuai for calling the update_entity service.
...
async def test_multiple_manual_update_entity(menuai: menuai) -> None:
    """Test multiple simultaneous manual update entity via service menuai/update_entity.
...
async def test_sensor_unknown(menuai: menuai) -> None:
    ...
    await async_init_integration(menuai, status=MOCK_MINIMAL_STATUS)

    ups_mode_id = "sensor.apc_ups_mode"
    last_self_test_id = "sensor.apc_ups_last_self_test"

    assert menuai.states.get(ups_mode_id).state == MOCK_MINIMAL_STATUS["UPSMODE"]
    ...
    assert menuai.states.get(last_self_test_id) is not None
    assert menuai.states.get(last_self_test_id).state == STATE_UNKNOWN
    ...
    with patch("aioapcaccess.request_status") as mock_request_status:
        ...
        async_fire_time_changed(menuai, future)
        await menuai.async_block_till_done()
    assert menuai.states.get(last_self_test_id).state == "1970-01-01 00:00:00 0000"
