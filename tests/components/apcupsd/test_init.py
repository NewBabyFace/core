...
from menuai.components.apcupsd.coordinator import UPDATE_INTERVAL
...
from menuai.util import slugify, utcnow
...
async def test_async_setup_entry(
    ...
    config_entry = await async_init_integration(menuai, status=status)
    ...
    platforms = async_get_platforms(menuai, DOMAIN)
    assert len(platforms) > 0
    assert all(len(p.entities) > 0 for p in platforms)


async def test_multiple_integrations(menuai: menuai) -> None:
    ...
    entries = (
        await async_init_integration(
            menuai, host="test1", status=status1, entry_id="entry-id-1"
        ),
        await async_init_integration(
            menuai, host="test2", status=status2, entry_id="entry-id-2"
        ),
    )
    ...
    state1 = menuai.states.get(f"sensor.{device_slug}_load")
    state2 = menuai.states.get(f"sensor.{device_slug}_load_2")
    assert state1 is not None and state2 is not None
    assert state1.state != state2.state


async def test_multiple_integrations_different_devices(menuai: menuai) -> None:
    ...
    state1 = menuai.states.get("sensor.myups1_load")
    state2 = menuai.states.get("sensor.myups2_load")
    assert state1 is not None and state2 is not None
...
async def test_connection_error(menuai: menuai, error: Exception) -> None:
...
async def test_unload_remove_entry(menuai: menuai) -> None:
    """Test successful unload and removal of an entry."""
    # Load two integrations from two mock hosts.
    entries = (
        await async_init_integration(
            menuai, host="test1", status=MOCK_STATUS, entry_id="entry-id-1"
        ),
        await async_init_integration(
            menuai, host="test2", status=MOCK_MINIMAL_STATUS, entry_id="entry-id-2"
        ),
    )
    ...
    assert await menuai.config_entries.async_unload(entries[1].entry_id)
    await menuai.async_block_till_done()
...
async def test_availability(menuai: menuai) -> None:
    ...
    device_slug = slugify(MOCK_STATUS["UPSNAME"])
    state = menuai.states.get(f"sensor.{device_slug}_load")
    assert state
