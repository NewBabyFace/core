...
async def test_lock_device_registry(
    ...
    lock_one = await _mock_doorsense_enabled_yale_lock_detail(menuai)
...
async def test_lock_changed_by(menuai: menuai) -> None:
    ...
    await _create_yale_with_devices(menuai, [lock_one], activities=activities)
...
async def test_state_unlocking(menuai: menuai) -> None:
    ...
    lock_online_with_doorsense_name = menuai.states.get("lock.online_with_doorsense_name")
...
async def test_one_lock_operation(
    ...
    operator_state = menuai.states.get("sensor.online_with_doorsense_name_operator")
...
async def test_open_lock_operation(menuai: menuai) -> None:
    ...
    await _create_yale_with_devices(menuai, [lock_with_unlatch])
...
async def test_open_lock_operation_socketio_connected(
    ...
    _, socketio = await _create_yale_with_devices(menuai, [lock_with_unlatch])
...
async def test_one_lock_operation_socketio_connected(
    ...
    _, socketio = await _create_yale_with_devices(menuai, [lock_one])
    socketio.connected = True
...
async def test_lock_update_via_socketio(menuai: menuai) -> None:
    """Test creation of a lock with doorsense and bridge."""
    lock_one = await _mock_doorsense_enabled_yale_lock_detail(menuai)
    assert lock_one.pubsub_channel == "pubsub"
    ...
    config_entry, socketio = await _create_yale_with_devices(
        menuai, [lock_one], activities=activities
    )
    socketio.connected = True
    states = menuai.states
