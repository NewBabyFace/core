...
def mock_discovery_fixture():
    ...
    with patch(
        "menuai.components.yale.data.discovery_flow.async_create_flow"
    ) as mock_discovery:
...
async def mock_client_credentials_fixture(menuai: menuai) -> None:
    """Mock client credentials."""
    await mock_client_credentials(menuai)
...
def skip_cloud_fixture():
    ...
    """
    with patch("menuai.components.cloud.async_setup", return_value=True):
        yield
