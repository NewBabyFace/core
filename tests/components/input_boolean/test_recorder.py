...
from menuai.components.input_boolean import DOMAIN
...
async def test_exclude_attributes(menuai: menuai) -> None:
    ...
    now = dt_util.utcnow()
    assert await async_setup_component(menuai, DOMAIN, {DOMAIN: {"test": {}}})

    state = menuai.states.get("input_boolean.test")
    assert state
