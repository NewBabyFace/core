...
from menuai.components.here_travel_time.coordinator import BACKOFF_MULTIPLIER
...
async def test_sensor(
    ...
    duration = menuai.states.get("sensor.test_duration")
    ...
    assert float(menuai.states.get("sensor.test_distance").state) == pytest.approx(13.682)
    assert menuai.states.get("sensor.test_duration_in_traffic").state == "30"
    assert menuai.states.get("sensor.test_origin").state == "22nd St NW"
    assert (
        menuai.states.get("sensor.test_origin").attributes.get(ATTR_LATITUDE)
        == "38.8999937"
    )
    assert (
        menuai.states.get("sensor.test_origin").attributes.get(ATTR_LONGITUDE)
        == "-77.0479682"
    )

    assert menuai.states.get("sensor.test_destination").state == "Service Rd S"
    assert (
        menuai.states.get("sensor.test_destination").attributes.get(ATTR_LATITUDE)
        == "38.99997"
    )
    assert (
        menuai.states.get("sensor.test_destination").attributes.get(ATTR_LONGITUDE)
        == "-77.10014"
    )
...
@pytest.mark.usefixtures("valid_response")
async def test_public_transport(menuai: menuai) -> None:
    ...
    assert (
        menuai.states.get("sensor.test_duration").attributes.get(ATTR_ATTRIBUTION)
    ...
    assert menuai.states.get("sensor.test_distance").state == "1.883"


@pytest.mark.usefixtures("no_attribution_response")
async def test_no_attribution_response(menuai: menuai) -> None:
    ...
    assert (
        menuai.states.get("sensor.test_duration").attributes.get(ATTR_ATTRIBUTION) is None
    )


async def test_entity_ids(menuai: menuai, valid_response: MagicMock) -> None:
    ...
    assert await async_setup_component(menuai, "zone", zone_config)
    menuai.states.async_set(
        "device_tracker.test",
        "not_home",
        {
    ...
    assert menuai.states.get("sensor.test_distance").state == "13.682"
...
async def test_restore_state(menuai: menuai) -> None:
    ...
    # restore from cache
    state = menuai.states.get("sensor.test_duration")
    assert state.state == "1234"
    assert state.attributes.get(ATTR_UNIT_OF_MEASUREMENT) == UnitOfTime.MINUTES
    assert state.attributes.get(ATTR_STATE_CLASS) == SensorStateClass.MEASUREMENT

    state = menuai.states.get("sensor.test_duration_in_traffic")
    assert state.state == "5678"
    assert state.attributes.get(ATTR_UNIT_OF_MEASUREMENT) == UnitOfTime.MINUTES
    assert state.attributes.get(ATTR_STATE_CLASS) == SensorStateClass.MEASUREMENT

    state = menuai.states.get("sensor.test_distance")
    assert state.state == "123"
    assert state.attributes.get(ATTR_UNIT_OF_MEASUREMENT) == UnitOfLength.KILOMETERS
    assert state.attributes.get(ATTR_STATE_CLASS) == SensorStateClass.MEASUREMENT

    state = menuai.states.get("sensor.test_origin")
    assert state.state == "Origin Address 1"

    state = menuai.states.get("sensor.test_destination")
...
async def test_transit_errors(
    menuai: menuai, caplog: pytest.LogCaptureFixture, exception, expected_message
) -> None:
...
async def test_routing_rate_limit(
    ...
    with patch(
        ...
        assert menuai.states.get("sensor.test_distance").state == "unavailable"
...
async def test_multiple_sections(
    ...
    assert float(menuai.states.get("sensor.test_distance").state) == pytest.approx(3.583)
    assert menuai.states.get("sensor.test_duration_in_traffic").state == "18"
    assert menuai.states.get("sensor.test_origin").state == "Chemin de Halage"
    assert (
        menuai.states.get("sensor.test_origin").attributes.get(ATTR_LATITUDE)
        == "49.1260894"
    )
    ...
    assert menuai.states.get("sensor.test_destination").state == "Rue Charles Sadoul"
    assert (
        menuai.states.get("sensor.test_destination").attributes.get(ATTR_LATITUDE)
        == "49.1025668"
    )
