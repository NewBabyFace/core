...
from menuai.const import CONF_MAC, CONF_MODEL, CONF_TOKEN
...
async def test_setup(
    ...
    entry = await setup_samsungtv_entry(menuai, entry_data)
...
async def test_setup_h_j_model(
    menuai: menuai, rest_api: Mock, caplog: pytest.LogCaptureFixture
) -> None:
    ...
    entry = await setup_samsungtv_entry(
        menuai, {**ENTRYDATA_WEBSOCKET, CONF_MODEL: "UE48JU6400"}
    )
...
@pytest.mark.usefixtures("remote_websocket")
async def test_setup_updates_from_ssdp(menuai: menuai) -> None:
    ...
    async def _mock_async_get_discovery_info_by_st(menuai: menuai, mock_st: str):
    ...
    with patch(
        "menuai.components.samsungtv.ssdp.async_get_discovery_info_by_st",
...
async def test_reauth_triggered_encrypted(menuai: menuai) -> None:
    ...
    entry = await setup_samsungtv_entry(menuai, encrypted_entry_data)
...
async def test_incorrectly_formatted_mac_fixed(menuai: menuai) -> None:
