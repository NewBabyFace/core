...
async def test_speed_operation(
    ...
    bulb, _ = await async_setup_integration(menuai, bulb_type=FAKE_DUAL_HEAD_RGBWW_BULB)
    await async_push_update(menuai, bulb, {"mac": FAKE_MAC})
    ...
    await async_push_update(menuai, bulb, {"mac": FAKE_MAC, "speed": 50})
    assert menuai.states.get(entity_id).state == "50.0"
    ...
    await async_push_update(menuai, bulb, {"mac": FAKE_MAC, "speed": 30})
    assert menuai.states.get(entity_id).state == "30.0"
...
async def test_ratio_operation(
    ...
    await async_push_update(menuai, bulb, {"mac": FAKE_MAC, "ratio": 50})
    assert menuai.states.get(entity_id).state == "50.0"
    ...
    await async_push_update(menuai, bulb, {"mac": FAKE_MAC, "ratio": 30})
    assert menuai.states.get(entity_id).state == "30.0"
