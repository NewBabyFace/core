...
from menuai.components.select import ATTR_OPTION, DOMAIN as SELECT_DOMAIN
from menuai.components.wled.const import DOMAIN, SCAN_INTERVAL
from menuai.const import ATTR_ENTITY_ID, SERVICE_SELECT_OPTION, STATE_UNAVAILABLE
...
async def test_color_palette_dynamically_handle_segments(
    ...
    assert (segment0 := menuai.states.get("select.wled_rgb_light_color_palette"))
    assert segment0.state == "Default"
    assert not menuai.states.get("select.wled_rgb_light_segment_1_color_palette")
    ...
    assert segment0.state == "Default"
    assert (
        segment1 := menuai.states.get("select.wled_rgb_light_segment_1_color_palette")
    )
...
async def test_preset_unavailable_without_presets(menuai: menuai) -> None:
    """Test WLED preset entity is unavailable when presets are not available."""
    assert (state := menuai.states.get("select.wled_rgb_light_preset"))
    assert state.state == STATE_UNAVAILABLE


async def test_playlist_unavailable_without_playlists(menuai: menuai) -> None:
    """Test WLED playlist entity is unavailable when playlists are not available."""
    assert (state := menuai.states.get("select.wled_rgb_light_playlist"))
    assert state.state == STATE_UNAVAILABLE
