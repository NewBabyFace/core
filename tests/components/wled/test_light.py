...
from menuai.components.wled.const import (
...
async def test_rgb_light_state(
    ...
    assert (state := menuai.states.get("light.wled_rgb_light"))
    ...
    assert (state := menuai.states.get("light.wled_rgb_light_segment_1"))
    assert state.attributes.get(ATTR_BRIGHTNESS) == 255
    ...
    assert (state := menuai.states.get("light.wled_rgb_light_main"))
...
async def test_dynamically_handle_segments(
    ...
    assert (segment0 := menuai.states.get("light.wled_rgb_light"))
    assert segment0.state == STATE_ON
    assert not menuai.states.get("light.wled_rgb_light_main")
    assert not menuai.states.get("light.wled_rgb_light_segment_1")
    ...
    assert (main := menuai.states.get("light.wled_rgb_light_main"))
    assert main.state == STATE_ON
    assert (segment0 := menuai.states.get("light.wled_rgb_light"))
    assert segment0.state == STATE_ON
    assert (segment1 := menuai.states.get("light.wled_rgb_light_segment_1"))
...
async def test_single_segment_behavior(
    ...
    async_fire_time_changed(menuai)
    await menuai.async_block_till_done()
    state = menuai.states.get("light.wled_rgb_light")
    assert state
...
async def test_rgbw_light(menuai: menuai, mock_wled: MagicMock) -> None:
    """Test RGBW support for WLED."""
    assert (state := menuai.states.get("light.wled_rgbw_light"))
    assert state.state == STATE_ON
...
async def test_cct_light(menuai: menuai, mock_wled: MagicMock) -> None:
    """Test CCT support for WLED."""
    assert (state := menuai.states.get("light.wled_cct_light"))
    assert state.state == STATE_ON
