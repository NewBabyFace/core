...
from menuai.components import songpal
...
def _patch_media_setup():
    ...
    return patch(
        "menuai.components.songpal.media_player.async_setup_entry",
        side_effect=_async_return,
    )


async def test_setup_empty(menuai: menuai) -> None:
    """Test setup without any configuration."""
    with _patch_media_setup() as setup:
        assert await async_setup_component(menuai, songpal.DOMAIN, {}) is True
    ...
    with _patch_config_flow_device(mocked_device), _patch_media_setup() as setup:
        assert (
            await async_setup_component(
                menuai, songpal.DOMAIN, {songpal.DOMAIN: [CONF_DATA]}
            )
            is True
        )
        await menuai.async_block_till_done()
...
async def test_unload(menuai: menuai) -> None:
    ...
    assert await songpal.async_unload_entry(menuai, entry)
    await menuai.async_block_till_done()
