...
from menuai.components.input_button import DOMAIN, SERVICE_PRESS
...
async def test_config_options(menuai: menuai) -> None:
    ...
    state_1 = menuai.states.get("input_button.test_1")
    state_2 = menuai.states.get("input_button.test_2")
...
async def test_restore_state(menuai: menuai) -> None:
    ...
    state = menuai.states.get("input_button.b1")
    assert state
    assert state.state == "2021-01-01T23:59:59+00:00"

    state = menuai.states.get("input_button.b2")
    assert state
...
async def test_input_button_context(
    ...
    assert await async_setup_component(menuai, DOMAIN, {DOMAIN: {"update": {}}})

    state = menuai.states.get("input_button.update")
    ...
    state2 = menuai.states.get("input_button.update")
...
async def test_reload(
    ...
    state_1 = menuai.states.get("input_button.test_1")
    state_2 = menuai.states.get("input_button.test_2")
    state_3 = menuai.states.get("input_button.test_3")
...
async def test_reload_not_changing_state(menuai: menuai, storage_setup) -> None:
    ...
    async_track_state_change(menuai, [f"{DOMAIN}.from_storage"], state_changed_listener)
...
async def test_ws_create_update(
    ...
    state = menuai.states.get(f"{DOMAIN}.new")
    assert state is not None
    assert state.state == STATE_UNKNOWN
