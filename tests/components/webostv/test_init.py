...
from menuai.components.media_player import ATTR_INPUT_SOURCE_LIST
from menuai.components.webostv.const import CONF_SOURCES, DOMAIN
from menuai.config_entries import SOURCE_REAUTH, ConfigEntryState
from menuai.const import CONF_CLIENT_SECRET, EVENT_menuai_STOP
...
async def test_reauth_setup_entry(menuai: menuai, client) -> None:
...
async def test_key_update_setup_entry(menuai: menuai, client) -> None:
...
async def test_update_options(menuai: menuai, client) -> None:
...
async def test_disconnect_on_stop(menuai: menuai, client) -> None:
    """Test we disconnect the client and clear callbacks when MenuAIs stops."""
    config_entry = await setup_webostv(menuai)
