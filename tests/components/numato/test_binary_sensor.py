...
async def test_setup_callbacks(menuai: menuai, numato_fixture) -> None:
...
async def test_menuai_binary_sensor_notification(
    ...
    await menuai.async_block_till_done()  # wait until services are registered
    assert (
        menuai.states.get("binary_sensor.numato_binary_sensor_mock_port2").state == "on"
    )
    await menuai.async_add_executor_job(numato_fixture.devices[0].callbacks[2], 2, False)
    await menuai.async_block_till_done()
    assert (
        menuai.states.get("binary_sensor.numato_binary_sensor_mock_port2").state == "off"
    )
...
async def test_binary_sensor_setup_without_discovery_info(
    ...
    await discovery.async_load_platform(
        menuai, Platform.BINARY_SENSOR, "numato", None, config
    )
    for entity_id in MOCKUP_ENTITY_IDS:
        assert entity_id not in menuai.states.async_entity_ids()
