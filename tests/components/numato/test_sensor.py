...
async def test_failing_sensor_update(
    ...
    assert await async_setup_component(menuai, "numato", NUMATO_CFG)
    await menuai.async_block_till_done()
    assert menuai.states.get("sensor.numato_adc_mock_port1").state is STATE_UNKNOWN
...
async def test_sensor_setup_without_discovery_info(
    ...
    await discovery.async_load_platform(menuai, Platform.SENSOR, "numato", None, config)
    for entity_id in MOCKUP_ENTITY_IDS:
        assert entity_id not in menuai.states.async_entity_ids()
