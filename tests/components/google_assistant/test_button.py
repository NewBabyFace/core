"""Test buttons."""

from unittest.mock import patch

import pytest

from menuai.components import google_assistant as ga
from menuai.core import Context, menuai
from menuai.exceptions import menuaiError
from menuai.setup import async_setup_component

from .test_http import DUMMY_CONFIG

from tests.common import MockUser


async def test_sync_button(menuai: menuai, menuai_owner_user: MockUser) -> None:
    """Test sync button."""

    await async_setup_component(
        menuai,
        ga.DOMAIN,
        {"google_assistant": DUMMY_CONFIG},
    )

    await menuai.async_block_till_done()

    state = menuai.states.get("button.google_assistant_synchronize_devices")
    assert state

    config_entry = menuai.config_entries.async_entries("google_assistant")[0]
    google_config: ga.GoogleConfig = config_entry.runtime_data

    with patch.object(google_config, "async_sync_entities") as mock_sync_entities:
        mock_sync_entities.return_value = 200
        context = Context(user_id=menuai_owner_user.id)
        await menuai.services.async_call(
            "button",
            "press",
            {"entity_id": "button.google_assistant_synchronize_devices"},
            blocking=True,
            context=context,
        )
        mock_sync_entities.assert_called_once_with(menuai_owner_user.id)

        mock_sync_entities.return_value = 400
        with pytest.raises(menuaiError):
            await menuai.services.async_call(
                "button",
                "press",
                {"entity_id": "button.google_assistant_synchronize_devices"},
                blocking=True,
                context=context,
            )
