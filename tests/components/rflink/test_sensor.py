...
async def test_default_setup(
    ...
    event_callback, create, _, _ = await mock_rflink(menuai, CONFIG, DOMAIN, monkeypatch)

    # make sure arguments are passed
    assert create.call_args_list[0][1]["ignore"]

    # test default state of sensor loaded from config
    config_sensor = menuai.states.get("sensor.test")
    ...
    assert menuai.states.get("sensor.test").state == "1"
    ...
    temp_sensor = menuai.states.get("sensor.test2")
    assert temp_sensor
    ...
    bat_sensor = menuai.states.get("sensor.test3")
    assert bat_sensor
...
async def test_disable_automatic_add(
    ...
    assert not menuai.states.get("sensor.test2")
...
async def test_entity_availability(
    ...
    assert menuai.states.get("sensor.test").state == STATE_UNKNOWN
    ...
    assert menuai.states.get("sensor.test").state == "unavailable"
...
async def test_aliases(menuai: menuai, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # test default state of sensor loaded from config
    config_sensor = menuai.states.get("sensor.test_02")
    assert config_sensor
    assert config_sensor.state == "unknown"
    ...
    updated_sensor = menuai.states.get("sensor.test_02")
...
async def test_race_condition(
    ...
    assert tmp_entity in menuai.data[DATA_ENTITY_LOOKUP][EVENT_KEY_SENSOR]["test3"]
    ...
    # test state of new sensor
    updated_sensor = menuai.states.get("sensor.test3")
    assert updated_sensor
...
async def test_sensor_attributes(
    ...
    meter_state = menuai.states.get("sensor.meter_device")
    assert meter_state
    ...
    rain_state = menuai.states.get("sensor.rain_device")
    assert rain_state
    ...
    humidity_state = menuai.states.get("sensor.humidity_device")
    ...
    temperature_state = menuai.states.get("sensor.temperature_device")
    ...
    fahrenheit_state = menuai.states.get("sensor.fahrenheit_device")
