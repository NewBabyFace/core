...
from menuai.components.rflink import CONF_RECONNECT_INTERVAL
...
async def test_default_setup(
    ...
    config_sensor = menuai.states.get("binary_sensor.test")
    ...
    assert menuai.states.get("binary_sensor.test").state == STATE_ON

    # test off event for config sensor
    event_callback({"id": "test", "command": "off"})
    await menuai.async_block_till_done()

    assert menuai.states.get("binary_sensor.test").state == STATE_OFF
...
async def test_entity_availability(
    ...
    assert menuai.states.get("binary_sensor.test").state == STATE_UNKNOWN
    ...
    assert menuai.states.get("binary_sensor.test").state == STATE_UNAVAILABLE
...
async def test_off_delay(menuai: menuai, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    menuai.bus.async_listen(EVENT_STATE_CHANGED, listener)
...
async def test_restore_state(
    ...
    mock_restore_cache(
        menuai, (State(f"{DOMAIN}.test", STATE_ON), State(f"{DOMAIN}.test2", STATE_ON))
    )
    ...
    state = menuai.states.get(f"{DOMAIN}.test2")
    assert state
    assert state.state == STATE_OFF
