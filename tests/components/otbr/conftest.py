...
def compute_pskc_fixture(enable_compute_pskc: bool) -> Any:
    """Patch menuai.components.otbr.util.compute_pskc."""
    compute_pskc = otbr.util.compute_pskc if enable_compute_pskc else None

    with patch(
        "menuai.components.otbr.util.compute_pskc", side_effect=compute_pskc
...
def multiprotocol_addon_manager_mock_fixture(menuai: menuai):
    """Mock the Silicon Labs Multiprotocol add-on manager."""
    mock_manager = Mock()
    mock_manager.async_get_channel = Mock(return_value=None)
    with patch.dict(menuai.data, {"silabs_multiprotocol_addon_manager": mock_manager}):
        yield mock_manager
