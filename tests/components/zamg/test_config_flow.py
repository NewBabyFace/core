...
from menuai.components.zamg.const import CONF_STATION_ID, DOMAIN, LOGGER
...
async def test_error_closest_station(menuai: menuai, mock_zamg: MagicMock) -> None:
...
@pytest.mark.usefixtures("mock_setup_entry")
async def test_error_update(menuai: menuai, mock_zamg: MagicMock) -> None:
    """Test with error of reading from Zamg."""
    result = await menuai.config_entries.flow.async_init(
        DOMAIN,
        context={"source": SOURCE_USER},
...
@pytest.mark.usefixtures("mock_zamg", "mock_setup_entry")
async def test_user_flow_duplicate(menuai: menuai) -> None:
    """Test the full manual user flow from start to finish."""
    result = await menuai.config_entries.flow.async_init(
        DOMAIN,
        context={"source": SOURCE_USER},
