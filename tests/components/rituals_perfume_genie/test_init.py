...
async def test_config_entry_unload(menuai: menuai) -> None:
    ...
    await init_integration(menuai, config_entry, [mock_diffuser(hublot="lot123")])

    await menuai.config_entries.async_unload(config_entry.entry_id)
    await menuai.async_block_till_done()

    assert config_entry.state is ConfigEntryState.NOT_LOADED
    assert config_entry.entry_id not in menuai.data[DOMAIN]
