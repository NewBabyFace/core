...
async def test_loading_light(
    ...
    state = menuai.states.get("light.wl000000000099_1")
    assert state
...
async def test_on_off_light_state(
    menuai: menuai, dummy_device_from_host_pb
) -> None:
...
async def test_dimmer_light_state(
    menuai: menuai, dummy_device_from_host_dimmer
) -> None:
    """Test the change of state of the light switches."""
    await setup_integration(menuai)
...
async def test_color_light_state(
    menuai: menuai, dummy_device_from_host_color
) -> None:
    """Test the change of state of the light switches."""
    await setup_integration(menuai)
