...
from menuai.components.input_datetime import CONF_HAS_DATE, CONF_HAS_TIME, DOMAIN
from menuai.components.recorder.history import get_significant_states
from menuai.const import ATTR_EDITABLE
...
async def test_exclude_attributes(menuai: menuai) -> None:
    ...
    assert await async_setup_component(
        menuai, DOMAIN, {DOMAIN: {"test": {CONF_HAS_TIME: True}}}
    )

    state = menuai.states.get("input_datetime.test")
    assert state
