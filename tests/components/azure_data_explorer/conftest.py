...
async def mock_entry_fixture_managed(
    menuai: menuai, filter_schema: dict[str, Any]
) -> MockConfigEntry:
    ...
    await _entry(menuai, filter_schema, entry)
    return entry
...
async def _entry(menuai: menuai, filter_schema: dict[str, Any], entry) -> None:
