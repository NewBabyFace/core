...
from menuai.components.shelly import MacAddressMismatchError, config_flow
...
from menuai.components.shelly.coordinator import ENTRY_RELOAD_COOLDOWN
...
async def test_form(
    ...
    with (
        patch(
            "menuai.components.shelly.config_flow.get_info",
            return_value={
                "mac": "test-mac",
                "type": MODEL_1,
                "auth": False,
...
async def test_form_errors_get_info(
    ...
    with patch("menuai.components.shelly.config_flow.get_info", side_effect=exc):
        result = await menuai.config_entries.flow.async_configure(
            result["flow_id"],
            {CONF_HOST: "1.1.1.1"},
        )
...
async def test_zeroconf(
    ...
    with patch(
        "menuai.components.shelly.config_flow.get_info", return_value=get_info
    ):
        result = await menuai.config_entries.flow.async_init(
            DOMAIN,
            data=DISCOVERY_INFO,
...
async def test_zeroconf_sleeping_device_error(menuai: menuai) -> None:
...
async def test_options_flow_abort_zigbee_enabled(
    ...
    entry = await init_integration(menuai, 4)
...
async def test_zeroconf_ignored(menuai: menuai) -> None:
...
async def test_zeroconf_with_wifi_ap_ip(menuai: menuai) -> None:
...
async def test_reauth_get_info_error(menuai: menuai) -> None:
...
async def test_options_flow_disabled_gen_1(
    menuai: menuai, mock_block_device: Mock, menuai_ws_client: WebSocketGenerator
) -> None:
...
async def test_options_flow_enabled_gen_2(
    menuai: menuai, mock_rpc_device: Mock, menuai_ws_client: WebSocketGenerator
) -> None:
...
async def test_options_flow_disabled_sleepy_gen_2(
    ...
    await async_setup_component(menuai, "config", {})
    entry = await init_integration(menuai, 2, sleep_period=10)
...
async def test_options_flow_ble(menuai: menuai, mock_rpc_device: Mock) -> None:
...
async def test_zeroconf_already_configured_triggers_refresh_mac_in_name(
    ...
    async_fire_time_changed(
        menuai, dt_util.utcnow() + timedelta(seconds=ENTRY_RELOAD_COOLDOWN)
    )
    await menuai.async_block_till_done()
...
async def test_zeroconf_sleeping_device_attempts_configure_no_url_available(
    ...
    """Test zeroconf discovery for sleeping device with no menuai url."""
    menuai.config.internal_url = None
    menuai.config.external_url = None
...
async def test_sleeping_device_gen2_with_new_firmware(
    ...
    with (
        ...
        patch("menuai.components.shelly.async_setup", return_value=True),
        patch(
            "menuai.components.shelly.async_setup_entry",
            return_value=True,
        ),
    ):
        result = await menuai.config_entries.flow.async_configure(
