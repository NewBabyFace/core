...
async def test_climate_hvac_mode(
    ...
    await init_integration(menuai, 1, sleep_period=1000, model=MODEL_VALVE)
...
async def test_block_restored_climate(
    ...
    mock_restore_cache_with_extra_data(
        menuai, ((State(entity_id, HVACMode.OFF, attributes=attrs), extra_data),)
    )
...
async def test_block_restored_climate_unavailable(
    ...
    mock_restore_cache(menuai, [State(entity_id, STATE_UNAVAILABLE)])
...
async def test_block_restored_climate_set_preset_before_online(
    ...
    mock_restore_cache(menuai, [State(entity_id, HVACMode.HEAT)])
...
async def test_block_set_mode_auth_error(
    ...
    entry = await init_integration(menuai, 1, sleep_period=1000)
...
async def test_rpc_climate_hvac_mode(
    ...
    await init_integration(menuai, 2, model=MODEL_WALL_DISPLAY)

    assert (state := menuai.states.get(entity_id)) == snapshot(name=f"{entity_id}-state")
