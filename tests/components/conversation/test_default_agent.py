...
from menuaiil.recognize import Intent, IntentData, MatchEntity, RecognizeResult
import pytest
from syrupy.assertion import SnapshotAssertion
import yaml

from menuai.components import conversation, cover, media_player, weather
...
from menuai.components.conversation.default_agent import METADATA_CUSTOM_SENTENCE
from menuai.components.conversation.models import ConversationInput
from menuai.components.cover import SERVICE_OPEN_COVER
...
@pytest.fixture
async def init_components(menuai: menuai) -> None:
...
async def test_hidden_entities_skipped(
    menuai: menuai, er_kwargs: dict[str, Any], entity_registry: er.EntityRegistry
) -> None:
    ...
    calls = async_mock_service(menuai, menuai_DOMAIN, "turn_on")
    result = await conversation.async_converse(
        menuai, "turn on test light", None, Context(), None
    )
...
@pytest.mark.usefixtures("init_components")
async def test_exposed_domains(menuai: menuai) -> None:
    ...
    result = await conversation.async_converse(
        menuai, "unlock front door", None, Context(), None
    )
    assert result.response.response_type == intent.IntentResponseType.ERROR
    assert result.response.error_code == intent.IntentResponseErrorCode.NO_VALID_TARGETS

    result = await conversation.async_converse(
        menuai, "run my script", None, Context(), None
    )
    assert result.response.response_type == intent.IntentResponseType.ERROR
    assert result.response.error_code == intent.IntentResponseErrorCode.NO_VALID_TARGETS
...
async def test_exposed_areas(
    ...
    expose_entity(menuai, bedroom_light.entity_id, False)

    result = await conversation.async_converse(
        menuai, "turn on lights in the kitchen", None, Context(), None
    )
    ...
    result = await conversation.async_converse(
        menuai, "turn on lights in the bedroom", None, Context(), None
    )
    ...
    result = await conversation.async_converse(
        menuai, "how many lights are on in the bedroom?", None, Context(), None
    )
    assert result.response.response_type == intent.IntentResponseType.QUERY_ANSWER


@pytest.mark.usefixtures("init_components")
async def test_conversation_agent(menuai: menuai) -> None:
    """Test DefaultAgent."""
    agent = menuai.data[DATA_DEFAULT_ENTITY]
    with patch(
        "menuai.components.conversation.default_agent.get_languages",
        return_value=["dwarvish", "elvish", "entish"],
    ):
        assert agent.supported_languages == ["dwarvish", "elvish", "entish"]

    state = menuai.states.get(agent.entity_id)
    assert state
...
async def test_expose_flag_automatically_set(
    ...
    assert async_get_assistant_settings(menuai, conversation.DOMAIN) == {}

    assert await async_setup_component(menuai, "conversation", {})
    await menuai.async_block_till_done()
    with patch("menuai.components.http.start_http_server_and_save_config"):
    ...
    menuai.states.async_set(new_light, "test")
    await menuai.async_block_till_done()
    assert async_get_assistant_settings(menuai, conversation.DOMAIN) == {
        "conversation.home_assistant": {"should_expose": False},
        light.entity_id: {"should_expose": True},
...
async def test_unexposed_entities_skipped(
    ...
    menuai.states.async_set(exposed_light.entity_id, "off")
    ...
    menuai.states.async_set(unexposed_light.entity_id, "off")

    # On light is exposed, the other is not
    expose_entity(menuai, exposed_light.entity_id, True)
    expose_entity(menuai, unexposed_light.entity_id, False)

    # Only one light should be turned on
    calls = async_mock_service(menuai, "light", "turn_on")
    result = await conversation.async_converse(
        menuai, "turn on kitchen lights", None, Context(), None
    )
    ...
    menuai.states.async_set(exposed_light.entity_id, "on")
    menuai.states.async_set(unexposed_light.entity_id, "on")
    result = await conversation.async_converse(
        menuai, "how many lights are on in the kitchen", None, Context(), None
    )
...
async def test_duplicated_names_resolved_with_device_area(
    ...
    for name in ("top light", "overhead light"):
        ...
        calls = async_mock_service(menuai, "light", "turn_on")
        result = await conversation.async_converse(
            menuai, f"turn on {name}", None, Context(), device_id=assist_device.id
        )
...
@pytest.mark.usefixtures("init_components")
async def test_trigger_sentences(menuai: menuai) -> None:
    ...
    result = await conversation.async_converse(menuai, "Not the trigger", None, Context())
    assert result.response.response_type == intent.IntentResponseType.ERROR
    ...
    for sentence in test_sentences:
        callback.reset_mock()
        result = await conversation.async_converse(menuai, sentence, None, Context())
...
async def test_trigger_sentence_response_translation(
    menuai: menuai, language: str, expected: str
) -> None:
    ...
    with patch(
        "menuai.components.conversation.default_agent.translation.async_get_translations",
        ...
        result = await conversation.async_converse(
            menuai, "test sentence", None, Context()
        )
        assert result.response.response_type == intent.IntentResponseType.ACTION_DONE
        assert result.response.speech == {
...
async def test_shopping_list_add_item(menuai: menuai) -> None:
...
@pytest.mark.usefixtures("init_components")
async def test_nevermind_intent(menuai: menuai) -> None:
    """Test menuaiNevermind intent through the default agent."""
    result = await conversation.async_converse(menuai, "nevermind", None, Context())
    assert result.response.intent is not None
...
@pytest.mark.usefixtures("init_components")
async def test_respond_intent(menuai: menuai) -> None:
    """Test menuaiRespond intent through the default agent."""
    result = await conversation.async_converse(menuai, "hello", None, Context())
    assert result.response.intent is not None
    assert result.response.intent.intent_type == intent.INTENT_RESPOND

    assert result.response.response_type == intent.IntentResponseType.ACTION_DONE
    assert result.response.speech["plain"]["speech"] == "Hello from MenuAI."
...
async def test_device_area_context(
    ...
    for command in ("on", "off"):
        result = await conversation.async_converse(
            menuai, f"turn {command} all lights", None, Context(), None
        )
        await menuai.async_block_till_done()
        assert result.response.response_type == intent.IntentResponseType.ACTION_DONE
...
@pytest.mark.usefixtures("init_components")
async def test_error_no_device(menuai: menuai) -> None:
    """Test error message when device/entity doesn't exist."""
    result = await conversation.async_converse(
        menuai, "turn on missing entity", None, Context(), None
    )
...
@pytest.mark.usefixtures("init_components")
async def test_error_no_device_exposed(menuai: menuai) -> None:
    """Test error message when device/entity exists but is not exposed."""
    menuai.states.async_set("light.kitchen_light", "off")
    expose_entity(menuai, "light.kitchen_light", False)

    result = await conversation.async_converse(
        menuai, "turn on kitchen light", None, Context(), None
    )
...
@pytest.mark.usefixtures("init_components")
async def test_error_no_area(menuai: menuai) -> None:
    """Test error message when area doesn't exist."""
    result = await conversation.async_converse(
        menuai, "turn on the lights in missing area", None, Context(), None
    )
...
@pytest.mark.usefixtures("init_components")
async def test_error_no_floor(menuai: menuai) -> None:
    """Test error message when floor doesn't exist."""
    result = await conversation.async_converse(
        menuai, "turn on all the lights on missing floor", None, Context(), None
    )
...
async def test_error_no_device_in_area(
    ...
    area_kitchen = area_registry.async_update(area_kitchen.id, name="kitchen")
    result = await conversation.async_converse(
        menuai, "turn on missing entity in the kitchen", None, Context(), None
    )
...
async def test_error_no_device_on_floor(
    ...
    result = await conversation.async_converse(
        menuai, "turn on missing entity on ground floor", None, Context(), None
    )
...
async def test_error_no_device_on_floor_exposed(
    ...
    expose_entity(menuai, kitchen_light.entity_id, False)
    await menuai.async_block_till_done()
    ...
    with patch(
        "menuai.components.conversation.default_agent.recognize_best",
        return_value=recognize_result,
    ):
        result = await conversation.async_converse(
            menuai, "turn on test light on the ground floor", None, Context(), None
        )
...
async def test_error_no_device_in_area_exposed(
    ...
    result = await conversation.async_converse(
        menuai, "turn on test light in the kitchen", None, Context(), None
    )
...
@pytest.mark.usefixtures("init_components")
async def test_error_no_domain(menuai: menuai) -> None:
    ...
    with patch(
        ...
        result = await conversation.async_converse(
            menuai, "turn on the fans", None, Context(), None
        )
...
@pytest.mark.usefixtures("init_components")
async def test_error_no_domain_exposed(menuai: menuai) -> None:
    """Test error message when devices/entities exist for a domain but are not exposed."""
    menuai.states.async_set("fan.test_fan", "off")
    expose_entity(menuai, "fan.test_fan", False)
    await menuai.async_block_till_done()
...
async def test_error_no_domain_in_area(
    ...
    area_kitchen = area_registry.async_update(area_kitchen.id, name="kitchen")
    result = await conversation.async_converse(
        menuai, "turn on the lights in the kitchen", None, Context(), None
    )
...
async def test_error_no_domain_on_floor(
    ...
    result = await conversation.async_converse(
        menuai, "turn on all lights on the ground floor", None, Context(), None
    )
    ...
    result = await conversation.async_converse(
        menuai, "turn on all lights upstairs", None, Context(), None
    )
...
@pytest.mark.usefixtures("init_components")
async def test_error_no_device_class(menuai: menuai) -> None:
    ...
    with patch(
        ...
        result = await conversation.async_converse(
            menuai, "open the windows", None, Context(), None
        )
...
@pytest.mark.usefixtures("init_components")
async def test_error_no_device_class_exposed(menuai: menuai) -> None:
    ...
    expose_entity(menuai, "cover.test_window", False)
    ...
    with patch(
        ...
        result = await conversation.async_converse(
            menuai, "open all the windows", None, Context(), None
        )
...
async def test_error_no_device_class_in_area(
    ...
    area_bedroom = area_registry.async_update(area_bedroom.id, name="bedroom")
    result = await conversation.async_converse(
        menuai, "open bedroom windows", None, Context(), None
    )
...
async def test_error_no_device_class_in_area_exposed(
    ...
    expose_entity(menuai, bedroom_window.entity_id, False)
    await menuai.async_block_till_done()
...
async def test_error_no_device_class_on_floor_exposed(
    ...
    with patch(
        ...
        result = await conversation.async_converse(
            menuai, "open ground floor windows", None, Context(), None
        )
...
@pytest.mark.usefixtures("init_components")
async def test_error_no_intent(menuai: menuai) -> None:
    ...
    with patch(
        ...
        result = await conversation.async_converse(
            menuai, "do something", None, Context(), None
        )
...
async def test_error_duplicate_names(
    ...
    for name in ("kitchen light", "overhead light"):
        # command
        result = await conversation.async_converse(
            menuai, f"turn on {name}", None, Context(), None
        )
        assert result.response.response_type == intent.IntentResponseType.ERROR
        assert (
            result.response.error_code
        ...
        # question
        result = await conversation.async_converse(
            menuai, f"is {name} on?", None, Context(), None
        )
        assert result.response.response_type == intent.IntentResponseType.ERROR
        assert (
            result.response.error_code
...
async def test_duplicate_names_but_one_is_exposed(
    ...
    # Only expose one
    expose_entity(menuai, kitchen_light_1.entity_id, True)
    expose_entity(menuai, kitchen_light_2.entity_id, False)

    # Check name and alias
    async_mock_service(menuai, "light", "turn_on")
    for name in ("kitchen light", "overhead light"):
        # command
        result = await conversation.async_converse(
            menuai, f"turn on {name}", None, Context(), None
...
async def test_error_duplicate_names_same_area(
    ...
    for name in ("kitchen light", "overhead light"):
        # command
        result = await conversation.async_converse(
            menuai, f"turn on {name} in {area_kitchen.name}", None, Context(), None
        )
        assert result.response.response_type == intent.IntentResponseType.ERROR
        assert (
            result.response.error_code
        ...
        # question
        result = await conversation.async_converse(
            menuai, f"is {name} on in the {area_kitchen.name}?", None, Context(), None
        )
        assert result.response.response_type == intent.IntentResponseType.ERROR
        assert (
            result.response.error_code
...
async def test_duplicate_names_different_areas(
    ...
    kitchen_entry.add_to_menuai(menuai)
    ...
    bedroom_entry.add_to_menuai(menuai)
    ...
    for name in ("test light", "overhead light"):
        ...
        result = await conversation.async_converse(
            menuai, f"turn on {name}", None, Context(), None, device_id=device_kitchen.id
        )
        assert result.response.response_type == intent.IntentResponseType.ACTION_DONE
        ...
        result = await conversation.async_converse(
            menuai, f"turn on {name}", None, Context(), None, device_id=device_bedroom.id
        )
        assert result.response.response_type == intent.IntentResponseType.ACTION_DONE
...
@pytest.mark.usefixtures("init_components")
async def test_error_wrong_state(menuai: menuai) -> None:
    """Test error message when no entities are in the correct state."""
    assert await async_setup_component(menuai, media_player.DOMAIN, {})
    ...
    result = await conversation.async_converse(
        menuai, "pause test player", None, Context(), None
    )
...
@pytest.mark.usefixtures("init_components")
async def test_error_feature_not_supported(menuai: menuai) -> None:
    ...
    result = await conversation.async_converse(
        menuai, "set test player volume to 100%", None, Context(), None
    )
...
async def test_error_no_timer_support(
    ...
    result = await conversation.async_converse(
        menuai, "set a 5 minute timer", None, Context(), None, device_id=device_id
    )
...
@pytest.mark.usefixtures("init_components")
async def test_error_timer_not_found(menuai: menuai) -> None:
    ...
    async_register_timer_handler(menuai, device_id, handle_timer)

    result = await conversation.async_converse(
        menuai, "pause timer", None, Context(), None, device_id=device_id
    )
...
async def test_error_multiple_timers_matched(
    ...
    result = await conversation.async_converse(
        menuai, "set a timer for 5 minutes", None, Context(), None, device_id=device_id
    )
    assert result.response.response_type == intent.IntentResponseType.ACTION_DONE
    ...
    result = await conversation.async_converse(
        menuai, "cancel timer", None, Context(), None, device_id=device_id
    )
    assert result.response.response_type == intent.IntentResponseType.ERROR
    assert result.response.error_code == intent.IntentResponseErrorCode.FAILED_TO_HANDLE
    assert (
...
async def test_no_states_matched_default_error(
    ...
    with patch(
        "menuai.components.conversation.default_agent.intent.async_handle",
...
async def test_empty_aliases(
    ...
    with patch(
        "menuai.components.conversation.default_agent.DefaultAgent._recognize",
        return_value=None,
...
@pytest.mark.usefixtures("init_components")
async def test_all_domains_loaded(menuai: menuai) -> None:
    """Test that sentences for all domains are always loaded."""

    # light domain is not loaded
    assert "light" not in menuai.config.components

    result = await conversation.async_converse(
        menuai, "set brightness of test light to 100%", None, Context(), None
    )
...
async def test_same_named_entities_in_different_areas(
    ...
    calls = async_mock_service(menuai, "light", "turn_on")
    result = await conversation.async_converse(
        menuai, "turn on overhead light in the kitchen", None, Context(), None
    )
    await menuai.async_block_till_done()
    ...
    calls.clear()
    result = await conversation.async_converse(
        menuai, "turn on overhead light in the bedroom", None, Context(), None
    )
    await menuai.async_block_till_done()
    ...
    result = await conversation.async_converse(
        menuai, "turn on overhead light", None, Context(), None
    )
    assert result.response.response_type == intent.IntentResponseType.ERROR

    # Querying a duplicate name should also fail
    result = await conversation.async_converse(
        menuai, "is the overhead light on?", None, Context(), None
    )
    assert result.response.response_type == intent.IntentResponseType.ERROR

    # But we can still ask questions that don't rely on the name
    result = await conversation.async_converse(
        menuai, "how many lights are on?", None, Context(), None
    )
    assert result.response.response_type == intent.IntentResponseType.QUERY_ANSWER
...
@pytest.mark.usefixtures("init_components")
async def test_device_id_in_handler(menuai: menuai) -> None:
...
@pytest.mark.usefixtures("init_components")
async def test_name_wildcard_lower_priority(menuai: menuai) -> None:
    ...
    intent.async_register(menuai, beer_handler)
    intent.async_register(menuai, food_handler)

    # Matches OrderBeer because more literal text is matched ("a")
    result = await conversation.async_converse(
        menuai, "I'd like to order a stout please", None, Context(), None
    )
    assert result.response.response_type == intent.IntentResponseType.ACTION_DONE
    ...
    result = await conversation.async_converse(
        menuai, "I'd like to order a cookie please", None, Context(), None
    )
    assert result.response.response_type == intent.IntentResponseType.ACTION_DONE
...
async def test_intent_entity_added_removed(
    ...
    result = await conversation.async_converse(
        menuai, "turn on my cool light", None, context
    )
    ...
    menuai.states.async_set("light.late", "off", {"friendly_name": "friendly light"})

    result = await conversation.async_converse(
        menuai, "turn on friendly light", None, context
    )
    data = result.as_dict()
    ...
    result = await conversation.async_converse(
        menuai, "turn on late added light", None, context
    )
    ...
    menuai.states.async_remove("light.late")
...
async def test_intent_alias_added_removed(
    ...
    menuai.states.async_set("light.kitchen", "off", {"friendly_name": "kitchen light"})

    calls = async_mock_service(menuai, LIGHT_DOMAIN, "turn_on")
    result = await conversation.async_converse(
        menuai, "turn on kitchen light", None, context
    )
    assert len(calls) == 1
    data = result.as_dict()
    ...
    result = await conversation.async_converse(
        menuai, "turn on late added alias", None, context
    )
...
async def test_intent_entity_renamed(
    ...
    result = await conversation.async_converse(
        menuai, "turn on renamed light", None, context
    )
...
async def test_intent_entity_fail_if_unexposed(
    ...
    # Unexpose the entity
    expose_entity(menuai, "light.kitchen", False)
...
async def test_custom_sentences_config(
    ...
    # Invoke intent via HTTP API
    result = await conversation.async_converse(
        menuai, "engage stealth mode", None, Context(), None
    )
...
async def test_language_region(menuai: menuai, init_components) -> None:
    """Test regional languages."""
    menuai.states.async_set("light.kitchen", "off")
    calls = async_mock_service(menuai, LIGHT_DOMAIN, "turn_on")

    # Add fake region
    language = f"{menuai.config.language}-YZ"
...
async def test_non_default_response(menuai: menuai, init_components) -> None:
    """Test intent response that is not the default."""
    menuai.states.async_set("cover.front_door", "closed")
...
async def test_turn_on_area(
    ...
    menuai.states.async_set("light.stove", "off")
...
async def test_custom_sentences_priority(
    ...
    with tempfile.NamedTemporaryFile(
        ...
        encoding="utf-8",
        suffix=".yaml",
        dir=os.path.join(menuai.config.config_dir, "custom_sentences", "en"),
        ...
        menuai.states.async_set("light.lamp", "off")
...
async def test_config_sentences_priority(
    ...
    intents = (
        await conversation.async_get_agent(menuai).async_get_or_load_intents(
...
@pytest.mark.usefixtures("init_components")
async def test_intent_cache_exposed(menuai: menuai) -> None:
    ...
    expose_entity(menuai, entity_id, True)
    await menuai.async_block_till_done()
    ...
    expose_entity(menuai, entity_id, False)
    result = await agent.async_recognize_intent(user_input)
    assert result is not None
...
@pytest.mark.usefixtures("init_components")
async def test_intent_cache_all_entities(menuai: menuai) -> None:
    ...
    entity_id = "light.test_light"
    menuai.states.async_set(entity_id, "off")
    expose_entity(menuai, entity_id, False)  # not exposed
    await menuai.async_block_till_done()
    ...
    menuai.states.async_set("light.new_light", "off")
    result = await agent.async_recognize_intent(user_input)
    assert result is not None
    assert getattr(result, mark, None) is None
...
@pytest.mark.usefixtures("init_components")
async def test_intent_cache_fuzzy(menuai: menuai) -> None:
...
@pytest.mark.usefixtures("init_components")
async def test_entities_filtered_by_input(menuai: menuai) -> None:
    ...
    menuai.states.async_set("switch.test_switch", "off")
    expose_entity(menuai, "light.test_light", False)
    expose_entity(menuai, "light.test_light_2", False)
    expose_entity(menuai, "cover.garage_door", False)
    expose_entity(menuai, "switch.test_switch", True)
    await menuai.async_block_till_done()
...
@pytest.mark.usefixtures("init_components")
async def test_entities_names_are_not_templates(menuai: menuai) -> None:
    """Test that entities names are not treated as menuaiil templates."""
    # Contains menuaiil template characters
    menuai.states.async_set(
        "light.test_light", "off", attributes={ATTR_FRIENDLY_NAME: "<test [light"}
    )

    async_mock_service(menuai, LIGHT_DOMAIN, "turn_on")
...
async def test_handle_intents_with_response_errors(
    ...
    agent: default_agent.DefaultAgent = menuai.data[DATA_DEFAULT_ENTITY]
    ...
    with patch(
        "menuai.components.conversation.default_agent.DefaultAgent._async_process_intent_result",
...
async def test_state_names_are_not_translated(
    ...
    await async_setup_component(menuai, "weather", {})

    menuai.states.async_set("weather.test_weather", weather.ATTR_CONDITION_PARTLYCLOUDY)
    expose_entity(menuai, "weather.test_weather", True)

    with patch(
        "menuai.helpers.template.Template.async_render"
    ) as mock_async_render:
        result = await conversation.async_converse(
            menuai, "what is the weather like?", None, Context(), None
        )
        assert result.response.response_type == intent.IntentResponseType.QUERY_ANSWER
...
async def test_language_with_alternative_code(
    ...
    for i, (lang_code, sentence, name) in enumerate(
        ...
        menuai.states.async_set(entity_id, "off", attributes={ATTR_FRIENDLY_NAME: name})
        calls = async_mock_service(menuai, LIGHT_DOMAIN, "turn_on")
        await menuai.services.async_call(
            "conversation",
            "process",
