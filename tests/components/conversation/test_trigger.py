...
from menuai.components.conversation import HOME_ASSISTANT_AGENT, default_agent
...
async def test_if_fires_on_event(
    ...
    context = Context()
    service_response = await menuai.services.async_call(
        "conversation",
        "process",
        {"text": "Ha ha ha"},
        blocking=True,
...
async def test_response(menuai: menuai) -> None:
...
async def test_empty_response(menuai: menuai) -> None:
...
async def test_fails_on_punctuation(menuai: menuai, command: str) -> None:
...
async def test_fails_on_empty(menuai: menuai, command: str) -> None:
...
async def test_fails_on_no_sentences(menuai: menuai) -> None:
...
async def test_wildcards(menuai: menuai, service_calls: list[ServiceCall]) -> None:
...
async def test_trigger_with_device_id(menuai: menuai) -> None:
