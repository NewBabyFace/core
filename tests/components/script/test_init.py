...
from menuai.components.script import DOMAIN, EVENT_SCRIPT_STARTED, ScriptEntity
...
from menuai.exceptions import ServiceNotFound, TemplateError
from menuai.helpers import device_registry as dr, entity_registry as er
from menuai.helpers.event import async_track_state_change
...
def calls(menuai: menuai) -> list[ServiceCall]:
    ...
    return async_mock_service(menuai, "test", "script")


async def test_passing_variables(menuai: menuai) -> None:
    """Test different ways of passing in variables."""
    mock_restore_cache(menuai, ())
    calls = []
    context = Context()
...
async def test_turn_on_off_toggle(
    menuai: menuai, toggle: bool, action_schema_variations: str
) -> None:
    ...
    menuai.bus.async_listen(event, event_mock)
    ...
    async_track_state_change(menuai, ENTITY_ID, state_listener, to_state="on")
    ...
    assert not script.is_on(menuai, ENTITY_ID)
    ...
    if toggle:
        ...
        await menuai.services.async_call(DOMAIN, split_entity_id(ENTITY_ID)[1])
    await menuai.async_block_till_done()
...
invalid_configs = [
    ...
    {"test": {"sequence": {"event": "test_event", "action": "menuai.turn_on"}}},
]
...
async def test_setup_with_invalid_configs(
    menuai: menuai, config, nbr_script_entities
) -> None:
    """Test setup with invalid configs."""
    assert await async_setup_component(menuai, "script", {"script": config})

    assert len(menuai.states.async_entity_ids("script")) == nbr_script_entities
...
async def test_bad_config_validation_critical(
    ...
    assert menuai.states.async_entity_ids("script") == ["script.good_script"]
...
async def test_bad_config_validation(
    ...
    assert set(menuai.states.async_entity_ids("script")) == {
        "script.bad_script",
        "script.good_script",
    }
    # The script failing validation should be unavailable
    assert menuai.states.get("script.bad_script").state == STATE_UNAVAILABLE
...
async def test_reload_service(menuai: menuai, running) -> None:
    ...
    menuai.bus.async_listen_once(event, event_handler)
    menuai.states.async_set("test.script", "off")
    ...
    assert menuai.services.has_service(script.DOMAIN, "test")

    if running != "no":
        _, object_id = split_entity_id(ENTITY_ID)
        await menuai.services.async_call(DOMAIN, object_id)
        await asyncio.wait_for(event_flag.wait(), 1)

        assert script.is_on(menuai, ENTITY_ID)
    ...
    if running != "same":
        ...
        assert not menuai.services.has_service(script.DOMAIN, "test")

        assert menuai.states.get("script.test2") is not None
        assert menuai.services.has_service(script.DOMAIN, "test2")
...
async def test_reload_unchanged_does_not_stop(
    ...
    assert await async_setup_component(menuai, script.DOMAIN, config)
    ...
    with patch(
        ...
        await menuai.services.async_call(script.DOMAIN, SERVICE_RELOAD, blocking=True)
...
async def test_reload_unchanged_script(
    menuai: menuai, calls: list[ServiceCall], script_config
) -> None:
    """Test an unmodified script is not reloaded."""
    with patch(
        "menuai.components.script.ScriptEntity", wraps=ScriptEntity
...
async def test_shared_context(menuai: menuai) -> None:
    ...
    menuai.bus.async_listen(event, event_mock)
    menuai.bus.async_listen(EVENT_SCRIPT_STARTED, run_mock)

    assert await async_setup_component(
        menuai, "script", {"script": {"test": {"sequence": [{"event": event}]}}}
    )
    ...
    state = menuai.states.get("script.test")
...
async def test_logging_script_error(
    ...
    with pytest.raises(ServiceNotFound) as err:
        await menuai.services.async_call("script", "hello", blocking=True)
...
async def test_turning_no_scripts_off(menuai: menuai) -> None:
    """Test it is possible to turn two scripts off."""
    assert await async_setup_component(menuai, "script", {})
...
async def test_async_get_descriptions_script(menuai: menuai) -> None:
    """Test async_set_service_schema for the script integration."""
    script_config = {
        DOMAIN: {
            "test1": {"sequence": [{"action": "menuai.restart"}]},
            "test2": {
                "description": "test2",
                "fields": {
                    "param": {
                ...
                "sequence": [{"action": "menuai.restart"}],
            },
        }
    }

    await async_setup_component(menuai, DOMAIN, script_config)
    descriptions = await async_get_all_descriptions(menuai)
...
async def test_extraction_functions_not_setup(menuai: menuai) -> None:
    ...
    assert script.scripts_with_area(menuai, "area-in-both") == []
    assert script.areas_in_script(menuai, "script.test") == []
    assert script.scripts_with_blueprint(menuai, "blabla.yaml") == []
    assert script.blueprint_in_script(menuai, "script.test") is None
    assert script.scripts_with_device(menuai, "device-in-both") == []
    assert script.devices_in_script(menuai, "script.test") == []
    assert script.scripts_with_entity(menuai, "light.in_both") == []
    assert script.entities_in_script(menuai, "script.test") == []
    assert script.scripts_with_floor(menuai, "floor-in-both") == []
    assert script.floors_in_script(menuai, "script.test") == []
    assert script.scripts_with_label(menuai, "label-in-both") == []
    assert script.labels_in_script(menuai, "script.test") == []


async def test_extraction_functions_unknown_script(menuai: menuai) -> None:
    """Test extraction functions for an unknown script."""
    assert await async_setup_component(menuai, DOMAIN, {})
    assert script.labels_in_script(menuai, "script.unknown") == []
    assert script.floors_in_script(menuai, "script.unknown") == []
    assert script.areas_in_script(menuai, "script.unknown") == []
    assert script.blueprint_in_script(menuai, "script.unknown") is None
    assert script.devices_in_script(menuai, "script.unknown") == []
    assert script.entities_in_script(menuai, "script.unknown") == []


async def test_extraction_functions_unavailable_script(menuai: menuai) -> None:
    ...
    assert script.scripts_with_area(menuai, "area-in-both") == []
    assert script.areas_in_script(menuai, entity_id) == []
    assert script.scripts_with_blueprint(menuai, "blabla.yaml") == []
    assert script.blueprint_in_script(menuai, entity_id) is None
    assert script.scripts_with_device(menuai, "device-in-both") == []
    assert script.devices_in_script(menuai, entity_id) == []
    assert script.scripts_with_entity(menuai, "light.in_both") == []
    assert script.entities_in_script(menuai, entity_id) == []
    assert script.scripts_with_floor(menuai, "floor-in-both") == []
    assert script.floors_in_script(menuai, entity_id) == []
    assert script.scripts_with_label(menuai, "label-in-both") == []
    assert script.labels_in_script(menuai, entity_id) == []
...
async def test_extraction_functions(
    ...
    assert set(script.scripts_with_entity(menuai, "light.in_both")) == {
        "script.test1",
        "script.test2",
        "script.test3",
    }
    assert set(script.entities_in_script(menuai, "script.test1")) == {
        "light.in_both",
        "light.in_first",
    }
    assert set(script.scripts_with_device(menuai, device_in_both.id)) == {
        "script.test1",
        "script.test2",
    }
    assert set(script.devices_in_script(menuai, "script.test2")) == {
        device_in_both.id,
        device_in_last.id,
    }
    assert set(script.scripts_with_area(menuai, "area-in-both")) == {
        "script.test1",
        "script.test3",
    }
    assert set(script.areas_in_script(menuai, "script.test3")) == {
        "area-in-both",
        "area-in-last",
    }
    assert set(script.scripts_with_floor(menuai, "floor-in-both")) == {
        "script.test1",
        "script.test3",
    }
    assert set(script.floors_in_script(menuai, "script.test3")) == {
        "floor-in-both",
        "floor-in-last",
    }
    assert set(script.scripts_with_label(menuai, "label-in-both")) == {
        "script.test1",
        "script.test3",
    }
    assert set(script.labels_in_script(menuai, "script.test3")) == {
        "label-in-both",
        "label-in-last",
    }
    assert script.blueprint_in_script(menuai, "script.test3") is None
...
async def test_config_basic(
    ...
    test_script = menuai.states.get("script.test_script")
...
async def test_config_multiple_domains(menuai: menuai) -> None:
    ...
    test_script = menuai.states.get("script.first_script")
    assert test_script
    assert test_script.name == "Main domain"

    test_script = menuai.states.get("script.second_script")
    assert test_script
    assert test_script.name == "Secondary domain"


async def test_logbook_humanify_script_started_event(menuai: menuai) -> None:
...
async def test_concurrent_script(menuai: menuai, concurrently) -> None:
    ...
    menuai.services.async_register("test", "script", async_service_handler)
    menuai.states.async_set("input_boolean.test1", "off")
    menuai.states.async_set("input_boolean.test2", "off")

    await menuai.services.async_call("script", "script1")
    await asyncio.wait_for(service_called.wait(), 1)
    service_called.clear()

    assert service_values[-1] == "script2a"
    assert script.is_on(menuai, "script.script1")
    assert script.is_on(menuai, "script.script2")

    if not concurrently:
        menuai.states.async_set("input_boolean.test2", "on")
        await asyncio.wait_for(service_called.wait(), 1)
        service_called.clear()

        assert service_values[-1] == "script2b"

    menuai.states.async_set("input_boolean.test1", "on")
    await asyncio.wait_for(service_called.wait(), 1)
    service_called.clear()

    assert service_values[-1] == "script1"
    assert concurrently == script.is_on(menuai, "script.script2")
    ...
    assert not script.is_on(menuai, "script.script1")
    assert not script.is_on(menuai, "script.script2")
...
async def test_script_variables(
    ...
    with pytest.raises(TemplateError):
        await menuai.services.async_call("script", "script3", blocking=True)
    assert "Error rendering variables" in caplog.text
    assert len(mock_calls) == 3

    await menuai.services.async_call("script", "script3", {"break": 0}, blocking=True)
...
async def test_script_this_var_always(
    ...
    await menuai.services.async_call("script", "script1", blocking=True)
...
async def test_script_restore_last_triggered(menuai: menuai) -> None:
    ...
    state = menuai.states.get("script.no_last_triggered")
    assert state
    assert state.attributes["last_triggered"] is None

    state = menuai.states.get("script.last_triggered")
    assert state
...
async def test_recursive_script(
    menuai: menuai, script_mode, warning_msg, caplog: pytest.LogCaptureFixture
) -> None:
...
async def test_recursive_script_turn_on(
    menuai: menuai, script_mode, caplog: pytest.LogCaptureFixture
) -> None:
...
async def test_setup_with_duplicate_scripts(
    ...
    assert len(menuai.states.async_entity_ids("script")) == 1
...
async def test_script_service_changed_entity_id(
    ...
    await menuai.services.async_call(DOMAIN, "test", {"greeting": "world"})
...
async def test_blueprint_script(menuai: menuai, calls: list[ServiceCall]) -> None:
    ...
    await menuai.async_block_till_done()
    assert len(calls) == 1
    assert script.blueprint_in_script(menuai, "script.test_script") == "test_service.yaml"
    assert script.scripts_with_blueprint(menuai, "test_service.yaml") == [
        "script.test_script"
    ]
...
async def test_responses(menuai: menuai, response: Any) -> None:
...
async def test_responses_no_response(menuai: menuai) -> None:
...
async def test_script_queued_mode(menuai: menuai) -> None:
    ...
    menuai.services.async_register("test", "simulated_remote", async_service_handler)
    assert await async_setup_component(
        menuai,
        script.DOMAIN,
        {
    ...
    await menuai.services.async_call("script", "test_main", blocking=True)
    assert calls == 4
