...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "menuai.components.airgradient.async_setup_entry",
        return_value=True,
    ) as mock_setup_entry:
...
def mock_airgradient_client() -> Generator[AsyncMock]:
    ...
    with (
        patch(
            "menuai.components.airgradient.AirGradientClient",
            autospec=True,
        ) as mock_client,
        patch(
            "menuai.components.airgradient.config_flow.AirGradientClient",
            new=mock_client,
        ),
    ):
