...
from menuai.const import ATTR_SUPPORTED_FEATURES, STATE_OFF, STATE_ON
...
async def test_hmip_notification_light(
    ...
    await async_manipulate_test_data(menuai, hmip_device, "dimLevel", 1, 2)
    await async_manipulate_test_data(
        menuai, hmip_device, "simpleRGBColorState", RGBColorState.PURPLE, 2
    )
    ha_state = menuai.states.get(entity_id)
    assert ha_state.state == STATE_ON
    ...
    await async_manipulate_test_data(menuai, hmip_device, "dimLevel", 0, 2)
    ha_state = menuai.states.get(entity_id)
    assert ha_state.state == STATE_OFF

    await async_manipulate_test_data(menuai, hmip_device, "dimLevel", None, 2)
    ha_state = menuai.states.get(entity_id)
    assert ha_state.state == STATE_OFF
...
async def test_hmip_dimmer(
    ...
    await async_manipulate_test_data(menuai, hmip_device, "dimLevel", 1)
    ha_state = menuai.states.get(entity_id)
    assert ha_state.state == STATE_ON
    assert ha_state.attributes[ATTR_BRIGHTNESS] == 255
    ...
    await async_manipulate_test_data(menuai, hmip_device, "dimLevel", 0)
    ha_state = menuai.states.get(entity_id)
    assert ha_state.state == STATE_OFF

    await async_manipulate_test_data(menuai, hmip_device, "dimLevel", None)
    ha_state = menuai.states.get(entity_id)
    assert ha_state.state == STATE_OFF
    assert not ha_state.attributes.get(ATTR_BRIGHTNESS)
...
async def test_hmip_wired_multi_dimmer(
    ...
    await async_manipulate_test_data(menuai, hmip_device, "dimLevel", 1, channel=1)
    ha_state = menuai.states.get(entity_id)
    assert ha_state.state == STATE_ON
    assert ha_state.attributes[ATTR_BRIGHTNESS] == 255
    assert ha_state.attributes[ATTR_COLOR_MODE] == ColorMode.BRIGHTNESS
    ...
    assert hmip_device.mock_calls[-1][0] == "set_dim_level_async"
    assert hmip_device.mock_calls[-1][1] == (0, 1)
    await async_manipulate_test_data(menuai, hmip_device, "dimLevel", 0, channel=1)
    ha_state = menuai.states.get(entity_id)
    assert ha_state.state == STATE_OFF

    await async_manipulate_test_data(menuai, hmip_device, "dimLevel", None, channel=1)
    ha_state = menuai.states.get(entity_id)
    assert ha_state.state == STATE_OFF
    assert not ha_state.attributes.get(ATTR_BRIGHTNESS)
...
async def test_hmip_din_rail_dimmer_3_channel2(
    ...
    await async_manipulate_test_data(menuai, hmip_device, "dimLevel", 1, channel=2)
    ha_state = menuai.states.get(entity_id)
    assert ha_state.state == STATE_ON
    assert ha_state.attributes[ATTR_BRIGHTNESS] == 255
    assert ha_state.attributes[ATTR_COLOR_MODE] == ColorMode.BRIGHTNESS
    ...
    await async_manipulate_test_data(menuai, hmip_device, "dimLevel", 0, channel=2)
    ha_state = menuai.states.get(entity_id)
    assert ha_state.state == STATE_OFF

    await async_manipulate_test_data(menuai, hmip_device, "dimLevel", None, channel=2)
    ha_state = menuai.states.get(entity_id)
    assert ha_state.state == STATE_OFF
    assert not ha_state.attributes.get(ATTR_BRIGHTNESS)
...
async def test_hmip_din_rail_dimmer_3_channel3(
    ...
    await async_manipulate_test_data(menuai, hmip_device, "dimLevel", 1, channel=3)
    ha_state = menuai.states.get(entity_id)
    assert ha_state.state == STATE_ON
    assert ha_state.attributes[ATTR_BRIGHTNESS] == 255
    assert ha_state.attributes[ATTR_COLOR_MODE] == ColorMode.BRIGHTNESS
    ...
    await async_manipulate_test_data(menuai, hmip_device, "dimLevel", 0, channel=3)
    ha_state = menuai.states.get(entity_id)
    assert ha_state.state == STATE_OFF

    await async_manipulate_test_data(menuai, hmip_device, "dimLevel", None, channel=3)
    ha_state = menuai.states.get(entity_id)
    assert ha_state.state == STATE_OFF
    assert not ha_state.attributes.get(ATTR_BRIGHTNESS)
