...
async def test_config_with_accesspoint_passed_to_config_entry(
    menuai: menuai, mock_connection, simple_mock_home
) -> None:
    ...
    with patch(
        ...
        assert await async_setup_component(menuai, DOMAIN, {DOMAIN: entry_config})
...
async def test_config_already_registered_not_passed_to_config_entry(
    menuai: menuai, simple_mock_home
) -> None:
    ...
    MockConfigEntry(domain=DOMAIN, data=mock_config).add_to_menuai(menuai)
...
async def test_load_entry_fails_due_to_connection_error(
    ...
    hmip_config_entry.add_to_menuai(menuai)
...
async def test_load_entry_fails_due_to_generic_exception(
    menuai: menuai, hmip_config_entry: MockConfigEntry
) -> None:
...
async def test_unload_entry(menuai: menuai) -> None:
    ...
    with patch("menuai.components.homematicip_cloud.HomematicipHAP") as mock_hap:
...
async def test_setup_services(menuai: menuai) -> None:
    ...
    hmipc_services = menuai.services.async_services()[DOMAIN]
