...
from menuai.components.camera.helper import get_camera_from_entity_id
...
from menuai.components.unifiprotect.utils import get_camera_base_name
...
@pytest.fixture
async def web_rtc_provider(menuai: menuai) -> None:
    """Fixture to enable WebRTC provider for camera entities."""
    await async_setup_component(menuai, "camera", {})
    async_register_webrtc_provider(menuai, MockWebRTCProvider())
...
async def validate_rtsps_camera_state(
    ...
    assert await async_get_stream_source(menuai, entity_id) == channel.rtsps_no_srtp_url
    validate_common_camera_state(menuai, channel, entity_id, features)
...
async def validate_rtsp_camera_state(
    ...
    assert await async_get_stream_source(menuai, entity_id) == channel.rtsp_url
    validate_common_camera_state(menuai, channel, entity_id, features)
...
async def validate_no_stream_camera_state(
    ...
    assert await async_get_stream_source(menuai, entity_id) is None
    validate_common_camera_state(menuai, channel, entity_id, features)
...
async def test_basic_setup(
    ...
    await init_entry(menuai, ufp, devices)

    assert_entity_counts(menuai, Platform.CAMERA, 14, 6)

    # test camera 1
    entity_id = validate_default_camera_entity(menuai, camera_high_only, 0)
    await validate_rtsps_camera_state(menuai, camera_high_only, 0, entity_id)

    entity_id = validate_rtsp_camera_entity(menuai, camera_high_only, 0)
    await enable_entity(menuai, ufp.entry.entry_id, entity_id)
    await validate_rtsp_camera_state(menuai, camera_high_only, 0, entity_id)

    # test camera 2
    entity_id = validate_default_camera_entity(menuai, camera_medium_only, 1)
    await validate_rtsps_camera_state(menuai, camera_medium_only, 1, entity_id)

    entity_id = validate_rtsp_camera_entity(menuai, camera_medium_only, 1)
    await enable_entity(menuai, ufp.entry.entry_id, entity_id)
    await validate_rtsp_camera_state(menuai, camera_medium_only, 1, entity_id)

    # test camera 3
    entity_id = validate_default_camera_entity(menuai, camera_all, 0)
    await validate_rtsps_camera_state(menuai, camera_all, 0, entity_id)

    entity_id = validate_rtsp_camera_entity(menuai, camera_all, 0)
    await enable_entity(menuai, ufp.entry.entry_id, entity_id)
    await validate_rtsp_camera_state(menuai, camera_all, 0, entity_id)

    entity_id = validate_rtsps_camera_entity(menuai, camera_all, 1)
    await enable_entity(menuai, ufp.entry.entry_id, entity_id)
    await validate_rtsps_camera_state(menuai, camera_all, 1, entity_id)

    entity_id = validate_rtsp_camera_entity(menuai, camera_all, 1)
    await enable_entity(menuai, ufp.entry.entry_id, entity_id)
    await validate_rtsp_camera_state(menuai, camera_all, 1, entity_id)

    entity_id = validate_rtsps_camera_entity(menuai, camera_all, 2)
    await enable_entity(menuai, ufp.entry.entry_id, entity_id)
    await validate_rtsps_camera_state(menuai, camera_all, 2, entity_id)

    entity_id = validate_rtsp_camera_entity(menuai, camera_all, 2)
    await enable_entity(menuai, ufp.entry.entry_id, entity_id)
    await validate_rtsp_camera_state(menuai, camera_all, 2, entity_id)

    # test camera 4
    entity_id = validate_default_camera_entity(menuai, camera_no_channels, 0)
    await validate_no_stream_camera_state(
        menuai, camera_no_channels, 0, entity_id, features=0
    )

    # test camera 5
    entity_id = validate_default_camera_entity(menuai, doorbell, 0)
    await validate_rtsps_camera_state(menuai, doorbell, 0, entity_id)

    entity_id = validate_rtsp_camera_entity(menuai, doorbell, 0)
    await enable_entity(menuai, ufp.entry.entry_id, entity_id)
    await validate_rtsp_camera_state(menuai, doorbell, 0, entity_id)

    entity_id = validate_default_camera_entity(menuai, doorbell, 3)
    await validate_no_stream_camera_state(menuai, doorbell, 3, entity_id, features=0)
...
async def test_webrtc_support(
    ...
    camera_high_only.channels[1].is_rtsp_enabled = False
    camera_high_only.channels[2].is_rtsp_enabled = False
    await init_entry(menuai, ufp, [camera_high_only])
    entity_id = validate_default_camera_entity(menuai, camera_high_only, 0)
    assert menuai.states.get(entity_id)
    camera_obj = get_camera_from_entity_id(menuai, entity_id)
...
async def test_adopt(
    menuai: menuai, ufp: MockUFPFixture, camera: ProtectCamera
) -> None:
    ...
    await init_entry(menuai, ufp, [camera1])
    assert_entity_counts(menuai, Platform.CAMERA, 0, 0)

    await remove_entities(menuai, ufp, [camera1])
    assert_entity_counts(menuai, Platform.CAMERA, 0, 0)
    camera1.channels = []
    await adopt_devices(menuai, ufp, [camera1])
    assert_entity_counts(menuai, Platform.CAMERA, 0, 0)
    ...
    assert_entity_counts(menuai, Platform.CAMERA, 2, 1)
...
async def test_camera_image(
    ...
    await async_get_image(menuai, "camera.test_camera_high_resolution_channel")
    ufp.api.get_camera_snapshot.assert_called_once()


async def test_package_camera_image(
    menuai: menuai, ufp: MockUFPFixture, doorbell: ProtectCamera
) -> None:
    """Test retrieving package camera image."""

    await init_entry(menuai, ufp, [doorbell])
    assert_entity_counts(menuai, Platform.CAMERA, 3, 2)

    ufp.api.get_package_camera_snapshot = AsyncMock()

    await async_get_image(menuai, "camera.test_camera_package_camera")
...
async def test_camera_interval_update(
    ...
    await time_changed(menuai, DEVICE_UPDATE_INTERVAL)
