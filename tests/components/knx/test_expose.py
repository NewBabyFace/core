...
from menuai.components.knx import CONF_KNX_EXPOSE, DOMAIN, KNX_ADDRESS
from menuai.components.knx.schema import ExposeSchema
...
async def test_binary_expose(menuai: menuai, knx: KNXTestKit) -> None:
    ...
    menuai.states.async_set(entity_id, "on", {"brightness": 180})
    await menuai.async_block_till_done()
    await knx.assert_no_telegram()

    # Change attribute and state
    menuai.states.async_set(entity_id, "off", {"brightness": 0})
    await menuai.async_block_till_done()
    await knx.assert_write("1/1/8", False)


async def test_expose_attribute(menuai: menuai, knx: KNXTestKit) -> None:
    ...
    # Change attribute; keep state
    menuai.states.async_set(entity_id, "on", {attribute: 1})
    await menuai.async_block_till_done()
    ...
    menuai.states.async_set(entity_id, "off", {attribute: 1})
    await menuai.async_block_till_done()
    await knx.assert_telegram_count(0)

    # Change state and attribute
    menuai.states.async_set(entity_id, "on", {attribute: 0})
    await menuai.async_block_till_done()
    ...
    menuai.states.async_set(entity_id, "off", {attribute: None})
    await menuai.async_block_till_done()
    await knx.assert_telegram_count(0)

    # Ignore "unavailable" state
    menuai.states.async_set(entity_id, "unavailable", {attribute: None})
    await menuai.async_block_till_done()
    await knx.assert_telegram_count(0)
...
async def test_expose_attribute_with_default(
    ...
    menuai.states.async_set(entity_id, "unavailable")
    await menuai.async_block_till_done()
    await knx.assert_write("1/1/8", (0,))

    # Change state and attribute
    menuai.states.async_set(entity_id, "on", {attribute: 3})
    await menuai.async_block_till_done()
...
async def test_expose_string(menuai: menuai, knx: KNXTestKit) -> None:
...
async def test_expose_cooldown(
    ...
    menuai.states.async_set(entity_id, "2", {})
    await menuai.async_block_till_done()
    await knx.assert_no_telegram()

    # Change state to 3
    menuai.states.async_set(entity_id, "3", {})
    await menuai.async_block_till_done()
    await knx.assert_no_telegram()
...
async def test_expose_value_template(
    menuai: menuai, knx: KNXTestKit, caplog: pytest.LogCaptureFixture
) -> None:
    ...
    menuai.states.async_set(entity_id, "off", {attribute: 255})
    await menuai.async_block_till_done()
...
async def test_expose_with_date(
    menuai: menuai, knx: KNXTestKit, time_type: str, raw: tuple[int, ...]
) -> None:
    ...
    assert await menuai.config_entries.async_unload(entries[0].entry_id)
