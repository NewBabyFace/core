...
from menuai.components.trace.const import DEFAULT_STORED_TRACES
from menuai.const import EVENT_menuai_STOP
from menuai.core import Context, CoreState, menuai, callback
...
async def _setup_automation_or_script(
    ...
    if script_config:
        if domain == "automation":
            assert await async_setup_component(
                menuai, "script", {"script": script_config}
            )
        else:
    ...
    assert await async_setup_component(menuai, domain, {domain: configs})
...
async def _run_automation_or_script(
    ...
    if domain == "automation":
        menuai.bus.async_fire(event, context=context)
    else:
        await menuai.services.async_call("script", config["id"], context=context)
...
async def test_get_trace(
    ...
    await _setup_automation_or_script(menuai, domain, [sun_config, moon_config])
    ...
    context = Context()
    await _run_automation_or_script(menuai, domain, sun_config, "test_event", context)
    ...
    await _run_automation_or_script(menuai, domain, moon_config, "test_event2", context)
    await menuai.async_block_till_done()
    ...
    # Fake stop
    assert "trace.saved_traces" not in menuai_storage
    menuai.bus.async_fire(EVENT_menuai_STOP)
    await menuai.async_block_till_done()

    # Check that saved data is same as the serialized traces
    assert "trace.saved_traces" in menuai_storage
    assert menuai_storage["trace.saved_traces"]["data"] == traces
...
async def test_restore_traces(
    ...
    saved_traces = json.loads(
        await async_load_fixture(menuai, f"{domain}_saved_traces.json", "trace")
    )
    menuai_storage["trace.saved_traces"] = saved_traces
    await _setup_automation_or_script(menuai, domain, [])
    ...
    # Fake stop
    menuai_storage.pop("trace.saved_traces")
    assert "trace.saved_traces" not in menuai_storage
    menuai.bus.async_fire(EVENT_menuai_STOP)
    await menuai.async_block_till_done()
    ...
    # Check that saved data is same as the serialized traces
    assert "trace.saved_traces" in menuai_storage
    assert menuai_storage["trace.saved_traces"] == saved_traces


@pytest.mark.parametrize("domain", ["automation", "script"])
async def test_get_invalid_trace(
    menuai: menuai, menuai_ws_client: WebSocketGenerator, domain
) -> None:
...
async def test_trace_overflow(
    menuai: menuai, menuai_ws_client: WebSocketGenerator, domain, stored_traces
) -> None:
    ...
    await _setup_automation_or_script(
        menuai, domain, [sun_config, moon_config], stored_traces=stored_traces
    )
    ...
    await _run_automation_or_script(menuai, domain, sun_config, "test_event")
    await _run_automation_or_script(menuai, domain, moon_config, "test_event2")
    await menuai.async_block_till_done()
    ...
    with patch(
        "menuai.components.trace.models.uuid_util.random_uuid_hex",
...
async def test_restore_traces_overflow(
    ...
    assert "trace_traces_restored" not in menuai.data
    ...
    assert "trace_traces_restored" in menuai.data
...
async def test_trace_no_traces(
    ...
    await _setup_automation_or_script(menuai, domain, [sun_config], stored_traces=0)
...
async def test_list_traces(
    ...
    await _run_automation_or_script(menuai, domain, moon_config, "test_event3")
    await menuai.async_block_till_done()
...
async def test_breakpoints(
    menuai: menuai, menuai_ws_client: WebSocketGenerator, domain, prefix
) -> None:
    ...
    await _setup_automation_or_script(menuai, domain, [sun_config])
...
async def test_script_mode(
    ...
    client = await menuai_ws_client()
    menuai.bus.async_listen(event, _handle_event)
...
async def test_trace_blueprint_automation(
    ...
    assert await async_setup_component(menuai, "automation", {"automation": sun_config})
    client = await menuai_ws_client()
