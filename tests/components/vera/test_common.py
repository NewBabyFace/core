...
from menuai.components.vera import SubscriptionRegistry
...
async def test_subscription_registry(menuai: menuai) -> None:
    ...
    await menuai.async_add_executor_job(subscription_registry.start)
    ...
    async_fire_time_changed(menuai, utcnow() + timedelta(seconds=2))
    await menuai.async_block_till_done()
    poll_server_once_mock.assert_called_once()
    ...
    poll_server_once_mock.reset_mock()
    await menuai.async_add_executor_job(subscription_registry.stop)

    # Assert no further polling is performed.
    async_fire_time_changed(menuai, utcnow() + timedelta(seconds=65))
    await menuai.async_block_till_done()
    poll_server_once_mock.assert_not_called()
