...
async def test_cover(
    ...
    assert menuai.states.get(entity_id).state == "closed"
    assert menuai.states.get(entity_id).attributes["current_position"] == 0
    ...
    await menuai.async_block_till_done()
    assert menuai.states.get(entity_id).state == "open"
    assert menuai.states.get(entity_id).attributes["current_position"] == 100
    ...
    await menuai.async_block_till_done()
    assert menuai.states.get(entity_id).state == "open"
    assert menuai.states.get(entity_id).attributes["current_position"] == 50
    ...
    await menuai.async_block_till_done()
    assert menuai.states.get(entity_id).state == "closed"
    assert menuai.states.get(entity_id).attributes["current_position"] == 00
