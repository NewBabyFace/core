...
from menuai.components.notify import ATTR_TITLE_DEFAULT
...
async def test_notify_file(
    ...
    filename = "mock_file"
    full_filename = os.path.join(menuai.config.path(), filename)
    ...
    m_open = mock_open()
    with (
        patch("menuai.components.file.notify.open", m_open, create=True),
        patch("menuai.components.file.notify.os.stat") as mock_st,
    ):
        ...
        await menuai.services.async_call(domain, service, params, blocking=True)
