...
from menuai.components.greeneye_monitor.sensor import (
    DATA_PULSES,
    DATA_WATT_SECONDS,
)
...
async def test_sensor_does_not_exist_before_monitor_connected(
    menuai: menuai, entity_registry: er.EntityRegistry, monitors: AsyncMock
) -> None:
    ...
    await setup_greeneye_monitor_component_with_config(
        menuai, SINGLE_MONITOR_CONFIG_VOLTAGE_SENSORS
    )
...
async def test_sensors_created_when_monitor_connected(
    menuai: menuai, monitors: AsyncMock
) -> None:
    ...
    await connect_monitor(menuai, monitors, SINGLE_MONITOR_SERIAL_NUMBER)
    assert len(monitors.listeners) == 0  # Make sure we cleaned up the listener
    assert_sensor_state(menuai, "sensor.voltage_1", "120.0")
...
async def test_disable_sensor_after_monitor_connected(
    ...
    monitor = await connect_monitor(menuai, monitors, SINGLE_MONITOR_SERIAL_NUMBER)

    assert len(monitor.voltage_sensor.listeners) == 1
    await disable_entity(menuai, "sensor.voltage_1")
...
async def test_updates_state_when_sensor_pushes(
    ...
    assert_sensor_state(menuai, "sensor.voltage_1", "119.8")
...
async def test_power_sensor_initially_unknown(
    ...
    await setup_greeneye_monitor_component_with_config(
        menuai, SINGLE_MONITOR_CONFIG_POWER_SENSORS
    )
    await connect_monitor(menuai, monitors, SINGLE_MONITOR_SERIAL_NUMBER)
    assert_sensor_state(
        menuai, "sensor.channel_1", STATE_UNKNOWN, {DATA_WATT_SECONDS: 1000}
    )
    # This sensor was configured with net metering on, so we should be taking the
    # polarized value
    assert_sensor_state(
        menuai, "sensor.channel_two", STATE_UNKNOWN, {DATA_WATT_SECONDS: -400}
    )


async def test_power_sensor(menuai: menuai, monitors: AsyncMock) -> None:
    ...
    assert_sensor_state(menuai, "sensor.channel_1", "120.0", {DATA_WATT_SECONDS: 1000})
    # This sensor was configured with net metering on, so we should be taking the
    # polarized value
    assert_sensor_state(menuai, "sensor.channel_two", "120.0", {DATA_WATT_SECONDS: -400})
...
async def test_pulse_counter_initially_unknown(
    ...
    await setup_greeneye_monitor_component_with_config(
        menuai, SINGLE_MONITOR_CONFIG_PULSE_COUNTERS
    )
    monitor = await connect_monitor(menuai, monitors, SINGLE_MONITOR_SERIAL_NUMBER)
    ...
    assert_sensor_state(menuai, "sensor.pulse_a", STATE_UNKNOWN, {DATA_PULSES: 1000})
    ...
    assert_sensor_state(menuai, "sensor.pulse_2", STATE_UNKNOWN, {DATA_PULSES: 1000})
    # This counter was configured with each pulse meaning 0.5 gallons and
    # wanting to show gallons per hour, so 10 pulses per second -> 18000 gal/hr
    assert_sensor_state(menuai, "sensor.pulse_3", STATE_UNKNOWN, {DATA_PULSES: 1000})


async def test_pulse_counter(menuai: menuai, monitors: AsyncMock) -> None:
    """Test that a pulse counter sensor reports its values properly, including calculating different units."""
    await setup_greeneye_monitor_component_with_config(
        menuai, SINGLE_MONITOR_CONFIG_PULSE_COUNTERS
    )
    await connect_monitor(menuai, monitors, SINGLE_MONITOR_SERIAL_NUMBER)
    assert_sensor_state(menuai, "sensor.pulse_a", "10.0", {DATA_PULSES: 1000})
    # This counter was configured with each pulse meaning 0.5 gallons and
    # wanting to show gallons per minute, so 10 pulses per second -> 300 gal/min
    assert_sensor_state(menuai, "sensor.pulse_2", "300.0", {DATA_PULSES: 1000})
    # This counter was configured with each pulse meaning 0.5 gallons and
    # wanting to show gallons per hour, so 10 pulses per second -> 18000 gal/hr
    assert_sensor_state(menuai, "sensor.pulse_3", "18000.0", {DATA_PULSES: 1000})


async def test_temperature_sensor(menuai: menuai, monitors: AsyncMock) -> None:
    """Test that a temperature sensor reports its values properly, including proper handling of when its native unit is different from that configured in menuai."""
    await setup_greeneye_monitor_component_with_config(
        menuai, SINGLE_MONITOR_CONFIG_TEMPERATURE_SENSORS
    )
    await connect_monitor(menuai, monitors, SINGLE_MONITOR_SERIAL_NUMBER)
    ...
    assert_sensor_state(menuai, "sensor.temp_a", "0.0")


async def test_voltage_sensor(menuai: menuai, monitors: AsyncMock) -> None:
...
async def test_multi_monitor_sensors(menuai: menuai, monitors: AsyncMock) -> None:
    """Test that sensors still work when multiple monitors are registered."""
    await setup_greeneye_monitor_component_with_config(menuai, MULTI_MONITOR_CONFIG)
    await connect_monitor(menuai, monitors, 1)
    await connect_monitor(menuai, monitors, 2)
    await connect_monitor(menuai, monitors, 3)
    assert_sensor_state(menuai, "sensor.unit_1_temp_1", "32.0")
    assert_sensor_state(menuai, "sensor.unit_2_temp_1", "0.0")
    assert_sensor_state(menuai, "sensor.unit_3_temp_1", "32.0")


async def disable_entity(menuai: menuai, entity_id: str) -> None:
