...
async def test_setup_fails_if_no_sensors_defined(
    ...
    success = await setup_greeneye_monitor_component_with_config(
        menuai, SINGLE_MONITOR_CONFIG_NO_SENSORS
    )
    assert not success
...
async def test_setup_creates_temperature_entities(
    ...
    assert_temperature_sensor_registered(
        menuai, SINGLE_MONITOR_SERIAL_NUMBER, 1, "temp_a"
    )
    assert_temperature_sensor_registered(
        menuai, SINGLE_MONITOR_SERIAL_NUMBER, 2, "temp_2"
    )
    assert_temperature_sensor_registered(
        menuai, SINGLE_MONITOR_SERIAL_NUMBER, 3, "temp_c"
    )
    assert_temperature_sensor_registered(
        menuai, SINGLE_MONITOR_SERIAL_NUMBER, 4, "temp_d"
    )
    assert_temperature_sensor_registered(
        menuai, SINGLE_MONITOR_SERIAL_NUMBER, 5, "temp_5"
    )
    assert_temperature_sensor_registered(
        menuai, SINGLE_MONITOR_SERIAL_NUMBER, 6, "temp_f"
    )
    assert_temperature_sensor_registered(
        menuai, SINGLE_MONITOR_SERIAL_NUMBER, 7, "temp_g"
    )
    assert_temperature_sensor_registered(
        menuai, SINGLE_MONITOR_SERIAL_NUMBER, 8, "temp_h"
    )
...
async def test_setup_creates_pulse_counter_entities(
    ...
    assert_pulse_counter_registered(
        menuai, SINGLE_MONITOR_SERIAL_NUMBER, 2, "pulse_2", "gal", "min"
    )
    assert_pulse_counter_registered(
        menuai,
        SINGLE_MONITOR_SERIAL_NUMBER,
...
async def test_setup_creates_power_sensor_entities(
    ...
    await connect_monitor(menuai, monitors, SINGLE_MONITOR_SERIAL_NUMBER)
    assert_power_sensor_registered(menuai, SINGLE_MONITOR_SERIAL_NUMBER, 1, "channel 1")
    assert_power_sensor_registered(menuai, SINGLE_MONITOR_SERIAL_NUMBER, 2, "channel two")
...
async def test_setup_creates_voltage_sensor_entities(
    ...
    await connect_monitor(menuai, monitors, SINGLE_MONITOR_SERIAL_NUMBER)
    assert_voltage_sensor_registered(menuai, SINGLE_MONITOR_SERIAL_NUMBER, 1, "voltage 1")


async def test_multi_monitor_config(menuai: menuai, monitors: AsyncMock) -> None:
    ...
    assert_temperature_sensor_registered(menuai, 1, 1, "unit_1_temp_1")
    assert_temperature_sensor_registered(menuai, 2, 1, "unit_2_temp_1")
    assert_temperature_sensor_registered(menuai, 3, 1, "unit_3_temp_1")


async def test_setup_and_shutdown(menuai: menuai, monitors: AsyncMock) -> None:
