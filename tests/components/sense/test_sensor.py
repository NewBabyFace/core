...
from menuai.components.sense.const import ACTIVE_UPDATE_RATE, TREND_UPDATE_RATE
...
async def test_sensors(
    ...
    """Test Sensor."""
    await setup_platform(menuai, config_entry, Platform.SENSOR)
...
async def test_device_power_sensors(
    ...
    device_1.power_w = 0
    device_2.power_w = 0
    await setup_platform(menuai, config_entry, SENSOR_DOMAIN)
    device_1, device_2 = mock_sense.devices

    state = menuai.states.get(f"sensor.{DEVICE_1_NAME.lower()}_power")
    assert state.state == "0"

    state = menuai.states.get(f"sensor.{DEVICE_2_NAME.lower()}_power")
    assert state.state == "0"
...
async def test_device_energy_sensors(
    ...
    state = menuai.states.get(f"sensor.{DEVICE_1_NAME.lower()}_daily_energy")
    assert state.state == f"{DEVICE_1_DAY_ENERGY:.0f}"

    state = menuai.states.get(f"sensor.{DEVICE_2_NAME.lower()}_daily_energy")
...
async def test_voltage_sensors(
    ...
    state = menuai.states.get(f"sensor.sense_{MONITOR_ID}_l1_voltage")
    assert state.state == "120"

    state = menuai.states.get(f"sensor.sense_{MONITOR_ID}_l2_voltage")
...
async def test_active_power_sensors(
    ...
    state = menuai.states.get(f"sensor.sense_{MONITOR_ID}_energy")
    assert state.state == "400"

    state = menuai.states.get(f"sensor.sense_{MONITOR_ID}_production")
...
async def test_trend_energy_sensors(
    ...
    state = menuai.states.get(f"sensor.sense_{MONITOR_ID}_daily_energy")
    assert state.state == "100"

    state = menuai.states.get(f"sensor.sense_{MONITOR_ID}_daily_production")
    assert state.state == "200"

    state = menuai.states.get(f"sensor.sense_{MONITOR_ID}_daily_from_grid")
    assert state.state == "300"

    state = menuai.states.get(f"sensor.sense_{MONITOR_ID}_daily_to_grid")
    assert state.state == "400"

    state = menuai.states.get(f"sensor.sense_{MONITOR_ID}_daily_net_production")
    assert state.state == "500"
    ...
    async_fire_time_changed(menuai, utcnow() + timedelta(seconds=600))
    await menuai.async_block_till_done()
