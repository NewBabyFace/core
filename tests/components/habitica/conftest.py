...
from menuai.components.habitica.const import CONF_API_USER, DEFAULT_URL, DOMAIN
...
async def mock_habiticalib(menuai: menuai) -> AsyncGenerator[AsyncMock]:
    """Mock habiticalib."""

    with (
        patch(
            "menuai.components.habitica.Habitica", autospec=True
        ) as mock_client,
        patch(
            "menuai.components.habitica.config_flow.Habitica", new=mock_client
        ),
    ):
        client = mock_client.return_value

        client.login.return_value = HabiticaLoginResponse.from_json(
            await async_load_fixture(menuai, "login.json", DOMAIN)
        )

        client.get_user.return_value = HabiticaUserResponse.from_json(
            await async_load_fixture(menuai, "user.json", DOMAIN)
        )

        client.cast_skill.return_value = HabiticaCastSkillResponse.from_json(
            await async_load_fixture(menuai, "cast_skill_response.json", DOMAIN)
        )
        ...
        client.update_score.return_value = HabiticaUserResponse.from_json(
            await async_load_fixture(menuai, "score_with_drop.json", DOMAIN)
        )
        client.get_group_members.return_value = HabiticaGroupMembersResponse.from_json(
            await async_load_fixture(menuai, "party_members.json", DOMAIN)
        )
        for func in (
            "leave_quest",
            "reject_quest",
            ...
            getattr(client, func).return_value = HabiticaQuestResponse.from_json(
                await async_load_fixture(menuai, "party_quest.json", DOMAIN)
            )
        client.get_content.return_value = HabiticaContentResponse.from_json(
            await async_load_fixture(menuai, "content.json", DOMAIN)
        )
        ...
        client.get_user_anonymized.return_value = (
            HabiticaUserAnonymizedResponse.from_json(
                await async_load_fixture(menuai, "anonymized.json", DOMAIN)
            )
        )
        client.update_task.return_value = HabiticaTaskResponse.from_json(
            await async_load_fixture(menuai, "task.json", DOMAIN)
        ...
        client.create_tag.return_value = HabiticaTagResponse.from_json(
            await async_load_fixture(menuai, "create_tag.json", DOMAIN)
        )
        client.create_task.return_value = HabiticaTaskResponse.from_json(
            await async_load_fixture(menuai, "task.json", DOMAIN)
        )
...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "menuai.components.habitica.async_setup_entry", return_value=True
    ) as mock_setup_entry:
...
def mock_uuid4() -> Generator[MagicMock]:
    """Mock uuid4."""
    with patch(
        "menuai.components.habitica.services.uuid4", autospec=True
    ) as mock_uuid4:
