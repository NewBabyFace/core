...
async def test_loading_sensors(menuai: menuai, init_integration) -> None:
...
async def test_srp_entity(menuai: menuai, init_integration) -> None:
    """Test the SrpEntity."""
    usage_state = menuai.states.get("sensor.srp_energy_mock_title_energy_usage")
...
async def test_srp_entity_timeout(
    ...
    with (
        ...
        patch("menuai.components.srp_energy.coordinator.TIMEOUT", 0),
    ):
        client = srp_energy_mock.return_value
        client.validate.return_value = True
