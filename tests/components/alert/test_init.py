...
from menuai.components import alert, notify
from menuai.components.alert.const import (
    CONF_ALERT_MESSAGE,
    CONF_DATA,
    CONF_DONE_MESSAGE,
    CONF_NOTIFIERS,
...
@pytest.fixture
def mock_notifier(menuai: menuai) -> list[ServiceCall]:
    """Mock for notifier."""
    return async_mock_service(menuai, notify.DOMAIN, NOTIFIER)


async def test_setup(menuai: menuai) -> None:
    """Test setup method."""
    assert await async_setup_component(menuai, DOMAIN, TEST_CONFIG)
    assert menuai.states.get(ENTITY_ID).state == STATE_IDLE


async def test_fire(menuai: menuai, mock_notifier: list[ServiceCall]) -> None:
...
async def test_silence(menuai: menuai, mock_notifier: list[ServiceCall]) -> None:
    ...
    menuai.states.async_set("sensor.test", STATE_OFF)
    await menuai.async_block_till_done()
    assert menuai.states.get(ENTITY_ID).state == STATE_IDLE
    menuai.states.async_set("sensor.test", STATE_ON)
    await menuai.async_block_till_done()
...
async def test_silence_can_acknowledge_false(menuai: menuai) -> None:
...
async def test_reset(menuai: menuai, mock_notifier: list[ServiceCall]) -> None:
...
async def test_toggle(menuai: menuai, mock_notifier: list[ServiceCall]) -> None:
    """Test toggling alert."""
    assert await async_setup_component(menuai, DOMAIN, TEST_CONFIG)
    menuai.states.async_set("sensor.test", STATE_ON)
    await menuai.async_block_till_done()
...
async def test_notification_no_done_message(
    menuai: menuai, mock_notifier: list[ServiceCall]
) -> None:
    """Test notifications."""
    config = deepcopy(TEST_CONFIG)
...
async def test_skipfirst(menuai: menuai, mock_notifier: list[ServiceCall]) -> None:
...
async def test_done_message_state_tracker_reset_on_cancel(menuai: menuai) -> None:
    """Test that the done message is reset when canceled."""
    entity = alert.AlertEntity(menuai, *TEST_NOACK)
