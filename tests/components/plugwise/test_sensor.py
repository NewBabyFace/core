...
async def test_adam_climate_sensor_entities(
    ...
    state = menuai.states.get("sensor.adam_outdoor_temperature")
    assert state
    assert float(state.state) == 7.81

    state = menuai.states.get("sensor.cv_pomp_electricity_consumed")
    assert state
    assert float(state.state) == 35.6

    state = menuai.states.get("sensor.onoff_water_temperature")
    assert state
    assert float(state.state) == 70.0

    state = menuai.states.get("sensor.cv_pomp_electricity_consumed_interval")
    assert state
    assert float(state.state) == 7.37

    await async_update_entity(menuai, "sensor.zone_lisa_wk_battery")

    state = menuai.states.get("sensor.zone_lisa_wk_battery")
    assert state
...
async def test_adam_climate_sensor_entity_2(
    ...
    """Test creation of climate related sensor entities."""
    state = menuai.states.get("sensor.woonkamer_humidity")
    assert state
...
async def test_unique_id_migration_humidity(
    ...
    assert menuai.states.get("sensor.woonkamer_humidity") is not None
    assert menuai.states.get("sensor.woonkamer_battery") is not None
...
async def test_anna_as_smt_climate_sensor_entities(
    ...
    """Test creation of climate related sensor entities."""
    state = menuai.states.get("sensor.opentherm_outdoor_air_temperature")
    assert state
    assert float(state.state) == 3.0

    state = menuai.states.get("sensor.opentherm_water_temperature")
    assert state
    assert float(state.state) == 29.1

    state = menuai.states.get("sensor.opentherm_dhw_temperature")
    assert state
    assert float(state.state) == 46.3

    state = menuai.states.get("sensor.anna_illuminance")
    assert state
...
async def test_p1_dsmr_sensor_entities(
    ...
    state = menuai.states.get("sensor.p1_net_electricity_point")
    assert state
    assert int(state.state) == 486

    state = menuai.states.get("sensor.p1_electricity_consumed_off_peak_cumulative")
    assert state
    assert float(state.state) == 17643.423

    state = menuai.states.get("sensor.p1_electricity_produced_peak_point")
    assert state
    assert int(state.state) == 0

    state = menuai.states.get("sensor.p1_electricity_consumed_peak_cumulative")
    assert state
    assert float(state.state) == 13966.608

    state = menuai.states.get("sensor.p1_gas_consumed_cumulative")
    assert not state
...
async def test_p1_3ph_dsmr_sensor_entities(
    ...
    """Test creation of power related sensor entities."""
    state = menuai.states.get("sensor.p1_electricity_phase_one_consumed")
    assert state
    assert int(state.state) == 1763

    state = menuai.states.get("sensor.p1_electricity_phase_two_consumed")
    assert state
    assert int(state.state) == 1703

    state = menuai.states.get("sensor.p1_electricity_phase_three_consumed")
    assert state
    assert int(state.state) == 2080

    # Default disabled sensor test
    state = menuai.states.get("sensor.p1_voltage_phase_one")
    assert state
...
async def test_stretch_sensor_entities(
    ...
    """Test creation of power related sensor entities."""
    state = menuai.states.get("sensor.koelkast_92c4a_electricity_consumed")
    assert state
    assert float(state.state) == 50.5

    state = menuai.states.get("sensor.droger_52559_electricity_consumed_interval")
    assert state
