...
from menuai.components.hydrawise.const import DEFAULT_WATERING_TIME
...
async def test_manual_watering_services(
    ...
    state = menuai.states.get("switch.zone_one_manual_watering")
...
async def test_auto_watering_services(
    ...
    state = menuai.states.get("switch.zone_one_automatic_watering")
    assert state is not None
    assert state.state == "off"
    mock_pydrawise.reset_mock()
