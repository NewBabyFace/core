...
from menuai.components.nextbus.const import DOMAIN
from menuai.components.nextbus.coordinator import NextBusDataUpdateCoordinator
...
async def test_predictions(
    ...
    await assert_setup_sensor(menuai, CONFIG_BASIC)

    state = menuai.states.get(SENSOR_ID)
...
async def test_prediction_exceptions(
    ...
    await assert_setup_sensor(menuai, CONFIG_BASIC)
    coordinator: NextBusDataUpdateCoordinator = menuai.data[DOMAIN][VALID_COORDINATOR_KEY]
...
async def test_custom_name(
    ...
    await assert_setup_sensor(menuai, config)
    state = menuai.states.get("sensor.custom_name")
    assert state is not None
...
async def test_unload_entry(
    ...
    config_entry1 = await assert_setup_sensor(menuai, CONFIG_BASIC)
    await assert_setup_sensor(menuai, CONFIG_BASIC_2, route_title=ROUTE_TITLE_2)
    ...
    state = menuai.states.get(SENSOR_ID_2)
    assert state is not None
    ...
    await menuai.config_entries.async_unload(config_entry1.entry_id)
