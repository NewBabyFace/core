...
from menuai.components.youtube import DOMAIN
...
class MockYouTube:
    ...
    async def get_user_channels(self) -> AsyncGenerator[YouTubeChannel]:
        ...
        channels = json.loads(
            await async_load_fixture(self.menuai, self._channel_fixture, DOMAIN)
        )
    ...
    async def get_playlist_items(
        ...
        """Get channels."""
        channels = json.loads(
            await async_load_fixture(self.menuai, self._playlist_items_fixture, DOMAIN)
        )
        for item in channels["items"]:
    ...
    async def get_user_subscriptions(self) -> AsyncGenerator[YouTubeSubscription]:
        """Get channels for authenticated user."""
        channels = json.loads(
            await async_load_fixture(self.menuai, self._subscriptions_fixture, DOMAIN)
        )
        for item in channels["items"]:
