...
async def test_refresh_token_validity_failures(
    ...
    with patch(
        "menuai.helpers.config_entry_oauth2_flow.OAuth2Session.async_ensure_token_valid",
...
async def test_unique_id_migration(
    ...
    await menuai.async_block_till_done()
    new_entry = menuai.config_entries.async_get_entry(config_entry.entry_id)
...
async def test_unique_id_migration_failure(
    menuai: menuai, config_entry: MockConfigEntry, electrickiwi_api: AsyncMock
) -> None:
    """Test that the unique ID is migrated to the customer number."""
