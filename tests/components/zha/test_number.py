...
async def test_number(menuai: menuai, setup_zha, zigpy_device_mock) -> None:
    ...
    entity_id = find_entity_id(Platform.NUMBER, zha_device_proxy, menuai)
    assert entity_id is not None

    assert menuai.states.get(entity_id).state == "15.0"

    # test attributes
    assert menuai.states.get(entity_id).attributes.get("min") == 1.0
    assert menuai.states.get(entity_id).attributes.get("max") == 100.0
    assert menuai.states.get(entity_id).attributes.get("step") == 1.1
    assert menuai.states.get(entity_id).attributes.get("icon") == "mdi:percent"
    assert menuai.states.get(entity_id).attributes.get("unit_of_measurement") == "%"

    assert (
        menuai.states.get(entity_id).attributes.get("friendly_name")
        == "FakeManufacturer FakeModel PWM1"
    )

    # change value from device
    await send_attributes_report(menuai, cluster, {0x0055: 15})
    assert menuai.states.get(entity_id).state == "15.0"

    # update value from device
    await send_attributes_report(menuai, cluster, {0x0055: 20})
    assert menuai.states.get(entity_id).state == "20.0"
    ...
    assert menuai.states.get(entity_id).state == "40.0"
