...
async def test_fan(menuai: menuai, setup_zha, zigpy_device_mock) -> None:
    ...
    entity_id = find_entity_id(Platform.FAN, zha_device_proxy, menuai)
    ...
    # turn off at fan
    await send_attributes_report(
        menuai, cluster, {hvac.Fan.AttributeDefs.fan_mode.id: hvac.FanMode.Off}
    )
    assert menuai.states.get(entity_id).state == STATE_OFF

    # turn on from HA
    cluster.write_attributes.reset_mock()
    await async_turn_on(menuai, entity_id)
    ...
    # turn off from HA
    cluster.write_attributes.reset_mock()
    await async_turn_off(menuai, entity_id)
    assert cluster.write_attributes.mock_calls == [
    ...
    cluster.write_attributes.reset_mock()
    await async_set_percentage(menuai, entity_id, percentage=100)
    assert cluster.write_attributes.mock_calls == [
    ...
    cluster.write_attributes.reset_mock()
    await async_set_preset_mode(menuai, entity_id, preset_mode=PRESET_MODE_ON)
    assert cluster.write_attributes.mock_calls == [
    ...
    with pytest.raises(NotValidPresetModeError) as exc:
        await async_set_preset_mode(
            menuai, entity_id, preset_mode="invalid does not exist"
        )
    assert exc.value.translation_key == "not_valid_preset_mode"
    assert len(cluster.write_attributes.mock_calls) == 0


async def async_turn_on(menuai: menuai, entity_id, percentage=None):
    """Turn fan on."""
    data = {
        key: value
    ...
    await menuai.services.async_call(Platform.FAN, SERVICE_TURN_ON, data, blocking=True)


async def async_turn_off(menuai: menuai, entity_id):
    """Turn fan off."""
    data = {ATTR_ENTITY_ID: entity_id} if entity_id else {}

    await menuai.services.async_call(Platform.FAN, SERVICE_TURN_OFF, data, blocking=True)


async def async_set_percentage(menuai: menuai, entity_id, percentage=None):
...
async def async_set_preset_mode(menuai: menuai, entity_id, preset_mode=None):
