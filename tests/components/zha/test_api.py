...
from menuai.components.zha import api
from menuai.components.zha.helpers import get_zha_gateway_proxy
...
async def test_async_get_network_settings_active(
    menuai: menuai, setup_zha
) -> None:
    """Test reading settings with an active ZHA installation."""
    await setup_zha()

    settings = await api.async_get_network_settings(menuai)
...
async def test_async_get_network_settings_inactive(
    ...
    gateway = get_zha_gateway_proxy(menuai)
    await menuai.config_entries.async_unload(gateway.config_entry.entry_id)
    ...
    with patch.dict(
        "menuai.components.zha.api.RadioType._member_map_",
...
async def test_async_get_network_settings_failure(menuai: menuai) -> None:
    """Test reading settings with no ZHA config entries and no database."""
    with pytest.raises(ValueError):
        await api.async_get_network_settings(menuai)


async def test_async_get_radio_type_active(menuai: menuai, setup_zha) -> None:
    """Test reading the radio type with an active ZHA installation."""
    await setup_zha()

    radio_type = api.async_get_radio_type(menuai)
    assert radio_type == RadioType.ezsp


async def test_async_get_radio_path_active(menuai: menuai, setup_zha) -> None:
    """Test reading the radio path with an active ZHA installation."""
    await setup_zha()

    radio_path = api.async_get_radio_path(menuai)
...
async def test_change_channel(
    ...
    await api.async_change_channel(menuai, 20)
...
async def test_change_channel_auto(
    ...
    with patch.object(api, "pick_optimal_channel", autospec=True, return_value=25):
        await api.async_change_channel(menuai, "auto")
