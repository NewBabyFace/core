...
from menuai.const import STATE_HOME, STATE_NOT_HOME, Platform
...
async def test_device_tracker(
    ...
    entity_id = find_entity_id(Platform.DEVICE_TRACKER, zha_device_proxy, menuai)
    cluster = zigpy_device.endpoints[1].power
    assert entity_id is not None

    # test that the state has changed from unavailable to not home
    assert menuai.states.get(entity_id).state == STATE_NOT_HOME

    # turn state flip
    await send_attributes_report(
        menuai, cluster, {0x0000: 0, 0x0020: 23, 0x0021: 200, 0x0001: 2}
    )
    ...
    assert menuai.states.get(entity_id).state == STATE_HOME

    entity = menuai.data[Platform.DEVICE_TRACKER].get_entity(entity_id)
