...
from menuai.components.zha import config_flow, radio_manager
...
from menuai.components.zha.radio_manager import ProbeResult
...
def mock_multipan_platform():
    ...
    with (
        patch(
            "menuai.components.zha.silabs_multiprotocol.async_get_channel",
            return_value=None,
        ),
        patch(
            "menuai.components.zha.silabs_multiprotocol.async_using_multipan",
            return_value=False,
        ),
    ):
        yield
...
async def test_zeroconf_discovery(
    ...
    result_confirm = await menuai.config_entries.flow.async_configure(
    ...
    result_form = await menuai.config_entries.flow.async_configure(
...
async def test_legacy_zeroconf_discovery_zigate(
    setup_entry_mock, menuai: menuai
) -> None:
    ...
    result_confirm_deprecated = await menuai.config_entries.flow.async_configure(
        result_init["flow_id"], user_input={}
    )
...
async def test_zeroconf_discovery_bad_payload(menuai: menuai) -> None:
...
@patch("menuai.components.zha.async_setup_entry", AsyncMock(return_value=True))
@patch(f"zigpy_znp.{PROBE_FUNCTION_PATH}", AsyncMock(return_value=True))
async def test_legacy_zeroconf_discovery_ip_change_ignored(menuai: menuai) -> None:
...
async def test_legacy_zeroconf_discovery_confirm_final_abort_if_entries(
    ...
    with patch(
        "menuai.config_entries.ConfigFlow._async_current_entries",
...
async def test_discovery_via_usb(menuai: menuai) -> None:
    ...
    with patch("menuai.components.zha.async_setup_entry", return_value=True):
...
async def test_zigate_discovery_via_usb(probe_mock, menuai: menuai) -> None:
...
async def test_discovery_via_usb_no_radio(menuai: menuai) -> None:
...
@patch("menuai.components.zha.async_setup_entry", AsyncMock(return_value=True))
async def test_discovery_via_usb_path_does_not_change(menuai: menuai) -> None:
...
@patch(f"zigpy_znp.{PROBE_FUNCTION_PATH}", AsyncMock(return_value=True))
async def test_discovery_via_usb_deconz_already_discovered(menuai: menuai) -> None:
...
@patch(f"zigpy_znp.{PROBE_FUNCTION_PATH}", AsyncMock(return_value=True))
async def test_discovery_via_usb_deconz_already_setup(menuai: menuai) -> None:
    """Test usb flow -- deconz setup."""
    MockConfigEntry(domain="deconz", data={}).add_to_menuai(menuai)
    await menuai.async_block_till_done()
    discovery_info = UsbServiceInfo(
        device="/dev/ttyZIGBEE",
        pid="AAAA",
...
@patch(f"zigpy_znp.{PROBE_FUNCTION_PATH}", AsyncMock(return_value=True))
async def test_discovery_via_usb_deconz_ignored(menuai: menuai) -> None:
...
@patch(f"zigpy_znp.{PROBE_FUNCTION_PATH}", AsyncMock(return_value=True))
async def test_discovery_via_usb_zha_ignored_updates(menuai: menuai) -> None:
...
@patch("menuai.components.zha.async_setup_entry", AsyncMock(return_value=True))
@patch(f"zigpy_znp.{PROBE_FUNCTION_PATH}", AsyncMock(return_value=True))
async def test_legacy_zeroconf_discovery_already_setup(menuai: menuai) -> None:
    """Test zeroconf flow -- radio detected."""
    service_info = ZeroconfServiceInfo(
        ip_address=ip_address("192.168.1.200"),
        ip_addresses=[ip_address("192.168.1.200")],
...
@patch("serial.tools.list_ports.comports", MagicMock(return_value=[com_port()]))
async def test_user_flow_not_detected(menuai: menuai) -> None:
...
@patch("serial.tools.list_ports.comports", MagicMock(return_value=[com_port()]))
async def test_user_flow_show_form(menuai: menuai) -> None:
    """Test user step form."""
    result = await menuai.config_entries.flow.async_init(
        DOMAIN,
        context={CONF_SOURCE: SOURCE_USER},
...
async def test_user_flow_show_manual(menuai: menuai) -> None:
...
async def test_user_flow_manual(menuai: menuai) -> None:
...
async def test_pick_radio_flow(menuai: menuai, radio_type) -> None:
    """Test radio picker."""
...
async def test_user_flow_existing_config_entry(menuai: menuai) -> None:
...
@patch(f"bellows.{PROBE_FUNCTION_PATH}", return_value=False)
async def test_user_port_config_fail(probe_mock, menuai: menuai) -> None:
...
async def test_user_port_config(probe_mock, menuai: menuai) -> None:
    """Test port config."""
...
@patch("menuai.components.zha.async_setup_entry", AsyncMock(return_value=True))
async def test_hardware(onboarded, menuai: menuai) -> None:
    """Test hardware flow."""
    data = {
        "name": "Yellow",
        "radio_type": "efr32",
    ...
    with patch(
        "menuai.components.onboarding.async_is_onboarded", return_value=onboarded
    ):
        result1 = await menuai.config_entries.flow.async_init(
...
async def test_hardware_already_setup(menuai: menuai) -> None:
...
async def test_hardware_invalid_data(menuai: menuai, data) -> None:
...
def pick_radio(
    ...
    p2 = patch("menuai.components.zha.async_setup_entry")
...
async def test_strategy_no_network_settings(
    pick_radio: RadioPicker, mock_app: AsyncMock, menuai: menuai
) -> None:
...
async def test_onboarding_auto_formation_new_hardware(
    mock_app: AsyncMock, menuai: menuai
) -> None:
...
@patch("menuai.components.zha.config_flow.process_uploaded_file")
...
@patch("menuai.components.zha.radio_manager._allow_overwrite_ezsp_ieee")
async def test_formation_strategy_restore_manual_backup_non_ezsp(
    ...
    with patch(
        "menuai.components.zha.config_flow.ZhaConfigFlowHandler._parse_uploaded_backup",
...
@patch(
    "menuai.components.zha.config_flow._format_backup_choice",
    lambda s, **kwargs: "choice:" + repr(s),
)
@patch("menuai.components.zha.async_setup_entry", AsyncMock(return_value=True))
async def test_formation_strategy_restore_automatic_backup_ezsp(
    pick_radio: RadioPicker, mock_app: AsyncMock, make_backup, menuai: menuai
) -> None:
...
async def test_formation_strategy_restore_automatic_backup_non_ezsp(
    ...
    with patch(
        "menuai.config_entries.ConfigFlow.show_advanced_options",
...
@patch("menuai.components.zha.async_setup_entry", return_value=True)
async def test_options_flow_defaults(
    async_setup_entry, async_unload_effect, menuai: menuai
) -> None:
    ...
    # ZHA gets unloaded
    with patch(
        "menuai.config_entries.ConfigEntries.async_unload",
        side_effect=[async_unload_effect],
    ) as mock_async_unload:
        result1 = await menuai.config_entries.options.async_configure(
            flow["flow_id"], user_input={}
        )
...
@patch("menuai.components.zha.async_setup_entry", AsyncMock(return_value=True))
async def test_options_flow_defaults_socket(menuai: menuai) -> None:
    ...
    # ZHA gets unloaded
    with patch(
        "menuai.config_entries.ConfigEntries.async_unload", return_value=True
    ):
        result1 = await menuai.config_entries.options.async_configure(
            flow["flow_id"], user_input={}
        )
...
async def test_options_flow_restarts_running_zha_if_cancelled(
    async_setup_entry, menuai: menuai
) -> None:
    ...
    # Abort the flow
    menuai.config_entries.options.async_abort(result2["flow_id"])
    await menuai.async_block_till_done()

    # ZHA was set up once more
    async_setup_entry.assert_called_once_with(menuai, entry)


@patch("serial.tools.list_ports.comports", MagicMock(return_value=[com_port()]))
@patch("menuai.components.zha.async_setup_entry", AsyncMock(return_value=True))
async def test_options_flow_migration_reset_old_adapter(
    menuai: menuai, mock_app
) -> None:
...
async def test_config_flow_port_yellow_port_name(
    ...
    with (
        patch("menuai.components.zha.config_flow.yellow_hardware.async_info"),
...
async def test_config_flow_ports_no_menuaiio(menuai: menuai) -> None:
    """Test config flow serial port name when this is not a menuaiio install."""

    with (
        patch("menuai.components.zha.config_flow.is_menuaiio", return_value=False),
        patch("serial.tools.list_ports.comports", MagicMock(return_value=[])),
    ):
        ports = await config_flow.list_serial_ports(menuai)

    assert ports == []


async def test_config_flow_port_multiprotocol_port_name(menuai: menuai) -> None:
    """Test config flow serial port name for multiprotocol add-on."""

    with (
        patch("menuai.components.zha.config_flow.is_menuaiio", return_value=True),
        patch(
            "menuai.components.menuaiio.addon_manager.AddonManager.async_get_addon_info"
...
async def test_config_flow_port_no_multiprotocol(menuai: menuai) -> None:
...
@patch("serial.tools.list_ports.comports", MagicMock(return_value=[com_port()]))
async def test_probe_wrong_firmware_installed(menuai: menuai) -> None:
...
async def test_discovery_wrong_firmware_installed(menuai: menuai) -> None:
    """Test auto-probing failing because the wrong firmware is installed."""
...
async def test_migration_ti_cc_to_znp(
    old_type: str, new_type: str, menuai: menuai, config_entry: MockConfigEntry
) -> None:
