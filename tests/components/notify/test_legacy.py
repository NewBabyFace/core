...
class NotificationService(notify.BaseNotificationService):
    ...
    def __init__(
        ...
        async def _async_make_reloadable(menuai: menuai) -> None:
            """Initialize the reload service."""
            await async_setup_reload_service(menuai, name, [notify.DOMAIN])

        self.menuai = menuai
        self.target_list = target_list or {"a": 1, "b": 2}
        menuai.async_create_task(_async_make_reloadable(menuai))
...
def mock_notify_platform(
    ...
    mock_platform(menuai, f"{integration}.notify", loaded_platform)

    return loaded_platform


async def help_setup_notify(
    menuai: menuai, tmp_path: Path, targets: dict[str, None] | None = None
) -> MagicMock:
    ...
    mock_notify_platform(menuai, tmp_path, "test", async_get_service=async_get_service)
    # Setup the platform
    await async_setup_component(menuai, "notify", {"notify": [{"platform": "test"}]})
...
async def test_same_targets(menuai: menuai) -> None:
    """Test not changing the targets in a legacy notify service."""
    test = NotificationService(menuai)
    await test.async_setup(menuai, "notify", "test")
...
async def test_change_targets(menuai: menuai) -> None:
...
async def test_add_targets(menuai: menuai) -> None:
...
async def test_remove_targets(menuai: menuai) -> None:
...
async def test_invalid_platform(
    ...
    mock_notify_platform(menuai, tmp_path, "testnotify1")
    # Setup the platform
    await async_setup_component(
        menuai, "notify", {"notify": [{"platform": "testnotify1"}]}
    )
    await menuai.async_block_till_done()
    assert "Invalid notify platform" in caplog.text
    caplog.clear()
    # Setup the second testnotify2 platform dynamically
    mock_notify_platform(menuai, tmp_path, "testnotify2")
    await async_load_platform(
        menuai,
        "notify",
        "testnotify2",
        {},
        menuai_config={"notify": [{"platform": "testnotify2"}]},
    )
    await menuai.async_block_till_done()
    assert "Invalid notify platform" in caplog.text
...
async def test_invalid_service(
    ...
    mock_notify_platform(menuai, tmp_path, "testnotify", get_service=get_service)
    # Setup the second testnotify2 platform dynamically
    await async_load_platform(
        menuai,
        "notify",
        "testnotify",
        {},
        menuai_config={"notify": [{"platform": "testnotify"}]},
    )
    await menuai.async_block_till_done()
    ...
    await async_load_platform(
        ...
        menuai_config={"notify": [{"platform": "testnotifyinvalid"}]},
    )
    await menuai.async_block_till_done()
...
async def test_platform_setup_with_error(
    ...
    mock_notify_platform(
        menuai, tmp_path, "testnotify", async_get_service=async_get_service
    )
    # Setup the second testnotify2 platform dynamically
    await async_load_platform(
        menuai,
...
async def test_reload_with_notify_builtin_platform_reload(
    ...
    async def async_get_service(
        ...
        """Get notify service for mocked platform."""
        targetlist = {"a": 1, "b": 2}
        return NotificationService(menuai, targetlist, "testnotify")
    ...
    await notify.async_reload(menuai, "testnotify")

    # Setup the platform
    await async_setup_component(
        menuai, "notify", {"notify": [{"platform": "testnotify"}]}
    )
    await menuai.async_block_till_done()
    assert menuai.services.has_service(notify.DOMAIN, "testnotify_a")
    assert menuai.services.has_service(notify.DOMAIN, "testnotify_b")
...
async def test_setup_platform_and_reload(menuai: menuai, tmp_path: Path) -> None:
    ...
    async def async_get_service2(
        ...
        get_service_called(config, discovery_info)
        targetlist = {"c": 3, "d": 4}
        return NotificationService(menuai, targetlist, "testnotify2")
    ...
    mock_notify_platform(
        menuai, tmp_path, "testnotify2", async_get_service=async_get_service2
    )
    ...
    await menuai.async_block_till_done()
    assert menuai.services.has_service("testnotify", SERVICE_RELOAD)
    assert menuai.services.has_service(notify.DOMAIN, "testnotify_a")
    assert menuai.services.has_service(notify.DOMAIN, "testnotify_b")
    ...
    await menuai.async_block_till_done()
    assert menuai.services.has_service("testnotify2", SERVICE_RELOAD)
    assert menuai.services.has_service(notify.DOMAIN, "testnotify2_c")
    assert menuai.services.has_service(notify.DOMAIN, "testnotify2_d")
    assert get_service_called.call_count == 1
    ...
    with patch.object(menuai_config, "YAML_CONFIG_FILE", new_yaml_config_file):
    ...
    assert not menuai.services.has_service(notify.DOMAIN, "testnotify2_c")
    assert not menuai.services.has_service(notify.DOMAIN, "testnotify2_d")
...
async def test_setup_platform_before_notify_setup(
    ...
    load_coro = async_load_platform(
        menuai, Platform.NOTIFY, "testnotify2", {}, menuai_config=menuai_config
    )

    # Setup the testnotify platform
    setup_coro = async_setup_component(menuai, "notify", menuai_config)
...
async def test_sending_none_message(menuai: menuai, tmp_path: Path) -> None:
    """Test send with None as message."""
    send_message_mock = await help_setup_notify(menuai, tmp_path)
...
async def test_method_forwards_correct_data(
    ...
    await menuai.services.async_call(notify.DOMAIN, notify.SERVICE_NOTIFY, data)
    await menuai.async_block_till_done()
...
async def test_targets_are_services(menuai: menuai, tmp_path: Path) -> None:
    """Test that all targets are exposed as individual services."""
    await help_setup_notify(menuai, tmp_path, targets={"a": 1, "b": 2})
    assert menuai.services.has_service("notify", "notify") is not None
    assert menuai.services.has_service("notify", "test_a") is not None
    assert menuai.services.has_service("notify", "test_b") is not None


async def test_messages_to_targets_route(menuai: menuai, tmp_path: Path) -> None:
    """Test message routing to specific target services."""
    send_message_mock = await help_setup_notify(
        menuai, tmp_path, targets={"target_name": "test target id"}
    )
