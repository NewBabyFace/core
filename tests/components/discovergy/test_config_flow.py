...
async def test_form(menuai: menuai, discovergy: AsyncMock) -> None:
    ...
    with patch(
        "menuai.components.discovergy.async_setup_entry",
        return_value=True,
    ) as mock_setup_entry:
...
async def test_form_fail(
    menuai: menuai, discovergy: AsyncMock, error: Exception, message: str
) -> None:
