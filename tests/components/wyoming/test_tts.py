...
from menuai.components import tts, wyoming
from menuai.core import menuai
from menuai.exceptions import menuaiError
from menuai.helpers.entity_component import DATA_INSTANCES

from . import MockAsyncTcpClient


async def test_support(menuai: menuai, init_wyoming_tts) -> None:
    """Test supported properties."""
    state = menuai.states.get("tts.test_tts")
    assert state is not None

    entity = menuai.data[DATA_INSTANCES]["tts"].get_entity("tts.test_tts")
...
async def test_get_tts_audio(
    menuai: menuai, init_wyoming_tts, snapshot: SnapshotAssertion
) -> None:
    """Test get audio."""
    audio = bytes(100)
    audio_events = [
    ...
    with patch(
        "menuai.components.wyoming.tts.AsyncTcpClient",
...
async def test_get_tts_audio_connection_lost(
    menuai: menuai, init_wyoming_tts
) -> None:
    """Test streaming audio and losing connection."""
    stream = tts.async_create_stream(menuai, "tts.test_tts", "en-US")
    with patch(
        "menuai.components.wyoming.tts.AsyncTcpClient",
...
async def test_get_tts_audio_audio_oserror(
    ...
    with (
        ...
        await tts.async_get_media_source_audio(
            menuai,
            tts.generate_media_source_id(
                menuai, "Hello world", "tts.test_tts", menuai.config.language
            ),
        )
