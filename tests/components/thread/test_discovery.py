...
from . import (
    ...
    ROUTER_DISCOVERY_menuai_BAD_DATA,
    ROUTER_DISCOVERY_menuai_BAD_STATE_BITMAP,
    ROUTER_DISCOVERY_menuai_MISSING_DATA,
    ROUTER_DISCOVERY_menuai_MISSING_MANDATORY_DATA_XA,
    ROUTER_DISCOVERY_menuai_MISSING_MANDATORY_DATA_XP,
    ROUTER_DISCOVERY_menuai_NO_ACTIVE_TIMESTAMP,
    ROUTER_DISCOVERY_menuai_NO_STATE_BITMAP,
    ROUTER_DISCOVERY_menuai_STATE_BITMAP_NOT_ACTIVE,
)
...
async def test_discover_routers(
    ...
    listener.remove_service(None, ROUTER_DISCOVERY_menuai["type_"], "unknown")
    await menuai.async_block_till_done()
    assert len(discovered) == 2
    assert len(removed) == 1
...
@pytest.mark.parametrize(
    ("data", "unconfigured"),
    [
        (ROUTER_DISCOVERY_menuai_NO_ACTIVE_TIMESTAMP, True),
        (ROUTER_DISCOVERY_menuai_BAD_STATE_BITMAP, None),
        (ROUTER_DISCOVERY_menuai_NO_STATE_BITMAP, None),
        (ROUTER_DISCOVERY_menuai_STATE_BITMAP_NOT_ACTIVE, True),
    ],
)
async def test_discover_routers_unconfigured(
    menuai: menuai, mock_async_zeroconf: MagicMock, data, unconfigured
) -> None:
    ...
    thread_disovery = discovery.ThreadRouterDiscovery(
        menuai, router_discovered_removed, router_discovered_removed
    )
    await thread_disovery.async_start()
    listener: discovery.ThreadRouterDiscovery.ThreadServiceListener = (
        mock_async_zeroconf.async_add_service_listener.mock_calls[0][1][1]
...
@pytest.mark.parametrize(
    "data", [ROUTER_DISCOVERY_menuai_BAD_DATA, ROUTER_DISCOVERY_menuai_MISSING_DATA]
)
async def test_discover_routers_bad_or_missing_optional_data(
    menuai: menuai, mock_async_zeroconf: MagicMock, data
) -> None:
...
async def test_discover_routers_bad_or_missing_mandatory_data(
    menuai: menuai, mock_async_zeroconf: MagicMock, service
) -> None:
