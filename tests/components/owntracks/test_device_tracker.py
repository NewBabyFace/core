...
from menuai.const import STATE_NOT_HOME
...
async def setup_comp(
    ...
    menuai.states.async_set("zone.inner", "zoning", INNER_ZONE)

    menuai.states.async_set("zone.inner_2", "zoning", INNER_ZONE)

    menuai.states.async_set("zone.outer", "zoning", OUTER_ZONE)


async def setup_owntracks(
    menuai: menuai, config: dict[str, Any], ctx_cls=owntracks.OwnTracksContext
) -> None:
    """Set up OwnTracks."""
    MockConfigEntry(
    ...
    with patch.object(owntracks, "OwnTracksContext", ctx_cls):
        assert await async_setup_component(menuai, "owntracks", {"owntracks": config})
        await menuai.async_block_till_done()


@pytest.fixture
async def context(menuai: menuai, setup_comp: None) -> OwnTracksContextFactory:
...
async def send_message(
    menuai: menuai, topic: str, message: dict[str, Any], corrupt: bool = False
) -> None:
    ...
    async_fire_mqtt_message(menuai, topic, mod_message)
    await menuai.async_block_till_done()
    await menuai.async_block_till_done()


def assert_location_state(menuai: menuai, location: str) -> None:
    """Test the assertion of a location state."""
    state = menuai.states.get(DEVICE_TRACKER_STATE)
    assert state.state == location


def assert_location_latitude(menuai: menuai, latitude: float) -> None:
    """Test the assertion of a location latitude."""
    state = menuai.states.get(DEVICE_TRACKER_STATE)
    assert state.attributes.get("latitude") == latitude


def assert_location_longitude(menuai: menuai, longitude: float) -> None:
    """Test the assertion of a location longitude."""
    state = menuai.states.get(DEVICE_TRACKER_STATE)
    assert state.attributes.get("longitude") == longitude


def assert_location_accuracy(menuai: menuai, accuracy: float) -> None:
    """Test the assertion of a location accuracy."""
    state = menuai.states.get(DEVICE_TRACKER_STATE)
    assert state.attributes.get("gps_accuracy") == accuracy


def assert_location_source_type(menuai: menuai, source_type: str) -> None:
...
def assert_mobile_tracker_state(
    menuai: menuai, location: str, beacon: str = IBEACON_DEVICE
) -> None:
    """Test the assertion of a mobile beacon tracker state."""
    dev_id = MOBILE_BEACON_FMT.format(beacon)
    state = menuai.states.get(dev_id)
    assert state.state == location


def assert_mobile_tracker_latitude(
    menuai: menuai, latitude: float, beacon: str = IBEACON_DEVICE
) -> None:
...
def assert_mobile_tracker_accuracy(
    menuai: menuai, accuracy: int, beacon: str = IBEACON_DEVICE
) -> None:
...
async def test_location_invalid_devid(menuai: menuai) -> None:
    """Test the update of a location."""
    await send_message(menuai, "owntracks/paulus/nexus-5x", LOCATION_MESSAGE)
    state = menuai.states.get("device_tracker.paulus_nexus_5x")
    assert state.state == "outer"


@pytest.mark.usefixtures("context")
async def test_location_update(menuai: menuai) -> None:
    """Test the update of a location."""
    await send_message(menuai, LOCATION_TOPIC, LOCATION_MESSAGE)

    assert_location_source_type(menuai, "gps")
    assert_location_latitude(menuai, LOCATION_MESSAGE["lat"])
    assert_location_accuracy(menuai, LOCATION_MESSAGE["acc"])
    assert_location_state(menuai, "outer")


@pytest.mark.usefixtures("context")
async def test_location_update_no_t_key(menuai: menuai) -> None:
    """Test the update of a location when message does not contain 't'."""
    message = LOCATION_MESSAGE.copy()
    message.pop("t")
    await send_message(menuai, LOCATION_TOPIC, message)
...
@pytest.mark.usefixtures("context")
async def test_location_inaccurate_gps(menuai: menuai) -> None:
    """Test the location for inaccurate GPS information."""
    await send_message(menuai, LOCATION_TOPIC, LOCATION_MESSAGE)
    await send_message(menuai, LOCATION_TOPIC, LOCATION_MESSAGE_INACCURATE)

    # Ignored inaccurate GPS. Location remains at previous.
    assert_location_latitude(menuai, LOCATION_MESSAGE["lat"])
    assert_location_longitude(menuai, LOCATION_MESSAGE["lon"])


@pytest.mark.usefixtures("context")
async def test_location_zero_accuracy_gps(menuai: menuai) -> None:
    """Ignore the location for zero accuracy GPS information."""
    await send_message(menuai, LOCATION_TOPIC, LOCATION_MESSAGE)
    await send_message(menuai, LOCATION_TOPIC, LOCATION_MESSAGE_ZERO_ACCURACY)
...
async def test_event_gps_entry_exit(
    menuai: menuai, context: OwnTracksContextFactory
) -> None:
    """Test the entry event."""
    # Entering the owntracks circular region named "inner"
    await send_message(menuai, EVENT_TOPIC, REGION_GPS_ENTER_MESSAGE)

    # Enter uses the zone's gps co-ords
    assert_location_latitude(menuai, INNER_ZONE["latitude"])
    assert_location_accuracy(menuai, INNER_ZONE["radius"])
    assert_location_state(menuai, "inner")
    ...
    await send_message(menuai, EVENT_TOPIC, REGION_GPS_LEAVE_MESSAGE)

    # Exit switches back to GPS
    assert_location_latitude(menuai, REGION_GPS_LEAVE_MESSAGE["lat"])
    assert_location_accuracy(menuai, REGION_GPS_LEAVE_MESSAGE["acc"])
    assert_location_state(menuai, "outer")
...
async def test_event_gps_with_spaces(
    ...
    await send_message(menuai, EVENT_TOPIC, message)
    assert_location_state(menuai, "inner 2")
...
@pytest.mark.usefixtures("context")
async def test_event_gps_entry_inaccurate(menuai: menuai) -> None:
    """Test the event for inaccurate entry."""
    # Set location to the outer zone.
    await send_message(menuai, LOCATION_TOPIC, LOCATION_MESSAGE)

    await send_message(menuai, EVENT_TOPIC, REGION_GPS_ENTER_MESSAGE_INACCURATE)
...
async def test_event_gps_entry_exit_inaccurate(
    ...
    await send_message(menuai, EVENT_TOPIC, REGION_GPS_LEAVE_MESSAGE_INACCURATE)
...
async def test_event_gps_entry_exit_zero_accuracy(
    ...
    await send_message(menuai, EVENT_TOPIC, REGION_GPS_ENTER_MESSAGE_ZERO)
    ...
    await send_message(menuai, EVENT_TOPIC, REGION_GPS_LEAVE_MESSAGE_ZERO)
...
@pytest.mark.usefixtures("context")
async def test_event_gps_exit_outside_zone_sets_away(menuai: menuai) -> None:
    ...
    assert_location_state(menuai, STATE_NOT_HOME)


@pytest.mark.usefixtures("context")
async def test_event_gps_entry_exit_right_order(menuai: menuai) -> None:
    ...
    await send_message(menuai, EVENT_TOPIC, message)
    assert_location_state(menuai, "inner_2")
...
@pytest.mark.usefixtures("context")
async def test_event_gps_entry_exit_wrong_order(menuai: menuai) -> None:
...
@pytest.mark.usefixtures("context")
async def test_event_gps_entry_unknown_zone(menuai: menuai) -> None:
    """Test the event for unknown zone."""
    # Just treat as location update
    message = build_message({"desc": "unknown"}, REGION_GPS_ENTER_MESSAGE)
    await send_message(menuai, EVENT_TOPIC, message)
    assert_location_latitude(menuai, REGION_GPS_ENTER_MESSAGE["lat"])
    assert_location_state(menuai, "inner")


@pytest.mark.usefixtures("context")
async def test_event_gps_exit_unknown_zone(menuai: menuai) -> None:
    """Test the event for unknown zone."""
    # Just treat as location update
...
@pytest.mark.usefixtures("context")
async def test_event_entry_zone_loading_dash(menuai: menuai) -> None:
...
async def test_events_only_on(
    ...
    await send_message(menuai, LOCATION_TOPIC, LOCATION_MESSAGE_NOT_HOME)
    assert_location_state(menuai, STATE_NOT_HOME)

    context().events_only = True

    # Enter and Leave messages
    await send_message(menuai, EVENT_TOPIC, REGION_GPS_ENTER_MESSAGE_OUTER)
    assert_location_state(menuai, "outer")
    await send_message(menuai, EVENT_TOPIC, REGION_GPS_LEAVE_MESSAGE_OUTER)
    assert_location_state(menuai, STATE_NOT_HOME)
...
@pytest.mark.usefixtures("context")
async def test_event_source_type_entry_exit(menuai: menuai) -> None:
    ...
    await send_message(
        menuai, EVENT_TOPIC, build_message({"acc": 1}, REGION_BEACON_ENTER_MESSAGE)
    )

    # We should be able to enter a beacon zone even inside a gps zone
    assert_location_source_type(menuai, "bluetooth_le")
    ...
    # owntracks shouldn't send beacon events with acc = 0
    await send_message(
        menuai, EVENT_TOPIC, build_message({"acc": 1}, REGION_BEACON_LEAVE_MESSAGE)
    )
...
async def test_event_region_entry_exit(
    ...
    await send_message(menuai, EVENT_TOPIC, REGION_BEACON_ENTER_MESSAGE)
    ...
    await send_message(menuai, EVENT_TOPIC, REGION_BEACON_LEAVE_MESSAGE)
...
@pytest.mark.usefixtures("context")
async def test_event_region_entry_exit_right_order(menuai: menuai) -> None:
    """Test the event for ordering."""
    # Enter inner zone
    # Set location to the outer zone.
    await send_message(menuai, LOCATION_TOPIC, LOCATION_MESSAGE)
...
@pytest.mark.usefixtures("context")
async def test_event_region_entry_exit_wrong_order(menuai: menuai) -> None:
    """Test the event for wrong order."""
    # Enter inner zone
    await send_message(menuai, EVENT_TOPIC, REGION_BEACON_ENTER_MESSAGE)
    assert_location_state(menuai, "inner")
...
@pytest.mark.usefixtures("context")
async def test_event_beacon_unknown_zone_no_location(menuai: menuai) -> None:
    """Test the event for unknown zone."""
    ...
    menuai.states.async_set(DEVICE_TRACKER_STATE, None)
    ...
    assert_location_state(menuai, "None")

    # We have had no location yet, so the beacon status
    # set to unknown.
    assert_mobile_tracker_state(menuai, "unknown", "unknown")


@pytest.mark.usefixtures("context")
async def test_event_beacon_unknown_zone(menuai: menuai) -> None:
    """Test the event for unknown zone."""
    # A beacon which does not match a HA zone is the
    # definition of a mobile beacon. In this case, "unknown"
    # will be turned into device_tracker.beacon_unknown and
    ...
    assert_location_state(menuai, "outer")
    assert_mobile_tracker_state(menuai, "outer", "unknown")


@pytest.mark.usefixtures("context")
async def test_event_beacon_entry_zone_loading_dash(menuai: menuai) -> None:
...
@pytest.mark.usefixtures("context")
async def test_mobile_enter_move_beacon(menuai: menuai) -> None:
    ...
    await send_message(menuai, EVENT_TOPIC, MOBILE_BEACON_ENTER_EVENT_MESSAGE)

    assert_mobile_tracker_latitude(menuai, LOCATION_MESSAGE["lat"])
    assert_mobile_tracker_state(menuai, "outer")
    ...
    assert_location_state(menuai, STATE_NOT_HOME)
    assert_mobile_tracker_state(menuai, STATE_NOT_HOME)

    not_home_lat = LOCATION_MESSAGE_NOT_HOME["lat"]
    assert_location_latitude(menuai, not_home_lat)
    assert_mobile_tracker_latitude(menuai, not_home_lat)


@pytest.mark.usefixtures("context")
async def test_mobile_enter_exit_region_beacon(menuai: menuai) -> None:
    ...
    await send_message(menuai, EVENT_TOPIC, MOBILE_BEACON_ENTER_EVENT_MESSAGE)
    assert_mobile_tracker_latitude(menuai, OUTER_ZONE["latitude"])
    assert_mobile_tracker_state(menuai, "outer")

    # GPS enter message should move beacon
    await send_message(menuai, EVENT_TOPIC, REGION_GPS_ENTER_MESSAGE)

    assert_mobile_tracker_latitude(menuai, INNER_ZONE["latitude"])
    assert_mobile_tracker_state(menuai, REGION_GPS_ENTER_MESSAGE["desc"])

    # Exit inner zone to outer zone should move beacon to
    # center of outer zone
    await send_message(menuai, EVENT_TOPIC, REGION_GPS_LEAVE_MESSAGE)
    assert_mobile_tracker_latitude(menuai, REGION_GPS_LEAVE_MESSAGE["lat"])
    assert_mobile_tracker_state(menuai, "outer")


@pytest.mark.usefixtures("context")
async def test_mobile_exit_move_beacon(menuai: menuai) -> None:
    ...
    await send_message(menuai, EVENT_TOPIC, MOBILE_BEACON_LEAVE_EVENT_MESSAGE)
...
async def test_mobile_multiple_async_enter_exit(
    ...
    for _ in range(20):
        async_fire_mqtt_message(
            menuai, EVENT_TOPIC, json.dumps(MOBILE_BEACON_ENTER_EVENT_MESSAGE)
        )
        async_fire_mqtt_message(
            menuai, EVENT_TOPIC, json.dumps(MOBILE_BEACON_LEAVE_EVENT_MESSAGE)
        )
...
@pytest.mark.usefixtures("context")
async def test_complex_movement(menuai: menuai) -> None:
    ...
    assert_mobile_tracker_latitude(menuai, INNER_ZONE["latitude"])
    assert_location_state(menuai, "inner")
    assert_mobile_tracker_state(menuai, "inner")
    ...
    await send_message(menuai, LOCATION_TOPIC, lost_keys_location_message)
    await send_message(menuai, EVENT_TOPIC, MOBILE_BEACON_LEAVE_EVENT_MESSAGE)
    assert_location_latitude(menuai, lost_keys_location_message["lat"])
    assert_mobile_tracker_latitude(menuai, lost_keys_location_message["lat"])
    assert_location_state(menuai, "outer")
    assert_mobile_tracker_state(menuai, "outer")
    ...
    await send_message(menuai, LOCATION_TOPIC, LOCATION_MESSAGE_NOT_HOME)
    await send_message(menuai, EVENT_TOPIC, REGION_GPS_LEAVE_MESSAGE_OUTER)
    assert_location_latitude(menuai, LOCATION_MESSAGE_NOT_HOME["lat"])
    assert_mobile_tracker_latitude(menuai, lost_keys_location_message["lat"])
    assert_location_state(menuai, "not_home")
    assert_mobile_tracker_state(menuai, "outer")
...
@pytest.mark.usefixtures("context")
async def test_complex_movement_sticky_keys_beacon(menuai: menuai) -> None:
    ...
    await send_message(menuai, EVENT_TOPIC, REGION_GPS_LEAVE_MESSAGE)
    await send_message(menuai, LOCATION_TOPIC, leave_location_message)
    assert_location_state(menuai, "outer")
    assert_mobile_tracker_state(menuai, "inner")
    assert_location_latitude(menuai, REGION_GPS_LEAVE_MESSAGE["lat"])
    assert_mobile_tracker_latitude(menuai, INNER_ZONE["latitude"])
...
@pytest.mark.usefixtures("context")
async def test_waypoint_import_simple(menuai: menuai) -> None:
    """Test a simple import of list of waypoints."""
    waypoints_message = WAYPOINTS_EXPORTED_MESSAGE.copy()
    await send_message(menuai, WAYPOINTS_TOPIC, waypoints_message)
    # Check if it made it into states
    wayp = menuai.states.get(WAYPOINT_ENTITY_NAMES[0])
    assert wayp is not None
    wayp = menuai.states.get(WAYPOINT_ENTITY_NAMES[1])
    assert wayp is not None


@pytest.mark.usefixtures("context")
async def test_waypoint_import_block(menuai: menuai) -> None:
    """Test import of list of waypoints for blocked user."""
    waypoints_message = WAYPOINTS_EXPORTED_MESSAGE.copy()
    await send_message(menuai, WAYPOINTS_TOPIC_BLOCKED, waypoints_message)
    # Check if it made it into states
    wayp = menuai.states.get(WAYPOINT_ENTITY_NAMES[2])
    assert wayp is None
    wayp = menuai.states.get(WAYPOINT_ENTITY_NAMES[3])
    assert wayp is None


async def test_waypoint_import_no_whitelist(menuai: menuai, setup_comp) -> None:
...
@pytest.mark.usefixtures("context")
async def test_waypoint_import_bad_json(menuai: menuai) -> None:
    """Test importing a bad JSON payload."""
    waypoints_message = WAYPOINTS_EXPORTED_MESSAGE.copy()
    await send_message(menuai, WAYPOINTS_TOPIC, waypoints_message, True)
    # Check if it made it into states
    wayp = menuai.states.get(WAYPOINT_ENTITY_NAMES[2])
    assert wayp is None
    wayp = menuai.states.get(WAYPOINT_ENTITY_NAMES[3])
...
@pytest.mark.usefixtures("context")
async def test_waypoint_import_existing(menuai: menuai) -> None:
    ...
    await send_message(menuai, WAYPOINTS_TOPIC, waypoints_message)
    new_wayp = menuai.states.get(WAYPOINT_ENTITY_NAMES[0])
    assert wayp == new_wayp


@pytest.mark.usefixtures("context")
async def test_single_waypoint_import(menuai: menuai) -> None:
    """Test single waypoint message."""
    waypoint_message = WAYPOINT_MESSAGE.copy()
    await send_message(menuai, WAYPOINT_TOPIC, waypoint_message)
    wayp = menuai.states.get(WAYPOINT_ENTITY_NAMES[0])
    assert wayp is not None


@pytest.mark.usefixtures("context")
async def test_not_implemented_message(menuai: menuai) -> None:
    """Handle not implemented message type."""
    patch_handler = patch(
        "menuai.components.owntracks.messages.async_handle_not_impl_msg",
        return_value=False,
    )
    patch_handler.start()
    assert not await send_message(menuai, LWT_TOPIC, LWT_MESSAGE)
    patch_handler.stop()


@pytest.mark.usefixtures("context")
async def test_unsupported_message(menuai: menuai) -> None:
    """Handle not implemented message type."""
    patch_handler = patch(
        "menuai.components.owntracks.messages.async_handle_unsupported_msg",
        return_value=False,
    )
    patch_handler.start()
    assert not await send_message(menuai, BAD_TOPIC, BAD_MESSAGE)
    patch_handler.stop()
...
def config_context(setup_comp: None) -> Generator[None]:
    """Set up the mocked context."""
    patch_load = patch(
        "menuai.components.device_tracker.async_load_config",
        return_value=[],
    )
    patch_load.start()

    patch_save = patch(
        "menuai.components.device_tracker.DeviceTracker.async_update_config"
    )
    patch_save.start()
...
def mock_not_supports_encryption():
    ...
    with patch(
        "menuai.components.owntracks.messages.supports_encryption",
        return_value=False,
    ):
        yield
...
def mock_get_cipher_error():
    ...
    with patch(
        "menuai.components.owntracks.messages.get_cipher", side_effect=OSError()
    ):
        yield


@patch("menuai.components.owntracks.messages.get_cipher", mock_cipher)
async def test_encrypted_payload(menuai: menuai, setup_comp) -> None:
    """Test encrypted payload."""
    await setup_owntracks(menuai, {CONF_SECRET: TEST_SECRET_KEY})
    await send_message(menuai, LOCATION_TOPIC, MOCK_ENCRYPTED_LOCATION_MESSAGE)
    assert_location_latitude(menuai, LOCATION_MESSAGE["lat"])


@patch("menuai.components.owntracks.messages.get_cipher", mock_cipher)
async def test_encrypted_payload_topic_key(menuai: menuai, setup_comp) -> None:
    """Test encrypted payload with a topic key."""
    await setup_owntracks(menuai, {CONF_SECRET: {LOCATION_TOPIC: TEST_SECRET_KEY}})
    await send_message(menuai, LOCATION_TOPIC, MOCK_ENCRYPTED_LOCATION_MESSAGE)
    assert_location_latitude(menuai, LOCATION_MESSAGE["lat"])


async def test_encrypted_payload_not_supports_encryption(
    menuai: menuai, setup_comp, not_supports_encryption
) -> None:
    """Test encrypted payload with no supported encryption."""
    await setup_owntracks(menuai, {CONF_SECRET: TEST_SECRET_KEY})
    await send_message(menuai, LOCATION_TOPIC, MOCK_ENCRYPTED_LOCATION_MESSAGE)
    assert menuai.states.get(DEVICE_TRACKER_STATE) is None


async def test_encrypted_payload_get_cipher_error(
    menuai: menuai, setup_comp, get_cipher_error
) -> None:
    """Test encrypted payload with no supported encryption."""
    await setup_owntracks(menuai, {CONF_SECRET: TEST_SECRET_KEY})
    await send_message(menuai, LOCATION_TOPIC, MOCK_ENCRYPTED_LOCATION_MESSAGE)
...
@patch("menuai.components.owntracks.messages.get_cipher", mock_cipher)
async def test_encrypted_payload_no_key(menuai: menuai, setup_comp) -> None:
    """Test encrypted payload with no key, ."""
    assert menuai.states.get(DEVICE_TRACKER_STATE) is None
    await setup_owntracks(menuai, {CONF_SECRET: {}})
    await send_message(menuai, LOCATION_TOPIC, MOCK_ENCRYPTED_LOCATION_MESSAGE)
    assert menuai.states.get(DEVICE_TRACKER_STATE) is None


@patch("menuai.components.owntracks.messages.get_cipher", mock_cipher)
async def test_encrypted_payload_wrong_key(menuai: menuai, setup_comp) -> None:
    """Test encrypted payload with wrong key."""
    await setup_owntracks(menuai, {CONF_SECRET: "wrong key"})
    await send_message(menuai, LOCATION_TOPIC, MOCK_ENCRYPTED_LOCATION_MESSAGE)
    assert menuai.states.get(DEVICE_TRACKER_STATE) is None
...
async def test_encrypted_payload_wrong_topic_key(
    menuai: menuai, setup_comp
) -> None:
    """Test encrypted payload with wrong  topic key."""
    await setup_owntracks(menuai, {CONF_SECRET: {LOCATION_TOPIC: "wrong key"}})
    await send_message(menuai, LOCATION_TOPIC, MOCK_ENCRYPTED_LOCATION_MESSAGE)
    assert menuai.states.get(DEVICE_TRACKER_STATE) is None


@patch("menuai.components.owntracks.messages.get_cipher", mock_cipher)
async def test_encrypted_payload_no_topic_key(menuai: menuai, setup_comp) -> None:
    """Test encrypted payload with no topic key."""
    await setup_owntracks(
        menuai, {CONF_SECRET: {f"owntracks/{USER}/otherdevice": "foobar"}}
    )
    await send_message(menuai, LOCATION_TOPIC, MOCK_ENCRYPTED_LOCATION_MESSAGE)
    assert menuai.states.get(DEVICE_TRACKER_STATE) is None


async def test_encrypted_payload_libsodium(menuai: menuai, setup_comp) -> None:
    """Test sending encrypted message payload."""
    await setup_owntracks(menuai, {CONF_SECRET: TEST_SECRET_KEY})

    await send_message(menuai, LOCATION_TOPIC, ENCRYPTED_LOCATION_MESSAGE)
    assert_location_latitude(menuai, LOCATION_MESSAGE["lat"])


async def test_customized_mqtt_topic(menuai: menuai, setup_comp) -> None:
    """Test subscribing to a custom mqtt topic."""
    await setup_owntracks(menuai, {CONF_MQTT_TOPIC: "mytracks/#"})

    topic = f"mytracks/{USER}/{DEVICE}"

    await send_message(menuai, topic, LOCATION_MESSAGE)
    assert_location_latitude(menuai, LOCATION_MESSAGE["lat"])


async def test_region_mapping(menuai: menuai, setup_comp) -> None:
    """Test region to zone mapping."""
    await setup_owntracks(menuai, {CONF_REGION_MAPPING: {"foo": "inner"}})
...
async def test_restore_state(
    ...
    state_1 = menuai.states.get("device_tracker.paulus_pixel")
    assert state_1 is not None

    await menuai.config_entries.async_reload(entry.entry_id)
    await menuai.async_block_till_done()

    state_2 = menuai.states.get("device_tracker.paulus_pixel")
...
async def test_returns_array_friends(
    ...
    otracks.add_to_menuai(menuai)

    await menuai.config_entries.async_setup(otracks.entry_id)
    await menuai.async_block_till_done()
