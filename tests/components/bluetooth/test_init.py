...
async def test_setup_and_stop(
    menuai: menuai, mock_bleak_scanner_start: MagicMock
) -> None:
    ...
    with (
        patch(
            "menuai.components.bluetooth.async_get_bluetooth",
            return_value=mock_bt,
        ),
        patch.object(menuai.config_entries.flow, "async_init"),
    ):
        assert await async_setup_component(
            menuai, bluetooth.DOMAIN, {bluetooth.DOMAIN: {}}
        )
...
async def test_setup_and_stop_no_bluetooth(
    ...
    with (
        ...
        patch("menuai.components.bluetooth.discovery_flow.async_create_flow"),
    ):
        await async_setup_with_one_adapter(menuai)
        menuai.bus.async_fire(EVENT_menuai_STARTED)
        await menuai.async_block_till_done()
...
async def test_setup_and_stop_broken_bluetooth(
    ...
    with (
        ...
        await async_setup_with_default_adapter(menuai)
        menuai.bus.async_fire(EVENT_menuai_STARTED)
        await menuai.async_block_till_done()
    ...
    assert len(bluetooth.async_discovered_service_info(menuai)) == 0
...
async def test_setup_and_retry_adapter_not_yet_available(
    ...
    entry = menuai.config_entries.async_entries(bluetooth.DOMAIN)[0]
...
async def test_calling_async_discovered_devices_no_bluetooth(
    ...
    await menuai.async_block_till_done()
    assert "Failed to initialize Bluetooth" in caplog.text
    assert not bluetooth.async_discovered_service_info(menuai)
    assert not bluetooth.async_address_present(menuai, "aa:bb:bb:dd:ee:ff")
...
async def test_discovery_match_by_service_uuid(
    ...
    with (
        ...
        patch.object(menuai.config_entries.flow, "async_init") as mock_config_flow,
    ):
        await async_setup_with_default_adapter(menuai)
        menuai.bus.async_fire(EVENT_menuai_STARTED)
        await menuai.async_block_till_done()
        ...
        inject_advertisement(menuai, wrong_device, wrong_adv)
        await menuai.async_block_till_done()
...
async def test_discovery_match_by_service_uuid_and_short_local_name(
    ...
    with patch.object(menuai.config_entries.flow, "async_init") as mock_config_flow:
        ...
        inject_advertisement(menuai, ht1_device, ht1_adv)
        await menuai.async_block_till_done()
...
async def test_discovery_match_by_service_uuid_connectable(
    ...
    with (
        ...
        inject_advertisement_with_time_and_source_connectable(
            menuai, wrong_device, wrong_adv, time.monotonic(), "any", True
        )
        await menuai.async_block_till_done()
        ...
        inject_advertisement_with_time_and_source_connectable(
            menuai, switchbot_device, switchbot_adv, time.monotonic(), "any", True
        )
        await menuai.async_block_till_done()
...
async def test_discovery_match_by_service_uuid_not_connectable(
    ...
    with (
        ...
        inject_advertisement_with_time_and_source_connectable(
            menuai, wrong_device, wrong_adv, time.monotonic(), "any", False
        )
        await menuai.async_block_till_done()
        ...
        inject_advertisement_with_time_and_source_connectable(
            menuai, switchbot_device, switchbot_adv, time.monotonic(), "any", False
        )
        await menuai.async_block_till_done()
...
async def test_discovery_match_by_name_connectable_false(
    ...
    with (
        ...
        inject_advertisement_with_time_and_source_connectable(
            menuai, qingping_device, qingping_adv, time.monotonic(), "any", False
        )
        await menuai.async_block_till_done()
...
async def test_discovery_match_by_manufacturer_id_and_manufacturer_data_start(
    ...
    with patch.object(menuai.config_entries.flow, "async_init") as mock_config_flow:
        ...
        inject_advertisement(menuai, hkc_device, hkc_adv_no_mfr_data)
        await menuai.async_block_till_done()
        assert len(mock_config_flow.mock_calls) == 0
        mock_config_flow.reset_mock()
        ...
        inject_advertisement(menuai, hkc_device, hkc_adv)
        await menuai.async_block_till_done()
        assert len(mock_config_flow.mock_calls) == 1
        ...
        inject_advertisement(menuai, not_hkc_device, not_hkc_adv)
        await menuai.async_block_till_done()
        ...
        inject_advertisement(menuai, not_apple_device, not_apple_adv)
        await menuai.async_block_till_done()
...
async def test_discovery_match_by_service_data_uuid_then_others(
    ...
    with patch.object(menuai.config_entries.flow, "async_init") as mock_config_flow:
        ...
        inject_advertisement(menuai, device, adv_without_service_data_uuid)
        await menuai.async_block_till_done()
        assert len(mock_config_flow.mock_calls) == 0
        mock_config_flow.reset_mock()
        ...
        inject_advertisement(menuai, device, adv_with_mfr_data)
        await menuai.async_block_till_done()
        assert len(mock_config_flow.mock_calls) == 1
        ...
        inject_advertisement(menuai, device, adv_with_service_data_uuid)
        await menuai.async_block_till_done()
        assert len(mock_config_flow.mock_calls) == 1
        ...
        inject_advertisement(menuai, device, adv_with_service_data_uuid_and_mfr_data)
        await menuai.async_block_till_done()
        assert len(mock_config_flow.mock_calls) == 0
        mock_config_flow.reset_mock()
        ...
        inject_advertisement(
            menuai, device, adv_with_service_data_uuid_and_mfr_data_and_service_uuid
        )
        await menuai.async_block_till_done()
        ...
        # since all fields have been seen at this point
        inject_advertisement(menuai, device, adv_with_service_uuid)
        await menuai.async_block_till_done()
        assert len(mock_config_flow.mock_calls) == 0
...
async def test_discovery_match_by_service_data_uuid_when_format_changes(
    ...
    with patch.object(menuai.config_entries.flow, "async_init") as mock_config_flow:
        ...
        inject_advertisement(menuai, device, xiaomi_format_adv)
        await menuai.async_block_till_done()
        assert len(mock_config_flow.mock_calls) == 1
        ...
        inject_advertisement(menuai, device, qingping_format_adv)
        await menuai.async_block_till_done()
        assert len(mock_config_flow.mock_calls) == 1
...
async def test_discovery_match_by_service_data_uuid_bthome(
    ...
    with patch.object(menuai.config_entries.flow, "async_init") as mock_config_flow:
        ...
        inject_advertisement(menuai, device, button_adv)
        await menuai.async_block_till_done()
        assert len(mock_config_flow.mock_calls) == 1
        mock_config_flow.reset_mock()
...
async def test_discovery_match_first_by_service_uuid_and_then_manufacturer_id(
    ...
    with patch.object(menuai.config_entries.flow, "async_init") as mock_config_flow:
        ...
        inject_advertisement(menuai, device, adv_service_uuids)
        await menuai.async_block_till_done()
        assert len(mock_config_flow.mock_calls) == 1
        assert mock_config_flow.mock_calls[0][1][0] == "my_domain"
        mock_config_flow.reset_mock()
        ...
        # 2nd discovery with manufacturer data
        # should trigger a config flow
        inject_advertisement(menuai, device, adv_manufacturer_data)
        await menuai.async_block_till_done()
        assert len(mock_config_flow.mock_calls) == 1
        assert mock_config_flow.mock_calls[0][1][0] == "my_domain"
        mock_config_flow.reset_mock()
...
async def test_rediscovery(
    ...
    with (
        ...
        async_rediscover_address(menuai, "44:44:33:11:23:45")

        inject_advertisement(menuai, switchbot_device, switchbot_adv_2)
        await menuai.async_block_till_done()
...
async def test_async_discovered_device_api(
    ...
    with (
        ...
        with pytest.raises(RuntimeError, match="BluetoothManager has not been set"):
            assert not bluetooth.async_discovered_service_info(menuai)
        with pytest.raises(RuntimeError, match="BluetoothManager has not been set"):
            assert not bluetooth.async_address_present(menuai, "44:44:22:22:11:22")
        await async_setup_with_default_adapter(menuai)

        with patch.object(menuai.config_entries.flow, "async_init"):
            menuai.bus.async_fire(EVENT_menuai_STARTED)
            await menuai.async_block_till_done()
            ...
            wrong_device_unavailable_cancel = async_track_unavailable(
                menuai, _wrong_device_unavailable_callback, wrong_device.address
            )
            switchbot_device_unavailable_cancel = async_track_unavailable(
                menuai, _switchbot_device_unavailable_callback, switchbot_device.address
            )
            ...
            service_infos = bluetooth.async_discovered_service_info(menuai)
            ...
            assert bluetooth.async_address_present(menuai, "44:44:33:11:23:42") is False
            assert bluetooth.async_address_present(menuai, "44:44:33:11:23:45") is True
...
async def test_register_callbacks(
    ...
    with (
        ...
        inject_advertisement(menuai, seen_switchbot_device, seen_switchbot_adv)
        ...
        inject_advertisement(menuai, empty_device, empty_adv)
        await menuai.async_block_till_done()
...
async def test_register_callback_by_address_connectable_only(
    ...
    with patch.object(menuai.config_entries.flow, "async_init"):
        ...
        inject_advertisement_with_time_and_source_connectable(
            menuai, switchbot_device, switchbot_adv, time.monotonic(), "test", False
        )
        inject_advertisement_with_time_and_source_connectable(
            menuai,
            switchbot_device,
...
async def test_register_callback_by_manufacturer_id(
    ...
    with patch.object(menuai.config_entries.flow, "async_init"):
        ...
        inject_advertisement(menuai, apple_device, apple_adv)
...
async def test_not_filtering_wanted_apple_devices(
    ...
    with patch.object(menuai.config_entries.flow, "async_init"):
        ...
        inject_advertisement(menuai, ibeacon_device, ibeacon_adv)
        ...
        inject_advertisement(menuai, homekit_device, homekit_adv)
...
async def test_register_callback_by_address_connectable_manufacturer_id(
    ...
    with patch.object(menuai.config_entries.flow, "async_init"):
        ...
        inject_advertisement(menuai, apple_device_wrong_address, apple_adv)
        await menuai.async_block_till_done()
...
async def test_register_callback_by_manufacturer_id_and_address(
    ...
    with patch.object(menuai.config_entries.flow, "async_init"):
        ...
        inject_advertisement(menuai, rtx_device, rtx_adv)
        ...
        inject_advertisement(menuai, yale_device, yale_adv)
        await menuai.async_block_till_done()
        ...
        inject_advertisement(menuai, other_apple_device, other_apple_adv)
...
async def test_register_callback_by_service_uuid_and_address(
    ...
    with patch.object(menuai.config_entries.flow, "async_init"):
        ...
        inject_advertisement(menuai, switchbot_dev, switchbot_adv)
        ...
        inject_advertisement(
            menuai, switchbot_missing_service_uuid_dev, switchbot_missing_service_uuid_adv
        )
        await menuai.async_block_till_done()
        ...
        inject_advertisement(
            menuai, service_uuid_wrong_address_dev, service_uuid_wrong_address_adv
        )
...
async def test_register_callback_by_local_name(
    ...
    with patch.object(menuai.config_entries.flow, "async_init"):
        ...
        inject_advertisement(menuai, rtx_device_2, rtx_adv_2)
...
async def test_process_advertisements_bail_on_good_advertisement(
    ...
    handle = menuai.async_create_task(
        async_process_advertisements(
            menuai,
            _callback,
    ...
    while not done.done():
        ...
        inject_advertisement(menuai, device, adv)
        inject_advertisement(menuai, device, adv)
        inject_advertisement(menuai, device, adv)
...
async def test_process_advertisements_ignore_bad_advertisement(
    ...
    while not done.is_set():
        inject_advertisement(menuai, device, adv)
        inject_advertisement(menuai, device, adv2)
        await asyncio.sleep(0)
...
async def test_process_advertisements_timeout(
    ...
    with pytest.raises(TimeoutError):
        await async_process_advertisements(
            menuai, _callback, {}, BluetoothScanningMode.ACTIVE, 0
        )
...
async def test_wrapped_instance_with_filter(
    ...
    with patch(
        "menuai.components.bluetooth.async_get_bluetooth", return_value=[]
    ):
        await async_setup_with_default_adapter(menuai)
...
async def test_async_ble_device_from_address(
    ...
    with (
        ...
        with pytest.raises(RuntimeError, match="BluetoothManager has not been set"):
            assert (
                bluetooth.async_ble_device_from_address(menuai, "44:44:33:11:23:45")
                is None
            )
        ...
        assert (
            bluetooth.async_ble_device_from_address(menuai, "00:66:33:22:11:22") is None
        )
...
async def test_default_address_config_entries_removed_linux(
    ...
    entry = MockConfigEntry(domain=bluetooth.DOMAIN, data={}, unique_id=DEFAULT_ADDRESS)
    entry.add_to_menuai(menuai)
    await async_setup_component(menuai, bluetooth.DOMAIN, {})
    await menuai.async_block_till_done()
    assert not menuai.config_entries.async_entries(bluetooth.DOMAIN)
...
@pytest.mark.usefixtures("one_adapter")
async def test_auto_detect_bluetooth_adapters_linux(menuai: menuai) -> None:
    """Test we auto detect bluetooth adapters on linux."""
    assert await async_setup_component(menuai, bluetooth.DOMAIN, {})
    await menuai.async_block_till_done()
    assert not menuai.config_entries.async_entries(bluetooth.DOMAIN)
    assert len(menuai.config_entries.flow.async_progress(bluetooth.DOMAIN)) == 1
...
async def test_auto_detect_bluetooth_adapters_linux_multiple(
    ...
    await menuai.async_block_till_done()
    assert not menuai.config_entries.async_entries(bluetooth.DOMAIN)
    assert len(menuai.config_entries.flow.async_progress(bluetooth.DOMAIN)) == 2


async def test_auto_detect_bluetooth_adapters_skips_crashed(
    menuai: menuai, crashed_adapter: None
) -> None:
    """Test we skip crashed adapters on linux."""
    assert await async_setup_component(menuai, bluetooth.DOMAIN, {})
    await menuai.async_block_till_done()
    assert not menuai.config_entries.async_entries(bluetooth.DOMAIN)
    assert len(menuai.config_entries.flow.async_progress(bluetooth.DOMAIN)) == 0
...
async def test_auto_detect_bluetooth_adapters_macos(menuai: menuai) -> None:
...
async def test_no_auto_detect_bluetooth_adapters_windows(menuai: menuai) -> None:
...
async def test_getting_the_scanner_returns_the_wrapped_instance(
    ...
    scanner = bluetooth.async_get_scanner(menuai)
    assert isinstance(scanner, HaBleakScannerWrapper)


@pytest.mark.usefixtures("enable_bluetooth")
async def test_scanner_count_connectable(menuai: menuai) -> None:
    """Test getting the connectable scanner count."""
    scanner = FakeScanner("any", "any")
    cancel = bluetooth.async_register_scanner(menuai, scanner)
    assert bluetooth.async_scanner_count(menuai, connectable=True) == 1
    cancel()


@pytest.mark.usefixtures("enable_bluetooth")
async def test_scanner_remove(menuai: menuai) -> None:
    ...
    inject_advertisement_with_time_and_source_connectable(
        menuai, device, adv, time.monotonic(), scanner.source, True
    )
    cancel()
    bluetooth.async_remove_scanner(menuai, scanner.source)
    manager: menuaiBluetoothManager = _get_manager()
    assert not manager.storage.async_get_advertisement_history(scanner.source)


@pytest.mark.usefixtures("enable_bluetooth")
async def test_scanner_count(menuai: menuai) -> None:
    """Test getting the connectable and non-connectable scanner count."""
    scanner = FakeScanner("any", "any")
    cancel = bluetooth.async_register_scanner(menuai, scanner)
    assert bluetooth.async_scanner_count(menuai, connectable=False) == 2
    cancel()
...
async def test_discover_new_usb_adapters(
    ...
    def _async_register_scan_request_callback(_menuai, _callback):
        nonlocal saved_callback
        saved_callback = _callback
        return lambda: None

    with patch(
        "menuai.components.bluetooth.usb.async_register_scan_request_callback",
    ...
    with (
        ...
        for wait_sec in range(10, 20):
            async_fire_time_changed(
                menuai, dt_util.utcnow() + timedelta(seconds=wait_sec)
            )
            await menuai.async_block_till_done()

    assert len(menuai.config_entries.flow.async_progress(DOMAIN)) == 1
...
async def test_discover_new_usb_adapters_with_firmware_fallback_delay(
    ...
    with (
        ...
        async_fire_time_changed(
            menuai, dt_util.utcnow() + timedelta(BLUETOOTH_DISCOVERY_COOLDOWN_SECONDS * 2)
        )
        await menuai.async_block_till_done()

    assert len(menuai.config_entries.flow.async_progress(DOMAIN)) == 0
...
async def test_fix_incorrect_mac_remote_scanner_config_entry(
    ...
    source_entry.add_to_menuai(menuai)
    connector = (
        HaBluetoothConnector(MockBleakClient, "mock_bleak_client", lambda: False),
    )
    ...
    await menuai.config_entries.async_setup(entry.entry_id)
    await menuai.async_block_till_done()
    assert menuai.config_entries.async_entry_for_domain_unique_id(
        "bluetooth", scanner.source
    )
