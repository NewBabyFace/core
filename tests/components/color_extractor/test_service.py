...
from menuai.components.color_extractor import (
    ATTR_PATH,
    ATTR_URL,
    DOMAIN,
    SERVICE_TURN_ON,
...
@pytest.fixture(autouse=True)
async def setup_light(menuai: menuai):
    ...
    state = menuai.states.get(LIGHT_ENTITY)
    assert state
...
async def test_missing_url_and_path(menuai: menuai, setup_integration) -> None:
...
async def _async_execute_service(menuai: menuai, service_data: dict[str, Any]):
    # Validate pre service call
    state = menuai.states.get(LIGHT_ENTITY)
    assert state
    assert state.state == STATE_OFF
    ...
    await menuai.services.async_call(DOMAIN, SERVICE_TURN_ON, service_data, blocking=True)

    await menuai.async_block_till_done()


async def test_url_success(
    menuai: menuai, aioclient_mock: AiohttpClientMocker, setup_integration
) -> None:
    ...
    aioclient_mock.get(
        ...
        content=base64.b64decode(
            await async_load_fixture(menuai, "color_extractor_url.txt", DOMAIN)
        ),
    )

    # Allow access to this URL using the proper mechanism
    menuai.config.allowlist_external_urls.add("http://example.com/images/")

    await _async_execute_service(menuai, service_data)
...
async def test_file(menuai: menuai, setup_integration) -> None:
    ...
    menuai.config.allowlist_external_dirs.add("/opt/")
    ...
    with patch("menuai.components.color_extractor._get_file", _get_file_mock):
        await menuai.services.async_call(DOMAIN, SERVICE_TURN_ON, service_data)
        await menuai.async_block_till_done()
...
@patch("os.path.isfile", Mock(return_value=True))
@patch("os.access", Mock(return_value=True))
async def test_file_denied_dir(menuai: menuai, setup_integration) -> None:
