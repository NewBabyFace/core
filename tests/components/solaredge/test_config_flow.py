...
from menuai.components.solaredge.const import CONF_SITE_ID, DEFAULT_NAME, DOMAIN
...
def mock_controller():
    ...
    with patch(
        "menuai.components.solaredge.config_flow.aiosolaredge.SolarEdge",
        return_value=api,
    ):
        yield api


async def test_user(menuai: menuai, test_api: Mock) -> None:
    """Test user config."""
...
async def test_abort_if_already_setup(menuai: menuai, test_api: str) -> None:
...
async def test_asserts(menuai: menuai, test_api: Mock) -> None:
