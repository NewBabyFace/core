...
async def test_entities_removed(
    ...
    state = menuai.states.get("binary_sensor.test_mybinary_sensor_to_be_removed")
    assert state is not None
    assert state.state == STATE_ON
    ...
    assert len(menuai_storage[storage_key]["data"]["binary_sensor"]) == 2
    ...
    await menuai.config_entries.async_unload(entry.entry_id)
    await menuai.async_block_till_done()
    assert len(menuai_storage[storage_key]["data"]["binary_sensor"]) == 1
...
async def test_entities_removed_after_reload(
    ...
    assert await menuai.config_entries.async_setup(entry.entry_id)
    on_future = menuai.loop.create_future()
    ...
    async_track_state_change_event(
        menuai, ["binary_sensor.test_mybinary_sensor"], _async_wait_for_on
    )
    await menuai.async_block_till_done()
...
async def test_entities_for_entire_platform_removed(
    ...
    await menuai.config_entries.async_unload(entry.entry_id)
    await menuai.async_block_till_done()
    assert len(menuai_storage[storage_key]["data"]["binary_sensor"]) == 0
...
async def test_entity_info_object_ids(
    ...
    state = menuai.states.get("binary_sensor.test_object_id_is_used")
    assert state is not None
...
async def test_deep_sleep_device(
    ...
    assert state is not None
    assert state.state == STATE_ON
    state = menuai.states.get("sensor.test_my_sensor")
    assert state is not None
...
async def test_entity_without_name_device_with_friendly_name(
    ...
    state = menuai.states.get("binary_sensor.mixer")
    assert state is not None
    assert state.state == STATE_ON
...
async def test_entity_id_preserved_on_upgrade(
    ...
    state = menuai.states.get("binary_sensor.should_not_change")
    assert state is not None
...
async def test_entity_id_preserved_on_upgrade_old_format_entity_id(
    ...
    state = menuai.states.get("binary_sensor.my")
    assert state is not None
...
async def test_entity_id_preserved_on_upgrade_when_in_storage(
    ...
    state = menuai.states.get("binary_sensor.mixer_my")
    assert state is not None
    ...
    await menuai.config_entries.async_unload(device.entry.entry_id)
    await menuai.async_block_till_done()
    entry = device.entry
    entry_id = entry.entry_id
    storage_key = f"esphome.{entry_id}"
    assert len(menuai_storage[storage_key]["data"]["binary_sensor"]) == 1
    binary_sensor_data: dict[str, Any] = menuai_storage[storage_key]["data"][
        "binary_sensor"
    ][0]
    ...
    state = menuai.states.get("binary_sensor.user_named")
    assert state is not None
