...
async def platforms() -> AsyncGenerator[None]:
    ...
    with patch(
        "menuai.components.bosch_alarm.PLATFORMS", [Platform.BINARY_SENSOR]
    ):
        yield
...
async def test_panel_faults(
    ...
    await call_observable(menuai, mock_panel.faults_observer)
...
async def test_area_ready_to_arm(
    ...
    assert menuai.states.get(entity_id).state == STATE_ON
    assert menuai.states.get(entity_id_2).state == STATE_ON
    ...
    await call_observable(menuai, area.status_observer)
    assert menuai.states.get(entity_id).state == STATE_OFF
    assert menuai.states.get(entity_id_2).state == STATE_OFF
