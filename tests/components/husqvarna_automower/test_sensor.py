...
async def test_sensor_unknown_states(
    ...
    state = menuai.states.get("sensor.test_mower_1_mode")
...
async def test_cutting_blade_usage_time_sensor(
    ...
    await setup_integration(menuai, mock_config_entry)
    state = menuai.states.get("sensor.test_mower_1_cutting_blade_usage_time")
    assert state is not None
...
async def test_next_start_sensor(
    ...
    """Test if this sensor is only added, if data is available."""
    await setup_integration(menuai, mock_config_entry)
    state = menuai.states.get("sensor.test_mower_1_next_start")
    assert state is not None
...
async def test_work_area_sensor(
    ...
    await setup_integration(menuai, mock_config_entry)
    state = menuai.states.get("sensor.test_mower_1_work_area")
    assert state is not None
...
async def test_statistics_not_available(
    ...
    mock_automower_client.get_status.return_value = values
    await setup_integration(menuai, mock_config_entry)
    state = menuai.states.get(f"sensor.test_mower_1_{sensor_to_test}")
...
async def test_error_sensor(
    ...
    for state, error_key, expected_state in (
        ...
        async_fire_time_changed(menuai)
        await menuai.async_block_till_done()
        state = menuai.states.get("sensor.test_mower_1_error")
