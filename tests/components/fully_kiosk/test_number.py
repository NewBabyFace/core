...
async def test_numbers(
    ...
    state = menuai.states.get("number.amazon_fire_screensaver_timer")
    assert state
    ...
    await set_value(menuai, "number.amazon_fire_screensaver_timer", 600)
    assert len(mock_fully_kiosk.setConfigurationString.mock_calls) == 1

    state = menuai.states.get("number.amazon_fire_screensaver_brightness")
    assert state
    ...
    state = menuai.states.get("number.amazon_fire_screen_off_timer")
    assert state
    assert state.state == "0"
    ...
    state = menuai.states.get("number.amazon_fire_screen_brightness")
    assert state
...
async def set_value(
    menuai: menuai, entity_id: str, value: float
) -> ServiceResponse:
