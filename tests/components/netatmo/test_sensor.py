...
from menuai.components.netatmo import sensor
...
async def test_indoor_sensor(
    ...
    assert menuai.states.get(f"{prefix}temperature").state == "20.3"
    assert menuai.states.get(f"{prefix}humidity").state == "63"
    assert menuai.states.get(f"{prefix}carbon_dioxide").state == "494"
    assert menuai.states.get(f"{prefix}atmospheric_pressure").state == "1014.5"
...
async def test_weather_sensor(
    ...
    assert menuai.states.get(f"{prefix}temperature").state == "unavailable"
...
async def test_public_weather_sensor(
    ...
    assert menuai.states.get(f"{prefix}temperature").state == "27.4"
    assert menuai.states.get(f"{prefix}humidity").state == "76"
    assert menuai.states.get(f"{prefix}atmospheric_pressure").state == "1014.4"

    prefix = "sensor.home_min_"

    assert menuai.states.get(f"{prefix}temperature").state == "19.8"
    assert menuai.states.get(f"{prefix}humidity").state == "56"
    assert menuai.states.get(f"{prefix}atmospheric_pressure").state == "1005.4"

    prefix = "sensor.home_avg_"

    assert menuai.states.get(f"{prefix}temperature").state == "22.7"
    assert menuai.states.get(f"{prefix}humidity").state == "63.2"
    assert menuai.states.get(f"{prefix}atmospheric_pressure").state == "1010.4"

    entities_before_change = len(menuai.states.async_all())
    ...
    assert len(menuai.states.async_all()) == entities_before_change
    assert menuai.states.get(f"{prefix}temperature").state == "27.4"
...
async def test_weather_sensor_enabling(
    ...
    with selected_platforms([Platform.SENSOR]):
        states_before = len(menuai.states.async_all())
        assert menuai.states.get(f"sensor.{name}") is None
        ...
        assert len(menuai.states.async_all()) > states_before
        assert menuai.states.get(f"sensor.{name}").state == expected
...
async def test_climate_battery_sensor(
    ...
    assert menuai.states.get(f"{prefix}battery").state == "75"
