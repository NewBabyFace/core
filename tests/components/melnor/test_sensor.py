...
from menuai.const import PERCENTAGE, SIGNAL_STRENGTH_DECIBELS_MILLIWATT
...
async def test_battery_sensor(menuai: menuai) -> None:
    ...
    with (
        ...
        battery_sensor = menuai.states.get("sensor.test_melnor_battery")
...
async def test_minutes_remaining_sensor(menuai: menuai) -> None:
    ...
    with (
        ...
        minutes_sensor = menuai.states.get("sensor.zone_1_manual_cycle_end")
        ...
        minutes_remaining_sensor = menuai.states.get("sensor.zone_1_manual_cycle_end")

        assert minutes_remaining_sensor is not None
        assert minutes_remaining_sensor.state == end_time.isoformat(timespec="seconds")


async def test_schedule_next_cycle_sensor(menuai: menuai) -> None:
    ...
    with (
        ...
        # Valve is off, report 0
        minutes_sensor = menuai.states.get("sensor.zone_1_next_cycle")
        ...
        # Valve is on, report 10
        next_cycle_sensor = menuai.states.get("sensor.zone_1_next_cycle")
