...
from menuai.components.recollect_waste.const import (
    CONF_PLACE_ID,
    CONF_SERVICE_ID,
    DOMAIN,
)
...
def mock_aiorecollect_fixture(client):
    ...
    with (
        patch(
            "menuai.components.recollect_waste.Client",
            return_value=client,
        ),
        patch(
            "menuai.components.recollect_waste.config_flow.Client",
            return_value=client,
        ),
    ):
        yield
...
async def setup_config_entry_fixture(
    menuai: menuai, config_entry: MockConfigEntry, mock_aiorecollect: None
) -> None:
