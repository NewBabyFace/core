...
from menuai.components.forked_daapd.browse_media import create_media_content_id
from menuai.components.forked_daapd.const import (
...
from menuai.components.media_source import PlayMedia
...
async def mock_api_object_fixture(
    ...
    with patch(
        "menuai.components.forked_daapd.ForkedDaapdAPI",
        autospec=True,
    ) as mock_api:
...
async def test_unload_config_entry(
    ...
    assert menuai.states.get(TEST_MASTER_ENTITY_NAME)
    assert menuai.states.get(TEST_ZONE_ENTITY_NAMES[0])
    await menuai.config_entries.async_unload(config_entry.entry_id)
    assert menuai.states.get(TEST_MASTER_ENTITY_NAME).state == STATE_UNAVAILABLE
    assert menuai.states.get(TEST_ZONE_ENTITY_NAMES[0]).state == STATE_UNAVAILABLE


@pytest.mark.usefixtures("mock_api_object")
def test_master_state(menuai: menuai) -> None:
    """Test master state attributes."""
    state = menuai.states.get(TEST_MASTER_ENTITY_NAME)
...
async def test_no_update_when_get_request_returns_none(
    menuai: menuai, config_entry: MockConfigEntry, mock_api_object: Mock
) -> None:
    ...
    signal_output_call = async_mock_signal(
        menuai, SIGNAL_UPDATE_OUTPUTS.format(config_entry.entry_id)
    )
    signal_player_call = async_mock_signal(
        menuai, SIGNAL_UPDATE_PLAYER.format(config_entry.entry_id)
    )
    signal_queue_call = async_mock_signal(
        menuai, SIGNAL_UPDATE_QUEUE.format(config_entry.entry_id)
    )
...
async def _service_call(
    ...
    return await menuai.services.async_call(
        MP_DOMAIN,
        service,
...
async def test_zone(menuai: menuai, mock_api_object: Mock) -> None:
    """Test zone attributes and methods."""
    zone_entity_name = TEST_ZONE_ENTITY_NAMES[0]
    state = menuai.states.get(zone_entity_name)
    ...
    assert not state.attributes[ATTR_MEDIA_VOLUME_MUTED]
    await _service_call(menuai, zone_entity_name, SERVICE_TURN_ON)
    await _service_call(menuai, zone_entity_name, SERVICE_TURN_OFF)
    await _service_call(menuai, zone_entity_name, SERVICE_TOGGLE)
    await _service_call(
        menuai, zone_entity_name, SERVICE_VOLUME_SET, {ATTR_MEDIA_VOLUME_LEVEL: 0.3}
    )
    await _service_call(
        menuai, zone_entity_name, SERVICE_VOLUME_MUTE, {ATTR_MEDIA_VOLUME_MUTED: True}
    )
    await _service_call(
        menuai, zone_entity_name, SERVICE_VOLUME_MUTE, {ATTR_MEDIA_VOLUME_MUTED: False}
    )
...
async def test_last_outputs_master(menuai: menuai, mock_api_object: Mock) -> None:
    """Test restoration of _last_outputs."""
    # Test turning on sends API call
    await _service_call(menuai, TEST_MASTER_ENTITY_NAME, SERVICE_TURN_ON)
    assert mock_api_object.change_output.call_count == 0
    assert mock_api_object.set_enabled_outputs.call_count == 1
    await _service_call(
        menuai, TEST_MASTER_ENTITY_NAME, SERVICE_TURN_OFF
...
async def test_bunch_of_stuff_master(
    ...
    """Run bunch of stuff."""
    await _service_call(menuai, TEST_MASTER_ENTITY_NAME, SERVICE_TURN_ON)
    await _service_call(menuai, TEST_MASTER_ENTITY_NAME, SERVICE_TURN_OFF)
    await _service_call(menuai, TEST_MASTER_ENTITY_NAME, SERVICE_TOGGLE)
    await _service_call(
        menuai,
    ...
    await _service_call(menuai, TEST_MASTER_ENTITY_NAME, SERVICE_MEDIA_PAUSE)
    await _service_call(menuai, TEST_MASTER_ENTITY_NAME, SERVICE_MEDIA_PLAY)
    await _service_call(menuai, TEST_MASTER_ENTITY_NAME, SERVICE_MEDIA_STOP)
    await _service_call(menuai, TEST_MASTER_ENTITY_NAME, SERVICE_MEDIA_PREVIOUS_TRACK)
    await _service_call(menuai, TEST_MASTER_ENTITY_NAME, SERVICE_MEDIA_NEXT_TRACK)
    await _service_call(
        menuai,
        TEST_MASTER_ENTITY_NAME,
        SERVICE_MEDIA_SEEK,
    ...
    await _service_call(menuai, TEST_MASTER_ENTITY_NAME, SERVICE_CLEAR_PLAYLIST)
    await _service_call(
        menuai, TEST_MASTER_ENTITY_NAME, SERVICE_SHUFFLE_SET, {ATTR_MEDIA_SHUFFLE: False}
    )
...
@pytest.mark.usefixtures("mock_api_object")
async def test_async_play_media_from_paused(menuai: menuai) -> None:
    """Test async play media from paused."""
    initial_state = menuai.states.get(TEST_MASTER_ENTITY_NAME)
    await _service_call(
        menuai,
        TEST_MASTER_ENTITY_NAME,
        SERVICE_PLAY_MEDIA,
...
@pytest.mark.usefixtures("mock_api_object")
async def test_async_play_media_unsupported(menuai: menuai) -> None:
...
async def test_async_play_media_announcement_tts_timeout(
    menuai: menuai, mock_api_object: Mock
) -> None:
    """Test async play media announcement with TTS timeout."""
    mock_api_object.add_to_queue.side_effect = None
    with patch("menuai.components.forked_daapd.media_player.TTS_TIMEOUT", 0):
        initial_state = menuai.states.get(TEST_MASTER_ENTITY_NAME)
        await _service_call(
            menuai,
            TEST_MASTER_ENTITY_NAME,
...
@pytest.mark.usefixtures("mock_api_object")
async def test_clear_source(menuai: menuai) -> None:
...
async def pipe_control_api_object_fixture(
    ...
    with patch(
        "menuai.components.forked_daapd.media_player.LibrespotJavaAPI",
        autospec=True,
    ) as pipe_control_api:
        menuai.config_entries.async_update_entry(config_entry, options=OPTIONS_DATA)
        await menuai.async_block_till_done()
...
async def test_librespot_java_play_announcement(menuai: menuai) -> None:
...
async def test_librespot_java_play_media_pause_timeout(
    menuai: menuai, pipe_control_api_object: Mock
) -> None:
    ...
    with patch(
        "menuai.components.forked_daapd.media_player.CALLBACK_TIMEOUT", 0
    ):
        initial_state = menuai.states.get(TEST_MASTER_ENTITY_NAME)
        await _service_call(
            menuai,
...
async def test_unsupported_update(menuai: menuai, mock_api_object: Mock) -> None:
    """Test unsupported update type."""
    last_updated = menuai.states.get(TEST_MASTER_ENTITY_NAME).last_updated
    updater_update = mock_api_object.start_websocket_handler.call_args[0][2]
    await updater_update(["config"])
    await menuai.async_block_till_done()
    assert menuai.states.get(TEST_MASTER_ENTITY_NAME).last_updated == last_updated
...
async def test_websocket_disconnect(menuai: menuai, mock_api_object: Mock) -> None:
...
async def test_play_owntone_media(menuai: menuai, mock_api_object: Mock) -> None:
...
async def test_play_spotify_media(menuai: menuai, mock_api_object: Mock) -> None:
...
async def test_play_media_source(menuai: menuai, mock_api_object: Mock) -> None:
    """Test async play media with a spotify source."""
    initial_state = menuai.states.get(TEST_MASTER_ENTITY_NAME)
    with patch(
        "menuai.components.media_source.async_resolve_media",
        return_value=PlayMedia("http://my_menuai/song.m4a", "audio/aac"),
    ):
        await _service_call(
            menuai,
            TEST_MASTER_ENTITY_NAME,
