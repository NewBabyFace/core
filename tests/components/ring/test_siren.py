...
async def test_entity_registry(
    ...
    await setup_platform(menuai, Platform.SIREN)
...
async def test_sirens_report_correctly(menuai: menuai, mock_ring_client) -> None:
    """Tests that the initial state of a device that should be on is correct."""
    await setup_platform(menuai, Platform.SIREN)

    state = menuai.states.get("siren.downstairs_siren")
    assert state.attributes.get("friendly_name") == "Downstairs Siren"
    assert state.state == "unknown"


async def test_default_ding_chime_can_be_played(
    menuai: menuai, mock_ring_client, mock_ring_devices
) -> None:
