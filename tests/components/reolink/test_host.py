...
from menuai.components.reolink.host import (
...
async def test_webhook_callback(
    ...
    signal_ch = MagicMock()
    async_dispatcher_connect(menuai, f"{unique_id}_all", signal_all)
    async_dispatcher_connect(menuai, f"{unique_id}_0", signal_ch)
...
async def test_register_webhook_errors(
    ...
    with patch(
        "menuai.components.reolink.host.get_url",
