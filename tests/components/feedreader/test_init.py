...
from menuai.components.feedreader.const import DOMAIN
...
async def test_setup(
    ...
    assert await async_setup_config_entry(menuai, config, return_value=feed_one_event)
...
async def test_storage_data_writing(
    ...
    with (
        patch("menuai.components.feedreader.coordinator.DELAY_SAVE", new=0),
    ):
        assert await async_setup_config_entry(
            menuai, VALID_CONFIG_DEFAULT, return_value=feed_one_event
        )
    ...
    assert menuai_storage[DOMAIN]["data"] == storage_data


async def test_feed(menuai: menuai, events, feed_one_event) -> None:
...
async def test_atom_feed(menuai: menuai, events, feed_atom_event) -> None:
    """Test simple atom feed with valid data."""
    assert await async_setup_config_entry(
        menuai, VALID_CONFIG_DEFAULT, return_value=feed_atom_event
    )
...
async def test_feed_identical_timestamps(
    menuai: menuai, events, feed_identically_timed_events
) -> None:
    """Test feed with 2 entries with identical timestamps."""
    with (
        patch(
            "menuai.components.feedreader.coordinator.StoredData.get_timestamp",
            return_value=gmtime(
        ...
        assert await async_setup_config_entry(
            menuai, VALID_CONFIG_DEFAULT, return_value=feed_identically_timed_events
        )
...
async def test_feed_with_only_summary(
    menuai: menuai, events, feed_only_summary
) -> None:
    """Test simple feed with only summary, no content."""
    assert await async_setup_config_entry(
        menuai, VALID_CONFIG_DEFAULT, return_value=feed_only_summary
    )
...
async def test_feed_updates(
    menuai: menuai, events, feed_one_event, feed_two_event
) -> None:
    """Test feed updates."""
    side_effect = [
        feed_one_event,
...
async def test_feed_default_max_length(
    menuai: menuai, events, feed_21_events
) -> None:
    """Test long feed beyond the default 20 entry limit."""
    assert await async_setup_config_entry(
        menuai, VALID_CONFIG_DEFAULT, return_value=feed_21_events
    )
    await menuai.async_block_till_done()
...
async def test_feed_max_length(menuai: menuai, events, feed_21_events) -> None:
    """Test long feed beyond a configured 5 entry limit."""
    assert await async_setup_config_entry(
        menuai, VALID_CONFIG_5, return_value=feed_21_events
    )
    await menuai.async_block_till_done()
...
async def test_feed_without_publication_date_and_title(
    menuai: menuai, events, feed_three_events
) -> None:
    """Test simple feed with entry without publication date and title."""
    assert await async_setup_config_entry(
        menuai, VALID_CONFIG_DEFAULT, return_value=feed_three_events
    )
    await menuai.async_block_till_done()
...
async def test_feed_with_unrecognized_publication_date(
    menuai: menuai, events, feed_four_events
) -> None:
    """Test simple feed with entry with unrecognized publication date."""
    assert await async_setup_config_entry(
        menuai, VALID_CONFIG_DEFAULT, return_value=feed_four_events
    )
    await menuai.async_block_till_done()
...
async def test_feed_without_items(
    menuai: menuai, events, feed_without_items, caplog: pytest.LogCaptureFixture
) -> None:
    """Test simple feed without any items."""
    assert "No new entries to be published in feed" not in caplog.text
    assert await async_setup_config_entry(
        menuai, VALID_CONFIG_DEFAULT, return_value=feed_without_items
    )
    await menuai.async_block_till_done()
...
async def test_feed_invalid_data(menuai: menuai, events) -> None:
    """Test feed with invalid data."""
    assert await async_setup_config_entry(
        menuai, VALID_CONFIG_DEFAULT, return_value=bytes("INVALID DATA", "utf-8")
    )
    await menuai.async_block_till_done()
...
async def test_feed_parsing_failed(
    menuai: menuai, events, feed_one_event, caplog: pytest.LogCaptureFixture
) -> None:
...
async def test_feed_errors(
    ...
    with patch(
        ...
        freezer.tick(timedelta(hours=1, seconds=1))
        with patch(
            "menuai.components.feedreader.coordinator.feedparser.parse",
            return_value=None,
        ):
            async_fire_time_changed(menuai)
            await menuai.async_block_till_done(wait_background_tasks=True)
...
async def test_feed_atom_htmlentities(
    menuai: menuai, feed_atom_htmlentities, device_registry: dr.DeviceRegistry
) -> None:
