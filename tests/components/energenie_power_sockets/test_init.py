...
async def test_device_not_found_on_load_entry(
    ...
    assert not await menuai.config_entries.async_setup(valid_config_entry.entry_id)
    await menuai.async_block_till_done()
...
async def test_usb_error(
    menuai: menuai, valid_config_entry: MockConfigEntry, mock_get_device: MagicMock
) -> None:
