...
async def test_setup_and_remove_config_entry(
    ...
    state = menuai.states.get(derivative_entity_id)
    assert state.state == "0.0"
    assert "unit_of_measurement" not in state.attributes
    assert state.attributes["source"] == "sensor.input"

    menuai.states.async_set(input_sensor_entity_id, 10, {"unit_of_measurement": "dog"})
    menuai.states.async_set(input_sensor_entity_id, 11, {"unit_of_measurement": "dog"})
    await menuai.async_block_till_done()
    state = menuai.states.get(derivative_entity_id)
    ...
    assert menuai.states.get(derivative_entity_id) is None
...
async def test_device_cleaning(
    ...
    # Config entry reload
    await menuai.config_entries.async_reload(derivative_config_entry.entry_id)
    await menuai.async_block_till_done()
