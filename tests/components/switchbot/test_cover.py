...
async def test_curtain3_setup(
    menuai: menuai, mock_entry_factory: Callable[[str], MockConfigEntry]
) -> None:
    """Test setting up the Curtain3."""
    inject_bluetooth_service_info(menuai, WOCURTAIN3_SERVICE_INFO)
...
async def test_curtain3_controlling(
    ...
    with (
        patch(
            "menuai.components.switchbot.cover.switchbot.SwitchbotCurtain.open",
            new=AsyncMock(return_value=True),
        ) as mock_open,
        patch(
            "menuai.components.switchbot.cover.switchbot.SwitchbotCurtain.close",
            new=AsyncMock(return_value=True),
        ) as mock_close,
        patch(
            "menuai.components.switchbot.cover.switchbot.SwitchbotCurtain.stop",
            new=AsyncMock(return_value=True),
        ) as mock_stop,
        patch(
            "menuai.components.switchbot.cover.switchbot.SwitchbotCurtain.set_position",
            new=AsyncMock(return_value=True),
        ...
        inject_bluetooth_service_info(
            menuai, make_advertisement(address, manufacturer_data, service_data)
        )
        await menuai.async_block_till_done()
...
async def test_blindtilt_setup(
    ...
    inject_bluetooth_service_info(menuai, WOBLINDTILT_SERVICE_INFO)
    ...
    entry.add_to_menuai(menuai)
    with patch(
        "menuai.components.switchbot.cover.switchbot.SwitchbotBlindTilt.update",
        new=AsyncMock(return_value=True),
    ):
        assert await menuai.config_entries.async_setup(entry.entry_id)
        await menuai.async_block_till_done()
...
async def test_blindtilt_controlling(
    ...
    with (
        patch(
            "menuai.components.switchbot.cover.switchbot.SwitchbotBlindTilt.get_basic_info",
            new=AsyncMock(return_value=info),
        ),
        patch(
            "menuai.components.switchbot.cover.switchbot.SwitchbotBlindTilt.open",
            new=AsyncMock(return_value=True),
        ) as mock_open,
        patch(
            "menuai.components.switchbot.cover.switchbot.SwitchbotBlindTilt.close",
            new=AsyncMock(return_value=True),
        ) as mock_close,
        patch(
            "menuai.components.switchbot.cover.switchbot.SwitchbotBlindTilt.stop",
            new=AsyncMock(return_value=True),
        ) as mock_stop,
        patch(
            "menuai.components.switchbot.cover.switchbot.SwitchbotBlindTilt.set_position",
            new=AsyncMock(return_value=True),
        ) as mock_set_position,
    ):
        assert await menuai.config_entries.async_setup(entry.entry_id)
...
async def test_roller_shade_setup(
    ...
    entry.add_to_menuai(menuai)
    with patch(
        "menuai.components.switchbot.cover.switchbot.SwitchbotRollerShade.update",
        new=AsyncMock(return_value=True),
    ):
        assert await menuai.config_entries.async_setup(entry.entry_id)
        await menuai.async_block_till_done()
...
async def test_roller_shade_controlling(
    ...
    inject_bluetooth_service_info(menuai, ROLLER_SHADE_SERVICE_INFO)
    ...
    with (
        patch(
            "menuai.components.switchbot.cover.switchbot.SwitchbotRollerShade.get_basic_info",
            new=AsyncMock(return_value=info),
        ),
        patch(
            "menuai.components.switchbot.cover.switchbot.SwitchbotRollerShade.open",
            new=AsyncMock(return_value=True),
        ) as mock_open,
        patch(
            "menuai.components.switchbot.cover.switchbot.SwitchbotRollerShade.close",
            new=AsyncMock(return_value=True),
        ) as mock_close,
        patch(
            "menuai.components.switchbot.cover.switchbot.SwitchbotRollerShade.stop",
            new=AsyncMock(return_value=True),
        ) as mock_stop,
        patch(
            "menuai.components.switchbot.cover.switchbot.SwitchbotRollerShade.set_position",
            new=AsyncMock(return_value=True),
        ) as mock_set_position,
    ):
        assert await menuai.config_entries.async_setup(entry.entry_id)
...
async def test_exception_handling_cover_service(
    ...
    with patch.multiple(
        f"menuai.components.switchbot.cover.switchbot.{class_name}",
