...
async def test_sensors(
    ...
    """Test sensors."""
    snapshot_matter_entities(menuai, entity_registry, snapshot, Platform.SENSOR)
...
async def test_sensor_null_value(
    ...
    """Test flow sensor."""
    state = menuai.states.get("sensor.mock_flow_sensor_flow")
    assert state
...
async def test_humidity_sensor(
    ...
    state = menuai.states.get("sensor.mock_humidity_sensor_humidity")
    assert state
    assert state.state == "0.0"
...
async def test_light_sensor(
    ...
    """Test light sensor."""
    state = menuai.states.get("sensor.mock_light_sensor_illuminance")
    assert state
...
async def test_temperature_sensor(
    ...
    state = menuai.states.get("sensor.mock_temperature_sensor_temperature")
    assert state
...
async def test_battery_sensor_description(
    ...
    state = menuai.states.get("sensor.smoke_sensor_battery_type")
    assert state
    ...
    state = menuai.states.get("sensor.smoke_sensor_battery_type") is None
...
async def test_eve_thermo_sensor(
    ...
    """Test Eve Thermo."""
    # Valve position
    state = menuai.states.get("sensor.eve_thermo_valve_position")
    assert state
    ...
    # LocalTemperature
    state = menuai.states.get("sensor.eve_thermo_temperature")
    assert state
    assert state.state == "21.0"
...
async def test_pressure_sensor(
    ...
    state = menuai.states.get("sensor.mock_pressure_sensor_pressure")
    assert state
    assert state.state == "0.0"
...
async def test_eve_weather_sensor_custom_cluster(
    ...
    state = menuai.states.get("sensor.eve_weather_pressure")
    assert state
...
async def test_air_quality_sensor(
    ...
    # Carbon Dioxide
    state = menuai.states.get("sensor.lightfi_aq1_air_quality_sensor_carbon_dioxide")
    assert state
    ...
    # PM1
    state = menuai.states.get("sensor.lightfi_aq1_air_quality_sensor_pm1")
    assert state
    ...
    # PM2.5
    state = menuai.states.get("sensor.lightfi_aq1_air_quality_sensor_pm2_5")
    assert state
    assert state.state == "3.0"
    ...
    # PM10
    state = menuai.states.get("sensor.lightfi_aq1_air_quality_sensor_pm10")
    assert state
    assert state.state == "3.0"
...
async def test_operational_state_sensor(
    ...
    state = menuai.states.get("sensor.dishwasher_operational_state")
    assert state
...
async def test_draft_electrical_measurement_sensor(
    ...
    state = menuai.states.get("sensor.yndx_00540_power")
    assert state
...
async def test_list_sensor(
    ...
    state = menuai.states.get("sensor.laundrywasher_current_phase")
    assert state
...
async def test_evse_sensor(
    ...
    state = menuai.states.get("sensor.evse_fault_state")
    assert state
    ...
    state = menuai.states.get("sensor.evse_circuit_capacity")
    assert state
    ...
    state = menuai.states.get("sensor.evse_min_charge_current")
    assert state
    assert state.state == "2.0"
    ...
    state = menuai.states.get("sensor.evse_max_charge_current")
    assert state
    ...
    state = menuai.states.get("sensor.evse_user_max_charge_current")
    assert state
    assert state.state == "32.0"
...
async def test_water_heater(
    ...
    # TankVolume
    state = menuai.states.get("sensor.water_heater_tank_volume")
    assert state
    ...
    state = menuai.states.get("sensor.water_heater_required_heating_energy")
    assert state
    ...
    # TankPercentage
    state = menuai.states.get("sensor.water_heater_hot_water_level")
    assert state
    ...
    state = menuai.states.get("sensor.water_heater_appliance_energy_state")
    assert state
...
async def test_pump(
    ...
    """Test pump sensors."""
    # ControlMode
    state = menuai.states.get("sensor.mock_pump_control_mode")
    assert state
    ...
    # Speed
    state = menuai.states.get("sensor.mock_pump_rotation_speed")
    assert state
