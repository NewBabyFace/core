...
from menuai.components import folder_watcher
...
def test_event() -> None:
    """Check that MenuAI events are fired correctly on watchdog event."""
    ...
    with patch(
        "menuai.components.folder_watcher.PatternMatchingEventHandler",
        MockPatternMatchingEventHandler,
    ):
        menuai = Mock()
        handler = folder_watcher.create_event_handler(["*"], menuai, "1")
        handler.on_created(
            SimpleNamespace(
                is_directory=False, src_path="/hello/world.txt", event_type="created"
            )
        )
        assert menuai.bus.fire.called
        assert menuai.bus.fire.mock_calls[0][1][0] == folder_watcher.DOMAIN
        assert menuai.bus.fire.mock_calls[0][1][1] == {
            "event_type": "created",
