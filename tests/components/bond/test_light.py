...
from menuai.components.bond.light import (
...
async def test_sbb_trust_state(menuai: menuai) -> None:
    ...
    await setup_platform(
        menuai, LIGHT_DOMAIN, ceiling_fan("name-1"), bond_version=version, bridge={}
    )

    device = menuai.states.get("light.name_1")
    assert device.attributes.get(ATTR_ASSUMED_STATE) is not True


async def test_trust_state_not_specified(menuai: menuai) -> None:
    """Assumed state should be True if Trust State is not specified."""
    await setup_platform(menuai, LIGHT_DOMAIN, ceiling_fan("name-1"))

    device = menuai.states.get("light.name_1")
    assert device.attributes.get(ATTR_ASSUMED_STATE) is True


async def test_trust_state(menuai: menuai) -> None:
    """Assumed state should be True if Trust State is False."""
    await setup_platform(
        menuai, LIGHT_DOMAIN, ceiling_fan("name-1"), props={"trust_state": False}
    )
...
async def test_no_trust_state(menuai: menuai) -> None:
    """Assumed state should be False if Trust State is True."""
    await setup_platform(
        menuai, LIGHT_DOMAIN, ceiling_fan("name-1"), props={"trust_state": True}
    )
    device = menuai.states.get("light.name_1")
    assert device.attributes.get(ATTR_ASSUMED_STATE) is not True


async def test_light_set_brightness_belief_full(menuai: menuai) -> None:
...
async def test_light_set_brightness_belief_api_error(menuai: menuai) -> None:
    """Tests that the set brightness belief throws menuaiError in the event of an api error."""
    await setup_platform(
        menuai,
        LIGHT_DOMAIN,
        light("name-1"),
...
async def test_fp_light_set_brightness_belief_full(menuai: menuai) -> None:
    """Tests that the set brightness belief function of a light delegates to API."""
    await setup_platform(
        menuai,
        LIGHT_DOMAIN,
...
async def test_fp_light_set_brightness_belief_api_error(menuai: menuai) -> None:
    """Tests that the set brightness belief throws menuaiError in the event of an api error."""
    await setup_platform(
        menuai,
        LIGHT_DOMAIN,
...
async def test_light_set_brightness_belief_brightness_not_supported(
    ...
    with pytest.raises(menuaiError), patch_bond_device_state():
        await menuai.services.async_call(
            DOMAIN,
            SERVICE_SET_LIGHT_BRIGHTNESS_TRACKED_STATE,
            {ATTR_ENTITY_ID: "light.name_1", ATTR_BRIGHTNESS: 255},
...
async def test_light_set_brightness_belief_zero(menuai: menuai) -> None:
    """Tests that the set brightness belief function of a light delegates to API."""
    await setup_platform(
        menuai,
        LIGHT_DOMAIN,
...
async def test_fp_light_set_brightness_belief_zero(menuai: menuai) -> None:
    """Tests that the set brightness belief function of a light delegates to API."""
    await setup_platform(
        menuai,
        LIGHT_DOMAIN,
...
async def test_light_set_power_belief(menuai: menuai) -> None:
    """Tests that the set brightness belief function of a light delegates to API."""
    await setup_platform(
        menuai,
        LIGHT_DOMAIN,
...
async def test_light_set_power_belief_api_error(menuai: menuai) -> None:
    """Tests that the set brightness belief function of a light throws menuaiError in the event of an api error."""
    await setup_platform(
        menuai,
        LIGHT_DOMAIN,
        light("name-1"),
...
async def test_fp_light_set_power_belief(menuai: menuai) -> None:
    """Tests that the set brightness belief function of a light delegates to API."""
    await setup_platform(
        menuai,
        LIGHT_DOMAIN,
...
async def test_fp_light_set_power_belief_api_error(menuai: menuai) -> None:
    """Tests that the set brightness belief function of a light throws menuaiError in the event of an api error."""
    await setup_platform(
        menuai,
        LIGHT_DOMAIN,
...
async def test_light_start_increasing_brightness(menuai: menuai) -> None:
...
async def test_light_start_increasing_brightness_missing_service(
    ...
    await setup_platform(
        menuai, LIGHT_DOMAIN, light("name-1"), bond_device_id="test-device-id"
    )
...
async def test_light_start_decreasing_brightness(menuai: menuai) -> None:
...
async def test_light_stop(menuai: menuai) -> None:
...
async def test_turn_on_light(menuai: menuai) -> None:
    """Tests that turn on command delegates to API."""
    await setup_platform(
        menuai, LIGHT_DOMAIN, ceiling_fan("name-1"), bond_device_id="test-device-id"
    )
...
async def test_turn_off_light(menuai: menuai) -> None:
...
async def test_brightness_support(menuai: menuai) -> None:
    ...
    state = menuai.states.get("light.name_1")
...
async def test_brightness_not_supported(menuai: menuai) -> None:
...
async def test_turn_on_light_with_brightness(menuai: menuai) -> None:
...
async def test_turn_on_up_light(menuai: menuai) -> None:
...
async def test_turn_off_up_light(menuai: menuai) -> None:
...
async def test_turn_on_down_light(menuai: menuai) -> None:
...
async def test_turn_off_down_light(menuai: menuai) -> None:
...
async def test_update_reports_light_is_on(menuai: menuai) -> None:
    ...
    assert menuai.states.get("light.name_1").state == "on"


async def test_update_reports_light_is_off(menuai: menuai) -> None:
    ...
    assert menuai.states.get("light.name_1").state == "off"


async def test_update_reports_up_light_is_on(menuai: menuai) -> None:
    """Tests that update command sets correct state when Bond API reports the up light is on."""
    await setup_platform(menuai, LIGHT_DOMAIN, up_light_ceiling_fan("name-1"))

    with patch_bond_device_state(return_value={"up_light": 1, "light": 1}):
        async_fire_time_changed(menuai, utcnow() + timedelta(seconds=30))
        await menuai.async_block_till_done()

    assert menuai.states.get("light.name_1_up_light").state == "on"


async def test_update_reports_up_light_is_off(menuai: menuai) -> None:
    ...
    assert menuai.states.get("light.name_1_up_light").state == "off"


async def test_update_reports_down_light_is_on(menuai: menuai) -> None:
    """Tests that update command sets correct state when Bond API reports the down light is on."""
    await setup_platform(menuai, LIGHT_DOMAIN, down_light_ceiling_fan("name-1"))

    with patch_bond_device_state(return_value={"down_light": 1, "light": 1}):
        async_fire_time_changed(menuai, utcnow() + timedelta(seconds=30))
        await menuai.async_block_till_done()

    assert menuai.states.get("light.name_1_down_light").state == "on"


async def test_update_reports_down_light_is_off(menuai: menuai) -> None:
    ...
    assert menuai.states.get("light.name_1_down_light").state == "off"


async def test_turn_on_fireplace_with_brightness(menuai: menuai) -> None:
    """Tests that turn on command delegates to set flame API."""
    await setup_platform(
        menuai, LIGHT_DOMAIN, fireplace("name-1"), bond_device_id="test-device-id"
    )
...
async def test_turn_on_fireplace_without_brightness(menuai: menuai) -> None:
...
async def test_turn_off_fireplace(menuai: menuai) -> None:
    """Tests that turn off command delegates to API."""
    await setup_platform(
        menuai, LIGHT_DOMAIN, fireplace("name-1"), bond_device_id="test-device-id"
    )
...
async def test_flame_converted_to_brightness(menuai: menuai) -> None:
    """Tests that reported flame level (0..100) converted to HA brightness (0...255)."""
    await setup_platform(menuai, LIGHT_DOMAIN, fireplace("name-1"))

    with patch_bond_device_state(return_value={"power": 1, "flame": 50}):
        async_fire_time_changed(menuai, utcnow() + timedelta(seconds=30))
        await menuai.async_block_till_done()

    assert menuai.states.get("light.name_1").attributes[ATTR_BRIGHTNESS] == 128


async def test_light_available(menuai: menuai) -> None:
    """Tests that available state is updated based on API errors."""
    await help_test_entity_available(
        menuai, LIGHT_DOMAIN, ceiling_fan("name-1"), "light.name_1"
    )


async def test_parse_brightness(menuai: menuai) -> None:
    """Tests that reported brightness level (0..100) converted to HA brightness (0...255)."""
    await setup_platform(menuai, LIGHT_DOMAIN, dimmable_ceiling_fan("name-1"))
