...
from menuai.components.light import ATTR_BRIGHTNESS, ATTR_TRANSITION
from menuai.components.litejet.const import CONF_DEFAULT_TRANSITION
...
async def test_on_brightness(menuai: menuai, mock_litejet) -> None:
    """Test turning the light on with brightness."""
    await async_init_integration(menuai)

    assert menuai.states.get(ENTITY_LIGHT).state == "off"
    assert menuai.states.get(ENTITY_OTHER_LIGHT).state == "off"

    assert not light.is_on(menuai, ENTITY_LIGHT)
...
async def test_default_transition(menuai: menuai, mock_litejet) -> None:
    """Test turning the light on with the default transition option."""
    entry = await async_init_integration(menuai)

    menuai.config_entries.async_update_entry(entry, options={CONF_DEFAULT_TRANSITION: 12})
...
async def test_transition(menuai: menuai, mock_litejet) -> None:
...
async def test_activated_event(menuai: menuai, mock_litejet) -> None:
    ...
    assert light.is_on(menuai, ENTITY_LIGHT)
    assert not light.is_on(menuai, ENTITY_OTHER_LIGHT)
    assert menuai.states.get(ENTITY_LIGHT).state == "on"
    assert menuai.states.get(ENTITY_OTHER_LIGHT).state == "off"
    assert menuai.states.get(ENTITY_LIGHT).attributes.get(ATTR_BRIGHTNESS) == 255
    ...
    assert light.is_on(menuai, ENTITY_LIGHT)
    assert light.is_on(menuai, ENTITY_OTHER_LIGHT)
    assert menuai.states.get(ENTITY_LIGHT).state == "on"
    assert menuai.states.get(ENTITY_OTHER_LIGHT).state == "on"
    assert menuai.states.get(ENTITY_OTHER_LIGHT).attributes.get(ATTR_BRIGHTNESS) == 103


async def test_deactivated_event(menuai: menuai, mock_litejet) -> None:
    """Test handling an event from LiteJet."""
    await async_init_integration(menuai)
...
async def test_connected_event(menuai: menuai, mock_litejet) -> None:
    ...
    assert menuai.states.get(ENTITY_LIGHT).state == STATE_OFF
