...
async def setup_fixture(
    ...
    if request.param == "mock_setup":
        ...
        await mock_setup(menuai, tmp_path, provider)
...
async def mock_setup(
    ...
    assert await async_setup_component(menuai, "stt", {"stt": {"platform": TEST_DOMAIN}})
...
async def mock_config_entry_setup(
    ...
    async def async_unload_entry_init(
        ...
        await menuai.config_entries.async_forward_entry_unload(config_entry, Platform.STT)
        return True
    ...
    mock_stt_entity_platform(menuai, tmp_path, test_domain, async_setup_entry_platform)
...
async def test_get_provider(
    ...
    await mock_setup(menuai, tmp_path, mock_provider)
    assert mock_provider == async_get_provider(menuai, TEST_DOMAIN)

    # Test getting the default provider
    assert mock_provider == async_get_provider(menuai)


async def test_config_entry_unload(
    menuai: menuai, tmp_path: Path, mock_provider_entity: MockSTTProviderEntity
) -> None:
...
async def test_default_engine_none(menuai: menuai, tmp_path: Path) -> None:
    ...
    assert await async_setup_component(menuai, "stt", {"stt": {}})
    await menuai.async_block_till_done()

    assert async_default_engine(menuai) is None
...
async def test_default_engine(
    ...
    assert async_default_engine(menuai) == TEST_DOMAIN
...
async def test_default_engine_entity(
    ...
    assert async_default_engine(menuai) == f"{DOMAIN}.{TEST_DOMAIN}"
...
async def test_default_engine_prefer_entity(
    ...
    await mock_config_entry_setup(
        menuai, tmp_path, mock_provider_entity, test_domain=config_flow_test_domains[0]
    )
    await menuai.async_block_till_done()

    entity_engine = async_get_speech_to_text_engine(menuai, "stt.new_test")
    assert entity_engine is not None
    assert entity_engine.name == "New test"
    provider_engine = async_get_speech_to_text_engine(menuai, "test")
    assert provider_engine is not None
    assert provider_engine.name == "test"
    assert async_default_engine(menuai) == "stt.new_test"
...
async def test_default_engine_prefer_cloud_entity(
    ...
    for domain in config_flow_test_domains:
        ...
        await mock_config_entry_setup(menuai, tmp_path, entity, test_domain=domain)
    await menuai.async_block_till_done()

    for domain in config_flow_test_domains:
        entity_engine = async_get_speech_to_text_engine(
            menuai, f"stt.{domain}_stt_entity"
        )
        assert entity_engine is not None
    ...
    assert provider_engine is not None
    assert provider_engine.name == "test"
    assert async_default_engine(menuai) == "stt.cloud_stt_entity"


async def test_get_engine_legacy(
    menuai: menuai, tmp_path: Path, mock_provider: MockSTTProvider
) -> None:
    ...
    assert await async_setup_component(
        menuai, "stt", {"stt": [{"platform": TEST_DOMAIN}, {"platform": "cloud"}]}
    )
    await menuai.async_block_till_done()

    assert async_get_speech_to_text_engine(menuai, "no_such_provider") is None
    assert async_get_speech_to_text_engine(menuai, "test") is mock_provider
...
async def test_get_engine_entity(
    ...
    assert async_get_speech_to_text_engine(menuai, "stt.test") is mock_provider_entity
