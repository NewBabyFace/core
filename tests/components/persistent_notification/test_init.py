...
async def test_create(menuai: menuai) -> None:
    """Test creating notification without title or notification id."""
    notifications = pn._async_get_or_create_notifications(menuai)
    assert len(menuai.states.async_entity_ids(pn.DOMAIN)) == 0
    assert len(notifications) == 0

    pn.async_create(menuai, "Hello World 2", title="2 beers")
...
async def test_create_notification_id(menuai: menuai) -> None:
    """Ensure overwrites existing notification with same id."""
    notifications = pn._async_get_or_create_notifications(menuai)
    assert len(menuai.states.async_entity_ids(pn.DOMAIN)) == 0
    assert len(notifications) == 0

    pn.async_create(menuai, "test", notification_id="Beer 2")
    ...
    pn.async_create(menuai, "test 2", notification_id="Beer 2")
...
async def test_dismiss_notification(menuai: menuai) -> None:
    ...
    assert len(notifications) == 1
    pn.async_dismiss(menuai, notification_id="Does Not Exist")

    assert len(notifications) == 1

    pn.async_dismiss(menuai, notification_id="Beer 2")

    assert len(notifications) == 0


async def test_dismiss_all_notifications(menuai: menuai) -> None:
    """Ensure removal of all notifications."""
    notifications = pn._async_get_or_create_notifications(menuai)
    assert len(notifications) == 0

    pn.async_create(menuai, "test", notification_id="Beer 2")
    pn.async_create(menuai, "test", notification_id="Beer 3")
    pn.async_create(menuai, "test", notification_id="Beer 4")
    pn.async_create(menuai, "test", notification_id="Beer 5")

    assert len(notifications) == 4
    pn.async_dismiss_all(menuai)
...
async def test_ws_get_notifications(
    ...
    await async_setup_component(menuai, pn.DOMAIN, {})
    ...
    # Dismiss
    pn.async_dismiss(menuai, "Beer 2")
...
async def test_manual_notification_id_round_trip(menuai: menuai) -> None:
...
async def test_manual_dismiss_all(menuai: menuai) -> None:
