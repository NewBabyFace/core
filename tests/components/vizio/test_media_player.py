...
async def _add_config_entry_to_menuai(
...
def _get_attr_and_assert_base_attr(
    menuai: menuai, device_class: str, power_state: str
) -> dict[str, Any]:
    """Return entity attributes  after asserting name, device class, and power state."""
    attr = menuai.states.get(ENTITY_ID).attributes
    assert attr["friendly_name"] == NAME
    assert attr["device_class"] == device_class

    assert menuai.states.get(ENTITY_ID).state == power_state
    return attr
...
async def _cm_for_test_setup_without_apps(
    ...
    with (
        patch(
            "menuai.components.vizio.media_player.VizioAsync.get_all_settings",
            return_value=all_settings,
        ),
        patch(
            "menuai.components.vizio.media_player.VizioAsync.get_setting_options",
            return_value=EQ_LIST,
        ),
        patch(
            "menuai.components.vizio.media_player.VizioAsync.get_power_state",
...
async def _test_setup_tv(menuai: menuai, vizio_power_state: bool | None) -> None:
    ...
    async with _cm_for_test_setup_without_apps(
        ...
        await _add_config_entry_to_menuai(menuai, config_entry)

        attr = _get_attr_and_assert_base_attr(
            menuai, MediaPlayerDeviceClass.TV, ha_power_state
        )
...
async def _test_setup_speaker(
    menuai: menuai, vizio_power_state: bool | None
) -> None:
    ...
    async with _cm_for_test_setup_without_apps(
        ...
        with patch(
            "menuai.components.vizio.media_player.VizioAsync.get_current_app_config",
        ) as service_call:
            await _add_config_entry_to_menuai(menuai, config_entry)

            attr = _get_attr_and_assert_base_attr(
                menuai, MediaPlayerDeviceClass.SPEAKER, ha_power_state
            )
            if ha_power_state == STATE_ON:
...
async def _cm_for_test_setup_tv_with_apps(
    menuai: menuai, device_config: dict[str, Any], app_config: dict[str, Any]
) -> AsyncIterator[None]:
    ...
    async with _cm_for_test_setup_without_apps(
        ...
        with patch(
            ...
            attr = _get_attr_and_assert_base_attr(
                menuai, MediaPlayerDeviceClass.TV, STATE_ON
            )
            assert (
                attr["volume_level"]
...
async def _test_service(
    ...
    **kwargs,
    ...
    with patch(
        f"menuai.components.vizio.media_player.VizioAsync.{vizio_func_name}"
    ) as service_call:
...
async def test_speaker_on(menuai: menuai) -> None:
    """Test Vizio Speaker entity setup when on."""
    await _test_setup_speaker(menuai, True)


@pytest.mark.usefixtures("vizio_connect", "vizio_update")
async def test_speaker_off(menuai: menuai) -> None:
    """Test Vizio Speaker entity setup when off."""
    await _test_setup_speaker(menuai, False)
...
async def test_speaker_unavailable(
    ...
    await _test_setup_speaker(menuai, None)


@pytest.mark.usefixtures("vizio_connect", "vizio_update")
async def test_init_tv_on(menuai: menuai) -> None:
    """Test Vizio TV entity setup when on."""
    await _test_setup_tv(menuai, True)


@pytest.mark.usefixtures("vizio_connect", "vizio_update")
async def test_init_tv_off(menuai: menuai) -> None:
    """Test Vizio TV entity setup when off."""
    await _test_setup_tv(menuai, False)


@pytest.mark.usefixtures("vizio_connect", "vizio_update")
async def test_init_tv_unavailable(menuai: menuai) -> None:
    """Test Vizio TV entity setup when unavailable."""
    await _test_setup_tv(menuai, None)


@pytest.mark.usefixtures("vizio_cant_connect")
async def test_setup_unavailable_speaker(menuai: menuai) -> None:
    """Test speaker entity sets up as unavailable."""
    config_entry = MockConfigEntry(
        domain=DOMAIN, data=MOCK_SPEAKER_CONFIG, unique_id=UNIQUE_ID
    )
    await _add_config_entry_to_menuai(menuai, config_entry)
    assert len(menuai.states.async_entity_ids(MP_DOMAIN)) == 1
    assert menuai.states.get("media_player.vizio").state == STATE_UNAVAILABLE


@pytest.mark.usefixtures("vizio_cant_connect")
async def test_setup_unavailable_tv(menuai: menuai) -> None:
...
async def test_services(menuai: menuai) -> None:
    ...
    await _test_service(menuai, MP_DOMAIN, "pow_on", SERVICE_TURN_ON, None)
    await _test_service(menuai, MP_DOMAIN, "pow_off", SERVICE_TURN_OFF, None)
    await _test_service(
        menuai,
        MP_DOMAIN,
        "mute_on",
    ...
    await _test_service(
        menuai, MP_DOMAIN, "vol_up", SERVICE_VOLUME_UP, None, num=DEFAULT_VOLUME_STEP
    )
    await _test_service(
        menuai, MP_DOMAIN, "vol_down", SERVICE_VOLUME_DOWN, None, num=DEFAULT_VOLUME_STEP
    )
    await _test_service(
        menuai,
        MP_DOMAIN,
    ...
    await _test_service(menuai, MP_DOMAIN, "ch_up", SERVICE_MEDIA_NEXT_TRACK, None)
    await _test_service(menuai, MP_DOMAIN, "ch_down", SERVICE_MEDIA_PREVIOUS_TRACK, None)
    await _test_service(
        menuai,
        MP_DOMAIN,
        "set_setting",
    ...
    await _test_service(menuai, MP_DOMAIN, "play", SERVICE_MEDIA_PLAY, None)
    await _test_service(menuai, MP_DOMAIN, "pause", SERVICE_MEDIA_PAUSE, None)


@pytest.mark.usefixtures("vizio_connect", "vizio_update")
async def test_options_update(menuai: menuai) -> None:
    ...
    await menuai.async_block_till_done()
    await _test_service(
        menuai, MP_DOMAIN, "vol_up", SERVICE_VOLUME_UP, None, num=VOLUME_STEP
    )
...
async def _test_update_availability_switch(
    ...
    with freeze_time(now):
        await _test_setup_speaker(menuai, initial_power_state)
    ...
    vizio_log_list = [
        ...
        if log.name == "menuai.components.vizio.media_player"
    ]
...
async def test_update_unavailable_to_available(
    ...
    await _test_update_availability_switch(menuai, None, True, caplog)
...
async def test_update_available_to_unavailable(
    ...
    await _test_update_availability_switch(menuai, True, None, caplog)
...
async def test_setup_with_apps(
    ...
    async with _cm_for_test_setup_tv_with_apps(
        menuai, MOCK_USER_VALID_TV_CONFIG, CURRENT_APP_CONFIG
    ):
        attr = menuai.states.get(ENTITY_ID).attributes
...
async def test_setup_with_apps_include(
    ...
    async with _cm_for_test_setup_tv_with_apps(
        menuai, MOCK_TV_WITH_INCLUDE_CONFIG, CURRENT_APP_CONFIG
    ):
        attr = menuai.states.get(ENTITY_ID).attributes
...
async def test_setup_with_apps_exclude(
    ...
    async with _cm_for_test_setup_tv_with_apps(
        menuai, MOCK_TV_WITH_EXCLUDE_CONFIG, CURRENT_APP_CONFIG
    ):
        attr = menuai.states.get(ENTITY_ID).attributes
        _assert_source_list_with_apps([*INPUT_LIST_WITH_APPS, CURRENT_APP], attr)
        assert CURRENT_APP in attr[ATTR_INPUT_SOURCE_LIST]
...
async def test_setup_with_apps_additional_apps_config(
    ...
    with (
        patch(
            "menuai.components.vizio.media_player.VizioAsync.launch_app"
        ) as service_call1,
        patch(
            "menuai.components.vizio.media_player.VizioAsync.launch_app_config"
        ) as service_call2,
    ):
        await menuai.services.async_call(
            MP_DOMAIN,
...
async def test_setup_with_unknown_app_config(
    ...
    async with _cm_for_test_setup_tv_with_apps(
        menuai, MOCK_USER_VALID_TV_CONFIG, UNKNOWN_APP_CONFIG
    ):
        attr = menuai.states.get(ENTITY_ID).attributes
        _assert_source_list_with_apps(list(INPUT_LIST_WITH_APPS + APP_NAME_LIST), attr)
...
async def test_setup_with_no_running_app(
    ...
    async with _cm_for_test_setup_tv_with_apps(
        menuai, MOCK_USER_VALID_TV_CONFIG, vars(AppConfig())
    ):
        attr = menuai.states.get(ENTITY_ID).attributes
        _assert_source_list_with_apps(list(INPUT_LIST_WITH_APPS + APP_NAME_LIST), attr)
...
@pytest.mark.usefixtures("vizio_connect", "vizio_update")
async def test_setup_tv_without_mute(menuai: menuai) -> None:
    ...
    async with _cm_for_test_setup_without_apps(
        ...
        attr = _get_attr_and_assert_base_attr(menuai, MediaPlayerDeviceClass.TV, STATE_ON)
        _assert_sources_and_volume(attr, VIZIO_DEVICE_CLASS_TV)
        assert "sound_mode" not in attr
...
async def test_apps_update(
    ...
    """Test device setup with apps where no app is running."""
    with patch(
        "menuai.components.vizio.coordinator.gen_apps_list_from_url",
        return_value=None,
    ):
        async with _cm_for_test_setup_tv_with_apps(
            menuai, MOCK_USER_VALID_TV_CONFIG, vars(AppConfig())
            ...
            with patch(
                ...
                async_fire_time_changed(menuai, dt_util.now() + timedelta(days=2))
                await menuai.async_block_till_done()
                async_fire_time_changed(menuai, dt_util.now() + timedelta(days=2))
                await menuai.async_block_till_done()
                # Check source list, remove TV inputs, and verify that the integration is
...
async def test_vizio_update_with_apps_on_input(menuai: menuai) -> None:
