...
from menuai.components.template import trigger as template_trigger
...
def setup_comp(menuai: menuai, calls: list[ServiceCall]) -> None:
...
async def test_general(
    menuai: menuai, call_setup, calls: list[ServiceCall]
) -> None:
    """Test for firing on change."""
    assert len(calls) == 0

    for call_len, call_name, call_force in call_setup:
        menuai.states.async_set("test.entity", call_name, force_update=call_force)
        await menuai.async_block_till_done()
...
async def test_if_not_fires_because_fail(
    ...
    for call_len, call_number in call_setup:
        menuai.states.async_set("test.number", call_number)
        await menuai.async_block_till_done()
        assert len(calls) == call_len
...
@pytest.mark.usefixtures("start_ha")
async def test_if_action(menuai: menuai, calls: list[ServiceCall]) -> None:
...
async def test_if_not_fires_on_change_with_for(
    ...
    await menuai.async_block_till_done()
    assert len(calls) == 0
    async_fire_time_changed(menuai, dt_util.utcnow() + timedelta(seconds=6))
    await menuai.async_block_till_done()
    assert len(calls) == 0
...
async def test_if_fires_on_time_change(
    menuai: menuai, freezer: FrozenDateTimeFactory, calls: list[ServiceCall]
) -> None:
    ...
    async_fire_time_changed(menuai, second_time)
    await menuai.async_block_till_done()
    await menuai.async_block_till_done()
    assert len(calls) == 1
    ...
    async_fire_time_changed(menuai, third_time)
    await menuai.async_block_till_done()
    await menuai.async_block_till_done()
    assert len(calls) == 1
    ...
    async_fire_time_changed(menuai, forth_time)
    await menuai.async_block_till_done()
    await menuai.async_block_till_done()
    assert len(calls) == 2
