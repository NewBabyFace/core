...
from menuai.components.pvpc_hourly_pricing.const import (
    ATTR_POWER,
    ATTR_POWER_P3,
    ATTR_TARIFF,
    CONF_USE_API_TOKEN,
...
async def test_config_flow(
    ...
    await menuai.config.async_set_time_zone("Europe/Madrid")
    tst_config = {
        CONF_NAME: "test",
    ...
    state = menuai.states.get("sensor.esios_pvpc")
    ...
    state_inyection = menuai.states.get("sensor.injection_price")
    ...
    assert await menuai.config_entries.async_remove(registry_entity.config_entry_id)
    ...
    state_inyection = menuai.states.get("sensor.esios_injection_price")
    state_mag = menuai.states.get("sensor.esios_mag_tax")
    state_omie = menuai.states.get("sensor.esios_omie_price")
...
async def test_reauth(
    ...
    freezer.move_to(_MOCK_TIME_BAD_AUTH_RESPONSES)
    async_fire_time_changed(menuai, _MOCK_TIME_BAD_AUTH_RESPONSES)
    ...
    freezer.move_to(_MOCK_TIME_VALID_RESPONSES)
    async_fire_time_changed(menuai, _MOCK_TIME_VALID_RESPONSES)
    result = await menuai.config_entries.flow.async_configure(
        result["flow_id"], user_input={CONF_API_TOKEN: "test-token"}
    )
    assert result["type"] is FlowResultType.ABORT
