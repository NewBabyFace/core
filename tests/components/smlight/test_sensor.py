...
async def test_disabled_by_default_sensors(
    ...
    for sensor in ("core_uptime", "filesystem_usage", "ram_usage", "zigbee_uptime"):
        assert not menuai.states.get(f"sensor.mock_title_{sensor}")
...
async def test_zigbee_uptime_disconnected(
    ...
    state = menuai.states.get("sensor.mock_title_zigbee_uptime")
...
async def test_zigbee2_temp_sensor(
    ...
    state = menuai.states.get("sensor.mock_title_zigbee_chip_temp_2")
    assert state
...
async def test_zigbee_type_sensors(
    ...
    state = menuai.states.get("sensor.mock_title_zigbee_type")
    assert state
    assert state.state == "coordinator"

    state = menuai.states.get("sensor.mock_title_zigbee_type_2")
    assert state
