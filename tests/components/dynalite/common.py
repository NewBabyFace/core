...
from menuai.const import ATTR_SERVICE, CONF_HOST
...
async def get_entry_id_from_menuai(menuai: menuai) -> str:
    """Get the config entry id from menuai."""
    conf_entries = menuai.config_entries.async_entries(dynalite.DOMAIN)
    assert len(conf_entries) == 1
    return conf_entries[0].entry_id


async def create_entity_from_device(menuai: menuai, device: DynaliteBaseDevice):
...
async def run_service_tests(menuai: menuai, device, platform, services):
    ...
    for cur_item in services:
        ...
        await menuai.services.async_call(platform, service, service_data, blocking=True)
        await menuai.async_block_till_done()
