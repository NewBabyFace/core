...
from menuai.components.broadlink.heartbeat import BroadlinkHeartbeat
...
DEVICE_PING = "menuai.components.broadlink.heartbeat.blk.ping"


async def test_heartbeat_trigger_startup(menuai: menuai) -> None:
    """Test that the heartbeat is initialized with the first config entry."""
    device = get_device("Office")

    with patch(DEVICE_PING) as mock_ping:
        await device.setup_entry(menuai)
...
async def test_heartbeat_trigger_right_time(menuai: menuai) -> None:
    ...
    with patch(DEVICE_PING) as mock_ping:
        async_fire_time_changed(
            menuai, dt_util.utcnow() + BroadlinkHeartbeat.HEARTBEAT_INTERVAL
        )
...
async def test_heartbeat_do_not_trigger_before_time(menuai: menuai) -> None:
...
async def test_heartbeat_unload(menuai: menuai) -> None:
    ...
    await menuai.config_entries.async_remove(mock_setup.entry.entry_id)
    await menuai.async_block_till_done()
...
async def test_heartbeat_do_not_unload(menuai: menuai) -> None:
    ...
    mock_setup = await device_a.setup_entry(menuai)
    await device_b.setup_entry(menuai)
    await menuai.async_block_till_done()
