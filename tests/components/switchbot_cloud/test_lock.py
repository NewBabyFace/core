...
async def test_lock(menuai: menuai, mock_list_devices, mock_get_status) -> None:
    ...
    lock_id = "lock.lock_1"
    assert menuai.states.get(lock_id).state == LockState.LOCKED

    with patch.object(SwitchBotAPI, "send_command"):
        await menuai.services.async_call(
            LOCK_DOMAIN, SERVICE_UNLOCK, {ATTR_ENTITY_ID: lock_id}, blocking=True
        )
    assert menuai.states.get(lock_id).state == LockState.UNLOCKED
