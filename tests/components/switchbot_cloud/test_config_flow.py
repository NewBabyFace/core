"""Test the SwitchBot via API config flow."""

from unittest.mock import AsyncMock, patch

import pytest

from menuai import config_entries
from menuai.components.switchbot_cloud.config_flow import (
    SwitchBotAuthenticationError,
    SwitchBotConnectionError,
)
from menuai.components.switchbot_cloud.const import DOMAIN, ENTRY_TITLE
from menuai.const import CONF_API_KEY, CONF_API_TOKEN
from menuai.core import menuai
from menuai.data_entry_flow import FlowResultType


async def _fill_out_form_and_assert_entry_created(
    menuai: menuai, flow_id: str, mock_setup_entry: AsyncMock
) -> None:
    """Util function to fill out a form and assert that a config entry is created."""
    with patch(
        "menuai.components.switchbot_cloud.config_flow.SwitchBotAPI.list_devices",
        return_value=[],
    ):
        result_configure = await menuai.config_entries.flow.async_configure(
            flow_id,
            {
                CONF_API_TOKEN: "test-token",
                CONF_API_KEY: "test-secret-key",
            },
        )
        await menuai.async_block_till_done()

        assert result_configure["type"] is FlowResultType.CREATE_ENTRY
        assert result_configure["title"] == ENTRY_TITLE
        assert result_configure["data"] == {
            CONF_API_TOKEN: "test-token",
            CONF_API_KEY: "test-secret-key",
        }
        mock_setup_entry.assert_called_once()


async def test_form(menuai: menuai, mock_setup_entry: AsyncMock) -> None:
    """Test we get the form."""
    result_init = await menuai.config_entries.flow.async_init(
        DOMAIN, context={"source": config_entries.SOURCE_USER}
    )
    assert result_init["type"] is FlowResultType.FORM
    assert not result_init["errors"]

    await _fill_out_form_and_assert_entry_created(
        menuai, result_init["flow_id"], mock_setup_entry
    )


@pytest.mark.parametrize(
    ("error", "message"),
    [
        (SwitchBotAuthenticationError, "invalid_auth"),
        (SwitchBotConnectionError, "cannot_connect"),
        (Exception, "unknown"),
    ],
)
async def test_form_fails(
    menuai: menuai, error: Exception, message: str, mock_setup_entry: AsyncMock
) -> None:
    """Test we handle error cases."""
    result_init = await menuai.config_entries.flow.async_init(
        DOMAIN, context={"source": config_entries.SOURCE_USER}
    )

    with patch(
        "menuai.components.switchbot_cloud.config_flow.SwitchBotAPI.list_devices",
        side_effect=error,
    ):
        result_configure = await menuai.config_entries.flow.async_configure(
            result_init["flow_id"],
            {
                CONF_API_TOKEN: "test-token",
                CONF_API_KEY: "test-secret-key",
            },
        )

        assert result_configure["type"] is FlowResultType.FORM
        assert result_configure["errors"] == {"base": message}
        await menuai.async_block_till_done()

    await _fill_out_form_and_assert_entry_created(
        menuai, result_init["flow_id"], mock_setup_entry
    )
