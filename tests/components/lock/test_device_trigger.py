...
from menuai.components.lock import DOMAIN, LockEntityFeature, LockState
...
async def test_if_fires_on_state_change_with_for(
    ...
    await menuai.async_block_till_done()
    assert len(service_calls) == 1
    async_fire_time_changed(menuai, dt_util.utcnow() + timedelta(seconds=16))
    await menuai.async_block_till_done()
    assert len(service_calls) == 2
    await menuai.async_block_till_done()
    assert (
    ...
    await menuai.async_block_till_done()
    assert len(service_calls) == 3
    async_fire_time_changed(menuai, dt_util.utcnow() + timedelta(seconds=27))
    await menuai.async_block_till_done()
