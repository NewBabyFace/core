...
async def test_show_menu(menuai: menuai) -> None:
...
async def test_direct_setup(menuai: menuai) -> None:
    """Test the standard direct setup case."""
    show_form_result = await menuai.config_entries.flow.async_init(
        DOMAIN, context={"source": config_entries.SOURCE_USER}
    )
    with patch(
        "menuai.components.elmax.async_setup_entry",
        return_value=True,
    ):
        set_mode_result = await menuai.config_entries.flow.async_configure(
...
async def test_direct_show_form(menuai: menuai) -> None:
...
async def test_cloud_setup(menuai: menuai) -> None:
    ...
    with patch(
        ...
        show_form_result = await menuai.config_entries.flow.async_configure(
            show_form_result["flow_id"],
            {"next_step_id": CONF_ELMAX_MODE_CLOUD},
        )
        login_result = await menuai.config_entries.flow.async_configure(
            show_form_result["flow_id"],
            {
...
async def test_zeroconf_form_setup_api_not_supported(menuai: menuai) -> None:
...
async def test_zeroconf_discovery_ipv6(menuai: menuai) -> None:
    """Test discovery of Elmax local api panel."""
    result = await menuai.config_entries.flow.async_init(
        DOMAIN,
        context={"source": config_entries.SOURCE_ZEROCONF},
...
async def test_zeroconf_setup_show_form(menuai: menuai) -> None:
...
async def test_zeroconf_ipv6_setup(menuai: menuai) -> None:
    """Test the successful creation of config entry via discovery flow."""
    result = await menuai.config_entries.flow.async_init(
        DOMAIN,
        context={"source": config_entries.SOURCE_ZEROCONF},
...
async def test_zeroconf_panel_changed_ip(menuai: menuai) -> None:
...
async def test_one_config_allowed_cloud(menuai: menuai) -> None:
...
async def test_cloud_invalid_credentials(menuai: menuai) -> None:
...
async def test_cloud_connection_error(menuai: menuai) -> None:
...
async def test_direct_connection_error(menuai: menuai) -> None:
...
async def test_direct_wrong_panel_code(menuai: menuai) -> None:
...
async def test_unhandled_error(menuai: menuai) -> None:
...
async def test_invalid_pin(menuai: menuai) -> None:
...
async def test_no_online_panel(menuai: menuai) -> None:
...
async def test_show_reauth(menuai: menuai) -> None:
...
async def test_reauth_flow(menuai: menuai) -> None:
    ...
    # Trigger reauth
    reauth_result = await entry.start_reauth_flow(menuai)
    with patch(
        "menuai.components.elmax.async_setup_entry",
        return_value=True,
    ):
...
async def test_reauth_panel_disappeared(menuai: menuai) -> None:
...
async def test_reauth_invalid_pin(menuai: menuai) -> None:
    """Test that the case where panel is no longer associated with the user."""
    entry = MockConfigEntry(
        domain=DOMAIN,
        data={
...
async def test_reauth_bad_login(menuai: menuai) -> None:
