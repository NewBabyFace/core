...
from menuai.components.ipp.const import CONF_BASE_PATH, DOMAIN
...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "menuai.components.ipp.async_setup_entry", return_value=True
    ) as mock_setup_entry:
...
async def mock_printer(
    ...
    return Printer.from_dict(json.loads(await async_load_fixture(menuai, fixture)))
...
def mock_ipp_config_flow(mock_printer: Printer) -> Generator[MagicMock]:
    ...
    with patch(
        "menuai.components.ipp.config_flow.IPP", autospec=True
    ) as ipp_mock:
...
def mock_ipp(mock_printer: Printer) -> Generator[MagicMock]:
    ...
    with patch(
        "menuai.components.ipp.coordinator.IPP", autospec=True
    ) as ipp_mock:
        client = ipp_mock.return_value
        client.printer.return_value = mock_printer
        yield client
...
async def init_integration(
    menuai: menuai, mock_config_entry: MockConfigEntry, mock_ipp: MagicMock
) -> MockConfigEntry:
