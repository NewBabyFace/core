...
from menuai.components.roborock.const import CONF_ENTRY_CODE, DOMAIN, DRAWABLES
...
async def test_config_flow_success(
    ...
    with patch(
        "menuai.components.roborock.async_setup_entry", return_value=True
    ) as mock_setup:
        ...
        with patch(
            "menuai.components.roborock.config_flow.RoborockApiClient.request_code"
        ):
        ...
        with patch(
            "menuai.components.roborock.config_flow.RoborockApiClient.code_login",
...
async def test_options_flow_drawables(
    ...
    with patch("menuai.components.roborock.roborock_storage"):
...
async def test_reauth_flow(
    ...
    """Test reauth flow."""
    # Start reauth
    result = mock_roborock_entry.async_start_reauth(menuai)
    await menuai.async_block_till_done()
...
async def test_discovery_not_setup(
    ...
    """Handle the config flow and make sure it succeeds."""
    with (
        patch("menuai.components.roborock.async_setup_entry", return_value=True),
    ):
        result = await menuai.config_entries.flow.async_init(
            DOMAIN,
