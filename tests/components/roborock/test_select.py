...
from menuai.const import SERVICE_SELECT_OPTION, STATE_UNKNOWN, Platform
...
async def test_update_failure(
    ...
    """Test that changing a value will raise a menuaierror when it fails."""
    with (
        patch(
            "menuai.components.roborock.coordinator.RoborockLocalClientV1.send_message",
            side_effect=RoborockException(),
        ),
        pytest.raises(menuaiError, match="Error while calling SET_MOP_MOD"),
    ):
        await menuai.services.async_call(
            "select",
            SERVICE_SELECT_OPTION,
...
async def test_none_map_select(
    ...
    select_entity = menuai.states.get("select.roborock_s7_maxv_selected_map")
