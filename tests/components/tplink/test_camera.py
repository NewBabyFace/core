...
from menuai.components.tplink.camera import TPLinkCameraEntity
...
async def test_states(
    ...
    with patch("random.SystemRandom.getrandbits", return_value=123123123123):
        await setup_platform_for_device(
            menuai, mock_camera_config_entry, Platform.CAMERA, mock_device
        )
...
async def test_handle_mjpeg_stream(
    ...
    state = menuai.states.get("camera.my_camera_live_view")
    assert state is not None

    mock_request = make_mocked_request("GET", "/", headers={"token": "x"})
    stream = await async_get_mjpeg_stream(
        menuai, mock_request, "camera.my_camera_live_view"
    )
...
async def test_camera_image(
    ...
    with patch(
        ...
        image = await async_get_image(menuai, "camera.my_camera_live_view")
        assert image
...
async def test_no_camera_image_when_streaming(
    ...
    with patch(
        ...
        await async_get_image(menuai, "camera.my_camera_live_view")
        mock_get_image.assert_called_once()
        ...
        mock_get_image.reset_mock()
        with patch(
            "menuai.components.tplink.camera.async_aiohttp_proxy_stream",
            new=_waiter,
        ):
...
async def test_no_concurrent_camera_image(
    ...
    with patch(
        ...
        tasks = asyncio.gather(
            async_get_image(menuai, "camera.my_camera_live_view"),
            async_get_image(menuai, "camera.my_camera_live_view"),
        )
...
async def test_camera_stream_attributes(
    ...
    camera = get_camera_from_entity_id(menuai, "camera.my_camera_live_view")
