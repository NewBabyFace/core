...
from menuai.components.kulersky.config_flow import DOMAIN
...
async def test_integration_discovery(menuai: menuai) -> None:
    """Test discovery via bluetooth with a valid device."""
    with patch(
        "menuai.components.kulersky.config_flow.async_last_service_info",
...
async def test_integration_discovery_no_last_service_info(menuai: menuai) -> None:
    """Test discovery via bluetooth with a valid device."""
    result = await menuai.config_entries.flow.async_init(
        DOMAIN,
        context={"source": SOURCE_INTEGRATION_DISCOVERY},
...
async def test_user_setup(menuai: menuai) -> None:
    ...
    with patch(
        "menuai.components.kulersky.config_flow.async_discovered_service_info",
        return_value=[
...
async def test_user_setup_no_devices(menuai: menuai) -> None:
    """Test the user manually setting up the integration."""
    with patch(
        "menuai.components.kulersky.config_flow.async_discovered_service_info",
        return_value=[],
...
async def test_unexpected_error(menuai: menuai) -> None:
