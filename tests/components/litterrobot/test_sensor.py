...
from menuai.components.litterrobot.sensor import icon_for_gauge_level
from menuai.components.sensor import DOMAIN as PLATFORM_DOMAIN, SensorDeviceClass
from menuai.const import PERCENTAGE, STATE_UNKNOWN, UnitOfMass
...
async def test_waste_drawer_sensor(
    ...
    sensor = menuai.states.get(WASTE_DRAWER_ENTITY_ID)
    assert sensor
...
async def test_sleep_time_sensor_with_sleep_disabled(
    menuai: menuai, mock_account_with_sleep_disabled_robot: MagicMock
) -> None:
    """Tests the sleep mode start time sensor where sleep mode is disabled."""
    await setup_integration(
        menuai, mock_account_with_sleep_disabled_robot, PLATFORM_DOMAIN
    )

    sensor = menuai.states.get(SLEEP_START_TIME_ENTITY_ID)
    assert sensor
    assert sensor.state == STATE_UNKNOWN
    assert sensor.attributes["device_class"] == SensorDeviceClass.TIMESTAMP

    sensor = menuai.states.get(SLEEP_END_TIME_ENTITY_ID)
    assert sensor.state == STATE_UNKNOWN
    assert sensor.attributes["device_class"] == SensorDeviceClass.TIMESTAMP
...
async def test_litter_robot_sensor(
    menuai: menuai, mock_account_with_litterrobot_4: MagicMock
) -> None:
    """Tests Litter-Robot sensors."""
    await setup_integration(menuai, mock_account_with_litterrobot_4, PLATFORM_DOMAIN)
    ...
    assert sensor.attributes["device_class"] == SensorDeviceClass.TIMESTAMP
    sensor = menuai.states.get("sensor.test_last_seen")
    assert sensor.state == "2022-09-17T12:06:37+00:00"
    assert sensor.attributes["device_class"] == SensorDeviceClass.TIMESTAMP
    sensor = menuai.states.get("sensor.test_status_code")
    assert sensor.state == "rdy"
    assert sensor.attributes["device_class"] == SensorDeviceClass.ENUM
    sensor = menuai.states.get("sensor.test_litter_level")
    assert sensor.state == "70.0"
    assert sensor.attributes["unit_of_measurement"] == PERCENTAGE
    sensor = menuai.states.get("sensor.test_pet_weight")
    assert sensor.state == "12.0"
    assert sensor.attributes["unit_of_measurement"] == UnitOfMass.POUNDS


async def test_feeder_robot_sensor(
    menuai: menuai, mock_account_with_feederrobot: MagicMock
) -> None:
    """Tests Feeder-Robot sensors."""
    await setup_integration(menuai, mock_account_with_feederrobot, PLATFORM_DOMAIN)
    sensor = menuai.states.get("sensor.test_food_level")
    assert sensor.state == "10"
    assert sensor.attributes["unit_of_measurement"] == PERCENTAGE


async def test_pet_weight_sensor(
    menuai: menuai, mock_account_with_pet: MagicMock
) -> None:
    """Tests pet weight sensors."""
    await setup_integration(menuai, mock_account_with_pet, PLATFORM_DOMAIN)
    sensor = menuai.states.get("sensor.kitty_weight")
    assert sensor.state == "9.1"
    assert sensor.attributes["unit_of_measurement"] == UnitOfMass.POUNDS


async def test_litterhopper_sensor(
    menuai: menuai, mock_account_with_litterhopper: MagicMock
) -> None:
    """Tests LitterHopper sensors."""
    await setup_integration(menuai, mock_account_with_litterhopper, PLATFORM_DOMAIN)
    sensor = menuai.states.get("sensor.test_hopper_status")
