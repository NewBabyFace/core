...
async def test_binary_sensors(
    ...
    await setup_integration(menuai, mock_account, BINARY_SENSOR_DOMAIN)

    state = menuai.states.get("binary_sensor.test_sleeping")
    assert state.state == "off"
    state = menuai.states.get("binary_sensor.test_sleep_mode")
    assert state.state == "on"
    state = menuai.states.get("binary_sensor.test_power_status")
...
async def test_litterhopper_binary_sensors(
    ...
    await setup_integration(menuai, mock_account_with_litterhopper, BINARY_SENSOR_DOMAIN)

    state = menuai.states.get("binary_sensor.test_hopper_connected")
    assert state.state == "on"
    assert (
