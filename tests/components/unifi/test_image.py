...
async def test_entity_and_device_data(
    ...
    with patch("menuai.components.unifi.PLATFORMS", [Platform.IMAGE]):
        config_entry = await config_entry_factory()
    if site_payload[0]["role"] == "admin":
        await snapshot_platform(menuai, entity_registry, snapshot, config_entry.entry_id)
    else:
        assert len(menuai.states.async_entity_ids(IMAGE_DOMAIN)) == 0
...
async def test_wlan_qr_code(
    ...
    image_state_1 = menuai.states.get("image.ssid_1_qr_code")
    mock_websocket_message(message=MessageKey.WLAN_CONF_UPDATED, data=WLAN)
    image_state_2 = menuai.states.get("image.ssid_1_qr_code")
    ...
    image_state_3 = menuai.states.get("image.ssid_1_qr_code")
...
async def test_hub_state_change(
    ...
    assert menuai.states.get("image.ssid_1_qr_code").state != STATE_UNAVAILABLE
