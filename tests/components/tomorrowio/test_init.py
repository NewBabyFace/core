...
from menuai.components.tomorrowio.const import CONF_TIMESTEP, DOMAIN
...
async def test_load_and_unload(menuai: menuai) -> None:
    """Test loading and unloading entry."""
    data = _get_config_schema(menuai, SOURCE_USER)(MIN_CONFIG)
    data[CONF_NAME] = "test"
    ...
    await menuai.async_block_till_done()
    assert len(menuai.states.async_entity_ids(WEATHER_DOMAIN)) == 0


async def test_update_intervals(
    menuai: menuai, freezer: FrozenDateTimeFactory, tomorrowio_config_entry_update
) -> None:
    ...
    config_entry_2 = MockConfigEntry(
        ...
        data=data,
        options={CONF_TIMESTEP: 1},
        unique_id=f"{_get_unique_id(menuai, data)}_1",
        version=1,
    )
