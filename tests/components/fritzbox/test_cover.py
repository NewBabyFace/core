...
async def test_setup(
    ...
    with patch("menuai.components.fritzbox.PLATFORMS", [Platform.COVER]):
...
async def test_unknown_position(menuai: menuai, fritz: Mock) -> None:
...
async def test_open_cover(menuai: menuai, fritz: Mock) -> None:
...
async def test_close_cover(menuai: menuai, fritz: Mock) -> None:
...
async def test_set_position_cover(menuai: menuai, fritz: Mock) -> None:
...
async def test_stop_cover(menuai: menuai, fritz: Mock) -> None:
    """Test stopping the device."""
    device = FritzDeviceCoverMock()
    await setup_config_entry(
        menuai, MOCK_CONFIG[DOMAIN][CONF_DEVICES][0], ENTITY_ID, device, fritz
...
async def test_discover_new_device(menuai: menuai, fritz: Mock) -> None:
    ...
    state = menuai.states.get(f"{COVER_DOMAIN}.new_climate")
    assert state
