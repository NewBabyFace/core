...
async def test_setup(
    ...
    with patch("menuai.components.fritzbox.PLATFORMS", [Platform.SWITCH]):
...
async def test_turn_on(menuai: menuai, fritz: Mock) -> None:
    """Test turn device on."""
    device = FritzDeviceSwitchMock()
...
async def test_turn_off(menuai: menuai, fritz: Mock) -> None:
...
async def test_toggle_while_locked(menuai: menuai, fritz: Mock) -> None:
...
async def test_assume_device_unavailable(menuai: menuai, fritz: Mock) -> None:
...
async def test_discover_new_device(menuai: menuai, fritz: Mock) -> None:
    ...
    state = menuai.states.get(f"{SWITCH_DOMAIN}.new_switch")
    assert state
