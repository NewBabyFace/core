...
from menuai.components.fritzbox.const import COLOR_MODE, COLOR_TEMP_MODE, DOMAIN
...
async def test_setup(
    ...
    with patch("menuai.components.fritzbox.PLATFORMS", [Platform.LIGHT]):
...
async def test_turn_on_color(menuai: menuai, fritz: Mock) -> None:
...
async def test_discover_new_device(menuai: menuai, fritz: Mock) -> None:
    ...
    state = menuai.states.get(f"{LIGHT_DOMAIN}.new_light")
    assert state
