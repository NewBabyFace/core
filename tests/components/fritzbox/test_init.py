...
async def test_setup(menuai: menuai, fritz: Mock) -> None:
...
async def test_unload_remove(menuai: menuai, fritz: Mock) -> None:
...
async def test_logout_on_stop(menuai: menuai, fritz: Mock) -> None:
    """Test we log out from fritzbox when MenuAIs stops."""
    fritz().get_devices.return_value = [FritzDeviceSwitchMock()]
    entity_id = f"{SWITCH_DOMAIN}.{CONF_FAKE_NAME}"
...
async def test_raise_config_entry_not_ready_when_offline(menuai: menuai) -> None:
    ...
    entry.add_to_menuai(menuai)
    with patch(
        "menuai.components.fritzbox.coordinator.Fritzhome.login",
...
async def test_raise_config_entry_error_when_login_fail(menuai: menuai) -> None:
