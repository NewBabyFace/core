...
from menuai.components.emulated_hue.config import (
    DATA_KEY,
    DATA_VERSION,
    SAVE_DELAY,
    Config,
)
from menuai.components.emulated_hue.upnp import UPNPResponderProtocol
...
async def test_config_google_home_entity_id_to_number(
    ...
    conf = Config(menuai, {"type": "google_home"}, "127.0.0.1")
    menuai_storage[DATA_KEY] = {
    ...
    async_fire_time_changed(menuai, utcnow() + timedelta(seconds=SAVE_DELAY))
    await menuai.async_block_till_done()
    assert menuai_storage[DATA_KEY]["data"] == {
        "1": "light.test2",
        "2": "light.test",
    }
...
async def test_config_google_home_entity_id_to_number_empty(
    ...
    """Test config adheres to the type."""
    conf = Config(menuai, {"type": "google_home"}, "127.0.0.1")
    menuai_storage[DATA_KEY] = {"version": DATA_VERSION, "key": DATA_KEY, "data": {}}
    ...
    async_fire_time_changed(menuai, utcnow() + timedelta(seconds=SAVE_DELAY))
    await menuai.async_block_till_done()
    assert menuai_storage[DATA_KEY]["data"] == {"1": "light.test"}
...
async def test_setup_works(menuai: menuai) -> None:
    """Test setup works."""
    menuai.config.components.add("network")
    with (
        patch(
            "menuai.components.emulated_hue.async_create_upnp_datagram_endpoint",
            AsyncMock(),
        ) as mock_create_upnp_datagram_endpoint,
        patch("menuai.components.emulated_hue.async_get_source_ip"),
        patch(
            "menuai.components.emulated_hue.web.TCPSite",
        ...
        assert await async_setup_component(menuai, "emulated_hue", {})
