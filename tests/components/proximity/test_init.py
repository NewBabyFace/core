...
async def async_setup_single_entry(
    ...
    assert await menuai.config_entries.async_setup(mock_config.entry_id)
...
async def test_proximities(menuai: menuai, config: dict) -> None:
    """Test a list of proximities."""
    title = menuai.states.get(config[CONF_ZONE]).name
    mock_config = MockConfigEntry(
        domain=DOMAIN,
        title=title,
        data=config,
    ...
    # sensor entities
    state = menuai.states.get(f"sensor.{zone_name}_nearest_device")
    assert state.state == STATE_UNKNOWN

    for device in config[CONF_TRACKED_ENTITIES]:
        entity_base_name = f"sensor.{zone_name}_{slugify(device.split('.')[-1])}"
        state = menuai.states.get(f"{entity_base_name}_distance")
        assert state.state == STATE_UNAVAILABLE
        state = menuai.states.get(f"{entity_base_name}_direction_of_travel")
        assert state.state == STATE_UNAVAILABLE


async def test_device_tracker_test1_in_zone(menuai: menuai) -> None:
    """Test for tracker in zone."""
    await async_setup_single_entry(menuai, "zone.home", ["device_tracker.test1"], [], 1)
    ...
    # sensor entities
    state = menuai.states.get("sensor.home_nearest_device")
...
async def test_device_tracker_test1_away(menuai: menuai) -> None:
...
async def test_all_device_trackers_in_ignored_zone(menuai: menuai) -> None:
    """Test for tracker in ignored zone."""
    await async_setup_single_entry(menuai, "zone.home", ["device_tracker.test1"], [], 1)

    menuai.states.async_set("device_tracker.test1", "work", {"friendly_name": "test1"})
    await menuai.async_block_till_done()
...
async def test_device_tracker_test1_no_coordinates(menuai: menuai) -> None:
...
async def test_device_tracker_test1_awayfurther_a_bit(menuai: menuai) -> None:
    """Test for tracker states."""
    await async_setup_single_entry(
        menuai, "zone.home", ["device_tracker.test1"], ["zone.work"], 1000
    )
...
async def test_device_trackers_in_zone(menuai: menuai) -> None:
...
async def test_device_tracker_test1_awayfurther_test2_in_ignored_zone(
    ...
    menuai.states.async_set("device_tracker.test2", "work", {"friendly_name": "test2"})
...
async def test_nearest_sensors(menuai: menuai, config_zones) -> None:
    """Test for nearest sensors."""
    await async_setup_single_entry(
        menuai, "zone.home", ["device_tracker.test1", "device_tracker.test2"], [], 1
    )
    ...
    state = menuai.states.get("sensor.home_nearest_device")
    assert state.state == "test1"
    state = menuai.states.get("sensor.home_nearest_distance")
    assert state.state == "1615580"
    state = menuai.states.get("sensor.home_test1_direction_of_travel")
    assert state.state == "towards"
    state = menuai.states.get("sensor.home_test1_distance")
    assert state.state == "1615580"
    state = menuai.states.get("sensor.home_test1_direction_of_travel")
    assert state.state == "towards"
    state = menuai.states.get("sensor.home_test2_distance")
    assert state.state == "5176048"
    state = menuai.states.get("sensor.home_test2_direction_of_travel")
    assert state.state == "away_from"
    ...
    state = menuai.states.get("sensor.home_nearest_distance")
    assert state.state == "1615580"
    state = menuai.states.get("sensor.home_nearest_direction_of_travel")
    assert state.state == "towards"
    state = menuai.states.get("sensor.home_test1_distance")
    assert state.state == "1615580"
    state = menuai.states.get("sensor.home_test1_direction_of_travel")
...
async def test_create_removed_tracked_entity_issue(
    ...
    menuai.states.async_set(t1.entity_id, "not_home")
    menuai.states.async_set(t2.entity_id, "not_home")

    await async_setup_single_entry(
        menuai, "zone.home", [t1.entity_id, t2.entity_id], [], 1
    )

    sensor_t1 = f"sensor.home_{t1.entity_id.split('.')[-1]}_distance"
    sensor_t2 = f"sensor.home_{t2.entity_id.split('.')[-1]}_distance"

    state = menuai.states.get(sensor_t1)
    assert state.state == STATE_UNKNOWN
    state = menuai.states.get(sensor_t2)
    assert state.state == STATE_UNKNOWN

    menuai.states.async_remove(t2.entity_id)
...
async def test_track_renamed_tracked_entity(
    ...
    mock_config = await async_setup_single_entry(
        menuai, "zone.home", [t1.entity_id], ["zone.work"], 1
    )
    ...
    entry = menuai.config_entries.async_get_entry(mock_config.entry_id)
    assert entry
...
async def test_sensor_unique_ids(
    ...
    menuai.states.async_set("device_tracker.test2", "not_home")

    mock_config = await async_setup_single_entry(
        menuai, "zone.home", [t1.entity_id, "device_tracker.test2"], ["zone.work"], 1
    )
...
async def test_tracked_zone_is_removed(menuai: menuai) -> None:
    ...
    assert menuai.states.async_remove("zone.home")
    menuai.states.async_set(
        "device_tracker.test1",
        "home",
...
async def test_tracked_zone_radius_is_changed(menuai: menuai) -> None:
    """Test that radius of the tracked zone is changed."""
    entry = await async_setup_single_entry(
        menuai, "zone.home", ["device_tracker.test1"], [], 1
    )
    ...
    await menuai.async_block_till_done()
    radius = menuai.states.get("zone.home").attributes["radius"]
    assert radius == 110
...
async def test_tracked_zone_location_is_changed(menuai: menuai) -> None:
    ...
    await menuai.config_entries.async_reload(entry.entry_id)
    await menuai.async_block_till_done()
    latitude = menuai.states.get("zone.home").attributes["latitude"]
    assert latitude == 10
    longitude = menuai.states.get("zone.home").attributes["longitude"]
    assert longitude == 5
