...
from menuai.components import camera, image_processing as ip
from menuai.components.openalpr_cloud.image_processing import OPENALPR_API_URL
...
@pytest.fixture
async def setup_openalpr_cloud(menuai: menuai) -> None:
    ...
    with patch(
        "menuai.components.openalpr_cloud.image_processing."
...
@pytest.fixture
async def alpr_events(menuai: menuai) -> list[Event]:
    """Listen for events."""
    return async_capture_events(menuai, "image_processing.found_plate")
...
async def test_setup_platform(menuai: menuai) -> None:
    ...
    assert menuai.states.get("image_processing.openalpr_demo_camera")


async def test_setup_platform_name(menuai: menuai) -> None:
...
async def test_setup_platform_without_api_key(menuai: menuai) -> None:
...
async def test_setup_platform_without_region(menuai: menuai) -> None:
...
async def test_openalpr_process_image(
    ...
    aioclient_mock.post(
        OPENALPR_API_URL,
        params=PARAMS,
        text=await async_load_fixture(menuai, "alpr_cloud.json", "openalpr_cloud"),
        status=200,
    )
