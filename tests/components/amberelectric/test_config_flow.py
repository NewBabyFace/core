...
from menuai.components.amberelectric.config_flow import filter_sites
...
async def test_single_pending_site(
    menuai: menuai, single_site_pending_api: Mock
) -> None:
    """Test single site."""
    initial_result = await menuai.config_entries.flow.async_init(
    ...
    enter_api_key_result = await menuai.config_entries.flow.async_init(
        DOMAIN,
    ...
    select_site_result = await menuai.config_entries.flow.async_configure(
...
async def test_single_site(menuai: menuai, single_site_api: Mock) -> None:
    """Test single site."""
    initial_result = await menuai.config_entries.flow.async_init(
        DOMAIN, context={"source": SOURCE_USER}
    )
...
async def test_single_closed_site_no_closed_date(
    menuai: menuai, single_site_closed_no_close_date_api: Mock
) -> None:
...
async def test_single_site_rejoin(
    menuai: menuai, single_site_rejoin_api: Mock
) -> None:
    """Test single site."""
    initial_result = await menuai.config_entries.flow.async_init(
        DOMAIN, context={"source": SOURCE_USER}
...
async def test_no_site(menuai: menuai, no_site_api: Mock) -> None:
    """Test no site."""
...
async def test_invalid_key(menuai: menuai, invalid_key_api: Mock) -> None:
...
async def test_unknown_error(menuai: menuai, api_error: Mock) -> None:
    """Test invalid api key."""
    result = await menuai.config_entries.flow.async_init(
        DOMAIN, context={"source": SOURCE_USER}
    )
