...
@pytest.fixture
async def setup_general(menuai: menuai) -> AsyncGenerator[Mock]:
...
@pytest.fixture
async def setup_general_and_feed_in(menuai: menuai) -> AsyncGenerator[Mock]:
...
async def test_general_price_sensor(menuai: menuai, setup_general: Mock) -> None:
    """Test the General Price sensor."""
    assert len(menuai.states.async_all()) == 6
    price = menuai.states.get("sensor.mock_title_general_price")
    assert price
...
async def test_general_and_controlled_load_price_sensor(menuai: menuai) -> None:
    """Test the Controlled Price sensor."""
    assert len(menuai.states.async_all()) == 9
    price = menuai.states.get("sensor.mock_title_controlled_load_price")
    assert price
    assert price.state == "0.08"
    attributes = price.attributes
    assert attributes["duration"] == 30
...
async def test_general_and_feed_in_price_sensor(menuai: menuai) -> None:
    """Test the Feed In sensor."""
    assert len(menuai.states.async_all()) == 9
    price = menuai.states.get("sensor.mock_title_feed_in_price")
    assert price
    assert price.state == "-0.08"
    attributes = price.attributes
    assert attributes["duration"] == 30
...
async def test_general_forecast_sensor(
    menuai: menuai, setup_general: Mock
) -> None:
    """Test the General Forecast sensor."""
    assert len(menuai.states.async_all()) == 6
    price = menuai.states.get("sensor.mock_title_general_forecast")
    assert price
...
@pytest.mark.usefixtures("setup_general_and_controlled_load")
async def test_controlled_load_forecast_sensor(menuai: menuai) -> None:
    """Test the Controlled Load Forecast sensor."""
    assert len(menuai.states.async_all()) == 9
    price = menuai.states.get("sensor.mock_title_controlled_load_forecast")
    assert price
    assert price.state == "0.09"
    attributes = price.attributes
    assert attributes["channel_type"] == "controlledLoad"
...
@pytest.mark.usefixtures("setup_general_and_feed_in")
async def test_feed_in_forecast_sensor(menuai: menuai) -> None:
    """Test the Feed In Forecast sensor."""
    assert len(menuai.states.async_all()) == 9
    price = menuai.states.get("sensor.mock_title_feed_in_forecast")
    assert price
    assert price.state == "-0.09"
    attributes = price.attributes
    assert attributes["channel_type"] == "feedIn"
...
def test_renewable_sensor(menuai: menuai) -> None:
    """Testing the creation of the Amber renewables sensor."""
    assert len(menuai.states.async_all()) == 6
    sensor = menuai.states.get("sensor.mock_title_renewables")
    assert sensor
    assert sensor.state == "51"


@pytest.mark.usefixtures("setup_general")
def test_general_price_descriptor_descriptor_sensor(menuai: menuai) -> None:
    """Test the General Price Descriptor sensor."""
    assert len(menuai.states.async_all()) == 6
    price = menuai.states.get("sensor.mock_title_general_price_descriptor")
    assert price
...
def test_general_and_controlled_load_price_descriptor_sensor(
    ...
    assert len(menuai.states.async_all()) == 9
    price = menuai.states.get("sensor.mock_title_controlled_load_price_descriptor")
    assert price
    assert price.state == "extremely_low"


@pytest.mark.usefixtures("setup_general_and_feed_in")
def test_general_and_feed_in_price_descriptor_sensor(menuai: menuai) -> None:
    """Test the Feed In Price Descriptor sensor."""
    assert len(menuai.states.async_all()) == 9
    price = menuai.states.get("sensor.mock_title_feed_in_price_descriptor")
    assert price
    assert price.state == "extremely_low"
