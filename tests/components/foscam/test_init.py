...
from menuai.components.foscam.const import DOMAIN
...
async def test_unique_id_new_entry(
    ...
    with (
        ...
        patch("menuai.components.foscam.FoscamCamera") as mock_foscam_camera,
    ):
...
async def test_switch_unique_id_migration_ok(
    ...
    assert menuai.states.get(entity_id_new)
...
async def test_unique_id_migration_not_needed(
    ...
    with (
        ...
        patch("menuai.components.foscam.FoscamCamera") as mock_foscam_camera,
        patch(
            "menuai.components.foscam.async_migrate_entry",
            return_value=True,
        ),
    ):
        setup_mock_foscam_camera(mock_foscam_camera)
