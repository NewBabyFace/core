...
async def test_auth_failure(
    ...
    assert menuai.states.get(entity_id(mock_devices[0])).state == STATE_UNAVAILABLE
    assert menuai.states.get(entity_id(mock_devices[1])).state == STATE_UNAVAILABLE
...
async def test_auth_regions(
    ...
    with patch(
        "menuai.components.fujitsu_fglair.new_ayla_api", return_value=AsyncMock()
...
async def test_migrate_entry_v11_v12(
    ...
    await setup_integration(menuai, v11_config_entry)
    updated_entry = menuai.config_entries.async_get_entry(v11_config_entry.entry_id)
...
async def test_device_offline(
    ...
    assert menuai.states.get(entity_id(mock_devices[0])).state == STATE_UNAVAILABLE
    assert menuai.states.get(entity_id(mock_devices[1])).state == HVACMode.COOL
