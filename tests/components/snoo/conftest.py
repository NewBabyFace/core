...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "menuai.components.snoo.async_setup_entry", return_value=True
    ) as mock_setup_entry:
...
def bypass_api() -> Generator[AsyncMock]:
    """Bypass the Snoo api."""
    with (
        patch("menuai.components.snoo.Snoo", autospec=True) as mock_client,
        patch("menuai.components.snoo.config_flow.Snoo", new=mock_client),
    ):
