...
def mock_tts_get_cache_files_fixture_helper() -> Generator[MagicMock]:
    ...
    with patch(
        "menuai.components.tts._get_cache_files", return_value={}
    ) as mock_cache_files:
...
def mock_tts_init_cache_dir_fixture_helper(
    ...
    with patch(
        "menuai.components.tts._init_tts_cache_dir",
...
def tts_mutagen_mock_fixture_helper() -> Generator[MagicMock]:
    """Mock writing tags."""
    with patch(
        "menuai.components.tts.SpeechManager.write_tags",
        side_effect=lambda *args: args[1],
    ) as mock_write_tags:
        yield mock_write_tags


async def get_media_source_url(menuai: menuai, media_content_id: str) -> str:
    """Get the media source url."""
    if media_source.DOMAIN not in menuai.config.components:
        assert await async_setup_component(menuai, media_source.DOMAIN, {})

    resolved = await media_source.async_resolve_media(menuai, media_content_id, None)
    return resolved.url


async def retrieve_media(
    menuai: menuai, menuai_client: ClientSessionGenerator, media_content_id: str
) -> HTTPStatus:
    """Get the media source url."""
    url = await get_media_source_url(menuai, media_content_id)
...
async def mock_setup(
    ...
    mock_integration(menuai, MockModule(domain=TEST_DOMAIN))
    mock_platform(menuai, f"{TEST_DOMAIN}.{DOMAIN}", MockTTS(mock_provider))

    await async_setup_component(menuai, DOMAIN, {DOMAIN: {"platform": TEST_DOMAIN}})
    await menuai.async_block_till_done()
...
async def mock_config_entry_setup(
    ...
    async def async_unload_entry_init(
        ...
        await menuai.config_entries.async_forward_entry_unload(config_entry, Platform.TTS)
        return True
    ...
    mock_platform(menuai, f"{test_domain}.{DOMAIN}", loaded_platform)
...
class MockResultStream(ResultStream):
    ...
    def __init__(self, menuai: menuai, extension: str, data: bytes) -> None:
        ...
        super().__init__(
            ...
            language="en",
            options={},
            _manager=menuai.data[DATA_TTS_MANAGER],
        )
        menuai.data[DATA_TTS_MANAGER].token_to_stream[self.token] = self
