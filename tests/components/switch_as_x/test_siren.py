...
async def test_default_state(menuai: menuai) -> None:
    ...
    state = menuai.states.get("siren.noise_maker")
...
async def test_service_calls(menuai: menuai) -> None:
    ...
    assert menuai.states.get("siren.decorative_lights").state == STATE_ON
    ...
    assert menuai.states.get("siren.decorative_lights").state == STATE_OFF
