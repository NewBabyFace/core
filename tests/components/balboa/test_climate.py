...
from menuai.const import ATTR_TEMPERATURE, Platform
...
async def test_climate(
    ...
    """Test spa climate."""
    with patch("menuai.components.balboa.PLATFORMS", [Platform.CLIMATE]):
...
async def test_spa_temperature(
    ...
    state = await _patch_spa_settemp(menuai, client, 0, 100)
...
async def test_spa_temperature_unit(
    ...
    state = await _patch_spa_settemp(menuai, client, 0, 15.4)
...
async def test_spa_hvac_modes(
    ...
    for heat_mode in list(HeatMode)[:2]:
        state = await _patch_spa_heatmode(menuai, client, heat_mode)
...
async def test_spa_hvac_action(
    ...
    state = await _patch_spa_heatstate(menuai, client, 0)
    assert state.attributes[ATTR_HVAC_ACTION] == HVACAction.OFF

    state = await _patch_spa_heatstate(menuai, client, 1)
    assert state.attributes[ATTR_HVAC_ACTION] == HVACAction.HEATING

    state = await _patch_spa_heatstate(menuai, client, 2)
    assert state.attributes[ATTR_HVAC_ACTION] == HVACAction.IDLE
...
async def test_spa_preset_modes(
    ...
    for mode in modelist:
        ...
        await common.async_set_preset_mode(menuai, mode, ENTITY_CLIMATE)

        state = await client_update(menuai, client, ENTITY_CLIMATE)
        assert state
        assert state.attributes[ATTR_PRESET_MODE] == mode

    with pytest.raises(ServiceValidationError):
        await common.async_set_preset_mode(menuai, 2, ENTITY_CLIMATE)
...
async def test_spa_with_blower(menuai: menuai, client: MagicMock) -> None:
    """Test supported features flags."""
    ...
    for fan_mode in (FAN_LOW, FAN_MEDIUM, FAN_HIGH):
        ...
        state = await _patch_blower(menuai, client, fan_mode)
...
async def _patch_blower(menuai: menuai, client: MagicMock, fan_mode: str) -> State:
    """Patch the blower state."""
    client.blowers[0].state = OffLowMediumHighState[fan_mode.upper()]
    await common.async_set_fan_mode(menuai, fan_mode)
    return await client_update(menuai, client, ENTITY_CLIMATE)


async def _patch_spa_settemp(
    menuai: menuai, client: MagicMock, tscale: int, settemp: float
) -> State:
    """Patch the settemp."""
    client.temperature_unit = tscale
    client.target_temperature = settemp
    await common.async_set_temperature(
        menuai, temperature=settemp, entity_id=ENTITY_CLIMATE
    )
    return await client_update(menuai, client, ENTITY_CLIMATE)


async def _patch_spa_heatmode(
    menuai: menuai, client: MagicMock, heat_mode: int
) -> State:
    """Patch the heatmode."""
    client.heat_mode.state = heat_mode
    await common.async_set_hvac_mode(menuai, HVAC_SETTINGS[heat_mode], ENTITY_CLIMATE)
    return await client_update(menuai, client, ENTITY_CLIMATE)


async def _patch_spa_heatstate(
    menuai: menuai, client: MagicMock, heat_state: int
) -> State:
    """Patch the heatmode."""
    client.heat_state = heat_state
    await common.async_set_hvac_mode(menuai, HVAC_SETTINGS[heat_state], ENTITY_CLIMATE)
    return await client_update(menuai, client, ENTITY_CLIMATE)
