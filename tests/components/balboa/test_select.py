...
async def test_selects(
    ...
    """Test spa climate."""
    with patch("menuai.components.balboa.PLATFORMS", [Platform.SELECT]):
        entry = await init_integration(menuai)

    await snapshot_platform(menuai, entity_registry, snapshot, entry.entry_id)


async def test_select(menuai: menuai, client: MagicMock, mock_select) -> None:
    ...
    # test high state
    await _select_option_and_wait(menuai, ENTITY_SELECT, LowHighRange.HIGH.name.lower())
    ...
    await _select_option_and_wait(menuai, ENTITY_SELECT, LowHighRange.LOW.name.lower())
...
async def test_selected_option(
    menuai: menuai, client: MagicMock, mock_select
) -> None:
    ...
    state = await client_update(menuai, client, ENTITY_SELECT)
    assert state.state == LowHighRange.HIGH.name.lower()


async def _select_option_and_wait(menuai: menuai | None, entity, option):
