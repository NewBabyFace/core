...
from menuai.components.met.const import DOMAIN, HOME_LOCATION_NAME
...
def met_setup_fixture(request: pytest.FixtureRequest) -> Generator[Any]:
    ...
    if "disable_autouse_fixture" in request.keywords:
        ...
        with patch("menuai.components.met.async_setup_entry", return_value=True):
            yield
...
async def test_onboarding_step(menuai: menuai) -> None:
...
async def test_onboarding_step_abort_no_home(
    menuai: menuai, latitude, longitude
) -> None:
    ...
    assert menuai.config.latitude == latitude
    assert menuai.config.longitude == longitude
...
async def test_options_flow(menuai: menuai) -> None:
    ...
    with patch(
        "menuai.components.met.coordinator.metno.MetWeatherData"
    ) as weatherdatamock:
