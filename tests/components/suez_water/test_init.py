...
from menuai.components.suez_water.const import (
    CONF_COUNTER_ID,
    DATA_REFRESH_INTERVAL,
    DOMAIN,
)
from menuai.components.suez_water.coordinator import (
    PySuezError,
    TelemetryMeasure,
)
...
async def test_migration_version_rollback(
    ...
    await setup_integration(menuai, future_entry)
...
async def test_no_migration_current_version(
    ...
    await setup_integration(menuai, current_entry)
...
async def test_migration_version_1_to_2(
    ...
    await setup_integration(menuai, past_entry)
