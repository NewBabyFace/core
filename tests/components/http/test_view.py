...
from menuai.components.http.view import (
    menuaiView,
    request_handler_factory,
)
from menuai.exceptions import ServiceNotFound, Unauthorized


@pytest.fixture
def mock_request() -> Mock:
    """Mock a request."""
    return Mock(app={KEY_menuai: Mock(is_stopping=False)}, match_info={})


@pytest.fixture
def mock_request_with_stopping() -> Mock:
    """Mock a request."""
    return Mock(app={KEY_menuai: Mock(is_stopping=True)}, match_info={})


async def test_invalid_json(caplog: pytest.LogCaptureFixture) -> None:
    """Test trying to return invalid JSON."""
    with pytest.raises(HTTPInternalServerError):
        menuaiView.json({"hello": Decimal("2.0")})
...
async def test_nan_serialized_to_null() -> None:
    ...
    response = menuaiView.json(float("NaN"))
...
async def test_not_running(mock_request_with_stopping: Mock) -> None:
    ...
    response = await request_handler_factory(
        mock_request_with_stopping.app[KEY_menuai],
        Mock(requires_auth=False),
        AsyncMock(side_effect=Unauthorized),
