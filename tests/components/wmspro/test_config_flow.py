...
async def test_config_flow(
    menuai: menuai, mock_setup_entry: AsyncMock, mock_hub_refresh: AsyncMock
) -> None:
...
async def test_config_flow_from_dhcp_add_mac(
    ...
    assert len(mock_setup_entry.mock_calls) == 1
    assert menuai.config_entries.async_entries(DOMAIN)[0].unique_id is None
    ...
    assert menuai.config_entries.async_entries(DOMAIN)[0].unique_id == "00:11:22:33:44:55"
...
async def test_config_flow_from_dhcp_ip_update(
    ...
    assert result["reason"] == "already_configured"
    assert menuai.config_entries.async_entries(DOMAIN)[0].unique_id == "00:11:22:33:44:55"
    assert menuai.config_entries.async_entries(DOMAIN)[0].data[CONF_HOST] == "5.6.7.8"
...
async def test_config_flow_from_dhcp_no_update(
    ...
    assert result["reason"] == "already_configured"
    assert menuai.config_entries.async_entries(DOMAIN)[0].unique_id == "00:11:22:33:44:55"
    assert menuai.config_entries.async_entries(DOMAIN)[0].data[CONF_HOST] == "webcontrol"
