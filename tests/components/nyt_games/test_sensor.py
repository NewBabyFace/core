...
async def test_updating_exception(
    ...
    assert menuai.states.get("sensor.wordle_played").state == STATE_UNAVAILABLE
...
async def test_new_account(
    ...
    mock_nyt_games_client.get_latest_stats.return_value = WordleStats.from_json(
        await async_load_fixture(menuai, "new_account.json", DOMAIN)
    ).player.stats
    await setup_integration(menuai, mock_config_entry)

    assert menuai.states.get("sensor.spelling_bee_played") is None
