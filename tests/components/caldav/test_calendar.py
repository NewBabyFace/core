...
def mock_tz() -> str | None:
    """Fixture to specify the MenuAI timezone to use during the test."""
    return None


@pytest.fixture(autouse=True)
async def set_tz(menuai: menuai, tz: str | None) -> None:
    """Fixture to set the default TZ to the one requested."""
    if tz is not None:
        await menuai.config.async_set_time_zone(tz)


@pytest.fixture(autouse=True)
def mock_http(menuai: menuai) -> None:
...
async def mock_setup_platform_cb(
    ...
    async def _run() -> None:
        assert await async_setup_component(
            menuai, "calendar", {"calendar": {**CALDAV_CONFIG, **config}}
        )
...
async def test_setup_component_config(
    ...
    all_calendar_entities = menuai.states.async_entity_ids("calendar")
...
async def test_ongoing_event(
    menuai: menuai, setup_platform_cb: Callable[[], Awaitable[None]]
) -> None:
...
async def test_matching_filter(
    ...
    state = menuai.states.get("calendar.example_example")
    assert state
    assert state.name == CALENDAR_NAME
    assert state.state == STATE_OFF
    assert dict(state.attributes) == {
...
async def test_calendar_components(menuai: menuai) -> None:
    """Test that only calendars that support events are created."""

    assert await async_setup_component(menuai, "calendar", {"calendar": CALDAV_CONFIG})
    await menuai.async_block_till_done()

    state = menuai.states.get("calendar.calendar_1")
    assert state

    state = menuai.states.get("calendar.calendar_2")
    assert state

    # No entity created for To-do only component
    state = menuai.states.get("calendar.calendar_3")
    assert not state

    # No entity created when no components exist
    state = menuai.states.get("calendar.calendar_4")
    assert not state
