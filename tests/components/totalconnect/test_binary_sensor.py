...
async def test_entity_registry(
    ...
    entry = await setup_platform(menuai, BINARY_SENSOR)

    await snapshot_platform(menuai, entity_registry, snapshot, entry.entry_id)


async def test_state_and_attributes(menuai: menuai) -> None:
    ...
    with patch(
        ...
        await setup_platform(menuai, BINARY_SENSOR)

        state = menuai.states.get(ZONE_ENTITY_ID)
        ...
        state = menuai.states.get(f"{ZONE_ENTITY_ID}_battery")
        assert state.state == STATE_OFF
        state = menuai.states.get(f"{ZONE_ENTITY_ID}_tamper")
        assert state.state == STATE_OFF

        # Zone 2 is fire with low battery
        state = menuai.states.get("binary_sensor.fire")
        assert state.state == STATE_OFF
        assert state.attributes.get("device_class") == BinarySensorDeviceClass.SMOKE
        state = menuai.states.get("binary_sensor.fire_battery")
        assert state.state == STATE_ON
        state = menuai.states.get("binary_sensor.fire_tamper")
        assert state.state == STATE_OFF

        # Zone 3 is gas with tamper
        state = menuai.states.get("binary_sensor.gas")
        assert state.state == STATE_OFF
        assert state.attributes.get("device_class") == BinarySensorDeviceClass.GAS
        state = menuai.states.get("binary_sensor.gas_battery")
        assert state.state == STATE_OFF
        state = menuai.states.get("binary_sensor.gas_tamper")
        assert state.state == STATE_ON

        # Zone 6 is unknown type, assume it is a security (door) sensor
        state = menuai.states.get("binary_sensor.unknown")
        assert state.state == STATE_OFF
        assert state.attributes.get("device_class") == BinarySensorDeviceClass.DOOR
        state = menuai.states.get("binary_sensor.unknown_battery")
        assert state.state == STATE_OFF
        state = menuai.states.get("binary_sensor.unknown_tamper")
        assert state.state == STATE_OFF

        # Zone 7 is temperature
        state = menuai.states.get("binary_sensor.temperature")
        assert state.state == STATE_OFF
        assert state.attributes.get("device_class") == BinarySensorDeviceClass.PROBLEM
        state = menuai.states.get("binary_sensor.temperature_battery")
        assert state.state == STATE_OFF
        state = menuai.states.get("binary_sensor.temperature_tamper")
        assert state.state == STATE_OFF
