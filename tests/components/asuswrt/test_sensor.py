...
from menuai.components import device_tracker, sensor
...
def _setup_entry(menuai: menuai, config, sensors, unique_id=None):
...
async def _test_sensors(
    ...
    config_entry, sensor_prefix = _setup_entry(
        menuai, config, SENSORS_DEFAULT, entry_unique_id
    )
    ...
    assert menuai.states.get(f"{device_tracker.DOMAIN}.test").state == STATE_HOME
    assert menuai.states.get(f"{device_tracker.DOMAIN}.testtwo").state == STATE_HOME
    assert menuai.states.get(f"{sensor_prefix}_sensor_rx_rates").state == "160.0"
    assert menuai.states.get(f"{sensor_prefix}_sensor_rx_bytes").state == "60.0"
    assert menuai.states.get(f"{sensor_prefix}_sensor_tx_rates").state == "80.0"
    assert menuai.states.get(f"{sensor_prefix}_sensor_tx_bytes").state == "50.0"
    assert menuai.states.get(f"{sensor_prefix}_devices_connected").state == "2"
    ...
    assert menuai.states.get(f"{device_tracker.DOMAIN}.test").state == STATE_HOME
    assert menuai.states.get(f"{device_tracker.DOMAIN}.testtwo").state == STATE_HOME
    assert menuai.states.get(f"{sensor_prefix}_devices_connected").state == "1"
    ...
    assert menuai.states.get(f"{device_tracker.DOMAIN}.test").state == STATE_NOT_HOME
    assert menuai.states.get(f"{device_tracker.DOMAIN}.testtwo").state == STATE_HOME
    assert menuai.states.get(f"{device_tracker.DOMAIN}.testthree").state == STATE_HOME
    assert menuai.states.get(f"{sensor_prefix}_devices_connected").state == "3"
...
async def test_sensors_legacy(
    ...
    await _test_sensors(
        menuai, freezer, mock_devices_legacy, CONFIG_DATA_TELNET, entry_unique_id
    )
...
async def test_sensors_http(
    ...
    await _test_sensors(
        menuai, freezer, mock_devices_http, CONFIG_DATA_HTTP, entry_unique_id
    )


async def _test_loadavg_sensors(
    menuai: menuai, freezer: FrozenDateTimeFactory, config
) -> None:
    """Test creating an AsusWRT load average sensors."""
    config_entry, sensor_prefix = _setup_entry(menuai, config, SENSORS_LOAD_AVG)
    config_entry.add_to_menuai(menuai)
    ...
    assert menuai.states.get(f"{sensor_prefix}_sensor_load_avg1").state == "1.1"
    assert menuai.states.get(f"{sensor_prefix}_sensor_load_avg5").state == "1.2"
    assert menuai.states.get(f"{sensor_prefix}_sensor_load_avg15").state == "1.3"


async def test_loadavg_sensors_legacy(
    menuai: menuai, freezer: FrozenDateTimeFactory, connect_legacy
) -> None:
    """Test creating an AsusWRT load average sensors."""
    await _test_loadavg_sensors(menuai, freezer, CONFIG_DATA_TELNET)


async def test_loadavg_sensors_http(
    menuai: menuai, freezer: FrozenDateTimeFactory, connect_http
) -> None:
    """Test creating an AsusWRT load average sensors."""
    await _test_loadavg_sensors(menuai, freezer, CONFIG_DATA_HTTP)
...
async def test_loadavg_sensors_unaivalable_http(
    ...
    config_entry, sensor_prefix = _setup_entry(menuai, CONFIG_DATA_HTTP, SENSORS_LOAD_AVG)
    config_entry.add_to_menuai(menuai)
    ...
    assert not menuai.states.get(f"{sensor_prefix}_sensor_load_avg1")
    assert not menuai.states.get(f"{sensor_prefix}_sensor_load_avg5")
    assert not menuai.states.get(f"{sensor_prefix}_sensor_load_avg15")


async def test_temperature_sensors_http_fail(
    menuai: menuai, connect_http_sens_fail
) -> None:
    """Test fail creating AsusWRT temperature sensors."""
    config_entry, sensor_prefix = _setup_entry(
        menuai, CONFIG_DATA_HTTP, SENSORS_TEMPERATURES
    )
    config_entry.add_to_menuai(menuai)
    ...
    assert not menuai.states.get(f"{sensor_prefix}_2_4ghz")
    assert not menuai.states.get(f"{sensor_prefix}_5_0ghz")
    assert not menuai.states.get(f"{sensor_prefix}_cpu")
    assert not menuai.states.get(f"{sensor_prefix}_5_0ghz_2")
    assert not menuai.states.get(f"{sensor_prefix}_6_0ghz")


async def _test_temperature_sensors(
    menuai: menuai, freezer: FrozenDateTimeFactory, config, sensors
) -> str:
    """Test creating a AsusWRT temperature sensors."""
    config_entry, sensor_prefix = _setup_entry(menuai, config, sensors)
    config_entry.add_to_menuai(menuai)
...
async def test_temperature_sensors_legacy(
    ...
    sensor_prefix = await _test_temperature_sensors(
        menuai, freezer, CONFIG_DATA_TELNET, SENSORS_TEMPERATURES_LEGACY
    )
    # assert temperature sensor available
    assert menuai.states.get(f"{sensor_prefix}_2_4ghz").state == "40.2"
    assert menuai.states.get(f"{sensor_prefix}_cpu").state == "71.2"
    assert not menuai.states.get(f"{sensor_prefix}_5_0ghz")
...
async def test_temperature_sensors_http(
    ...
    """Test creating a AsusWRT temperature sensors."""
    sensor_prefix = await _test_temperature_sensors(
        menuai, freezer, CONFIG_DATA_HTTP, SENSORS_TEMPERATURES
    )
    # assert temperature sensor available
    assert menuai.states.get(f"{sensor_prefix}_2_4ghz").state == "40.2"
    assert menuai.states.get(f"{sensor_prefix}_cpu").state == "71.2"
    assert menuai.states.get(f"{sensor_prefix}_5_0ghz_2").state == "40.3"
    assert menuai.states.get(f"{sensor_prefix}_6_0ghz").state == "40.4"
    assert not menuai.states.get(f"{sensor_prefix}_5_0ghz")
...
async def test_cpu_sensors_http_fail(
    ...
    config_entry, sensor_prefix = _setup_entry(menuai, CONFIG_DATA_HTTP, SENSORS_CPU)
    config_entry.add_to_menuai(menuai)
    ...
    assert not menuai.states.get(f"{sensor_prefix}_cpu1_usage")
    assert not menuai.states.get(f"{sensor_prefix}_cpu2_usage")
    assert not menuai.states.get(f"{sensor_prefix}_cpu3_usage")
    assert not menuai.states.get(f"{sensor_prefix}_cpu4_usage")
    assert not menuai.states.get(f"{sensor_prefix}_cpu5_usage")
    assert not menuai.states.get(f"{sensor_prefix}_cpu6_usage")
    assert not menuai.states.get(f"{sensor_prefix}_cpu7_usage")
    assert not menuai.states.get(f"{sensor_prefix}_cpu8_usage")
    assert not menuai.states.get(f"{sensor_prefix}_cpu_total_usage")
...
async def test_cpu_sensors_http(
    ...
    assert menuai.states.get(f"{sensor_prefix}_cpu1_usage").state == "0.1"
    assert menuai.states.get(f"{sensor_prefix}_cpu2_usage").state == "0.2"
    assert menuai.states.get(f"{sensor_prefix}_cpu3_usage").state == "0.3"
    assert menuai.states.get(f"{sensor_prefix}_cpu4_usage").state == "0.4"
    assert menuai.states.get(f"{sensor_prefix}_cpu5_usage").state == "0.5"
    assert menuai.states.get(f"{sensor_prefix}_cpu6_usage").state == "0.6"
    assert menuai.states.get(f"{sensor_prefix}_cpu7_usage").state == "0.7"
    assert menuai.states.get(f"{sensor_prefix}_cpu8_usage").state == "0.8"
    assert menuai.states.get(f"{sensor_prefix}_cpu_total_usage").state == "0.9"
...
async def test_memory_sensors_http(
    ...
    config_entry, sensor_prefix = _setup_entry(menuai, CONFIG_DATA_HTTP, SENSORS_MEMORY)
    config_entry.add_to_menuai(menuai)
    ...
    assert menuai.states.get(f"{sensor_prefix}_mem_usage_perc").state == "52.4"
    assert menuai.states.get(f"{sensor_prefix}_mem_free").state == "384.0"
    assert menuai.states.get(f"{sensor_prefix}_mem_used").state == "640.0"
...
async def test_uptime_sensors_http(
    ...
    config_entry, sensor_prefix = _setup_entry(menuai, CONFIG_DATA_HTTP, SENSORS_UPTIME)
    config_entry.add_to_menuai(menuai)
    ...
    assert (
        menuai.states.get(f"{sensor_prefix}_sensor_last_boot").state
        == "2024-08-02T00:47:00+00:00"
    )
    assert menuai.states.get(f"{sensor_prefix}_sensor_uptime").state == "1625927"
...
async def test_connect_fail_legacy(
    menuai: menuai, connect_legacy, side_effect
) -> None:
...
async def test_connect_fail_http(
    menuai: menuai, connect_http, side_effect
) -> None:
    """Test AsusWRT connect fail."""
...
async def _test_sensors_polling_fails(
    ...
    for sensor_name in sensors:
        assert (
            menuai.states.get(f"{sensor_prefix}_{slugify(sensor_name)}").state
            == STATE_UNAVAILABLE
        )
    assert menuai.states.get(f"{sensor_prefix}_devices_connected").state == "0"
...
async def test_sensors_polling_fails_legacy(
    ...
    await _test_sensors_polling_fails(
        menuai, freezer, CONFIG_DATA_TELNET, SENSORS_ALL_LEGACY
    )
...
async def test_sensors_polling_fails_http(
    ...
    """Test AsusWRT sensors are unavailable when polling fails."""
    await _test_sensors_polling_fails(menuai, freezer, CONFIG_DATA_HTTP, SENSORS_ALL_HTTP)
...
async def test_unique_id_migration(
    menuai: menuai, entity_registry: er.EntityRegistry, connect_legacy
) -> None:
...
async def test_decorator_errors(
    ...
    config_entry, sensor_prefix = _setup_entry(menuai, CONFIG_DATA_TELNET, sensors)
    config_entry.add_to_menuai(menuai)
