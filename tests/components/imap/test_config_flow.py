...
from menuai.components.imap.const import (
    CONF_CHARSET,
    CONF_FOLDER,
    CONF_SEARCH,
    DOMAIN,
...
async def test_form(menuai: menuai, mock_setup_entry: AsyncMock) -> None:
    ...
    with patch(
        "menuai.components.imap.config_flow.connect_to_server"
    ) as mock_client:
...
async def test_entry_already_configured(menuai: menuai) -> None:
...
async def test_form_invalid_charset(menuai: menuai) -> None:
...
async def test_form_invalid_folder(menuai: menuai) -> None:
...
async def test_form_invalid_search(menuai: menuai) -> None:
...
async def test_reauth_success(menuai: menuai, mock_setup_entry: AsyncMock) -> None:
    """Test we can reauth."""
...
async def test_reauth_failed(menuai: menuai) -> None:
    """Test we can reauth."""
    entry = MockConfigEntry(
        domain=DOMAIN,
        data=MOCK_CONFIG,
...
async def test_reauth_failed_conn_error(menuai: menuai) -> None:
    """Test we can reauth."""
    entry = MockConfigEntry(
        domain=DOMAIN,
        data=MOCK_CONFIG,
...
async def test_key_options_in_options_form(menuai: menuai) -> None:
    ...
    result = await menuai.config_entries.options.async_init(entry2.entry_id)
...
async def test_config_flow_with_cipherlist_and_ssl_verify(
    menuai: menuai, mock_setup_entry: AsyncMock, cipher_list: str, verify_ssl: True
) -> None:
...
async def test_config_flow_with_event_message_data(
    menuai: menuai, mock_setup_entry: AsyncMock, event_message_data: list
) -> None:
