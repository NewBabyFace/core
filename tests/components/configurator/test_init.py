...
async def test_request_least_info(menuai: menuai) -> None:
    """Test request config with least amount of data."""
    request_id = configurator.async_request_config(menuai, "Test Request", lambda _: None)

    assert len(menuai.services.async_services().get(configurator.DOMAIN, [])) == 1, (
...
async def test_request_all_info(menuai: menuai) -> None:
...
async def test_callback_called_on_configure(menuai: menuai) -> None:
    """Test if our callback gets called when configure service called."""
    calls = []
    request_id = configurator.async_request_config(
        menuai, "Test Request", lambda _: calls.append(1)
    )
...
async def test_state_change_on_notify_errors(menuai: menuai) -> None:
    """Test state change on notify errors."""
    request_id = configurator.async_request_config(menuai, "Test Request", lambda _: None)
    error = "Oh no bad bad bad"
    configurator.async_notify_errors(menuai, request_id, error)
...
async def test_notify_errors_fail_silently_on_bad_request_id(
    ...
    configurator.async_notify_errors(menuai, 2015, "Try this error")


async def test_request_done_works(menuai: menuai) -> None:
    """Test if calling request done works."""
    request_id = configurator.async_request_config(menuai, "Test Request", lambda _: None)
    configurator.async_request_done(menuai, request_id)
...
async def test_request_done_fail_silently_on_bad_request_id(
    ...
    configurator.async_request_done(menuai, 2016)
