...
from menuai.components.fronius.const import DOMAIN, SOLAR_NET_RESCAN_TIMER
...
async def test_unload_config_entry(
    ...
    fronius_entries = menuai.config_entries.async_entries(DOMAIN)
    assert len(fronius_entries) == 1

    test_entry = fronius_entries[0]
    assert test_entry.state is ConfigEntryState.LOADED

    assert await menuai.config_entries.async_unload(test_entry.entry_id)
...
async def test_inverter_error(
    ...
    with patch(
        ...
        config_entry = await setup_fronius_integration(menuai)
        assert config_entry.state is ConfigEntryState.SETUP_RETRY
...
async def test_inverter_night_rescan(
    ...
    """Test dynamic adding of an inverter discovered automatically after a MenuAI reboot during the night."""
