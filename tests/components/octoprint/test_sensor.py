...
async def test_sensors(
    ...
    await init_integration(menuai, "sensor", printer=printer, job=job)

    state = menuai.states.get("sensor.octoprint_job_percentage")
    ...
    state = menuai.states.get("sensor.octoprint_current_state")
    assert state is not None
    ...
    state = menuai.states.get("sensor.octoprint_actual_tool1_temp")
    assert state is not None
    ...
    state = menuai.states.get("sensor.octoprint_target_tool1_temp")
    assert state is not None
    ...
    state = menuai.states.get("sensor.octoprint_start_time")
    assert state is not None
    ...
    state = menuai.states.get("sensor.octoprint_estimated_finish_time")
    assert state is not None
...
async def test_sensors_no_target_temp(
    ...
    await init_integration(menuai, "sensor", printer=printer)
...
async def test_sensors_printer_disconnected(
    ...
    freezer.move_to(datetime(2020, 2, 20, 9, 10, 0))
    await init_integration(menuai, "sensor", printer=None, job=job)
