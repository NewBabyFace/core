...
async def test_device_remove(
    ...
    async_fire_mqtt_message(
        menuai, f"{DEFAULT_PREFIX}/{mac}/sensors", json.dumps(sensor_config)
    )
...
async def test_device_remove_non_tasmota_device(
    ...
    await remove_device(menuai, menuai_ws_client, device_entry.id, config_entry.entry_id)
    await menuai.async_block_till_done()
...
async def test_tasmota_ws_remove_discovered_device(
    ...
    tasmota_config_entry = menuai.config_entries.async_entries(DOMAIN)[0]
    await remove_device(
        menuai, menuai_ws_client, device_entry.id, tasmota_config_entry.entry_id
    )
