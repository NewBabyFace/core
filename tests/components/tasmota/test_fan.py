...
async def test_controlling_state_via_mqtt(
    ...
    state = menuai.states.get("fan.tasmota")
    ...
    async_fire_mqtt_message(menuai, "tasmota_49A3BC/tele/STATE", '{"FanSpeed":1}')
    state = menuai.states.get("fan.tasmota")
    assert state.state == STATE_ON
    assert state.attributes["percentage"] == 33

    async_fire_mqtt_message(menuai, "tasmota_49A3BC/tele/STATE", '{"FanSpeed":2}')
    state = menuai.states.get("fan.tasmota")
    assert state.state == STATE_ON
    assert state.attributes["percentage"] == 66

    async_fire_mqtt_message(menuai, "tasmota_49A3BC/tele/STATE", '{"FanSpeed":3}')
    state = menuai.states.get("fan.tasmota")
    assert state.state == STATE_ON
    assert state.attributes["percentage"] == 100

    async_fire_mqtt_message(menuai, "tasmota_49A3BC/tele/STATE", '{"FanSpeed":0}')
    state = menuai.states.get("fan.tasmota")
    assert state.state == STATE_OFF
    assert state.attributes["percentage"] == 0

    async_fire_mqtt_message(menuai, "tasmota_49A3BC/stat/RESULT", '{"FanSpeed":1}')
    state = menuai.states.get("fan.tasmota")
    assert state.state == STATE_ON
    assert state.attributes["percentage"] == 33

    async_fire_mqtt_message(menuai, "tasmota_49A3BC/stat/RESULT", '{"FanSpeed":0}')
    state = menuai.states.get("fan.tasmota")
    assert state.state == STATE_OFF
    assert state.attributes["percentage"] == 0
...
async def test_sending_mqtt_commands(
    ...
    await common.async_turn_on(menuai, "fan.tasmota")
    ...
    await common.async_turn_off(menuai, "fan.tasmota")
    mqtt_mock.async_publish.assert_called_once_with(
    ...
    await common.async_set_percentage(menuai, "fan.tasmota", 0)
    mqtt_mock.async_publish.assert_called_once_with(
        "tasmota_49A3BC/cmnd/FanSpeed", "0", 0, False
    )
    mqtt_mock.async_publish.reset_mock()
    ...
    # Set speed percentage and verify MQTT message is sent
    await common.async_set_percentage(menuai, "fan.tasmota", 15)
    mqtt_mock.async_publish.assert_called_once_with(
    ...
    # Set speed percentage and verify MQTT message is sent
    await common.async_set_percentage(menuai, "fan.tasmota", 50)
    mqtt_mock.async_publish.assert_called_once_with(
        "tasmota_49A3BC/cmnd/FanSpeed", "2", 0, False
    )
    mqtt_mock.async_publish.reset_mock()
    ...
    # Set speed percentage and verify MQTT message is sent
    await common.async_set_percentage(menuai, "fan.tasmota", 90)
    mqtt_mock.async_publish.assert_called_once_with(
...
async def test_invalid_fan_speed_percentage(
    ...
    with pytest.raises(MultipleInvalid) as excinfo:
        await common.async_set_percentage(menuai, "fan.tasmota", 101)
...
async def test_availability_when_connection_lost(
    ...
    await help_test_availability_when_connection_lost(
        menuai, mqtt_client_mock, mqtt_mock, Platform.FAN, config, object_id="tasmota"
    )
...
async def test_availability(
    ...
    await help_test_availability(
        menuai, mqtt_mock, Platform.FAN, config, object_id="tasmota"
    )
...
async def test_availability_poll_state(
    ...
    await help_test_availability_poll_state(
        menuai, mqtt_client_mock, mqtt_mock, Platform.FAN, config, poll_topic, ""
    )
...
async def test_discovery_update_unchanged_fan(
    ...
    config["if"] = 1
    with patch(
        "menuai.components.tasmota.fan.TasmotaFan.discovery_update"
    ) as discovery_update:
...
async def test_discovery_device_remove(
    ...
    await help_test_discovery_device_remove(
        menuai, mqtt_mock, Platform.FAN, unique_id, config
    )
...
async def test_entity_id_update_subscriptions(
    ...
    await help_test_entity_id_update_subscriptions(
        menuai, mqtt_mock, Platform.FAN, config, topics, object_id="tasmota"
    )
